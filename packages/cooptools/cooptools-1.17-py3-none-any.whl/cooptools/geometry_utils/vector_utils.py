...
import math
import numpy as np
from cooptools.common import verify_len, verify_len_match, divided_length, verify_val

FloatVec = Iterable[float]
IterVec = Iterable[FloatVec]
LstVec = List[FloatVec]
VecTransformer = Callable[[IterVec], LstVec]
VecTransformerGetter = VecTransformer | Callable[[], VecTransformer]

def resolve_vec_transformer(vec_transformer_getter: VecTransformerGetter) -> VecTransformer:
    if callable(vec_transformer_getter):
        return vec_transformer_getter()
    return vec_transformer_getter

def resolve_vec_transformed_points(points: IterVec,
                                   vec_transformer_getter: VecTransformerGetter=None) -> LstVec:
    if vec_transformer_getter is None:
        return list(points)

    vec_transformer = resolve_vec_transformer(vec_transformer_getter)
...
def add_vectors(vectors: List[FloatVec],
...
def points_projected_to_plane(points: Iterable[FloatVec],
                              plane_points: Iterable[FloatVec]) -> List[FloatVec]:
