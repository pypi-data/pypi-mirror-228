...
class DataRefresher:
    def __init__(self,
        ...
        self._refresh_timer_start = None
        self._stagnation_timer_start = None
    ...
    def reset_refresh_timer(self, reset_stagnation: bool = True):
        timer = time.perf_counter()
        self._refresh_timer_start = timer
        if reset_stagnation: self._stagnation_timer_start = timer
    ...
    def _check_refresh_latest(self):
        ...
        # check if time to refresh cache
                ...
                time.perf_counter() - self._refresh_timer_start > self.refresh_interval_ms / 1000:
            self._try_refresh_cache()
    ...
    def __str__(self):
        return f"Latest: {df.latest}, " \
               f"Callback Count: {self.callback_count}, " \
              f"Avg Callback Time: {round(df.avg_callback_time_ms)}ms, " \
              f"Time Until Refresh: {round(df.ms_until_refresh)}ms, " \
              f"Time Until Stagnant: {round(df.ms_until_stagnant)}ms"
