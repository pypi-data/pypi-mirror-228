...
def rotateAroundArbitraryAxis(rotationPoint, rotationAxis, radians, right_handed: bool = False):
    ...
    calculated_matrix = np.array(
        [[u ** 2 + (v ** 2 + w ** 2) * c, u * v * t - w * s, u * w * t + v * s, (x * (v ** 2 + w ** 2) - u * (y * v + z * w)) * t + (y * w - z * v) * s],
...
def apply_transform_to_points(points: IterVec,
    ...
    for pt in scaled:
        if pt[3] != 0:
            ret.append(tuple(x / pt[3] for x in pt[:3]))
        else:
...
if __name__ == "__main__":
    ...
    assert apply_transform_to_points([(1, 0, 0)], lh_matrix=rotateYMatrix(math.pi / 2), sig_dig=0)[0] == (0.0, 0.0, -1.0)
