...
from ...common.style_sheet import isDarkTheme, MaterialStyleSheet, themeColor, ThemeColor
...
class ScrollItemDelegate(QStyledItemDelegate):
    ...
    def _drawText(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        ...
        if index == self.currentIndex:
            ...
            if not (self.min <= index.data(Qt.ItemDataRole.UserRole) <= self.max or
                    option.state & QStyle.StateFlag.State_MouseOver) or index == self.pressedIndex:
...
class CalendarViewBase(QFrame):
    ...
    def __initWidget(self):
        ...
        MaterialStyleSheet.CALENDAR_PICKER.apply(self)
...
class CalendarPicker(LineEdit):
    ...
    def __init__(self, parent=None):
        ...
        self.setTrailingAction(Action(LineEditIcon.CALENDAR, ''))

        self.view = CalendarView(self.window())
        self.view.hide()

        self.setLabel(self.tr('Date'))
        self.setClearButtonEnabled(False)
        self.setReadOnly(True)
        self.setPlaceholderText(self.tr('Pick a date'))
        self.setMinimumWidth(200)

        self.trailingButton.clicked.connect(self._showCalendarView)
