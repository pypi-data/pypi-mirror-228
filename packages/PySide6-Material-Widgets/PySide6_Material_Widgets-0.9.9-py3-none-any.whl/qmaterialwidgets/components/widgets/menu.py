...
from ...common.icon import MaterialIconEngine, Action, MaterialIconBase, Icon, drawSvgIcon, writeSvg, getIconColor
...
class SubMenuItemWidget(QWidget):
    ...
    def paintEvent(self, e):
        ...
        svg = writeSvg(":/qmaterialwidgets/images/menu/ArrowRight.svg", fill=palette.onSurfaceVariant.name())
...
class CheckableMenuIcon(MaterialIconBase, Enum):

    ACCEPT = "Accept"

    def path(self, theme=Theme.AUTO) -> str:
        return f':/qmaterialwidgets/images/menu/{self.value}_{getIconColor(theme)}.svg'
...
class CheckableMenu(RoundMenu):
    ...
    def _adjustItemText(self, item: QListWidgetItem, action: QAction):
        ...
        item.setSizeHint(QSize(w + 16, self.itemHeight))
