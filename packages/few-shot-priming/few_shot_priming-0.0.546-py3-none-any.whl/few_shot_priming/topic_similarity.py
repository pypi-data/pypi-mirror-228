...
def calc_all_similarity(df_train, df_test, model):
    ...
    preprocessed_train_documents, unpreprocessed_train_corpus, _, _= preprocess_dataset(df_all)
    tp = TopicModelDataPreparation("all-mpnet-base-v2")
    dataset = tp.fit(text_for_contextual=unpreprocessed_train_corpus, text_for_bow=preprocessed_train_documents)
    vectors = model.get_doc_topic_distribution(dataset, n_samples=20)
    training_vectors = vectors[:df_train.shape[0]]
    test_vectors = vectors[df_train.shape[0]:]
