...
from PyQt6.QtCore import Qt, QEvent, QRectF, QPoint, QObject, QSize
...
from ...common.style_sheet import themeColor, MaterialStyleSheet, isDarkTheme, Theme, palette
...
class InfoBadge(QLabel):
    ...
    def __init__(self, parent: QWidget = None, level=InfoLevel.ATTENTION):
        ...
        MaterialStyleSheet.INFO_BADGE.apply(self)
...
class IconInfoBadge(InfoBadge):
    ...
    @__init__.register
    def _(self, icon: MaterialIconBase, parent: QWidget = None, level=InfoLevel.ATTENTION):
        self.__init__(parent, level)
        self.setIcon(icon)
    ...
    def setIcon(self, icon: Union[QIcon, MaterialIconBase, str]):
    ...
    def paintEvent(self, e):
        ...
        if isinstance(self._icon, MaterialIconBase):
    ...
    @classmethod
    def make(cls, icon: Union[QIcon, MaterialIconBase], parent=None, level=InfoLevel.INFOAMTION, target: QWidget = None,
             position=InfoBadgePosition.TOP_RIGHT):
    ...
    @classmethod
    def info(cls, icon: Union[QIcon, MaterialIconBase], parent=None, target: QWidget = None,
             position=InfoBadgePosition.TOP_RIGHT):
        return cls.make(icon, parent, InfoLevel.INFOAMTION)

    @classmethod
    def success(cls, icon: Union[QIcon, MaterialIconBase], parent=None, target: QWidget = None,
                position=InfoBadgePosition.TOP_RIGHT):
        return cls.make(icon, parent, InfoLevel.SUCCESS, target, position)

    @classmethod
    def attension(cls, icon: Union[QIcon, MaterialIconBase], parent=None, target: QWidget = None,
                  position=InfoBadgePosition.TOP_RIGHT):
        return cls.make(icon, parent, InfoLevel.ATTENTION, target, position)

    @classmethod
    def warning(cls, icon: Union[QIcon, MaterialIconBase], parent=None, target: QWidget = None,
                position=InfoBadgePosition.TOP_RIGHT):
        return cls.make(icon, parent, InfoLevel.WARNING, target, position)

    @classmethod
    def error(cls, icon: Union[QIcon, MaterialIconBase], parent=None, target: QWidget = None,
              position=InfoBadgePosition.TOP_RIGHT):
        return cls.make(icon, parent, InfoLevel.ERROR, target, position)

    @classmethod
    def custom(cls, icon: Union[QIcon, MaterialIconBase], light: QColor, dark: QColor, parent=None,
               target: QWidget = None, position=InfoBadgePosition.TOP_RIGHT):
        """ create a badge with custom background color

        Parameters
        ----------
        icon: QIcon | MaterialIconBase
            the icon of badge
...
class BottomLeftInfoBadgeManager(InfoBadgeManager):
    ...
    def position(self):
        if isinstance(self.badge, DotInfoBadge):
            x = self.target.geometry().center().x() + 8
            y = self.target.y() + 10
        else:
            x = self.target.geometry().center().x() + 8 - self.badge.width() // 2
            y = self.target.y() + 8
