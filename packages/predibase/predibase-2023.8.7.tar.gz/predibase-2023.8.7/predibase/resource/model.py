...
class ModelRepo:
    ...
    _description: str = field(metadata=config(field_name="description"), default=None)
...
class Model:
    ...
    _experiment_id: int = field(metadata=config(field_name="experimentID"), default=None)
    ...
    def metrics(self) -> "ModelMetrics":
        ...
        return ModelMetrics(nested_metrics, global_metrics)
    ...
    def download(
        ...
        if framework is not None:
            endpoint = endpoint + f"?framework={framework.lower()}"
        resp = self.session.get_json(endpoint)
        uri = resp["uri"]
        ...
        if name:
            ...
            download_path = os.path.basename(urlparse(uri).path)
...
class ModelDraft:
    ...
    def train(self, engine: Optional[Engine] = None) -> Model:
    ...
    if not session.is_plan_expired():
        try:
            ...
            if exists_ok and e.code == 400:
                log_info(f"Model repository {name} already exists. exists_ok=True, so ignoring.")
            else:
                raise e
