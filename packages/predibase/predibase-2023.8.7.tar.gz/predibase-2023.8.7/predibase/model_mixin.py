...
from predibase.resource.model import create_model_repo
from predibase.resource.model import get_default_config as _get_default_config
from predibase.resource.model import get_model as _get_model
from predibase.resource.model import get_model_repo
from predibase.resource.model import get_suggested_configs as _get_suggested_configs
from predibase.resource.model import Model, ModelRepo
...
class ModelMixin:
    ...
    ) -> Union[pd.DataFrame, List[ModelRepo]]:
        """
    ...
    def get_model_repo(self, name: str) -> ModelRepo:
        return get_model_repo(self.session, name)
    ...
    def get_model(
        ...
        :return:
        """
        return _get_model(self.session, name, version, model_id)

    def get_suggested_configs(
        self,
        base_config: Dict[Any, str],
    ) -> List[ConfigSuggestion]:
        return _get_suggested_configs(self.session, base_config)

    def get_most_recent_model_with_description(self, repo_name: str, model_description: str) -> Union[Model, None]:
    ...
    def get_default_config(
        ...
        """
        return _get_default_config(self.session, dataset, targets, automl, hyperopt)
    ...
    def upload_model(self, zip_fp: str, repo: ModelRepo, dataset: Optional[Dataset] = None) -> Model:
        return repo.upload_model(zip_fp, dataset=dataset)
