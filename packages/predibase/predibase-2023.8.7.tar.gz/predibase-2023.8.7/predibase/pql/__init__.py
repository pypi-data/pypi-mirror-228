...
from predibase.pql.api import DEFAULT_API_ENDPOINT, DEFAULT_SERVING_ENDPOINT, Session
...
def connect(
    ...
    _session.url = url
    _session.serving_http_endpoint = serving_endpoint
    _session.serving_grpc_endpoint = serving_endpoint
    _session.token = token
    try:
        decoded = jwt.decode(token, options={"verify_signature": False})
        _session.tenant = decoded["sub"]
    except Exception as e:
        print(f"{e}\nPlease ensure the PREDIBASE_API_TOKEN contains a valid tenant for prediction")
