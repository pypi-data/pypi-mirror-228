...
class Model:
    ...
    def download(
        ...
        try:
            ...
            if e.message == "Error 400: record not found":
...
def wait_until_model_ready(model_id: int, session: Session) -> Dict[str, Any]:
    return session.get_json_until_with_logging(
...
class ModelFuture:
    ...
    def get(self) -> Model:
        ...
        for _ in range(5):
            ...
            node_counts[engine_template["headNodeGroup"]["nodeType"]["clouds"][cloud]["label"]] += 1
            for worker_node_group in engine_template.get("workerNodeGroups", []):
                label = worker_node_group.get("nodeType", {}).get("clouds", {}).get(cloud, {}).get("label", "")
                count = int(
