...
class TorchNetworkToVecParams:
    ...
    def setAsVec(self, x : torch.Tensor) -> None:
        parameters_old = torch.cat([param.view(-1) for param in self.model.parameters()])
        nd = parameters_old.numel()
        assert x.shape[0] == nd, "Expected x to have dimension [{}x1], but it is of {}".format(nd, x.shape)
        assert x.shape[1] == 1, "Expected x to have dimension [{}x1], but it is of {}".format(nd, x.shape)
...
class OptimRes:
    def __init__(self, iter: int, x: torch.Tensor, f: torch.Tensor, g: torch.Tensor, R: torch.Tensor, termination_notes:str="none", algorithmName:str=sys._getframe(1).f_code.co_name):
        self.iter = iter
        self.x = x
        self.f = f
        self.g = g
    ...
    def __str__(self) -> str:
        outstr =  "Algorithm {} terminated after {} iterations!\nNotes:\n{}\nx={}\nf={}\ng={}\n".format(self.algorithmName, self.iter, self.termination_notes, self.x.cpu().numpy().T, self.f, self.g.cpu().numpy().T)
        return outstr


def DBFGSTrustQR(fcn: Callable, x0: torch.Tensor, max_iter=200, grad_tol=1e-6, rel_grad_tol=1e-6, rel_fun_tol=1e-6, DeltaMax=1e16) -> OptimRes:
    ...
    """
    assert not x0.requires_grad, "Expected auto grad to not be enabled for input variable."
    device = x0.device
    dtype = x0.dtype
    n = x0.shape[0]
    ...
    f_hist      = torch.zeros([nhist,], dtype=dtype, device=device)
    # Newton decrement history
    nd_hist     = torch.zeros([nhist,], dtype=dtype, device=device) 
    ...
    header  = " %6s %13s %10s %13s %15s %15s %15s %15s" % ("Iter", "Func-count", "f(x)  ", "TR radius", "Newton Dec.", "1st order opt.", "rho  ", "Dec.  ")
    ...
    for i in range(0, max_iter):
        ...
        assert dm>=0, "Expected there to be a reduction in cost from the scaled trust region step"
        ...
        if rho > 0.5:
            ...
            Delta = 0.25*Delta; # more effective than 0.5*Delta
        ...
        if isWithinNewtonDecrement:
            return OptimRes(i,x,f, g,R,"CONVERGED: Newton decrement below threshold after {} iterations and {} steps.".format(i,j))
        ...
        if rho > 1e-8:
            ...
            f_hist = torch.hstack([torch.tensor(f,dtype=dtype, device=device), f_hist[:-1]])
            nd_hist =torch.hstack([torch.tensor(lambdaSQ,dtype=dtype, device=device), nd_hist[:-1]])
            j=j+1
            x   = xn
            f   = fn
            g   = gn
        ...
        if i == 0 :
            ...
            if yp < 0.2 * ps2:
                ...
                r     = theta*y + (1-theta)*torch.linalg.solve_triangular(R,ps, upper=True)
            else:
                r     = y
