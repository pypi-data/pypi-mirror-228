...
def get_git_revision_short_hash() -> str:
    out = subprocess.check_output(['git', 'rev-parse', '--short', 'HEAD']).decode("utf-8")
    out = out.strip()
    return out 

def GaussianInterpolant(image: torch.Tensor, sigma: float, rQOi: torch.Tensor) -> torch.Tensor:
    ...
    logw        = -0.5*z2 - logdetS - torch.tensor(torch.pi, dtype=dtype, device=device).log()
...
def logGaussianMixtureAtXSingleCov(x: torch.Tensor, mu: torch.Tensor, S: torch.Tensor, logW: torch.Tensor) -> torch.Tensor:
    ...
    z2norm  = (z*z).sum(1)
    lm      = -0.5*z2norm - logdetS - 0.5*n*torch.tensor(2*torch.pi, dtype=x.dtype, device=x.device).log() + logW.reshape((N, 1))
    l       = torch.logsumexp(lm, dim=0).reshape(1, m)
    return l

def entropyUpperBoundGaussianMixtureSingleCov(mu: torch.Tensor, S: torch.Tensor, logW: torch.Tensor) -> torch.Tensor:
    assert len(mu.shape) == 2
    [n,N]   = mu.shape
    assert N == logW.shape[1]
    ...
    w = logW.exp()
    logdetS = S.diag().abs().log().sum()
    Z = -logW + 0.5*n*(1+ torch.tensor(2*torch.pi, dtype=mu.dtype, device=mu.device).log()) + 0.5*logdetS
    H = w.inner(Z)
    return H

def getQuartiles(A, qvl=0.05, qvu=0.95):
...
def interQuartileRange(x: torch.Tensor):
...
class CalibrationDataset(Dataset):
    def __init__(self, mat_dir, files, maxSamples=-1, dtype=torch.float32, device="cpu"):
    ...
    def getItemFromFile(self, idx):
        ...
        if self.maxSamples>0:
            ...
            idx_choose      = np.random.choice(nValid, self.maxSamples)
        ...
        lidar_i     = torch.from_numpy(intensities_np).to(device=self.device, dtype=self.dtype)
...
def readCameraIntrinsicsFromYaml(camera_params_file : str) -> dict:
    assert os.path.isfile(camera_params_file), "Expected " + camera_params_file + " to be a file."
    ...
    names = ["camera_matrix", "distortion_coefficients", "rectification_matrix", "projection_matrix"]
    param = camera_params
    for name in names:
...
def vec2pix(rPCc: torch.Tensor, Kc: torch.Tensor, dist_theta: torch.Tensor) -> torch.Tensor:
    assert rPCc.shape[0] == 3, "Expected rPCc to have three rows, but its shape is " + str(rPCc.shape) + "."
    assert Kc.shape[0] == 3 & Kc.shape[1] == 3, "Expected Kc to have three rows and three columns, but its shape is " + str(Kc.shape)+ "."

    device  = rPCc.device
    dtype   = rPCc.dtype
    ntp     = dist_theta.shape[1]
    nt      = 12
    theta   = torch.hstack((dist_theta, torch.zeros(1,nt-ntp, device=device, dtype=dtype)))
...
def lidar2pix(Tlc: torch.Tensor, rPLl: torch.Tensor, Kc: torch.Tensor, dist_theta: torch.Tensor) -> torch.Tensor:
