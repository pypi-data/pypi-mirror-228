...
class CameraLiDARMutualInformation(torch.nn.Module):
    def __init__(
        ...
        self.useLieGroup = useLieGroup
        self.learnCamera = learnCamera
        ...
        self.xx = (
            torch.logspace(
                math.log10(1), math.log10(256), nbins, device=device, dtype=dtype
            )
            - 1
        )
    ...
    def getRelativePose(self) -> torch.Tensor:
        if self.useLieGroup:
            ...
            Tlc = torch.eye(4, device=self.device, dtype=self.dtype)
            rCLl = self.eta[0:3, [0]]
            Thetalc = self.eta[3:6, [0]]
            Tlc[0:3, 0:3] = rot.rot_zyx(Thetalc)
            Tlc[0:3, [3]] = rCLl
        return Tlc
    ...
    def forwardSingle(self, frame: dict) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        isConsidered = nrPLl < self.maxRange
        ...
        isValid = (isValidU & isValidV & isConsidered).flatten()
        I_image = gn.GaussianInterpolant(image, self.sigma, rQOi[:, isValid])

        return I_lidar.flatten()[isValid], I_image
    ...
    def cost(self, I_lidar, I_image) -> torch.Tensor:
        ...
        if self.entropy_approx == "trapz":
            ...
            HXY = torch.trapz(torch.trapz(fHXY, x), y)
