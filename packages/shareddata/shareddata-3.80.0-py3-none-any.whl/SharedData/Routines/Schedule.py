...
class Schedule:
    ...
    def UpdateRoutinesStatus(self):
        ...
        for i in dfpending.index:
            r = dfpending.loc[i]
            hasdependencies = (str(r['dependencies']) != '') & \
                (str(r['dependencies']) != 'nan')
            depcompleted = True
            if hasdependencies:
                dependencies = r['dependencies'].replace('\n', '').split(',')
                for dep in dependencies:
            ...
            if (not hasdependencies) | (depcompleted):
                if not r['isexternal']:
                    if (r['runmsgsent'] == False) & (r['lastmsg'] == 'nan'):
                        ...
                        if (idx.any()) & (r['lastmsgage'] <= 45):
                            sched.loc[i, 'status'] = 'RUNNING'
                        elif (r['lastmsgage'] <= 45):
                        ...
                        if (idx.any()) & (r['lastmsgage'] <= 45):
                            ...
                            if runmsgage <= 60:  # one minute to send first message
                                sched.loc[i, 'status'] = 'STARTING'
                            else:
                                sched.loc[i, 'status'] = 'RUNNING'
                        elif (r['lastmsgage'] > 45):
