...
__copyright__ = 'Copyright (C) 2020-2023 vb'
__version__ = '0.9.7'
__license__ = 'AGPLv3'
__author__ = 'vb'
...
def main():
    ...
    parser.add_argument('-p', f'--{Arguments.Name.PLAYLIST}', type=str,
                        help='Playlist ID ("PL-")')
    parser.add_argument('-k', f'--{Arguments.Name.COOKIE}', type=str,
                        help='Specify cookie file.')
    parser.add_argument('-l',  f'--{Arguments.Name.LINK_TO_SECTION}',  default='streams', type=str,
                        help='Specify a link to a section of the channel page. Option: "streams"(default), "videos", "shorts", "playlists"')
    ...
    if Arguments().cookie:
        print('\n[MESSAGE] Using cookie:' + Arguments().cookie + '\n')
    ...
    if Arguments().skip_duplicate:
        ...
        dir_videos.update([f[:11] for f in os.listdir(
            path) if os.path.isfile(os.path.join(path, f)) and f[-4:]=='.txt'])
            ...
            if Arguments().channels:
                if Arguments().link_to_section:
                    ...
                    if not arg in ( "streams","shorts","videos","playlists"):
                        ...
                        return
                Downloader(dir_videos).channels(Arguments().channels,tab=Arguments().link_to_section)
