import base64
from . import util, yt_data_extract, local_playlist, subscriptions
from . import yt_app
...
def get_channel_id(base_url):
    if base_url.startswith('UC'):
        return base_url # 'https://www.youtube.com/channel/' + base_url
    elif  base_url.startswith('@'):
        base_url =  'https://www.youtube.com/' + base_url
    # method that gives the smallest possible response at ~4 kb
    # needs to be as fast as possible
    base_url = base_url.replace('https://www', 'https://m') # avoid redirect

    # Some channels cannot be retrieved with '/about?pbj=1'. So omit this...
    # response = util.fetch_url(base_url + '/about?pbj=1', headers_mobile,
    response = util.fetch_url(base_url , headers_mobile,
...
def get_videos(channel_id, tab):
    # if duplicates > 20 then break 
    cache = set()
    cache.add("")
    counter = 0

    request = RequestStructure(page=1, sort='3', view='1')
    info = get_channel_page_general_url(
        base_url = 'https://www.youtube.com/channel/' + channel_id,
        request = request,
        tab=tab
    )
    cache_authorname = None    
    while True:
        ...
        for item in items:
            ...
            # Since the channel name cannot be retrieved from polymer_json after the second page, it should be cached.
            if cache_authorname is None:
                cache_authorname = video.get("author")
            if video.get("author") is None:
                video["author"] = cache_authorname
            yield video

        ctoken = info.get('ctoken')
        if ctoken is None:
            break
        polymer_json = get_channel_tab(channel_id=channel_id,tab=tab,ctoken=ctoken)
        info = yt_data_extract.extract_channel_info(json.loads(polymer_json), tab=tab,
                                                    continuation=ctoken)
...
def parse(item):
    ...
    try:
        ...
        video["time_published"] = item["time_published"]
