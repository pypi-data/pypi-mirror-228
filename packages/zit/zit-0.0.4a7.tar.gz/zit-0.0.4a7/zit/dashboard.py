...
def check_and_install_poetry(cn: bool = False):
    try:
    ...
    if cn:
        try:
            run_subprocess("poetry config repositories.aliyun https://mirrors.aliyun.com/pypi/simple/", shell=True)
            logger.info("Poetry source has been set to Aliyun.")
        except SubprocessError as e:
            logger.error("Error setting Poetry source to Aliyun:")
            logger.error(e.stderr)
            sys.exit(1)
