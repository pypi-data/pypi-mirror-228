...
def create(args, context: CLIContext, parser: argparse.ArgumentParser):
    ...
    if args.condition_json:
        ...
        if "operator" in condition_json.keys():
            condition = ConditionGroup.parse_obj(condition_json)
        elif "comparator" in condition_json.keys():
            condition = Condition.parse_obj(condition_json)
        else:
            parser.error(
...
commands = [create_command, get_command, delete_command, search_command]
