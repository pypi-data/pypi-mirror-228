...
class ReaderThread(threading.Thread):
    ...
    def __init__(self, *args, output_messages_queue, port, read_timeout=0.1, baudrate, reference_voltage, **kwargs):
        ...
        for frame in frames:
            ...
            if message:
                if MessageIds.has_value(message.header.messageId):
                    ret.append(decode_to_specific_message(message.header, decoded, reference_voltage=self.reference_voltage))
                    self.total_correct_messages_count += 1
                    logger.debug(f"Got message id {message.header.messageId}, len {len(message.payload)}")
                else:
                    logloggering.error("Unknown msgId " + str(message.header.messageId))
                    self.total_incorrect_messages_count += 1
            else:
                self.total_incorrect_messages_count += 1
                logger.error(f"Incorrect frame received: {error_msg}. Frame len: {len(frame)}")
        return ret
...
class AMSSerialModule():
    ...
    def __init__(self, *args, port="COM1", baudrate:int=1000_000, read_timeout=0.1, rx_max_queue_len=0, **kwargs):
        """
        ...
        self.reader_thread = ReaderThread(output_messages_queue=self.msg_queue, port=port, baudrate=baudrate, read_timeout=read_timeout, daemon=True, reference_voltage=self.reference_voltage)
    ...
    def workmode_freerunning(self, samples_count=0):
        ...
        Args:
            :samples_count (int, optional): Total number of samples to acquire. The module will enter STOP mode after collecting all samples. If zero, the module will stay in FREERUNNING mode. Defaults to 0.
        """
    ...
    def workmode_trigger_input(self, samples_count, delay):
        ...
        Args:
            :samples_count (int): Total number of samples to acquire after single trigger event.
            :delay (float): Delay between trigger event and start of sampling [in seconds].
        """
        logger.debug(f"Setting mode: trigger input with samples_count={samples_count} and delay={delay}")
    ...
    def workmode_trigger_output(self, samples_count, period, delay):
        ...
        Args:
            :samples_count (int): Number of samples to acquire after single trigger output event.
        ...
        """
        logger.debug(f"Setting mode: trigger output with samples_count={samples_count}, delay={delay}, period={period}")
    ...
    def workmode_simulation(self, samples: list, sample_size: SampleSize, noise_rms: float, period: int):
        ...
        logger.debug(f"Setting mode: simulation with sample_size={sample_size} and noise_rms={noise_rms}")
    ...
    def set_samplerate(self, physicalSampleRate:int):
        ...
        sampling_msg.physicalResolution = 1
        sampling_msg.processingDataResolution = 1
