...
def decode_to_specific_message(header, bytes, reference_voltage):
    if header.messageId == MessageIds.MESSAGE_OUTPUT_DATA.value:
        msg = MessageOutputData.from_buffer_and_fill_data(bytes, reference_voltage=reference_voltage)
        return msg
        ...
        if msg.sample_size == SampleSize.SAMPLE_SIZE_8b.value:
            ...
            msg = message_simulation_class(msg.samples_count, SampleSize.SAMPLE_SIZE_16b.value).from_buffer(bytes)
        elif msg.sample_size == SampleSize.SAMPLE_SIZE_32b.value:
            msg = message_simulation_class(msg.samples_count, SampleSize.SAMPLE_SIZE_32b.value).from_buffer(bytes)
        else:
...
class MessageProcessingSimpleAverage(AbstractMessage, AutoCastMessage, ctypes.Structure):
...
class MessageProcessingBufferDecimation(AbstractMessage, AutoCastMessage, ctypes.Structure):
...
class MessageOutputData(AbstractMessage, ctypes.Structure):
    ...
    _fields_ = [
        ...
        ('_data', ctypes.c_ubyte * 0), # data attribute will be set dynamically since it has variable size
    ]
    ...
    def from_buffer_and_fill_data(cls, source, reference_voltage, offset=0):
        ...
        if ret.sample_size == SampleSize.SAMPLE_SIZE_16b.value:
            ...
            ret.data = (ctypes.c_uint16 * count).from_buffer_copy(source, MessageOutputData._data.offset)
            divider = 65535
        elif ret.sample_size == SampleSize.SAMPLE_SIZE_8b.value:
            count = (len(source) - MessageOutputData._data.offset)
            ret.data = (ctypes.c_uint8 * count).from_buffer_copy(source, MessageOutputData._data.offset)
            divider = 255
            ...
            ret.data = (ctypes.c_uint32 * count).from_buffer_copy(source, MessageOutputData._data.offset)
            divider = 65536*65536 - 1
        else:
            raise Exception(f"Unknown sample_size = {ret.sample_size}")
        ret.voltages = (np.array(ret.data, dtype=np.float64) * 2 / divider - 1) * reference_voltage
        return ret
    
    def __repr__(self):
        return super().__repr__() + " : " + str(len(self.data)) + " samples. Cnt: " + str(self.counter) + ". SampleSize: " + str(self.sample_size)
...
class MessageConfigDetectorTemperature(AbstractMessage, AutoCastMessage, ctypes.Structure):
...
class MessageWorkmodeFreerunning(AbstractMessage, AbstractWorkmode, AutoCastMessage, ctypes.Structure):
...
class MessageWorkmodeStop(AbstractMessage, AbstractWorkmode, AutoCastMessage, ctypes.Structure):
...
class MessageWorkmodeTriggerInput(AbstractMessage, AbstractWorkmode, AutoCastMessage, ctypes.Structure):
...
class MessageWorkmodeTriggerOutput(AbstractMessage, AbstractWorkmode, AutoCastMessage, ctypes.Structure):
...
def message_simulation_class(samples_count, sample_size):
    payload_bytes = samples_count - ctypes.sizeof(Header) - ctypes.sizeof(ctypes.c_ubyte)
    if sample_size == SampleSize.SAMPLE_SIZE_8b.value or sample_size == SampleSize.SAMPLE_SIZE_8b:
        sample_class = ctypes.c_ubyte
        sample_size_bytes = 1
    elif sample_size == SampleSize.SAMPLE_SIZE_16b.value or sample_size == SampleSize.SAMPLE_SIZE_16b:
        sample_class = ctypes.c_uint16
        sample_size_bytes = 2
    elif sample_size == SampleSize.SAMPLE_SIZE_32b.value or sample_size == SampleSize.SAMPLE_SIZE_32b:
        sample_class = ctypes.c_uint32
        sample_size_bytes = 4
    else:
        raise ValueError(f"Wrong sample_size {sample_size}")
    if samples_count * sample_size_bytes > 8192:
        raise ValueError(f"Buffer overflow with total {samples_count * sample_size.value} bytes")
    ...
    class MessageWorkmodeSimulation(AbstractMessage, AutoCastMessage, ctypes.Structure):
        ...
        def __repr__(self):
            return super().__repr__() + " : " + str(len(self.samples)) + " samples, sample_size=" + str(self.sample_size) + ", noise_rms=" + str(self.noise_rms) + ", period=" + str(self.period_in_ms)
...
class MessageStatus(AbstractMessage, AutoCastMessage, ctypes.Structure):
    ...
    def __repr__(self):
        return f"Status: sampling={self.sampling_state}, det_temp={self.detector_temperature}, temp_ok={self.temperature_ok}"
