...
from flow_agent_package.tools.contracts import AgentSkillConfiguration, ArbitrationMethod
from flow_agent_package.tools.constants import format_response, intent_prompt, intent_prompt_with_history, extract_input, format_instructions_conversation, format_instructions_zero
...
from flow_agent_package.tools.agent_skill import AgentSkill


class ArbitrationAgent:

  def __init__(self, arbitration_method, agent_connection, deployment_name, skills, history):
    self.arbitration_method = arbitration_method
    self.agent_connection = agent_connection
    self.skills = skills
    self.deployment_name = deployment_name
    self.memory_helper = MemoryHelper(history)
  ...
  # TODO: Add better error handling to everything
  def use_langchain(self, query):
    ...
    for skill in self.skills:
      ...
      lang_tool = Tool(
        name=skill.name,
        func=run_func,
        description=skill.get_langchain_tool_description(), #TODO: Potentially use openai function desc - check with Prakhar
    ...
    if self.memory_helper.use_history():
      ...
      agent = initialize_agent(
              tools,
              llm,
              format_instructions = format_instructions_conversation,
              agent=agent_type,
              memory=memory,
              verbose=True)
    else:
...
def agent(
    ...
    deployment_name: str,
    skill_one: AgentSkillConfiguration,
    skill_two: AgentSkillConfiguration,
    skill_three: AgentSkillConfiguration
):
  ...
  deploy_config = PromptFlowRuntime.get_instance().config.deployment
  ...
  subscription_id = deploy_config.subscription_id if deploy_config.subscription_id != "" else "381b38e9-9840-4719-a5a0-61d9585e1e91"
  resource_group_name=deploy_config.resource_group if deploy_config.resource_group != "" else "baker-static"
  workspace_name=deploy_config.workspace_name if deploy_config.workspace_name != "" else "baker-eastus2"
  ...
  agent_skills = []
  agent_skills.append(AgentSkill(skill_one, pf, subscription_id, resource_group_name, workspace_name))
  agent_skills.append(AgentSkill(skill_two, pf, subscription_id, resource_group_name, workspace_name))
  agent_skills.append(AgentSkill(skill_three, pf, subscription_id, resource_group_name, workspace_name))

  arbitration_agent = ArbitrationAgent(arbitration_method, llm_connection, deployment_name, agent_skills, chat_history)

  return arbitration_agent.execute(query)
