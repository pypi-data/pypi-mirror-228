...
class MongoDBManager:
    ...
    def get_client(self) -> tuple((pymongo.MongoClient, str)):
        ...
        try:
            ...
            structlog.contextvars.bind_contextvars(function_name="get_client")
            logger.info(f"Unable to connect to the server - Error: {e}")
    ...
    def insert_document_in_collection(self, item: dict):
        """
    ...
    def update_document_in_collection(self, item: dict, collection_id: str, collection_id_name: str = None):
        ...
        collection.update_one(
            {
                collection_id_name if collection_id_name else f"{collection_name.lower()}_id": collection_id
            },
            {
                "$set": item
            },
    ...
    def update_many_documents_in_collection(self, items: list, collection_id_name: str = None):
        """
        Update document in MongoDB database and collection specified in the constructor of the class MongoDB
        ...
        for item in tqdm(items):
            item["update_date"] = datetime.datetime.now()
            filter_query = {
                collection_id_name if collection_id_name else f"{collection_name.lower()}_id": item[collection_id_name] if collection_id_name else item[f"{collection_name.lower()}_id"]
            }
    ...
    def get_all_documents_from_collection(self, mongodb_collection_name: str = None) -> list:
        """
    ...
    def get_distinct_documents_from_collection(self, mongodb_collection_name: str = None, field: str = None) -> list:
        """
