...
def get_instruments(symbols=None, exchanges=None, sec_types=None, names=None,
    ...
    convert_map = {"is_adjusted": "is_strike_price_adjusted"}
    if fields:
        fields = [
            convert_map.get(field)
            if field in convert_map else field
...
def get_history_instruments(symbols, fields=None, start_date=None, end_date=None, df=False):
    ...
    convert_map = {"is_adjusted": "is_strike_price_adjusted"}
    if fields:
        fields = ",".join(convert_map.get(
            field) if field in convert_map else field for field in fields.split(','))
...
def history(symbol, frequency, start_time, end_time, fields=None,
    ...
    if frequency == DATA_TYPE_TICK:
        ...
        datas = [protobuf_to_dict(bar) for bar in rsp.data]
...
def history_n(symbol, frequency, count, end_time=None, fields=None,
    ...
    if frequency == DATA_TYPE_TICK:
        ...
        rsp = Ticks()
        rsp.ParseFromString(result)
        datas = [protobuf_to_dict(bar) for bar
                 in rsp.data]
        fields_str, fields_list = standard_fields(fields, letter_upper=False)
        if not fields_list:
            datas = datas if not df else pd.DataFrame(datas)
