...
class AlignmentData:
    ...
    def setup(self) -> None:
        ...
        match_trajectories(traj_test=self.traj_from, traj_ref=self.traj_to, settings=self.matching_settings)
        ...
        self.set_var_vector(self._create_var_vector())
        logger.debug("Variance vector created.")
        self.set_res_vector(self._create_res_vector())
        logger.debug("Residual vector created.")

    @cached_property
    def observation_groups(self) -> dict[str, tuple[Union[int, None], Union[int, None]]]:
    ...
    def set_obs_vector(self, values: np.ndarray) -> None:
    ...
    def _create_obs_vector(
        ...
        """
        if self.alignment_settings.estimation_of.leverarm_enabled and rot_from is None:
            raise ValueError(
