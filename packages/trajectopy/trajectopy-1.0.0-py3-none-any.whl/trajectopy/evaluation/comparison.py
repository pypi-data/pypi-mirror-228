...
def _get_pair_indices_time(traj_ref: Trajectory, settings: ComparisonSettings, time_diff: float) -> np.ndarray:
    if settings.relative_settings.use_all_pose_pairs:
        time_steps = np.array(
            [
                [i, tstamp + time_diff]
                for i, tstamp in enumerate(traj_ref.tstamps)
                if (tstamp + time_diff) < traj_ref.tstamps[-1]
            ],
            dtype=float,
        )
        if len(time_steps) == 0:
    ...
    time_steps = np.arange(traj_ref.tstamps[0], traj_ref.tstamps[-1], time_diff)
...
def _get_pair_indices_dist(traj_ref: Trajectory, settings: ComparisonSettings, dist: float) -> np.ndarray:
    if settings.relative_settings.use_all_pose_pairs:
        dist_steps = np.array(
            [
                [i, arc_length + dist]
                for i, arc_length in enumerate(traj_ref.arc_lengths)
                if (arc_length + dist) < traj_ref.arc_lengths[-1]
            ],
            dtype=float,
        )
        if len(dist_steps) == 0:
    ...
    dist_steps = np.arange(traj_ref.arc_lengths[0], traj_ref.arc_lengths[-1], dist)
...
def pairwise_comparison(
    *, traj_test: Trajectory, traj_ref: Trajectory, settings: ComparisonSettings
    ...
    if settings.relative_settings.pair_min_distance > settings.relative_settings.pair_max_distance:
        raise ValueError("Maximum pose distance must be larger than minimum pose distance")

    pair_dists = np.arange(
        settings.relative_settings.pair_min_distance,
        settings.relative_settings.pair_max_distance + settings.relative_settings.pair_distance_step,
        settings.relative_settings.pair_distance_step,
    )
    ...
    for pair_dist in pair_dists:
        ...
        if settings.relative_settings.pair_distance_unit == Unit.METER:
            pair_indices = _get_pair_indices_dist(traj_ref, settings, dist=pair_dist)

        elif settings.relative_settings.pair_distance_unit == Unit.SECOND:
            pair_indices = _get_pair_indices_time(traj_ref, settings, time_diff=pair_dist)

        else:
            raise ValueError(f"Unknown unit {settings.relative_settings.pair_distance_unit}")
    ...
    return RelativeTrajectoryDeviations(
        ...
        pair_distance_unit=settings.relative_settings.pair_distance_unit,
    )
