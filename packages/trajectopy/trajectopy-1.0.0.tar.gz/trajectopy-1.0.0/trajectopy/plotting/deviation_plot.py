...
from trajectopy.trajectory import Sorting
import seaborn as sns
import trajectopy.util.datahandling as datahandling
from trajectopy.evaluation.trajectory_deviations import (
    DeviationCollection,
    AbsoluteTrajectoryDeviations,
    RelativeTrajectoryDeviations,
)
from trajectopy.plotting.util import norm_hist, scatter_plotter, stair_hist, vertical_subplots
...
def plot_combined_devs(devs: AbsoluteTrajectoryDeviations, plot_settings: PlotSettings = PlotSettings()) -> Figure:
    if devs.rot_dev is None:
        comb_devs = [[devs.comb_pos_devs * plot_settings.unit_multiplier]]
        ylabels = [f"position deviations {plot_settings.unit_str}"]
    else:
        comb_devs = [[devs.comb_pos_devs * plot_settings.unit_multiplier, np.rad2deg(devs.comb_rot_devs)]]
        ylabels = [f"position deviations {plot_settings.unit_str}", "minimum rotation angle [°]"]

    return vertical_subplots(
        x_list=[devs.function_of] * len(comb_devs),
        y_list=comb_devs,
        xlabel=XLABEL_DICT[devs.sorting],
        ylabels=ylabels,
...
def plot_raw_rotation_devs(devs: AbsoluteTrajectoryDeviations, plot_settings: PlotSettings = PlotSettings()) -> Figure:
    ...
    return vertical_subplots(
        x_list=[devs.function_of] * len(vert_sp_data_rot),
...
def plot_raw_position_devs(devs: AbsoluteTrajectoryDeviations, plot_settings: PlotSettings = PlotSettings()) -> Figure:
    deviations_xa = devs.directed_pos_dev[:, 0] if plot_settings.show_directed_devs else devs.pos_dev[:, 0]
    deviations_yh = devs.directed_pos_dev[:, 1] if plot_settings.show_directed_devs else devs.pos_dev[:, 1]
    deviations_zv = devs.directed_pos_dev[:, 2] if plot_settings.show_directed_devs else devs.pos_dev[:, 2]
    ...
    if plot_settings.show_directed_devs:
        ...
        ylabels = [f"x {plot_settings.unit_str}", f"y {plot_settings.unit_str}", f"z {plot_settings.unit_str}"]
    ...
    return vertical_subplots(
        x_list=[devs.function_of] * len(vert_sp_data_pos),
...
def plot_position_dof(devs: AbsoluteTrajectoryDeviations, plot_settings: PlotSettings = PlotSettings()):
    ...
    rows = 1 if devs.rot_dev is None else 2
    xyz_dev = [devs.pos.xyz]
...
def compact_deviations(
    ...
    if devs.rot_dev:
        ...
        data_rms_plot = [comb_pos_rms * plot_settings.unit_multiplier, np.rad2deg(comb_rot_rms)]
        titles_rms = [
            f"position rms ({plot_settings.rms_window_width:.2f} {UNIT_DICT[devs.sorting]} window width)",
            f"rotation rms ({plot_settings.rms_window_width:.2f} {UNIT_DICT[devs.sorting]} window width)",
        ]
        c_labels_dev = [plot_settings.unit_str, "[°]"]

    else:
        data_rms_plot = [comb_pos_rms * plot_settings.unit_multiplier]
        titles_rms = [f"position rms ({plot_settings.rms_window_width:.2f} {UNIT_DICT[devs.sorting]} window width)"]
        c_labels_dev = [plot_settings.unit_str]
...
def plot_position_edf(
    deviation_list: list[AbsoluteTrajectoryDeviations], plot_settings: PlotSettings = PlotSettings()
) -> None:
...
def plot_rotation_edf(deviation_list: list[AbsoluteTrajectoryDeviations]) -> None:
    if all(dev.rot_dev is None for dev in deviation_list):
        return
...
def plot_bars(
    ...
    spacings = np.linspace(
        -bar_width * (len(deviation_list) - 1) / 2, bar_width * (len(deviation_list) - 1) / 2, len(deviation_list)
    )
    x_positions = np.arange(len(characteristics))
    for deviation, spacing in zip(deviation_list, spacings):
        if mode == "rotations" and deviation.rot_dev is None:
            continue
...
def rms_heatmap(
    deviation_collection: DeviationCollection, plot_settings: PlotSettings = PlotSettings()
) -> Tuple[Figure, Union[Figure, None]]:
    pos_rms_fig = plot_position_rms_heatmap(deviation_collection, plot_settings)
    rpy_rms_fig = plot_rotation_rms_heatmap(deviation_collection, plot_settings)

    return pos_rms_fig, rpy_rms_fig


def plot_rotation_rms_heatmap(
    deviation_collection: DeviationCollection, plot_settings: PlotSettings = PlotSettings()
) -> Union[Figure, None]:
    ...
    sns.heatmap(
        ...
        cmap="YlOrRd",
        fmt=".3f",
        linewidth=plot_settings.heatmap_spacing,
        cbar_kws={"label": "[°]"},
    )
    rpy_rms_fig.subplots_adjust(left=0.3)
    ax.set_xticklabels(["roll rms", "pitch rms", "yaw rms"], rotation=45)
    ax.set_yticklabels(deviation_collection.rpy_labels, rotation=0)
...
def plot_position_rms_heatmap(
    ...
    sns.heatmap(
        ...
        fmt=".3f",
        linewidth=plot_settings.heatmap_spacing,
        cbar_kws={"label": plot_settings.unit_str},
    )
    pos_rms_fig.subplots_adjust(left=0.3)
    ax.set_xticklabels(pos_labels, rotation=45 if plot_settings.show_directed_devs else 0)
    ax.set_yticklabels(deviation_collection.names, rotation=0)
...
def bias_heatmap(
    ...
    rpy_bias_fig = plot_rotation_bias_heatmap(deviation_collection, plot_settings)

    return pos_bias_fig, rpy_bias_fig


def plot_rotation_bias_heatmap(
    deviation_collection: DeviationCollection, plot_settings: PlotSettings = PlotSettings()
) -> Union[Figure, None]:
    ...
    rpy_bias_fig.subplots_adjust(left=0.3)
    ax.set_xticklabels(["bias roll", "bias pitch", "bias yaw"], rotation=45)
    ax.set_yticklabels(deviation_collection.rpy_labels, rotation=0)
...
def plot_position_bias_heatmap(
    ...
    pos_bias_fig.subplots_adjust(left=0.3)
    ax.set_xticklabels(pos_labels, rotation=45 if plot_settings.show_directed_devs else 0)
    ax.set_yticklabels(deviation_collection.names, rotation=0)
    return pos_bias_fig
...
def plot_multiple_comb_deviations(
    ...
    if all(dev.state.sorting_known for dev in deviation_list):
        x_label = "Trajectory length [m]"
        target_sorting = Sorting.SPATIAL
    else:
        target_sorting = Sorting.CHRONO
        x_label = "Time [s]"
    ...
    if any(dev.rot_dev for dev in deviation_list):
        ax_rot = plt.subplot(2, 1, 2)
    ...
    for dev in deviation_list:
        dev_sorted: AbsoluteTrajectoryDeviations = dev.set_sorting(target_sorting, inplace=False)
        if len(dev_sorted.function_of) == 0:
            logger.warning("Skipping %s as it has no data", dev.name)
            continue

        arc_length_sorting = np.argsort(dev_sorted.function_of)
        ax_pos.plot(
            dev_sorted.function_of[arc_length_sorting],
            dev_sorted.comb_pos_devs[arc_length_sorting] * plot_settings.unit_multiplier,
        )
        if ax_rot is not None:
            ax_rot.plot(
                dev_sorted.function_of[arc_length_sorting], np.rad2deg(dev_sorted.comb_rot_devs[arc_length_sorting])
            )
...
def plot_multiple_deviations(
    ...
    if plot_settings.show_directed_devs:
        ...
        pos_labels = [f"x {plot_settings.unit_str}", f"y {plot_settings.unit_str}", f"z {plot_settings.unit_str}"]
    ...
    xyz_dev_fig = _plot_components(
        x_list=deviation_collection.lengths,
        y_list=pos_dev,
        ylabels=pos_labels,
    ...
    # rpy
    if deviation_collection.rpy_dev and deviation_collection.lengths:
        rpy_dev_fig = _plot_components(
            x_list=deviation_collection.lengths,
            y_list=deviation_collection.rpy_dev,
            ylabels=rpy_labels,
            legend_list=deviation_collection.rpy_labels,
            sharey=True,
            deg=True,
        )
    else:
...
def plot_rpe(devs: list[RelativeTrajectoryDeviations]) -> Tuple[Figure, Figure]:
    ...
    ret_sum = 1 if any(dev.pair_distance_unit == Unit.METER for dev in devs) else 0
    if any(dev.pair_distance_unit == Unit.SECOND for dev in devs):
        ret_sum += 2

    return {0: (None, None), 1: (fig_metric, None), 2: (None, fig_time), 3: (fig_metric, fig_time)}[ret_sum]


def _plot_rpe_pos(figure_dict: dict[Unit, Axes], devs: list[RelativeTrajectoryDeviations]) -> None:
    for dev in devs:
        line_plot = figure_dict[dev.pair_distance_unit].plot(dev.mean_distances, dev.mean_pos_devs, label=dev.name)

        if len(devs) > len({dev.pair_distance_unit for dev in devs}):
            continue

        violin_plot = figure_dict[dev.pair_distance_unit].violinplot(
            [
                [val * dev.drift_factor for val in pos_list]
                for pos_list in list(dev.result_dict["pos_dev"].values())
                if pos_list
            ],
...
def _plot_rpe_rot(figure_dict: dict[Unit, Axes], devs: list[RelativeTrajectoryDeviations]) -> None:
    ...
    for dev in devs:
        ...
        plot_sum += 1
        line_plot = figure_dict[dev.pair_distance_unit].plot(dev.mean_distances, np.rad2deg(dev.mean_rot_devs), label=dev.name)
        ...
        violin_plot = figure_dict[dev.pair_distance_unit].violinplot(
            [
                ...
                for rot_list in list(dev.result_dict["rot_dev"].values())
                if rot_list
            ],
            positions=dev.mean_distances,
            showmeans=True,
