...
import trajectopy.io.trajectory_io as trajectory_io
from trajectopy.alignment.functional_model.equations import leverarm_time_component
from trajectopy.alignment.parameters import AlignmentParameters
from trajectopy.util.rotationset import RotationSet
from trajectopy.util.spatialsorter import Sorting, SpatialSorter, detect_laps_by_length, infer_sort_index
...
class Trajectory:
    ...
    def __init__(
        ...
        self._setup_optional_data(sort_index=sort_index, arc_lengths=arc_lengths, speed_3d=speed_3d)
        ...
        self._state = state or TrajectoryProcessingState(sorting_known=sorting == Sorting.SPATIAL)
    ...
    def to_file(self, filename: str, mode: str = "w") -> None:
        ...
        if self.rot is None:
            ...
            trajectory_data = np.c_[self.tstamps, self.arc_lengths, self.pos.xyz, self.rot.as_quat(), self.speed_3d]
    ...
    def divide_into_laps(self) -> Union[None, list]:
        ...
        for i in range(len(lap_indices) - 1):
            lap_trajectory = self.crop(
                t_start=chrono_tstamps[lap_indices[i]], t_end=chrono_tstamps[lap_indices[i + 1] - 1], inplace=False
    ...
    def interpolate(self, tstamps: Union[list, np.ndarray], inplace: bool = True) -> "Trajectory":
        ...
        tstamps_cropped = traj_self._crop_timestamps(tstamps)

        traj_self.pos.xyz = traj_self._interpolate_positions(tstamps_cropped)
        traj_self.rot = traj_self._interpolate_rotations(tstamps_cropped)
        traj_self._speed_3d = datahandling.gradient_3d(xyz=traj_self.pos.xyz, tstamps=tstamps_cropped)
        traj_self.arc_lengths = np.interp(tstamps_cropped, traj_self.tstamps, traj_self.arc_lengths)
        traj_self.tstamps = tstamps_cropped

        # rebuild index
        traj_self._sort_index = np.arange(0, len(traj_self.pos), dtype=int)
        traj_self._state.sorting_known = False
        traj_self._state.interpolated = True

        return traj_self

    def _crop_timestamps(self, tstamps: Union[list, np.ndarray]) -> np.ndarray:
        """
        Helper function to crop tstamps so they fit inside
        the timestamp range of the trajectory.

        This method is called internally by interpolation().

        Args:
            tstamps (np.ndarray): Timestamps that should be cropped.

        Returns:
            np.ndarray: Timestamps that lie within the timespan of the
                        trajectory. Empty if there is no overlap.
        """
        tstamps_cropped = np.array([tstamp for tstamp in tstamps if self.tstamps[0] <= tstamp <= self.tstamps[-1]])

        if len(tstamps_cropped) < len(tstamps):
            logger.warning(
                (
                    f"interpolate: Some timestamps are not within the range ["
                    f"{self.tstamps[0]:.2f}, {self.tstamps[-1]:.2f}]"
                    f", both inclusive. Those timestamps were ignored during interpolation."
                )
            )

        return tstamps_cropped

    def _interpolate_rotations(self, tstamps: Union[list, np.ndarray]) -> Union[RotationSet, None]:
        """Helper function for rotation interpolation of a trajectory
        ...
        slerp = Slerp(self.tstamps, self.rot)
        r_i = slerp(tstamps)
        return RotationSet.from_quat(r_i.as_quat())

    def _interpolate_positions(self, tstamps: np.ndarray) -> np.ndarray:
        """Helper function for position interpolation of a trajectory
        ...
        """
        x_i = np.interp(tstamps, self.tstamps, self.pos.x)
        y_i = np.interp(tstamps, self.tstamps, self.pos.y)
        z_i = np.interp(tstamps, self.tstamps, self.pos.z)
        return np.c_[x_i, y_i, z_i]
    ...
    def intersect(self, tstamps: np.ndarray, max_gap_size: float = 2.0, inplace: bool = True) -> "Trajectory":
        ...
        traj_self._state.intersected = True
    ...
    def same_sampling(self, other: "Trajectory", inplace: bool = True) -> Tuple["Trajectory", "Trajectory"]:
        ...
        traj_self._adopt_sampling_and_sorting(traj_other)
    ...
    def _adopt_sampling_and_sorting(self, traj_other: "Trajectory") -> None:
        ...
        Always in-place, as this is an internal method.
        ...
        """
        self.interpolate(traj_other.tstamps)
        self.arc_lengths = copy.deepcopy(traj_other.arc_lengths)
        self._sort_index = copy.deepcopy(traj_other.sort_index)
        self._sorting = traj_other.sorting
        self._state.sorting_known = traj_other.state.sorting_known
    ...
    def apply_index(self, index: Union[list, np.ndarray], inplace: bool = True) -> "Trajectory":
        ...
        traj_self._sort_index = traj_self._sort_index[index]

        if traj_self._speed_3d is not None:
            traj_self._speed_3d = traj_self._speed_3d[index]
    ...
    def append(self, trajectory: "Trajectory", inplace: bool = True) -> Union["Trajectory", None]:
        ...
        traj_self._speed_3d = np.concatenate((traj_self.speed_3d, trajectory.speed_3d), axis=0)
        ...
        traj_self._sorting = Sorting.CHRONO
    ...
    def apply_sorter(self, sorter: SpatialSorter, inplace: bool = True) -> "Trajectory":
        ...
        traj_self._sort_index = np.array(sorter.idx_sort, dtype=int)
        traj_self.arc_lengths = sorter.function_of

        traj_self._sorting = Sorting.SPATIAL
        traj_self._state.sorting_known = True
    ...
    def set_sorting(self, sorting: Sorting, inplace: bool = True) -> "Trajectory":
        ...
        if traj_self._sorting == sorting:
            return traj_self

        if traj_self._sorting == Sorting.CHRONO and sorting == Sorting.SPATIAL and not traj_self.state.sorting_known:
            raise TrajectoryError(
                (
                    f"Could not change sorting from {traj_self._sorting} to {sorting}"
        ...
        traj_self._switch_sorting()
        traj_self._sorting = sorting

        return traj_self

    def _switch_sorting(self) -> None:
        """Switch sorting to previous state

        Internal method that switches the sorting
        in-place.
