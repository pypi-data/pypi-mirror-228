<div align="center">
    ...
    <br />
    <img src=.images/uni-bonn.svg height="50"/>
    <img src=.images/igg.png height="50"/>
...
On the other hand, the "Nearest-Spatial" method finds the closest data points between the two trajectories based on a distance metric. This approach can be beneficial when the timestamps are relatively similar, but some discrepancies still exist, and a direct one-to-one correspondence needs to be established.

To evaluate both aligned trajectories, you can select them and use the "Compare with Reference" option. In this example, we conduct an absolute comparison using timestamp-based matching. The deviations calculated during the comparison are added to the results table. You can visualize these deviations by right-clicking and selecting the "Plot" option. The program supports plotting a single trajectory comparison or multiple comparisons simultaneously.
...
5. Compute transformation between 3) and 4)
    ...
    $ \Delta =  \Delta_{P~ref}^{-1} \cdot \Delta_{P~est}$
