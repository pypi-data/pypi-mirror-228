...
class GetStatsCommand(qumulo.lib.opts.Subcommand):
    ...
    def main(rest_client: RestClient, _args: Namespace) -> None:
        print(fs.read_fs_stats(rest_client.conninfo, rest_client.credentials))
...
class LockIter:
    def __init__(
        ...
        self.dns_config_id = dns_config_id
    ...
    def _advance_to_page(self, new_page: Dict[str, Any]) -> None:
        ...
        if self.want_hostnames:
            ...
            if new_ips:
                new_hostnames = self.rest_client.dns.resolve_ips_to_names(
                    new_ips, dns_config_id=self.dns_config_id
                )
...
class ListLocksCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            type=int,
            help=(
                'The DNS config ID used to resolve. This value is required when resolution is'
                ' required.'
            ),
        )
    ...
    def main(rest_client: RestClient, args: Namespace) -> None:
        ...
        if args.resolve and not args.dns_config_id:
            raise ValueError('Resolving requires --dns-config-id')
