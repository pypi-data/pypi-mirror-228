...
class SemanticObjectSpecificLogic(object):
    ...
    def __init__(self, semantic_object):
        if not isinstance(semantic_object, self.TARGET_CLASS):
            raise UnexpectedObjectClassException("Expected object of class %s but got %s" %
                                                 (self.TARGET_CLASS, semantic_object))
...
class LambdaAbstractedPredicatePropositionLogic(SemanticObjectAndOntologySpecificLogic):
    ...
    def combine_with(self, answer):
        if (answer.is_proposition() and
            (answer.getPredicate() == self._semantic_object.getPredicate() or
             answer.getPredicate().is_feature_of(self._semantic_object.getPredicate()))):
            return answer
    ...
    def is_resolved_by(self, answer):
        try:
            return (answer.getPredicate() == self._semantic_object.getPredicate()
...
class PredicatePropositionLogic(PropositionLogic, SemanticObjectAndOntologySpecificLogic):
    ...
    def relevant(self, answer):
        return (self.is_combinable_with(answer) or
                self._has_feature_combinable_with(answer) or
                self._semantic_object._is_feature(answer))

    def _has_feature_combinable_with(self, answer):
        return (answer.is_individual() and
                any([predicate for predicate in list(self._ontology.get_predicates().values()) if predicate.is_feature_of(self._semantic_object.predicate) and predicate.getSort() == answer.getSort()]))
...
class GoalPropositionLogic(SemanticObjectSpecificLogic):
    ...
    def is_combinable_with(self, answer):
        if answer.is_yes() or answer.is_no():
            return True
        elif answer.is_proposition() and answer.is_goal_proposition() and answer.get_goal() == self._semantic_object.get_goal():
            return True
        else:
            return False
...
class PropositionSetLogic(SemanticObjectSpecificLogic):
    ...
    @property
    def _domain(self):
        ddd_components = self._ddd_component_manager.get_ddd_specific_components_of_ontology_name(
            self._semantic_object.ontology_name)
