...
class Client(object):
    ...
    @staticmethod
    def describe_taiji_hy_specs():
        """获取太极HY计费项信息

        :rtype:     tikit.tencentcloud.tione.v20211111.models.DescribeTaijiHYSpecsResponse
        """
        specs = [
            {"SpecName": "TI.GT4.1HUNYUAN.POST", "SpecAlias": "A100*1", "SpecId": "87.63"},
            {"SpecName": "TI.GT4.2HUNYUAN.POST", "SpecAlias": "A100*2", "SpecId": "175.29"},
            {"SpecName": "TI.GT4.4HUNYUAN.POST", "SpecAlias": "A100*4", "SpecId": "363.39"},
            {"SpecName": "TI.GT4.8HUNYUAN.POST", "SpecAlias": "A100*8", "SpecId": "726.78"},
            {"SpecName": "TI.GT4.16HUNYUAN.POST", "SpecAlias": "A100*16", "SpecId": "1,453.56"},
            {"SpecName": "TI.GN10.1HUNYUAN.POST", "SpecAlias": "V100*1", "SpecId": "55.20"},
            {"SpecName": "TI.GN10.2HUNYUAN.POST", "SpecAlias": "V100*2", "SpecId": "111.66"},
            {"SpecName": "TI.GN10.4HUNYUAN.POST", "SpecAlias": "V100*4", "SpecId": "220.80"},
            {"SpecName": "TI.GN10.8HUNYUAN.POST", "SpecAlias": "V100*8", "SpecId": "441.63"},
            {"SpecName": "TI.GN10.16HUNYUAN.POST", "SpecAlias": "V100*16", "SpecId": "883.26"}
        ]

        def gen_spec(d) -> models.Spec:
            s = models.Spec()
            s.SpecName = d["SpecName"]
            s.SpecAlias = d["SpecAlias"]
            s.SpecId = d["SpecId"]
            return s

        rsp = models.DescribeTaijiHYSpecsResponse()
    ...
    def describe_taiji_hy_tasks(self, filters=None, tag_filters=None, offset=0, limit=50, order="DESC",
        ...
        :return:
        :rtype: :class:`tikit.tencentcloud.tione.v20211111.models.DescribeTaiJiHYTasksResponse`
        """
        try:
            # 增加 TAIJI_HY 过滤
            if filters is None:
                filters = []
            filters.append({"Name": "TaskType", "Values": ["TAIJI_HY"]})

            # 先获取任务列表
            resp = self.describe_training_tasks(
                filters=filters,
            ...
            # 转成 TAIJI_HY 任务列表结果
            return models.DescribeTaiJiHYTasksResponse(
                training_task_set=resp.TrainingTaskSet,
    ...
    def create_taiji_hy_training_task(self,
        ...
        try:
            ...
            req.Remark = remark
            req.Tags = tags
            req.TAIJITemplateId = template_name
            req.TuningParameters = json.dumps(tuning_parameters_dict)
            req.ChargeType = worker_resource.ChargeType
            req.TaskType = "TAIJI_HY"
            worker_info = models.ResourceConfigInfo()
            worker_info.Role = "WORKER"
            worker_info.InstanceNum = worker_resource.InstanceNum
            worker_info.Cpu = worker_resource.Cpu
            ...
            if base_model_id is not None:
                pre_train_model = models.PreTrainModel()
                pre_train_model.ModelId = base_model_id
                req.PreTrainModel = pre_train_model
    ...
    def describe_train_logs(self, pod_name, start_time=None, end_time=None, limit=None, order=None,
        ...
        def get_next_data(xx):
            if result.Context != "":
                next_result = self.describe_train_logs(pod_name, start_time, end_time, limit, order, result.Context,
                                                       filters)
    ...
    def _append_model_by_task(self,
                              req,
                              training_job_id,
    ...
    def create_model_by_task(self,
        ...
        return self._create_model_by_task(req,
                                          ...
                                          tags,
                                          training_model_cos_path, is_qat)

    def create_taiji_hy_model_by_task(self,
                                      training_model_name,
                                      training_job_id,
                                      tags=None):
    ...
    def append_model_into_existed_version_by_task(self, 
        ...
        try:
            ...
            self._check_model_clean_params(req, model_auto_clean, model_clean_period, max_reserved_models)
            return self._append_model_by_task(req,
                                              training_job_id,
                                              training_model_cos_path,
                                              reasoning_env,
                                              model_format,
    ...
    def describe_taiji_hy_models(self, filters=None, order_field=None, order=None, offset=None, limit=None,
                                 tag_filters=None):
        """查看taiji_hy模型列表
    ...
    def delete_training_model_version(self, training_model_version_id, enable_delete_cos=True):
        """删除模型版本
        ...
        try:
            ...
            if model_info is not None:
                ...
                if model_info.ModelType == "NORMAL":
                    ...
                    model_version_list = self.describe_training_model_versions(modelInfo.ModelId)
                    for model_version_tmp in model_version_list.TrainingModelVersions:
                        if model_version_tmp.TrainingModelVersion == model_info.ModelVersion:
                            modelInfo.ModelVersion = model_info.ModelVersion
                            modelInfo.ModelVersionId = model_version_tmp.TrainingModelVersionId
                            modelInfo.AlgorithmFramework = model_version_tmp.AlgorithmFramework
                            modelInfo.CosPathInfo = model_version_tmp.TrainingModelCosPath
                            modelInfo.ModelSource = model_version_tmp.TrainingModelSource

                            imageInfo.ImageUrl = model_version_tmp.ReasoningEnvironment
                            imageInfo.ImageType = model_version_tmp.ReasoningEnvironmentSource
                            break
                    if modelInfo.ModelVersionId is None:
                        raise Exception("model version %s not found" % model_info.ModelVersion)
                elif model_info.ModelType == "ACCELERATE":
                    model_list = self._describe_model_accelerate_versions(
                        filters=self._new_name_filter("ModelJobName", model_info.ModelName))
                    if model_list.TotalCount == 0:
                        raise Exception("model %s not found" % model_info.ModelName)
                    for model_tmp in model_list.ModelAccelerateVersions:
                        if model_tmp.ModelVersion == model_info.ModelVersion:
                            ...
                            modelInfo.AlgorithmFramework = model_tmp.ModelSource.AlgorithmFramework
                            imageInfo.ImageUrl = model_tmp.ModelSource.ReasoningEnvironment
                            imageInfo.ImageType = model_tmp.ModelSource.ReasoningEnvironmentSource
                            break
    ...
    def describe_batch_tasks(self, filters=None, tag_filters=None, offset=0, limit=50, order="DESC",
                             order_field="UpdateTime"):
        """获取跑批任务列表
    ...
    def _describe_model_accelerate_versions(self, filters=None, order_field=None, order=None, offset=0, limit=50):
        try:
    ...
    def create_model_service_version(self,
                                     worker_resource,
                                     framework,
    ...
    def create_taiji_model_service(self,
        ...
        try:
            ...
            req.ChargeType = "POSTPAID_BY_HOUR"
            req.ServiceGroupName = service_name
            req.ServiceDescription = service_description
            req.ModelInfo = models.ModelInfo()
            req.ModelInfo.ModelVersionId = model_version_id
            req.ServiceCategory = "TAIJI_HY"
            req.AuthorizationEnable = enable_auth
            return self._tione_client.CreateModelService(req)
        except TencentCloudSDKException as err:
            raise

    def describe_taiji_model_service(self, service_id: str):
        """查询太极混元服务信息

        :param service_id: 混元服务ID
        :type service_id: str
        :return:
    ...
    def modify_taiji_model_service_replicas(self, service_id: str, replicas: int):
        """更新太极混元模型服务副本数
    ...
    def stop_taiji_model_services(self, service_id: str):
        """停止太极混元服务
        ...
        try:
            req = models.ModifyModelServiceRequest()
            req.ServiceId = service_id
            req.ServiceAction = "STOP"
            req.ServiceCategory = "TAIJI_HY"
            return self._tione_client.ModifyModelService(req)
        except TencentCloudSDKException as err:
            raise

    def delete_taiji_model_service(self, service_id: str):
        """删除太极混元模型服务
    ...
    def _create_model_service_from_file(self, filename: str):
        ...
        if (result.Service and result.Service.ServiceGroupId and result.Service.ServiceId):
    ...
    def _create_model_service_file_to_arguments(self, filename: str)-> dict:
        ...
        if data.get('new_version'):
            ...
            if not service_group_id:
                raise ValueError('service_group_id should not be empty when you want to add a new version')
        ...
        if charge_type == 'PREPAID':
            if not data.get('resource_group_id'):
                raise ValueError('resource_group_id should not be empty when charge_type is PREPAID')
            result['resource_group_id'] = data['resource_group_id']
            resources = data.get('resources')
            if not resources:
                raise ValueError('resources should not be empty when charge_type is PREPAID')
            result['worker_resource'] = TiModels.ModelServiceResourceConfigInfo.new_prepaid(
                resources.get('cpu') / 1000,
                resources.get('memory') / 1024,
                gpu=None if not resources.get('gpu') else resources['gpu'] / 100,
                gpu_type=None if not resources.get('gpu_type') else resources['gpu_type']
            )
        elif charge_type == 'POSTPAID_BY_HOUR':
            instance_type = data.get('instance_type')
            if not instance_type:
                raise ValueError('instance_type should not be empty when charge_type is POSTPAID_BY_HOUR')
            result['worker_resource'] = TiModels.ModelServiceResourceConfigInfo.new_postpaid(
                instance_type
            )
        elif charge_type == 'HYBRID_PAID':
            if not data.get('resource_group_id'):
                raise ValueError('resource_group_id should not be empty when charge_type is HYBRID_PAID')
            result['resource_group_id'] = data['resource_group_id']
            instance_type = data.get('instance_type')
            if not instance_type:
                raise ValueError('instance_type should not be empty when charge_type is HYBRID_PAID')
            result['worker_resource'] = TiModels.ModelServiceResourceConfigInfo.new_hybridpaid(instance_type)
            hybrid_billing_prepaid_replicas = data.get('hybrid_billing_prepaid_replicas')
            if not hybrid_billing_prepaid_replicas:
                raise ValueError('hybrid_billing_prepaid_replicas should not be empty when charge_type is HYBRID_PAID')
        ...
        if model_version_id:
            result['model_config_info'] = TiModels.ModelConfigInfo.new_model_reference_lite(
        ...
        if _svc_limit:
            ...
            service_limit.EnableInstanceRpsLimit = _svc_limit.get('enable_instance_rps_limit', False)
    ...
    def _modify_model_service_file_to_arguments(self, filename: str)-> dict:
        ...
        if not service_id:
            raise ValueError('service_id should not be empty when you want to add a new version')
    ...
    def stop_model_service(self, service_id:str):
        ...
        """
        try:
            return self.modify_model_service(service_id=service_id, service_action='STOP')
        except TencentCloudSDKException as err:
            raise
    ...
    def start_model_service(self, service_id):
        ...
        """
        try:
            return self.modify_model_service(service_id=service_id, service_action='RESUME')
        except TencentCloudSDKException as err:
            raise
