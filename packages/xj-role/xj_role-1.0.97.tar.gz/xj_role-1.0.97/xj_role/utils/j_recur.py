...
class JRecur:
    ...
    def create_tree(source_list, search_root_key, search_root_value, primary_key="id", parent_key="parent_id",
        ...
        result_tree = recur_node(
            parent_node=result_tree,
            source_dict=source_index_dict,
            pk=primary_key,
            ck=children_key
        )
        return result_tree
    ...
    def create_forest(source_list, primary_key="id", parent_key="parent_id", children_key='children'):
        ...
        def recur_node(parent_node={}, source_dict={}, pk='id', ck='children'):
            ...
            parent_node[ck] = source_dict.get(pid_str, [])
            for child in parent_node[ck]:
                recur_node(child, source_dict, pk=pk, ck=ck)
            return parent_node

        for index, item in enumerate(result_forest):
            result_forest[index] = recur_node(
                parent_node=item,
                source_dict=source_index_dict,
                pk=primary_key,
                ck=children_key
    ...
    def get_value_in_forest(source_forest=[], field="id", children_key='children', ):
        ...
        for it in source_forest:
            recur_node(it, k=field, ck=children_key)

        return [k for k in result_dict]

    @staticmethod
    def recursion_forest_handler(source_forest=None, callback=None, children_key='children'):
        """
        自定义回调函数处理树
        :param source_forest: 得到的树结构数据
        :param callback: 回调函数
        :param children_key: 子节点key
        :return:
        """
        ...
        # 递归树 递归每个节点执行回调
        def recur_node(node={}, method=None, ck='children'):
            get_value = ctypes.cast(id(node), ctypes.py_object).value
            print("get_value:", get_value)
            node = method(node)
            if node.get(ck, None):
                for child in node[ck]:
                    recur_node(child, method, ck)
            return node

        # 树结构
        if isinstance(source_forest, list):
            for it in source_forest:
                recur_node(it, method=callback, ck=children_key)
        elif isinstance(source_forest, dict):
            source_forest = recur_node(source_forest, method=callback, ck=children_key)
        else:
            return source_forest
