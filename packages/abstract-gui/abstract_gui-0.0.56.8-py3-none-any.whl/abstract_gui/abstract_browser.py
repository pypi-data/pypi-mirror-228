...
def create_new_entity(event:str=None, entity_type:str="Folder"):
    ...
    if event == "Create":
        ...
        if values['-ENTITY_NAME-'] and values['-PARENT_DIR-']:
            ...
            elif entity_type == "File" and not exists:
                with open(entity_path, 'w') as f:
                    pass  # creates an empty file, you can add content here if needed
...
def save_entity(initial_folder:str=os.getcwd(), entity_type:str="Folder",default_name_prompt:str='Enter the new folder name:'):
    file_browser_visible = False
    if entity_type == "File":
        if default_name_prompt == 'Enter the new folder name:':
            default_name_prompt = 'Enter the new file name or choose from File Browser:'
        file_browser_visible = True
    layout = [
        [get_gui_fun("Text", args={"text": default_name_prompt})],
        [get_gui_fun("Input", args={"key": '-ENTITY_NAME-',"enable_events":True}),get_gui_fun("FileBrowse", args={"button_text":"File Browse","key":"-FOLDER_BROWSE-","initial_folder": initial_folder,"enable_events":True,"visible":file_browser_visible})],
        ...
        [create_row_of_buttons('Create', 'Cancel'),get_gui_fun("Push"),get_gui_fun("Input", args={"disabled":True,"default_text":entity_type,"key":"-ENTITY_TYPE-","size":(len(entity_type),1)}),get_gui_fun("Input", args={"default_text":"","key":"-FINAL_OUTPUT-","visible":False})]
    ]
    window = window_browser_mgr.get_new_window(args={"layout": layout, "title": f"Create New {entity_type}", "event_function": "create_new_entity", "exit_events": ["Cancel"]})
