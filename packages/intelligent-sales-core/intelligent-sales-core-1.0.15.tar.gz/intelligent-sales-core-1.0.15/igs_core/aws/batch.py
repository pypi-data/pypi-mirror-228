...
from igs_core.local_stack.boto_client_moks.localstack_clients import boto3_client_localstack
...
class AmazonWebServicesBatchJobs:
    ...
    def run_batch_with_parameters(self):
        if self.environment_default_parameters_dict:
            ...
            igs_logger.info(message=f"Batch Job started succesfully with:{igs_logger.new_line()}"
                                    f"Job name:{response.get('jobName')}{igs_logger.new_line()}"
                                    f"Job ID:{response.get('jobId')}")
        else:
            # TODO: change this to a try except
            igs_logger.error("We have missing parameters!")
