...
def _aigc_post_product(request, function):
    return _post(url="https://api.mecordai.com/proxymsg", 
...
def _try_network():
    try:
        for url in ["https://www.baidu.com/", "https://www.apple.com/"]:
            s = requests.session()
            s.keep_alive = False
            s.headers.update({'Connection':'close'})
            res = s.get(url=url, timeout=30)
            print(f"=== probe url={url} result_code={res.status_code}")
            s.close()
            
        for ip in ["www.baidu.com", "1.1.1.1", "8.8.8.8", "208.67.222.222"]:
            res = ping3.ping(ip, timeout=5)
            if res:
                print(f"=== ping {ip} success")
            else:
                print(f"=== probe {url} fail")
    finally:
        return
...
def _post(url, objStr, request, function, token):
    ...
    try:
        ...
        s.keep_alive = False
        s.headers.update({'Connection':'close'})
        res = s.post(url=url, data=input_req.SerializeToString(), timeout=60)
...
def GetTask():
    ...
    extInfo["trace_id"] = datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S_%f')
    req.extend = json.dumps(extInfo)

    rsp = aigc_ext_pb2.GetTaskRes()
    if store.is_product():
        r1, r2, r3 = _aigc_post_product(req, "GetTask")
    else:
...
def retryLastTaskNotify():
    try:
        if os.path.exists(TASK_NOTIFY_DATA):
            ...
            for it in data:
                msg = it["msg"]
                if TaskNotify(it["taskUUID"], it["status"], f"{msg} *", it["dataStr"], True) == False:
                    newData.append(it)
            with open(TASK_NOTIFY_DATA, 'w') as f:
                json.dump(newData, f)
    finally:
...
def TaskNotify(taskUUID, status, msg, dataStr, isRetry=False):
    ...
    if store.is_product():
        r1, r2, r3 = _aigc_post_product(req, "TaskNotify")
    else:
        r1, r2, r3 = _aigc_post(req, "TaskNotify")
    if r1 != 0:
        if r1 == -99 and isRetry == False:
...
def GetOssUrl(ext):
    ...
    if store.is_product():
        r1, r2, r3 = _aigc_post_product(req, "UploadFileUrl")
    else:
...
def UploadMarketModel(name, cover, model_url, type, taskuuid):
    ...
    if store.is_product():
        r1, r2, r3 = _aigc_post_product(req, "MarketModelCreateWithTask")
    else:
