...
def notifyScriptError(msg):
    notifyWechatRobot({
        "msgtype": "markdown",
        "markdown": {
            "content": f"机器<<font color=\"warning\">{socket.gethostname()}</font>> 脚本执行异常\n<<font color=\"warning\">{msg}</font>>"
        }
    })
    notifyWechatRobot({
        "msgtype": "file",
        "file": {
            "media_id": uploadFile2Wechat(f"{os.path.dirname(os.path.abspath(__file__))}/log.log")
        }
    })
...
def notifyCounterIfNeed():
    ...
    if now_hour == 0 and len(data) > 2:
        ...
        notifyWechatRobot({
            ...
            "markdown": {
                "content": f"机器<<font color=\"warning\">{socket.gethostname()}</font>> {yesterday} 日报 \n\n\
                                >过去24小时执行任务<<font color=\"warning\">{s_cnt+f_cnt}</font>>个 \n\
        ...
        plt.title(f'[{socket.gethostname()}] [{yesterday}] success/fail/count={s_cnt}/{f_cnt}/{s_cnt+f_cnt}')
        plt.xlabel('time')
        plt.xticks(ticks=t_l)
        plt.ylabel('count')
        plt.legend()
...
def saveCounter(taskUUID, isSuccess):
    notifyCounterIfNeed()
    if os.path.exists(task_counter_file) == False:
