...
def setCurrentTaskUUID(taskUUID):
    sp = Store()
    read_data = sp.read()
    read_data["curTaskUUID"] = taskUUID
    sp.write(read_data)
    
def getCurrentTaskUUID():
    sp = Store()
    read_data = sp.read()
    if "curTaskUUID" in read_data:
        return read_data["curTaskUUID"]
    else:
        return ""
...
def is_product():    
    ...
    try:
        with open(env_file, 'r', encoding='UTF-8') as f:
            v = int(f.read())
            if v == 1:
                if mecord_service.check_is_running():
                    return True
                else:
                    save_product(False)
            return False
