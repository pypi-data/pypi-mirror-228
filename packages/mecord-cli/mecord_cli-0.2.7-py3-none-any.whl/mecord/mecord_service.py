...
class BaseService:
    ...
    def run(self):
        ...
        while (os.path.exists(stop_file) == False):
            idling_counter += 0 if self.run_internal() > 0 else 1
    ...
    def executeLocalPython(self, taskUUID, cmd, param):
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                print(f"{o1}\n{o2}")
                print("======================     end      ======================")
                utils.notifyScriptError(f"{o1}\n{o2}")
        except subprocess.CalledProcessError as e:
            print("====================== process error ======================")
            print(e)
            print("======================      end      ======================")
            utils.notifyScriptError(e)
    ...
    def run_internal(self):
        ...
        try:
            ...
            for it in datas:
                ...
                isProduct = "" if store.is_product() else " with test"
                print(f"=== receive task : {taskUUID} {isProduct}")
                ...
                #run
                print(f"=== start execute task : {taskUUID}")
                ...
                print(f"=== notify task({taskUUID}) complate")
                if xy_pb.TaskNotify(taskUUID, 
                                    is_ok, 
                                    msg, 
                                    json.dumps(result_obj["result"], separators=(',', ':'))):
                    utils.saveCounter(taskUUID, is_ok)
                    print(f"task : {taskUUID} notify server success", flush=True)
                else:
                    print(f"task : {taskUUID} server fail~~", flush=True)
        except Exception as e:
