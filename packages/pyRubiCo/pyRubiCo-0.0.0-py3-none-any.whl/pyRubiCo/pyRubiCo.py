from pyRubiCo.Copyright import copyright
from pyRubiCo.PostData import method_Rubika,httpregister,_download,_download_with_server
from pyRubiCo.Error import AuthError,TypeMethodError
from pyRubiCo.Device import DeviceTelephone
from re import findall
from pyRubiCo.Clien import clien
from random import randint,choice
import datetime
import io, PIL.Image
from pyRubiCo.Getheader import Upload
from pyRubiCo.Encoder import encoderjson
from tinytag import TinyTag
from pyRubiCo.TypeText import TypeText
import asyncio
import json
import load
class Bot:
    ...
    def HideChatGroup(self,guid,hiddenmsg):
        return self.methods.methodsRubika("json",methode ="editGroupInfo",indata = {"chat_history_for_new_members": "Hidden","group_guid": guid,"updated_parameters": hiddenmsg},wn = clien.web)
    ...
    def SendCodeWhithPassword(self,phone_number:str,pass_you):
        tmp = self.Authrandom()
        enc = encoderjson(tmp)
        return self.methods.methodsRubika("json",methode ="sendCode",indata = {"pass_key":pass_you,"phone_number":f"98{phone_number[1:]}","send_type":"SMS"},wn = clien.web)
    ...
    def registerDevice(self,auth):
        ...
        while 1:
            try:
                loop = asyncio.get_event_loop()
                ersal = load(enc.decrypt(load(loop.run_until_complete(httpregister(auth)))))
                return ersal
                break
            except:
                continue
