...
from pyRubiCo.GtM import default_api
...
async def http(js,auth):
	...
	async with aiohttp.ClientSession() as session:
		async with session.post(s, data = dumps({"api_version":"6","auth": auth,"data_enc":enc.encrypt(dumps(js))}) , headers = {'Content-Type': 'application/json'}) as response:
...
async def httpregister(auth):
	...
	enc = encoderjson(auth)
	async with aiohttp.ClientSession() as session:
		async with session.post(s, data = dumps({"api_version":"5","auth":auth,"client": clien.android,"data_enc":enc.encrypt(dumps(DeviceTelephone.defaultDevice)),"method":"registerDevice"})) as response:
