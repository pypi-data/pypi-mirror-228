    ...
    def get_amount(self, component=None, total_flow=None, to_units=None):
        ...
        if isinstance(self.java_natural_gas, list):
            ...
            for idx, ng in enumerate(self.java_natural_gas):
                quantity = jvm.Quantities.getQuantity(total_flow[0][idx], self.__parse_units(total_flow[1]))
                if component is not None:
                    results.append(ng.getAmount(component.value, quantity, self.__parse_units(to_units)))
                else:
                    results.append(ng.getAmount(quantity, self.__parse_units(to_units)))
            return results
        else:
            quantity = jvm.Quantities.getQuantity(total_flow[0], self.__parse_units(total_flow[1]))
            if component is not None:
                return self.java_natural_gas.getAmount(component.value, quantity, self.__parse_units(to_units))
            else:
                return self.java_natural_gas.getAmount(quantity, self.__parse_units(to_units))
