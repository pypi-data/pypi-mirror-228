...
"""
__version__ = 'v3.1.1 2023-08-29'# Added TLV493D magnetometer
...
class I2C_TLV493D(I2CDev):
    ...
    def __init__(self, devAddr):
        print(f'I2C_TLV493D {devAddr}')
        super().__init__(devAddr, 'Magnetometer', 'TLV493D')
        self.pPV.update({# Add PVs
            self.name+'_X': LDO('R','X-axis field', 0., units='G'),
            self.name+'_Y': LDO('R','Y-axis field', 0., units='G'),
            self.name+'_Z': LDO('R','Z-axis field', 0., units='G'),
            self.name+'_M': LDO('R','Magnitude', 0., units='G'),
            self.name+'_T': LDO('R','Magnitude', 0., units='C'),
        })
        regs = I2C.read_i2c_data(self.addr, 0x0, 10)
        print(f'regs: {[hex(i) for i in regs]}')
    ...
    def read(self, timestamp):
        ...
        m1 = self.UDataMax
        m2 = m1*2
        r = I2C.read_i2c_data(self.addr, 0x0, 10)
        ...
        if z > m1: z -= m2
        t = ((r[3]&0xF0)<<4) + r[6]
        printv(f'xyzt: {x,y,z,t}')
        m = round(float(np.sqrt(x**2 + y**2 + z**2)), 6)

        for suffix,v in zip('XYZM', (x,y,z,m)):
            v*= self.LSB
            self.pPV[self.name+'_'+suffix].set_valueAndTimestamp(v, timestamp)
        t = (t - 340.)*self.LSBT + 25.
        self.pPV[self.name+'_T'].set_valueAndTimestamp(t, timestamp)
