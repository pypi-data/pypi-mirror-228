...
from pysulog.db import DatabaseULog
from pysulog.migrate_db import migrate_db
...
class TestMigration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        '''
        self.db_path = os.path.join(TEST_PATH, 'test_pysulog.sqlite3')
    ...
    def tearDown(self):
        ...
        for filename in os.listdir(self.sql_dir):
            assert re.match(r'pysulog\.\d\.sql', filename), 'Only removing migration files.'
    ...
    def _make_migration_file(self, sql):
        ...
        sql_filename = f'pysulog.{migration_index}.sql'
...
COMMIT;
    ...
    def test_real_migrations(self):
        '''
        Verify that the migration scripts in the pysulog/sql directory execute
        successfully.
        '''
        migrate_db(self.db_path)
        db_version, _ = self._get_db_info()
