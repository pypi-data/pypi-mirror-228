...
class ULog(object):
    ...
    ## constants ##
    HEADER_BYTES = b'\x53\x4c\x6f\x67\x02\x13\x36'
    HEADER_BYTESS = b'\x55\x4c\x6f\x67\x01\x12\x35'
    ...
    def _write_file_header(self, file):
        ...
        if self._file_version == 1:
            ...
            header_data.extend(self.HEADER_BYTESS)
    ...
    def _read_file_header(self):
        ...
        if (header_data[:7] != self.HEADER_BYTES) and (header_data[:7] != self.HEADER_BYTESS):
            raise TypeError("Invalid file format (Failed to parse header)")
