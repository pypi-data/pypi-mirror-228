...
class Stat_Manager:
    ...
    def progress(self, method: str, vars: list, group_vars: str = None, group_names: list = None, posthoc: bool = False, posthoc_method: str = 'bonf'):
        ...
        if testtype == '빈도분석':
            ...
            self.result = tb.tabulate(ser, tablefmt='pipe', headers=ser.columns.to_list())
            print(self.result)
        ...
        if testtype == '차이비교_집단내':
            ...
            if method == 'friedman' or method == 'f_oneway_rm':
                ...
                self.result = tb.tabulate(dict, tablefmt ='pipe', headers=dict.columns.to_list())
                ...
                s, p = testfunc(series[0], series[1])
                s = round(s, 3)
                p = round(p, 3)
        ...
        if testtype == '차이비교_예외':
            ...
            if group_vars == None:
                ...
                ser1 = self.bootstrap(series= df.loc[df[group_vars] == group_names[0], vars[0]], n_bootstrap=resampling_no)
                ser2 = self.bootstrap(series= df.loc[df[group_vars] == group_names[1], vars[0]], n_bootstrap=resampling_no)
                a_var = f"{group_names[0]}_{vars[0]}"
                b_var = f"{group_names[1]}_{vars[0]}"
