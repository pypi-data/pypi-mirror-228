...
def test_insert_cost_zero_when_not_allowed():
    ...
    assert_allclose(insert_cost(route[0], route[1], data, cost_eval), 0)
    assert_allclose(insert_cost(route[3], route[2], data, cost_eval), 0)

    # Inserting after a node that's not in a route is not possible.
    assert_allclose(insert_cost(route[1], Node(loc=3), data, cost_eval), 0)
...
def test_insert_cost():
    ...
    assert_allclose(insert_cost(Node(loc=4), route[1], data, cost_eval), 696)

    # +5 load penalty, and delta dist is 1090 + 1475 - 1965 = 600. So total
    # delta cost is 605 (again no time warp).
    assert_allclose(insert_cost(Node(loc=4), route[2], data, cost_eval), 605)
    ...
    #      3 + 82 + 2087 = 2172.
    assert_allclose(insert_cost(Node(loc=3), route[1], data, cost_eval), 2172)
    ...
    #     3 + 719 + 3633 = 4355.
    assert_allclose(insert_cost(Node(loc=3), route[2], data, cost_eval), 4355)
...
def test_remove_cost_zero_when_not_allowed():
    ...
    assert_allclose(remove_cost(route[0], data, cost_eval), 0)
    assert_allclose(remove_cost(route[3], data, cost_eval), 0)

    # Removing a node that's not in a route is not possible.
    assert_allclose(remove_cost(Node(loc=3), data, cost_eval), 0)
...
def test_remove():
    ...
    assert_allclose(remove_cost(route[1], data, cost_eval), -1592)
