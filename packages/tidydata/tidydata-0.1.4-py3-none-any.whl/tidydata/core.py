...
ScalarNA = Union[str, int, float, bool, pd.NA, np.nan, None] 
...
class Table:
    ...
    def specify_valid_values(self, valid_values: Dict[str, List[Scalar | Tuple[Number, Number]]]):
        ...
        for col, values in valid_values.items():
            if col in df.columns:
                ...
                if scalar_values:
                    df.loc[~df[col].isin(scalar_values), col] = pd.NA
                    
                for value_range in tuple_values:
                    df.loc[~df[col].between(value_range[0], value_range[1]), col] = pd.NA
