...
class KeycloakEnvironment():
    ...
    def __init__(self,
                 ...
                 baseKeycloakUrl = "http://keycloak.url/auth"
                 ):
        ...
        if 'KEYCLOAK_ENABLED' in os.environ:
            ...
            self.keycloak_enabled = keycloakEnabled
        if 'KEYCLOAK_BASE_URL' in os.environ:
            self.keycloak_url = "{0}/auth".format(os.environ['KEYCLOAK_BASE_URL'])
        elif baseKeycloakUrl:
            self.keycloak_url = baseKeycloakUrl if baseKeycloakUrl.endswith('/auth') else baseKeycloakUrl + "/auth"
        else:
    ...
    def authenticate(self):
        if self.keycloak_enabled:
            if not self.keycloak_admin_auth_user or not self.keycloak_admin_auth_password:
                return self.__authenticateByServiceAccount()
            else:
                return self.__authenticateByUsernamePassword()
        else:
    ...
    def __authenticateByServiceAccount(self, client_id=None, client_realm=None):
        headers = {}
    ...
    def __authenticateByUsernamePassword(self):
        headers = {}
        if self.keycloak_enabled:
            if self.keycloak_auth_client_secret is None and (self.kc is not None or self.init_keycloak_api() is not None):
    ...
    def __get_test_access_token(self):
        decoded_access_token = {
            ...
            "sub": str(uuid.uuid1()),
            "typ": "ID",
            "azp": self.keycloak_auth_client_id,
        ...
        "sid": "d2935da1-3a86-4c95-9cea-9535cdb5c2f8",
