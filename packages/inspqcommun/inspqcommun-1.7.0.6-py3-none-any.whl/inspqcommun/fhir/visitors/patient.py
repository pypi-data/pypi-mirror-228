    ...
    def set_name(self, given_name=None, family_name=None):
        if not given_name and not family_name:
            ...
            if not self.getFhirResource().name:
                self.getFhirResource().name = HumanName()
                self.getFhirResource().name.given = [given_name] if given_name else None
                self.getFhirResource().name.family = [family_name] if family_name else None

    def get_given_name(self):
        return self.getFhirResource().name.given[0] if self.getFhirResource().name and self.getFhirResource().name.given else None

    def get_family_name(self):
        return self.getFhirResource().name.family[0] if self.getFhirResource().name and self.getFhirResource().name.family else None
