...
class CircularArc(Curve):
    ...
    def from_3_consecutive_points(cls, a: Vector2, b: Vector2, c: Vector2, naming_provider: Callable[[], str]):
        ...
        rads_ab = circle.rads_between_points(a.as_tuple(), b.as_tuple())
        rads_ac = circle.rads_between_points(a.as_tuple(), c.as_tuple())
...
class CubicUniformBasisSpline(Curve):
    ...
    def from_json(cls, json: Dict):
        ...
        return CubicUniformBasisSpline(control_points=cps, id=json.get('id', None))
    ...
    def _render_formula(self, t, deriv_order: int):
        ...
        t_in_seg = t - seg_idx / self.nSegments
...
class CubicBezier(Curve):
    ...
    def as_circular_arcs(cls, curve, tolerance: float = 10):
        ...
        inflection_points = utils.cubic_bezier_inflection_points(control_points=curve.ControlPoints)
    ...
    def max_delta_on_circle(cls, curve, biarc_circle: Circle):
        max_delta = 0
        for point in utils.cubic_bezier_points([(x.x, x.y) for x in curve.ControlPoints], 10):
