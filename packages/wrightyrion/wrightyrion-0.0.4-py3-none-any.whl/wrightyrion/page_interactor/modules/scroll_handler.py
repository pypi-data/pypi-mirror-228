        ...
        while True:
            if selector is not None:
                ...
                if count_check_counter >= count_check_interval:
                    count = await self._common_handler.count(selector=selector)
                    count_check_counter = 0
                    ...
                    if count == prev_count:
                        ...
                        self.debug_tool.info(f"Current count: {count}, previous count: {prev_count}, same selector count: {same_sel_count} / {same_sel_count_th}, threshold: {threshold}")
                        same_sel_count = 0
                        prev_count = count

                    if same_sel_count >= same_sel_count_th:
                        self.debug_tool.info(f"Same selector count: {same_sel_count}, same_sel_count_th: {same_sel_count_th}, stopping!!")
                        break
            ...
            if top == last_top:
                same_count += 1
                self.debug_tool.info(f"Same top count: {same_count}, threshold: {same_th}, top = {top}")
