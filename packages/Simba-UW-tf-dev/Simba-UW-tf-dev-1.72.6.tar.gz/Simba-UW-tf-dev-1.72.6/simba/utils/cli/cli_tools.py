...
def set_video_parameters(config_path: str,
    ...
    timer.stop_timer()
    stdout_success(msg='Video parameters set', elapsed_time=timer.elapsed_time_str, source=set_video_parameters.__name__)

def set_outlier_correction_criteria_cli(config_path: str,
    ...
    config.set(section='Outlier settings', option='movement_criterion', value=str(movement_criterion))
    config.set(section='Outlier settings', option='location_criterion', value=str(location_criterion))
    config.set(section='Outlier settings', option='mean_or_median', value=aggregation)
    for cnt, (k, v) in enumerate(body_parts.items()):
        config.set(section='Outlier settings', option=f'movement_bodypart1_mouse{cnt+1}', value=v['Movement'][0])
        config.set(section='Outlier settings', option=f'movement_bodypart2_mouse{cnt+1}', value=v['Movement'][1])
        config.set(section='Outlier settings', option=f'location_bodypart1_mouse{cnt+1}', value=v['Location'][0])
        config.set(section='Outlier settings', option=f'location_bodypart2_mouse{cnt+1}', value=v['Location'][1])
    with open(config_path, 'w') as file:
        config.write(file)

    timer.stop_timer()
    stdout_success('Outlier parameters set', elapsed_time=timer.elapsed_time_str, source=set_outlier_correction_criteria_cli.__name__)
