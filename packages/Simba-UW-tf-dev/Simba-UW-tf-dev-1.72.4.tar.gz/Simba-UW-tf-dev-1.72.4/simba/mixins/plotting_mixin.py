...
class PlottingMixin(object):
    ...
    def make_distance_plot(self,
        ...
        if save_img:
            ...
            stdout_success(f'Final distance plot saved at {save_path}', elapsed_time=timer.elapsed_time_str, source=self.__class__.__name__)
        else:
            return img
    ...
    def make_probability_plot(self,
        ...
        timer.stop_timer()
        cv2.imwrite(save_path, img)
        stdout_success(msg=f'Final distance plot saved at {save_path}', elapsed_time=timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def make_path_plot(self,
                       ...
                       clf_attr: dict,
                       save_path: str,
                       print_animal_names: bool = True) -> None:
        ...
        img = cv2.resize(img, (style_attr['width'], style_attr['height']))
        cv2.imwrite(save_path, img)
        stdout_success(msg=f'Final path plot saved at {save_path}', elapsed_time=video_timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def make_gantt_plot(self,
        ...
        stdout_success(msg=f'Final gantt frame for video {video_name} saved at {save_path}',
                       elapsed_time=video_timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def make_clf_heatmap_plot(frm_data: np.array,
        ...
        plt.savefig('/Users/simon/Desktop/envs/simba_dev/test.png')
        ...
        if final_img:
            ...
            stdout_success(msg=f'Final classifier heatmap image saved at at {file_name}', source='make_clf_heatmap_plot')
        else:
            return image
    ...
    def make_location_heatmap_plot(self,
        ...
        if final_img:
            cv2.imwrite(file_name, image)
            stdout_success(msg=f'Final location heatmap image saved at at {file_name}', source=self.__class__.__name__)
        else:
            return image
    ...
    def violin_plot(self,
        ...
        stdout_success(msg=f'Violin plot saved at {save_path}', source=self.__class__.__name__)
