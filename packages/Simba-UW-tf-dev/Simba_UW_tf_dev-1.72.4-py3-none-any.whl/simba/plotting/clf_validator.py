    ...
    def run(self):
        ...
        for file_cnt, file_path in enumerate(self.data_paths):
            ...
            if not self.video_path:
                raise NoFilesFoundError(msg=f'Could not find a video file representing {file_name} in the project_folder/videos directory', source=self.__class__.__name__)
            ...
            if len(clf_bouts) == 0:
                NoDataFoundWarning(msg=f'Skipping video {file_name}: No classified behavior detected...', source=self.__class__.__name__)
                continue
        ...
        stdout_success(msg='All validation clips complete. Files are saved in the project_folder/frames/output/classifier_validation directory of the SimBA project', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
