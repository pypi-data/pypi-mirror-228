...
from simba.utils.printing import stdout_success, SimbaTimer, perform_timing
...
def change_img_format(directory: Union[str, os.PathLike],
    ...
    if not os.path.isdir(directory):
        raise NotDirectoryError('SIMBA ERROR: {} is not a valid directory'.format(directory), source=change_img_format.__name__)
    files_found = glob.glob(directory + '/*.{}'.format(file_type_in))
    if len(files_found) < 1:
        raise NoFilesFoundError('SIMBA ERROR: No {} files (with .{} file ending) found in the {} directory'.format(file_type_in, file_type_in, directory), source=change_img_format.__name__)
...
def extract_frame_range(file_path: Union[str, os.PathLike],
    ...
    stdout_success(msg=f'{str(len(frame_range))} frames extracted for video {file_name}', source=extract_frame_range.__name__)
...
def change_fps_of_multiple_videos(directory: Union[str, os.PathLike],
    ...
    if not os.path.isdir(directory):
        raise NotDirectoryError(msg='SIMBA ERROR: {} is not a valid directory'.format(directory), source=change_fps_of_multiple_videos.__name__)
    check_int(name='New fps', value=fps)
    video_paths = []
    ...
    if len(video_paths) < 1:
        raise NoFilesFoundError(msg='SIMBA ERROR: No files with .mp4, .avi, .mov, .flv file ending found in the {} directory'.format(directory), source=change_fps_of_multiple_videos.__name__)
    ...
    timer.stop_timer()
    stdout_success(msg=f'SIMBA COMPLETE: FPS of {str(len(video_paths))} video(s) changed to {str(fps)}', elapsed_time=timer.elapsed_time_str, source=change_fps_of_multiple_videos.__name__)
...
def convert_video_powerpoint_compatible_format(file_path: Union[str, os.PathLike],
    ...
    """
    if gpu and not check_nvidea_gpu_available():
        raise FFMPEGCodecGPUError(msg='No GPU found (as evaluated by nvidea-smi returning None)', source=convert_video_powerpoint_compatible_format.__name__)
    timer = SimbaTimer(start=True)
    check_file_exist_and_readable(file_path=file_path)
    dir, file_name, ext = get_fn_ext(filepath=file_path)
    save_name = os.path.join(dir, file_name + '_powerpointready.mp4')
    if os.path.isfile(save_name):
        raise FileExistError(msg='SIMBA ERROR: The outfile file already exist: {}.'.format(save_name), source=convert_video_powerpoint_compatible_format.__name__)
    if gpu:
    ...
    timer.stop_timer()
    stdout_success(msg=f'SIMBA COMPLETE: Video converted! {save_name} generated!', elapsed_time=timer.elapsed_time_str, source=convert_video_powerpoint_compatible_format.__name__)
...
def convert_to_mp4(file_path: Union[str, os.PathLike],
    ...
    timer = SimbaTimer(start=True)
    if gpu and not check_nvidea_gpu_available():
        raise FFMPEGCodecGPUError(msg='No GPU found (as evaluated by nvidea-smi returning None)', source=convert_to_mp4.__name__)
    check_file_exist_and_readable(file_path=file_path)
    dir, file_name, ext = get_fn_ext(filepath=file_path)
    save_name = os.path.join(dir, file_name + '_converted.mp4')
    if os.path.isfile(save_name):
        raise FileExistError(msg='SIMBA ERROR: The outfile file already exist: {}.'.format(save_name), source=convert_to_mp4.__name__)
    if gpu:
    ...
    subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    timer.stop_timer()
    stdout_success(msg=f'SIMBA COMPLETE: Video converted! {save_name} generated!', elapsed_time=timer.elapsed_time_str, source=convert_to_mp4.__name__)
...
def video_to_greyscale(file_path: Union[str, os.PathLike],
    ...
    subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    timer.stop_timer()
    stdout_success(msg=f'SIMBA COMPLETE: Video converted! {save_name} generated!', elapsed_time=timer.elapsed_time_str, source=video_to_greyscale.__name__)
...
def superimpose_frame_count(file_path: Union[str, os.PathLike],
    ...
    timer = SimbaTimer(start=True)
    if gpu and not check_nvidea_gpu_available():
        raise FFMPEGCodecGPUError(msg='No GPU found (as evaluated by nvidea-smi returning None)', source=superimpose_frame_count.__name__)
    check_file_exist_and_readable(file_path=file_path)
    dir, file_name, ext = get_fn_ext(filepath=file_path)
    ...
    try:
        ...
        subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    timer.stop_timer()
    stdout_success(msg=f'SIMBA COMPLETE: Video converted! {save_name} generated!', elapsed_time=timer.elapsed_time_str, source=superimpose_frame_count.__name__)
...
def remove_beginning_of_video(file_path: Union[str, os.PathLike],
    ...
    if os.path.isfile(save_name):
        raise FileExistError(msg='SIMBA ERROR: The outfile file already exist: {}.'.format(save_name), source=remove_beginning_of_video.__name__)
    if gpu:
...
def clip_video_in_range(file_path: Union[str, os.PathLike],
    ...
    if os.path.isfile(save_name):
        raise FileExistError(msg='SIMBA ERROR: The outfile file already exist: {}.'.format(save_name), source=clip_video_in_range.__name__)
    if gpu:
    ...
    subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    timer.stop_timer()
    stdout_success(msg=f'SIMBA COMPLETE: Video converted! {save_name} generated!', elapsed_time=timer.elapsed_time_str, source=clip_video_in_range.__name__)
...
def downsample_video(file_path: Union[str, os.PathLike],
    ...
    subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    timer.stop_timer()
    stdout_success(msg=f'SIMBA COMPLETE: Video converted! {save_name} generated!', elapsed_time=timer.elapsed_time_str, source=downsample_video.__name__)
...
def gif_creator(file_path: str,
    ...
    if os.path.isfile(save_name):
        raise FileExistError('SIMBA ERROR: The outfile file already exist: {}.'.format(save_name), source=gif_creator.__name__)
    if gpu:
...
def batch_convert_video_format(directory: Union[str, os.PathLike],
    ...
    if gpu and not check_nvidea_gpu_available():
        raise FFMPEGCodecGPUError(msg='NVIDEA GPU not available (as evaluated by nvidea-smi returning None', source=batch_convert_video_format.__name__)
    if input_format == output_format:
        raise InvalidFileTypeError(msg=f'The input format ({input_format}) is the same as the output format ({output_format})', source=batch_convert_video_format.__name__)
    if not os.path.isdir(directory):
        raise NotDirectoryError(msg='SIMBA ERROR: {} is not a valid directory'.format(directory), source=batch_convert_video_format.__name__)
    video_paths = []
    file_paths_in_folder = [f for f in glob.glob(directory + '/*') if os.path.isfile(f)]
    for file_path in file_paths_in_folder:
        _, _, ext = get_fn_ext(filepath=file_path)
        if ext.lower() == '.{}'.format(input_format.lower()):
            video_paths.append(file_path)
    if len(video_paths) < 1:
        raise NoFilesFoundError(msg='SIMBA ERROR: No files with .{} file ending found in the {} directory'.format(input_format, directory), source=batch_convert_video_format.__name__)
    for file_cnt, file_path in enumerate(video_paths):
        dir_name, file_name, ext = get_fn_ext(filepath=file_path)
        print('Processing video {}...'.format(file_name))
        save_path = os.path.join(dir_name, file_name + '.{}'.format(output_format.lower()))
        if os.path.isfile(save_path):
            raise FileExistError(msg='SIMBA ERROR: The outfile file already exist: {}.'.format(save_path), source=batch_convert_video_format.__name__)
        if gpu:
    ...
    stdout_success(msg=f'SIMBA COMPLETE: {str(len(video_paths))} videos converted in {directory} directory!', source=batch_convert_video_format.__name__)
...
def batch_create_frames(directory: Union[str, os.PathLike]) -> None:
    ...
    if not os.path.isdir(directory):
        raise NotDirectoryError(msg='SIMBA ERROR: {} is not a valid directory'.format(directory), source=batch_create_frames.__name__)
    video_paths = []
    file_paths_in_folder = [f for f in glob.glob(directory + '/*') if os.path.isfile(f)]
    for file_path in file_paths_in_folder:
        _, _, ext = get_fn_ext(filepath=file_path)
        if ext.lower() in ['.avi', '.mp4', '.mov', '.flv']:
            video_paths.append(file_path)
    if len(video_paths) < 1:
        raise NoFilesFoundError(msg='SIMBA ERROR: No files with .mp4, .avi, .mov, .flv file ending found in the {} directory'.format(directory), source=batch_create_frames.__name__)
    for file_cnt, file_path in enumerate(video_paths):
        dir_name, file_name, ext = get_fn_ext(filepath=file_path)
        print('Processing video {}...'.format(file_name))
        ...
        print('Video {} complete, (Video {}/{})...'.format(file_name, str(file_cnt + 1), str(len(video_paths))))
    stdout_success(msg=f'{str(len(video_paths))} videos converted into frames in {directory} directory!', source=batch_create_frames.__name__)
...
def multi_split_video(file_path: Union[str, os.PathLike],
    ...
    for clip_cnt, (start_time, end_time) in enumerate(zip(start_times, end_times)):
        ...
        if os.path.isfile(save_path):
            raise FileExistError(msg=f'The outfile file already exist: {save_path}.', source=multi_split_video.__name__)
        if gpu:
            if not check_nvidea_gpu_available():
                raise FFMPEGCodecGPUError(msg='NVIDEA GPU not available (as evaluated by nvidea-smi returning None', source=multi_split_video.__name__)
        ...
        subprocess.call(command, shell=True, stdout=subprocess.PIPE)
    timer.stop_timer()
    stdout_success(msg=f'Video {file_name} converted into {str(len(start_times))} clips in directory {dir_name}!', elapsed_time=timer.elapsed_time_str, source=multi_split_video.__name__)
...
def crop_single_video(file_path: Union[str, os.PathLike],
    ...
    if (width == 0 and height == 0) or (width + height + top_lext_x + top_left_y == 0):
        raise CountError(msg='CROP FAILED: Cropping height and width are both 0. Please try again.', source=crop_single_video.__name__)
    save_path = os.path.join(dir_name, file_name + '_cropped.mp4')
    if os.path.isfile(save_path):
        raise FileExistError(msg='SIMBA ERROR: The out file file already exist: {}.'.format(save_path), source=crop_single_video.__name__)
    timer = SimbaTimer(start=True)
    if gpu:
    ...
    subprocess.call(command, shell=True)
    timer.stop_timer()
    stdout_success(f'Video {file_name} cropped and saved at {save_path}', elapsed_time=timer.elapsed_time_str, source=crop_single_video.__name__)
...
def crop_multiple_videos(directory_path: Union[str, os.PathLike],
    ...
    if not os.path.isdir(directory_path):
        raise NotDirectoryError(msg='SIMBA ERROR: {} is not a valid directory'.format(directory_path), source=crop_multiple_videos.__name__)
    video_paths = []
    ...
    for file_path in file_paths_in_folder:
        ...
        if ext.lower() in ['.avi', '.mp4', '.mov', '.flv']:
            video_paths.append(file_path)
    if len(video_paths) < 1:
        raise NoFilesFoundError(msg='SIMBA ERROR: No files with .mp4, .avi, .mov, .flv file ending found in the {} directory'.format(directory_path), source=crop_multiple_videos.__name__)
...
def frames_to_movie(directory: Union[str, os.PathLike],
    ...
    if not os.path.isdir(directory):
        raise NotDirectoryError(msg='SIMBA ERROR: {} is not a valid directory'.format(directory), source=frames_to_movie.__name__)
    ...
    if len(img_paths_in_folder) < 1:
        raise NoFilesFoundError(msg='SIMBA ERROR: Zero images of file-type {} found in {} directory'.format(img_format, directory), source=frames_to_movie.__name__)
...
class VideoRotator(ConfigReader):
    ...
    def __init__(self,
        ...
        if ffmpeg and not check_ffmpeg_available():
            raise FFMPEGNotFoundError(msg='FFMPEG not found on the computer (as evaluated by "ffmpeg" returning None)', source=self.__class__.__name__)
    ...
    def __run_rotation(self):
        ...
        if self.ffmpeg or self.gpu:
            ...
            for video_cnt, (video_path, rotation) in enumerate(self.results.items()):
                ...
                cap.release()
                writer.release()
        stdout_success(msg=f'All videos rotated and saved in {self.save_dir}', elapsed_time=str(round((time.time() - start), 2)), source=self.__class__.__name__)
...
def extract_frames_from_all_videos_in_directory(config_path: Union[str, os.PathLike],
    ...
    if len(video_paths) == 0:
        raise NoFilesFoundError(msg='SIMBA ERROR: 0 video files in mp4 or avi format found in {}'.format(directory), source=extract_frames_from_all_videos_in_directory.__name__)
...
def copy_img_folder(config_path: Union[str, os.PathLike], source: Union[str, os.PathLike]) -> None:
    ...
    if not os.path.isdir(source):
        raise NotDirectoryError(msg=f'SIMBA ERROR: source {source} is not a directory.', source=copy_img_folder.__name__)
    if len(glob.glob(source + '/*.png')) == 0:
        raise NoFilesFoundError(msg=f'SIMBA ERROR: source {source} does not contain any .png files.', source=copy_img_folder.__name__)
    ...
    if os.path.isdir(destination):
        raise DirectoryExistError(msg=f'SIMBA ERROR: {destination} already exist in SimBA project.', source=copy_img_folder.__name__)
    print(f'Importing image files for {input_basename}...')
    shutil.copytree(source, destination)
    timer.stop_timer()
    stdout_success(msg=f'{destination} imported to SimBA project', elapsed_time=timer.elapsed_time_str, source=copy_img_folder.__name__)
...
def append_audio(video_path: Union[str, os.PathLike],
    ...
    if not check_ffmpeg_available():
        raise FFMPEGNotFoundError(msg='FFMpeg not found on computer. See SimBA docs for install instructions.', source=append_audio.__name__)
    ...
    try:
       track_type = subprocess.check_output(cmd, stderr=subprocess.STDOUT).decode('utf-8').strip()
        ...
        raise NoDataError(msg=f'No audio track found in file {audio_path}', source=append_audio.__name__)
    ...
    stdout_success(msg=f"Audio merged successfully, file saved at {save_path}!", source=append_audio.__name__)
