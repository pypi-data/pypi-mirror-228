...
class PlotSklearnResultsMultiProcess(ConfigReader, TrainModelMixin, PlottingMixin):
    ...
    def run(self):
        ...
        if self.video_setting:
            stdout_success(msg=f'{len(self.files_found)} videos saved in project_folder/frames/output/sklearn_results directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
        if self.frame_setting:
            stdout_success(f'Frames for {len(self.files_found)} videos saved in sub-folders within project_folder/frames/output/sklearn_results directory', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
