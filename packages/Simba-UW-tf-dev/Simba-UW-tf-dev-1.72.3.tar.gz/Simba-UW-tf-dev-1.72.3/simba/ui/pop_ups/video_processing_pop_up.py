...
class CropVideoPopUp(PopUpMixin):
    def __init__(self):
        ...
        use_gpu_cb_single = Checkbutton(crop_video_lbl_frm, text='Use GPU (reduced runtime)', variable=use_gpu_var_single)
        ...
        use_gpu_cb_multiple = Checkbutton(crop_video_lbl_frm_multiple, text='Use GPU (reduced runtime)', variable=use_gpu_var_multiple)
...
class ClipVideoPopUp(PopUpMixin):
    def __init__(self):
        ...
        self.use_gpu_cb = Checkbutton(use_gpu_frm, text='Use GPU (reduced runtime)', variable=self.use_gpu_var)
...
class MultiShortenPopUp(PopUpMixin):
    ...
    def batch_change(self, value: str):
        if not hasattr(self, 'table'):
            raise CountError(msg='Select the number of video clippings first', source=self.__class__.__name__)
    ...
    def run_clipping(self):
        ...
        multi_split_video(file_path=self.selected_video.file_path,
...
class ChangeImageFormatPopUp(PopUpMixin):
    ...
    def run_img_conversion(self):
        if len(os.listdir(self.input_folder_selected.folder_path)) == 0:
            raise NoFilesFoundError(msg='SIMBA ERROR: The input folder {} contains ZERO files.'.format(self.input_folder_selected.folder_path), source=self.__class__.__name__)
...
class ConvertVideoPopUp(PopUpMixin):
    def __init__(self):
        ...
        gpu_multiple_cb = Checkbutton(convert_multiple_videos_frm, text='Use GPU (reduced runtime)', variable=gpu_multiple_var)
        ...
        gpu_single_cb = Checkbutton(convert_single_video_frm, text='Use GPU (reduced runtime)', variable=self.gpu_single_var)
...
class ExtractSpecificFramesPopUp(PopUpMixin):
    ...
    def start_frm_extraction(self):
        ...
        if int(end_frame) < int(start_frame):
            raise FrameRangeError(msg='SIMBA ERROR: The end frame ({}) cannot come before the start frame ({})'.format(str(end_frame), str(start_frame)), source=self.__class__.__name__)
        ...
        if int(start_frame) > video_meta_data['frame_count']:
            raise FrameRangeError(msg='SIMBA ERROR: The start frame ({}) is larger than the number of frames in the video ({})'.format(str(start_frame), str(video_meta_data['frame_count'])), source=self.__class__.__name__)
        if int(end_frame) > video_meta_data['frame_count']:
            raise FrameRangeError(msg='SIMBA ERROR: The end frame ({}) is larger than the number of frames in the video ({})'.format(str(end_frame), str(video_meta_data['frame_count'])), source=self.__class__.__name__)
...
class MultiCropPopUp(PopUpMixin):
    def __init__(self):
        ...
        video_type_dropdown = DropDownMenu(self.main_frm, 'Video type:', video_options, '15')
        video_type_dropdown.setChoices('mp4')
        crop_cnt_dropdown = DropDownMenu(self.main_frm, 'Crop count:', list(range(1, 31)), '15')
        crop_cnt_dropdown.setChoices(2)
        use_gpu_var = BooleanVar(value=False)
        use_gpu_cb = Checkbutton(self.main_frm, text='Use GPU (reduced runtime)', variable=use_gpu_var)

        run_btn = Button(self.main_frm, text='Crop',command=lambda: MultiCropper(file_type=video_type_dropdown.getChoices(),
                                                                                input_folder=input_folder.folder_path,
                                                                                output_folder=output_folder.folder_path,
                                                                                crop_cnt=crop_cnt_dropdown.getChoices(),
                                                                                gpu=use_gpu_var.get()))

        input_folder.grid(row=0,sticky=NW)
        output_folder.grid(row=1,sticky=NW)
        video_type_dropdown.grid(row=2,sticky=NW)
...
class ChangeFpsSingleVideoPopUp(PopUpMixin):
    def __init__(self):
        ...
        gpu_cb = Checkbutton(self.main_frm, text='Use GPU (reduced runtime)', variable=gpu_var)
...
class ConcatenatingVideosPopUp(PopUpMixin):
    def __init__(self):
        ...
        use_gpu_cb = Checkbutton(settings_frm, text='Use GPU (reduced runtime)', variable=use_gpu_var)
...
class ConcatenatorPopUp(PopUpMixin, ConfigReader):
    ...
    def populate_table(self):
        ...
        self.gpu_frm = LabelFrame(self.main_frm, text='GPU', pady=5, padx=5, font=Formats.LABELFRAME_HEADER_FORMAT.value, fg='black')
        self.use_gpu_var = BooleanVar(value=False)
    ...
    def run(self):
        ...
        if (len(videos_info.keys()) < 3) & (self.join_type_var.get() == 'mixed_mosaic'):
            raise MixedMosaicError(msg='Ff using the mixed mosaic join type, please tick check-boxes for at least three video types.', source=self.__class__.__name__)
...
class VideoTemporalJoinPopUp(PopUpMixin):
    def __init__(self):
        ...
        self.use_gpu_var = BooleanVar(value=False)
        use_gpu_cb = Checkbutton(self.settings_frm, text='Use GPU (reduced runtime)', variable=self.use_gpu_var)
...
class ImportFrameDirectoryPopUp(PopUpMixin, ConfigReader):
    ...
    def run(self):
        if not os.path.isdir(self.frame_folder.folder_path):
            raise NotDirectoryError(msg=f'SIMBA ERROR: {self.frame_folder.folder_path} is not a valid directory.', source=self.__class__.__name__)
...
class ConvertROIDefinitionsPopUp(PopUpMixin):
    def __init__(self):
        ...
        settings_frm = LabelFrame(self.main_frm, text='SETTINGS', font=Formats.LABELFRAME_HEADER_FORMAT.value)
        self.roi_definitions_file_select = FileSelect(settings_frm, "ROI DEFINITIONS PATH (H5)", title='SELECT H5 FILE', lblwidth=20)
    ...
    def run(self):
        ...
        _ = convert_roi_definitions(roi_definitions_path=self.roi_definitions_file_select.file_path,
