...
from simba.utils.checks import check_if_filepath_list_is_empty, check_if_valid_input, check_that_column_exist
from simba.utils.enums import Methods, TagNames
from simba.utils.errors import NoFilesFoundError, DataHeaderError
...
class Interpolate(ConfigReader):
    ...
    def __init__(self,
        ...
        stdout_success(msg=f'{str(len(self.files_found))} data file(s) interpolated)', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
...
class Smooth(ConfigReader):
    ...
    def __init__(self,
        ...
        super().__init__(config_path=config_path, read_video_info=False)
        log_event(logger_name=str(self.__class__.__name__), log_type=TagNames.CLASS_INIT.value, msg=f'input_path: {input_path}, time_window: {time_window}, smoothing_method: {smoothing_method}, initial_import_multi_index: {initial_import_multi_index}')
        if os.path.isdir(input_path):
            self.files_found = glob.glob(input_path + '/*' + self.file_type)
            self.input_dir = input_path
            check_if_filepath_list_is_empty(filepaths=self.files_found, error_msg=f"SIMBA ERROR: {self.input_dir} does not contain any {self.file_type} files.")
        ...
        self.timer.stop_timer()
        stdout_success(msg=f'{str(len(self.files_found))} data file(s) smoothened', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
    ...
    def gaussian_smoother(self):
        for file_path in self.files_found:
            ...
            video_path = find_video_of_file(video_dir=self.video_dir, filename=video_name)
            if not video_path:
                raise NoFilesFoundError(msg=f'No video for file {video_name} found in SimBA project. Import the video before doing Gaussian smoothing. To perform smoothing, SimBA needs the video in order to read the video FPS.', source=self.__class__.__name__)
            video_meta_data = get_video_meta_data(video_path=video_path)
...
class AdvancedInterpolator(ConfigReader):
    ...
    def __init__(self,
        ...
        self.file_paths = find_files_of_filetypes_in_directory(directory=data_dir, extensions=[f'.{self.file_type}'], raise_warning=False, raise_error=True)
        check_if_valid_input(name='type', input=type, options=['animal', 'body-part'], raise_error=True)
        self.settings, self.initial_import_multi_index, self.overwrite = settings, initial_import_multi_index, overwrite
        self.move_dir = None
        if not overwrite:
            self.move_dir = os.path.join(data_dir, f'Pre_Advanced_Interpolation_{self.datetime}')
            if not os.path.isdir(self.move_dir): os.makedirs(self.move_dir)
        if type == 'animal': self._transpose_settings()
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.file_paths):
            df = read_df(file_path=file_path, file_type=self.file_type, check_multiindex=self.initial_import_multi_index).fillna(0).reset_index(drop=True)
            _, video_name, _ = get_fn_ext(filepath=file_path)
            if self.initial_import_multi_index:
            ...
            for animal_name, animal_body_parts in self.settings.items():
                for bp, interpolation_setting in animal_body_parts.items():
                    check_that_column_exist(df=df, column_name=f'{bp}_x', file_name=file_path)
                    check_that_column_exist(df=df, column_name=f'{bp}_y', file_name=file_path)
                    df[[f'{bp}_x', f'{bp}_y']] = df[[f'{bp}_x', f'{bp}_y']].astype(int)
                    idx = df.loc[(df[f'{bp}_x'] <= 0.0) & (df[f'{bp}_y'] <= 0.0)].index.tolist()
                    print(f'Interpolating {len(idx)} {bp} body-parts in video {video_name}...')
            ...
            if not self.overwrite:
                shutil.move(src=file_path, dst=os.path.join(self.move_dir, os.path.basename(file_path)))
            write_df(df=df, file_type=self.file_type, save_path=file_path, multi_idx_header=self.initial_import_multi_index)
        self.timer.stop_timer()
        stdout_success(msg='Interpolation complete!', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
...
class AdvancedSmoother(ConfigReader):
    ...
    def __init__(self,
        ...
        ConfigReader.__init__(self, config_path=config_path, read_video_info=False)
        log_event(logger_name=str(self.__class__.__name__), log_type=TagNames.CLASS_INIT.value, msg=f'data_dir: {data_dir}, type: {type}, settings: {settings}, initial_import_multi_index: {initial_import_multi_index}, overwrite: {overwrite}')
        self.file_paths = find_files_of_filetypes_in_directory(directory=data_dir, extensions=[f'.{self.file_type}'], raise_warning=False, raise_error=True)
        check_if_valid_input(name='type', input=type, options=['animal', 'body-part'], raise_error=True)
        self.settings, self.initial_import_multi_index, self.overwrite = settings, initial_import_multi_index, overwrite
        self.move_dir = None
        if not overwrite:
            self.move_dir = os.path.join(data_dir, f'Pre_Advanced_Smoothing_{self.datetime}')
            if not os.path.isdir(self.move_dir): os.makedirs(self.move_dir)
        if type == 'animal': self._transpose_settings()
    ...
    def run(self):
        for file_cnt, file_path in enumerate(self.file_paths):
            ...
            for animal_name, animal_body_parts in self.settings.items():
                for bp, smoothing_setting in animal_body_parts.items():
                    ...
                    frames_in_time_window = int(smoothing_setting['time_window'] / (1000 / int(video_meta_data['fps'])))
                    check_that_column_exist(df=df, column_name=f'{bp}_x', file_name=file_path)
                    check_that_column_exist(df=df, column_name=f'{bp}_y', file_name=file_path)
                    df[[f'{bp}_x', f'{bp}_y']] = df[[f'{bp}_x', f'{bp}_y']].astype(int)
                    if smoothing_setting['method'].lower == Methods.GAUSSIAN.value:
                        df[[f'{bp}_x', f'{bp}_y']] = df[[f'{bp}_x', f'{bp}_y']].rolling(window=int(frames_in_time_window), win_type='gaussian', center=True).mean(std=5).fillna(df[[f'{bp}_x', f'{bp}_y']]).abs()
                    if smoothing_setting['method'].lower == Methods.SAVITZKY_GOLAY.value:
                        if (frames_in_time_window % 2) == 0:
                            frames_in_time_window = frames_in_time_window - 1
                        if (frames_in_time_window % 2) <= 3:
                            frames_in_time_window = 5
                        df[[f'{bp}_x', f'{bp}_y']] = savgol_filter(x=df[[f'{bp}_x', f'{bp}_y']].to_numpy(), window_length=frames_in_time_window, polyorder=3, mode='nearest')
                    df[[f'{bp}_x', f'{bp}_y']][df[[f'{bp}_x', f'{bp}_y']] < 0] = 0
            if self.initial_import_multi_index:
                multi_idx_header = []
                for i in range(len(df.columns)): multi_idx_header.append(('IMPORTED_POSE', 'IMPORTED_POSE', list(df.columns)[i]))
        ...
        self.timer.stop_timer()
        stdout_success(msg='Smoothing complete complete!', elapsed_time=self.timer.elapsed_time_str, source=self.__class__.__name__)
