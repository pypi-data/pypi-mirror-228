...
class CircularStatisticsMixin(object):
    ...
    def rolling_rayleigh_z(self,
        ...
        resultant_vector_lengths = CircularStatisticsMixin().rolling_resultant_vector_length(data=data, fps=fps, time_window=time_window)
    ...
    @staticmethod
    @jit(nopython=True)
    def rolling_angular_difference(data: np.ndarray,
                                   fps: int,
        ...
        >>> CircularStatisticsMixin().rolling_angular_difference(data=data, fps=1, time_windows=np.array([1]))
        >>> [[ 0.], [ 0.], [11.], [ 0.]]
        """
    ...
    @staticmethod
    @jit(nopython=True)
    def agg_angular_difference_timebins(data: np.ndarray,
                                        fps: int,
                                        time_windows: np.ndarray):
        ...
        >>> CircularStatisticsMixin().agg_angular_difference_timebins(data=data,time_windows=np.array([1]), fps=5)
        """
    ...
    def sliding_rao_spacing(data: np.ndarray,
                            ...
                            fps: int) -> np.ndarray:
        """
        Jitted compute of the uniformity of a circular dataset in sliding window of size ``time_window``.
        ...
        >>> result = CircularStatisticsMixin().sliding_rao_spacing(data=data, time_window=0.5, fps=10)
        """
    ...
    @staticmethod
    @jit(nopython=True)
    def rolling_kuipers_test(data: np.ndarray,
                             time_windows: np.ndarray,
                             fps: int) -> np.ndarray:
        ...
        for time_window_cnt in prange(time_windows.shape[0]):
            ...
            for win_cnt, window_end in enumerate(prange(int(window_size * 2), data.shape[0] + 1, window_size)):
                window_start = (window_end - window_size) - 1
                sample_1, sample_2 = np.sort(data[prior_window[0]: prior_window[1]]), np.sort(data[window_start:window_end])
                sample_diffs = np.subtract(sample_1, sample_2)
                max_positive_diff, max_negative_diff = np.max(sample_diffs), np.min(sample_diffs)
    ...
    def sliding_hodges_ajne(data: np.ndarray,
        ...
        for i in range(window_size, data.shape[0]):
            ...
            test_statistic = (w_data.shape[0] ** 2) / 12 + abs(np.sum(deviations ** 2)) - ((w_data.shape[0] * ((w_data.shape[0] ** 2 + 1) ** 2)) / 12)
    ...
    def watson_williams_test(sample_1: np.ndarray,
        ...
        r1 = np.sqrt((np.sum(np.cos(sample_1)) / sample_1.shape[0]) ** 2 + (np.sum(np.sin(sample_1)) / sample_1.shape[0]) ** 2)
        r2 = np.sqrt((np.sum(np.cos(sample_2)) / sample_2.shape[0]) ** 2 + (np.sum(np.sin(sample_2)) / sample_2.shape[0]) ** 2)
        r12 = np.sqrt((np.sum(np.cos(t_sample)) / t_sample.shape[0]) ** 2 + (np.sum(np.sin(t_sample)) / t_sample.shape[0]) ** 2)

        f0 = (sample_1.shape[0] * sample_2.shape[0]) / (sample_1.shape[0] + sample_2.shape[0])
...
#
# data = np.array([180, 221, 32, 42, 212, 101, 139, 41, 69, 171, 149, 200]).astype(np.float32)
# print(np.rad2deg(stats.circstd(np.deg2rad(data))))
#
# results = CircularStatisticsMixin().circular_std(data=data)
