...
from simba.utils.enums import Formats, OS, TagNames
...
class PosePlotter():
    def __init__(self,
        ...
        if not os.path.isfile(config_path):
            raise InvalidFilepathError(msg=f'When visualizing pose-estimation, select an input sub-directory of the project_folder/csv folder. {in_dir} is not a sub-directory to the "project_folder/csv" directory and therefore SimBA cant locate the project_config.ini (expected at {config_path}', source=self.__class__.__name__)
    ...
    def run(self):
        for file_cnt, (pose_path, video_path) in enumerate(self.data.items()):
            ...
            if (self.sample_time is None) and (video_meta_data['frame_count'] != len(pose_df)):
                raise CountError(msg=f'The video {video_name} has pose-estimation data for {len(pose_df)} frames, but the video has {video_meta_data["frame_count"]} frames. Ensure the data and video has an equal number of frames.', source=self.__class__.__name__)
            ...
            stdout_success(msg=f'Pose video {video_name} complete', elapsed_time=video_timer.elapsed_time_str, source=self.__class__.__name__)
...
# test = PosePlotter(in_dir='/Users/simon/Desktop/envs/troubleshooting/sleap_dropped_frms/project_folder/csv/input_csv',
#                    out_dir='/Users/simon/Desktop/envs/troubleshooting/sleap_dropped_frms/project_folder/video_tests_',
#                    sample_time=5,
#                    circle_size=10,
#                    core_cnt=1,
#                    color_settings={'Animal_1':  'Green'})
# test.run()
