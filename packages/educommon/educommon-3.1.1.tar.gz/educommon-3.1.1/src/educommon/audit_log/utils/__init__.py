...
def _set_audit_log_tables():
    ...
    # Метод delete() загружает в память данные для отправки сигналов и проверки каскадов.
    # Поэтому, необходимо заранее удалить ненужные логи без загрузки в память, т.к. их может быть много:
    logs_to_delete = AuditLog.objects.filter(table__name__in=table_names_for_deleting)
    logs_to_delete._raw_delete(logs_to_delete.db)
    # Затем, можно спокойно удалить таблицы, изменения которых больше не нужно отслеживать,
