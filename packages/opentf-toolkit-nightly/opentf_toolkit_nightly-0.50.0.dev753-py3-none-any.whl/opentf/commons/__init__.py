...
def annotate_response(response: Response, **kwargs) -> Response:
    ...
    `response`
    """
    if processed := {k for k, v in kwargs.items() if v is not None}:
        response.headers['X-Processed-Query'] = ','.join(processed)
    return response
