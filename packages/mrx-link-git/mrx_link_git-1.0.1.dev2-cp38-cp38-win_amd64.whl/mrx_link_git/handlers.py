...
class GitInfoHandler(GitHandler):
    ...
    arg ex.
    {
        "path": "/home/user/project/test.ipynb" or "/home/user/project",
    }
    ...
    async def get(self, path: str = ""):
        ...
        if not path:
            self.set_status(HTTPStatus.BAD_REQUEST)
            self.finish(json.dumps({"code": 1, "message": "Path is required."}))
            return

        if os.path.isfile(path):
            git_top_level_result = await self.git.get_git_top_level(os.path.dirname(path))
        elif os.path.isdir(path):
            git_top_level_result = self.git.get_git_top_level(path)
        else:
            self.set_status(HTTPStatus.BAD_REQUEST)
            self.finish(json.dumps({"code": 1, "message": "Path must be a file path or a directory path."}))
            return

        if git_top_level_result["code"] != 0:
            self.set_status(HTTPStatus.INTERNAL_SERVER_ERROR)
            self.finish(json.dumps(git_top_level_result))
            return

        git_top_level = git_top_level_result.get("git_top_level", "")
        url_result = await self.git.get_remote_url(path=git_top_level)
        if url_result["code"] != 0:
            self.set_status(HTTPStatus.INTERNAL_SERVER_ERROR)
            self.finish(json.dumps(url_result))
            return

        commit_id_result = await self.git.get_last_commit_id(path=git_top_level)
        if commit_id_result["code"] != 0:
            self.set_status(HTTPStatus.INTERNAL_SERVER_ERROR)
            self.finish(json.dumps(commit_id_result))
            return

        result = {**url_result, **commit_id_result, **git_top_level_result}
        self.finish(json.dumps(result))
...
class GitCheckFileIsPushedHandler(GitHandler):
    ...
    async def get(self, path: str = ""):
        ...
        git_top_level_result = await self.git.get_git_top_level(os.path.dirname(file_path))
        if git_top_level_result["code"] != 0:
            self.set_status(HTTPStatus.INTERNAL_SERVER_ERROR)
            self.finish(json.dumps(git_top_level_result))
            return
        ...
        file_relpath = os.path.relpath(file_path, start=git_top_level)
        is_committed_result = await self.git.check_file_is_committed(path=git_top_level, file_path=file_relpath)
        ...
        if not is_committed:
            result = {"code": 0, "is_pushed": False}
            self.finish(json.dumps(result))
            return

        # Check the commit is pushed to remote
        last_commit_result = await self.git.get_last_commit_id(path=git_top_level)
        if last_commit_result["code"] != 0:
            self.set_status(HTTPStatus.INTERNAL_SERVER_ERROR)
            self.finish(json.dumps(last_commit_result))
            return

        remote_commit_result = await self.git.get_remote_commit_ids(path=git_top_level)
        if remote_commit_result["code"] != 0:
            self.set_status(HTTPStatus.INTERNAL_SERVER_ERROR)
            self.finish(json.dumps(remote_commit_result))
            return

        last_commit_id = last_commit_result.get("last_commit_id", "")
        remote_commit_ids = remote_commit_result.get("remote_commit_ids", [])
        is_pushed = last_commit_id in remote_commit_ids
