...
NEED_TO_MERGE_DRIVER_UPDATE = "NEED_TO_UPDATE"
...
class Git:
    ...
    async def set_credentials_store(self, path):
        """
        Execute git config --global credential.helper store command
        """
    ...
    async def unset_credentials_store(self, path):
        """
        Execute git config --global --unset credential.helper command
        """
    ...
    def _get_script_path() -> str:
        script_path: pathlib.Path = pathlib.Path(__file__).resolve().parent / "scripts" / "main.py"
    ...
    async def check_merge_driver(self, path) -> Dict[str, Any]:
        """
        It checks if Link Git merge driver is configured for a given path(top level repo)

        > Execute
        git config --get-regexp '^merge[.]mrx_link_git_merger[.]'
        git check-attr merge '*.ipynb'

        path: str -> a given git top level abs path to check merge driver
        ...
        {
            "code": 0,
            "message: "NEED_TO_UPDATE"
        }
        or
        {
            "code": 0,
            "message: "ALREADY_EXIST: <MERGE_DRIVER>"
        }
        or
        {
            "code": 0,
        ...
        """
        cfg_cmd = ["git", "config", "--get-regexp", "^merge[.]mrx_link_git_merger[.]"]
        attr_cmd = ["git", "check-attr", "merge", "*.ipynb"]
        msg = ""

        attr_code, attr_output, attr_error = await execute(
            attr_cmd,
            cwd=path,
        )
        if attr_code != 0:
            return {
                "code": attr_code,
                "command": " ".join(attr_cmd),
                "message": attr_error,
            }
        attr_msg = self._check_git_attribute(output=attr_output)
        if attr_msg == NEED_TO_MERGE_DRIVER_UPDATE:
            return {
                "code": 0,
                "message": attr_msg,
            }
        if attr_msg == "Invalid Git Attributes":
            return {
                "code": 0,
                "command": " ".join(attr_cmd),
                "message": attr_msg
        ...
        cfg_code, cfg_output, cfg_error = await execute(
            cfg_cmd,
            cwd=path,
        )
        if cfg_code != 0:
            if cfg_code == 1 and not cfg_output and not cfg_error:
                return {
                    "code": 0,
                    "message": NEED_TO_MERGE_DRIVER_UPDATE,
                }
            return {
                "code": cfg_code,
        ...
        cfg_msg = self._check_git_config(
            output=cfg_output, python_path=self._get_python_path(), script_path=self._get_script_path()
        )
        if cfg_msg == "Invalid Git Config":
            return {
                "code": 0,
                "message": cfg_msg,
            }
    ...
    @staticmethod
    def _check_git_config(output: str, python_path: str, script_path: str) -> str:
        """
        output: 'merge.mrx_link_git_merger.name Git Merge Driver for MRX Link Git\n'
                'merge.mrx_link_git_merger.driver <sys.excutable> <script_file>'
                ' --base-file %O --current-file %A --incoming-file %B --file-path %P\n'
        output.rstrip().split('\n'): [
            'merge.mrx_link_git_merger.name Git Merge Driver for MRX Link Git',
            'merge.mrx_link_git_merger.driver <sys.excutable> <script_file>'
            + ' --base-file %O --current-file %A --incoming-file %B --file-path %P',
        ]
        merge_config: {
        ...
        msg = ""
        merger_config = {
            each_config.split(' ', maxsplit=1)[0]: each_config.split(' ', maxsplit=1)[1]
            for each_config in output.rstrip().split("\n")
        }
        name_info = merger_config.get("merge.mrx_link_git_merger.name", "")
        driver_info = merger_config.get("merge.mrx_link_git_merger.driver", "")
        if not (
            name_info == "Git Merge Driver for MRX Link Git"
            and driver_info == f"{python_path} {script_path}"
            " --base-file %O --current-file %A --incoming-file %B --file-path %P"
        ):
            msg = "Invalid Git Config"
        return msg

    @staticmethod
    def _check_git_attribute(output: str) -> str:
        """
        output: '*.ipynb: merge: mrx_link_git_merger\n'
        """
        msg: str = ""
        merger_attr = output.rstrip().split(": ")

        if len(merger_attr) != 3:
            return "Invalid Git Attributes"
        if not (merger_attr[0] == "*.ipynb" and merger_attr[1] == "merge"):
            return "Invalid Git Attributes"
        if merger_attr[2] != "mrx_link_git_merger":
            if merger_attr[2] == "unspecified":
                return NEED_TO_MERGE_DRIVER_UPDATE
            msg = f"ALREADY_EXIST: {merger_attr[2]}"
        return msg

    async def update_merge_driver(self, path) -> Dict[str, Any]:
        """
        Update Merge Driver for top level repo with git config & git attributes
        ...
        > Edit .gitattributes file in person

        path: str -> The level repo path to update merge driver
        return:
        {
            "code": 0,
            "message: ""
        ...
        cfg_cmds = [
            ...
            [
                "git", "config", "merge.mrx_link_git_merger.driver",
                f"{self._get_python_path()} {self._get_script_path()}"
                " --base-file %O --current-file %A --incoming-file %B --file-path %P"
            ],
        ]
        for cfg_cmd in cfg_cmds:
            cfg_code, _, cfg_error = await execute(
                cfg_cmd,
                cwd=path
            )
            if cfg_code != 0:
        ...
        try:
            attr_comment = "# Merge Driver for MakinaRocks Link Git" + "\n" + "# Visit https://makinarocks.gitbook.io/link/features/version-control/conflict-management#link-git-merge-driver for details"
            attr_cmd = "*.ipynb merge=mrx_link_git_merger"
            attr_content = "\n" + attr_comment + "\n" + attr_cmd + "\n"

            with open(str(pathlib.Path(path) / ".gitattributes"), mode="a", encoding="utf-8") as attr_file:
                attr_file.write(attr_content)
    ...
    async def get_git_top_level(self, path):
        """
        Execute git rev-parse --show-toplevel command and return the git top level path
        """
        cmd = ["git", "rev-parse", "--show-toplevel"]
        code, output, error = await execute(cmd, cwd=path)
        ...
        git_top_level = output.rstrip("\n").strip("'")
        return {"code": code, "git_top_level": git_top_level}

    async def get_remote_commit_ids(self, path):
        """
        Execute git log -1 command & return the commid ID.
        """
        cmd = ["git", "log", "origin", "--format='%H'"]
        code, output, error = await execute(
            cmd,
            cwd=path,
        )
        ...
        if code != 0:
            return {"code": code, "command": " ".join(cmd), "message": error}
        commit_ids = [commit_id.strip("'") for commit_id in output.split("\n")]

        result = {"code": code, "remote_commit_ids": commit_ids}
        return result

    async def get_remote_url(self, path):
        """
        Execute git remote -v command & return the git remote url.
        """
        cmd = ["git", "remote", "-v"]
        code, output, error = await execute(
            cmd,
        ...
        if code != 0:
            return {"code": code, "command": " ".join(cmd), "message": error}
        remote_url = output.split("\n")[0].lstrip("origin\t").rstrip("(fetch)").strip(" ")

        result = {"code": code, "remote_url": remote_url}
        return result

    async def get_last_commit_id(self, path):
        """
        Execute git log -1 command & return the commit ID.
        """
        cmd = ["git", "log", "--format='%H'", "-1"]
        code, output, error = await execute(
            cmd,
            cwd=path,
        )
        ...
        result = {"code": code, "last_commit_id": commit_id}
        return result

    async def check_file_is_committed(self, path, file_path):
        """
        Execute git status 'file_path' command & return whether the file is committed.
        """
        ...
        #     수정함:        iris_classification_advanced.ipynb
        ...
        result = {"code": code, "is_committed": is_committed}
        return result
