...
class RayvisionTransferRaysync():
    ...
    def auto_download(self):
        ...
        try:
            ...
            raysync_name = RAYSYNC_NAME[self._system] + '.zip'
    ...
    def post(self, api_url, data):
        ...
        """
        request_address = self._baseUrl + api_url
        ...
        response = requests.post(
            request_address, json=data, headers=self._headers, proxies={"http": "http://127.0.0.1:%s" % self._port})
    ...
    def listening_raysync_server(self):
        ...
        try:
            self._baseUrl = "%s:%s" % (self._domain, self._port)
            if not self.service_statu:
                self.close_raysync()
                self.logger.info(
                    'Start the Raysync-man.exe to listen for the process(port=%s)....' % self._port)
                run_raysync_cmd = self._raysyncexe if self._system == "Windows" else\
                    '\"%s\" --localConfig' % self._raysyncexe
                self.raysync_app = subprocess.Popen(run_raysync_cmd, shell=True)
                result_code = True
                while result_code:
                    try:
                        url = self._baseUrl + self._url.check_raysync_http
                        response = requests.post(url, headers=self._headers, json={
                                                 "sign": "render-bus"}, proxies={"http": "http://127.0.0.1:%s" % self._port}).json()
    ...
    def convert_upload(self, upload_path, input_id):
        ...
        upload_list_file = os.path.join(os.path.dirname(upload_path), "upload_list.json")
    ...
    def start_transfer(self, server_ip, server_port, local_path, server_path, storage_id, input_id=None, task_type=None,
                       ...
                       max_timeout=18000, network_mode=0, proxy_ip=None, proxy_port=None):
        """
