...
def logs(
    ...
    try:
        # NOTE: type ignore comment below is temporarily required because adding the
        # `offset` param breaks compatibility with older brokers
        res = auth.broker_client.jobs.logs(JobLogRequest(id=job.last_execution_id))  # type: ignore
