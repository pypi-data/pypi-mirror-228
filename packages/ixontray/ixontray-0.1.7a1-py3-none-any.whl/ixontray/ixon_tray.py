...
from ixontray.types.api import Agent, IXapiApplicationID
...
class IxonTray:
    ...
    def add_server_menu(self, agent: Agent, ixon_id: str, sub_menu: QMenu) -> None:
        for name, url in agent.get_webaccess_urls().items():
            menu_action = sub_menu.addAction(name)

            xdg_open_command = Command(
                name=f"XDG_open {name}",
                icon="web-browser-symbolic",
                cmd=f"xdg-open {url}",
                force_connection=False,
            )
            menu_action.triggered.connect(
                functools.partial(self.run_command, command=xdg_open_command, ixon_id=ixon_id),
            )
    ...
    def add_menu_items_for_connected_client(self, menu: QMenu) -> None:
        ...
        if ixon_id:
            ...
            self.add_server_menu(agent, ixon_id, menu)
