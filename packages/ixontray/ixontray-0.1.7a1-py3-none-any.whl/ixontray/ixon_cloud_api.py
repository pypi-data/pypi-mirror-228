...
from ixontray.types.api import Agent, AgentsResponse, CompaniesResponse, Server
...
class IxonCloudAPIv1(IxonCloudAPI):
    ...
    def get_webaccess_url_from_server(self, agent: Agent, server: Server) -> str:
        ...
        if server.type == "vnc":
            return f"https://connect.ixon.cloud/agents/{agent.publicId}/web-access/vnc/{server.publicId}"

        data = {"method": "http", "server": {"publicId": server.publicId}}
        print(server)
        print(data)
        headers = {"IXapi-Company": agent.company_id}
        results = self.send_request(method="POST", data=json.dumps(data), endpoint=full_url, headers=headers)
        print(results)
        return ""
