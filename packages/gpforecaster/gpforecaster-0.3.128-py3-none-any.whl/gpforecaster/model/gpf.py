...
class GPF:
    ...
    def __init__(
        ...
        if isinstance(groups["predict"]["x_values"], torch.Tensor):
            self.last_x = max(groups["predict"]["x_values"].cpu()) + 1
        else:
            self.last_x = max(groups["predict"]["x_values"]) + 1
        ...
        self.groups = self._preprocess(self.groups, self.device)
        ...
        self.train_x = groups["train"]["x_values"]
        self.train_y = groups["train"]["data"].to(torch.float32).to(self.device)

        self.test_x = groups["predict"]["x_values"][-self.groups["h"] :]
        self.test_y = (
    ...
    def _preprocess(
        self,
        groups: Dict[str, Dict[str, Union[np.ndarray, torch.Tensor, list]]],
        device: torch.device,
        ...
        for key in ["train", "predict"]:
            if self.scale_x_values:
                groups[key]["x_values"] = self._convert_and_transform_data(
                    groups[key]["x_values"], self.scaler_x_values, fit=(key == "train")
                )
            else:
                tensor = self._convert_and_transform_data(
                    groups[key]["x_values"], None, fit=False
                )
                groups[key]["x_values"] = to_device(tensor, device)
        ...
        if self.scale_x_values:
            ...
            self.complete_x = to_device(tensor=complete_x, device=device)

        groups["train"]["data"] = self._convert_and_transform_data(
            groups["train"]["data"], self.scaler, fit=True
        )
        groups["predict"]["data_matrix"] = self._convert_and_transform_data(
            groups["predict"]["data_matrix"], self.scaler, fit=False
        )
    ...
    def _train_loop_iteration(
        ...
        Parameters:
            ...
            patience: Parameter for early stopping. Defaults to 4.
            verbose: Print outputs during training. Defaults to True.
        ...
        return loss.item(), val_loss, test_loss, model.state_dict()
    ...
    def train(
        ...
        self.changepoints = torch.tensor(
            changepoints, dtype=torch.float32, device=self.device
        )

        rbf_kernel_lengthscale = torch.tensor(
            [hyperparameters["rbf_kernel_lengthscale"]]
        )
        ...
        hierarchical_data = self.hierarchical_data_summary()
        concatenated_hierarchical_data = self.concatenate_arrays(hierarchical_data)
        ...
        for fold_idx, (train_indices, val_indices) in enumerate(
            ...
            for epoch in range(epochs):
                loss, val_loss, test_loss, state_dict = self._train_loop_iteration(
                    epoch,
                    model,
                    likelihood,
                    val_x=val_x,
                ...
                es, non_decreasing = self.early_stopping(
                    fold_val_losses, patience=hyperparameters["patience"]
                )

                if early_stopping and val_loss is not None and es:
                    print(
                ...
                if val_loss is not None and val_loss < best_val_loss:
                    ...
                    torch.save(
                        best_state_dict,
                        f"./model_weights/best_model_weights_fold_{fold_idx}_"
                        f"{self.gp_type}_{self.dataset}_{self.model_version}.pth",
                    )
        ...
        # Computing average losses over folds, note that we are getting the min
        # validation loss for each fold and the corresponding loss and test_loss
        self.avg_loss = (
            [
                self.losses[i][np.argmin(self.val_losses[i])]
        ...
        self.avg_val_loss = (
            [np.round(min(fold), 3) for fold in self.val_losses]
            if self.val_losses
            else None
        )
        self.avg_val_loss_idx = (
            [np.round(np.argmin(fold), 3) for fold in self.val_losses]
            if self.val_losses
            else None
        )
        self.avg_test_loss = (
            [
                np.round(self.test_losses[i][np.argmin(self.val_losses[i])], 3)
                for i in range(len(self.test_losses))
            ]
            if self.test_losses and self.val_losses
            else None
        )
        min_test_loss = (
            [np.round(min(fold), 3) for fold in self.test_losses]
            if self.test_losses
            else None
        )
        print(
            ...
            f"Average validation loss indexes: {self.avg_val_loss_idx}, \n"
    ...
    def validate(
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            # Moving average loss
            self.val_loss_ma_window.append(val_loss)
        ...
        return val_loss_ma, test_mase.item()
    ...
    def plot_losses(self, start_idx: int = 5):
        ...
        for i, (n_iter, losses, val_losses, test_losses) in enumerate(
            zip(n_iterations, self.losses, val_losses_interp, test_losses_interp)
        ):
    ...
    def _prepare_loss_data(self):
        ...
        test_losses_interp = []
        n_iterations = []
        for idx, (val_losses_fold, test_losses_fold) in enumerate(
            zip(self.val_losses, self.test_losses)
        ):  # Loop over each fold
            n_iterations_total = np.arange(len(self.losses[idx]))
            val_interval = round(len(self.losses[idx]) / len(self.val_losses[idx]))
            val_indices = np.arange(0, len(self.losses[idx]), val_interval)
    ...
    def predict(
        ...
        if self.scale_x_values:
            self.train_x = self.scaler_x_values.inverse_transform(self.train_x)
            self.complete_x = self.scaler_x_values.inverse_transform(self.complete_x)
            self.train_y = self.scaler.inverse_transform(self.train_y)
            self.test_x = self.scaler_x_values.inverse_transform(self.test_x)
