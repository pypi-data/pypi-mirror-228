...
async def check_url_safety(url: str, max_retries=5, retry_delay=5) -> Optional[bool]:
    headers = {'x-apikey': os.environ.get('TELLMGRAMBOT_VIRUSTOTAL_API_KEY')}
    data = {'url': url}
        ...
        if response.status_code == 200:  # OK
            ...
            if analysis_id:
                ...
                while retries <= max_retries:  # While we haven't exceeded max retries
                    ...
                    response = await client.get(url=f'https://www.virustotal.com/api/v3/analyses/{analysis_id}',
                                                headers=headers)
                    if response.status_code == 200:  # OK
                        json_response = response.json()
                        ...
                        if attributes:
                            ...
                            if status == "completed":
                                ...
                                if malicious == 0 and suspicious < 3:
                                    ...
                                    print(f"URL is not safe. Malicious: {malicious}, Suspicious: {suspicious}, Status: {status}")
