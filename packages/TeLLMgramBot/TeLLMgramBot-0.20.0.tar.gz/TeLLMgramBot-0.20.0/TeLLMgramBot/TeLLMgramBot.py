...
from .utils import generate_filename, log_error
...
class TelegramBot:
    ...
    async def tele_start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if current_uname == self.telegram['owner']:
            greeting_text = f'Oh, hello there, {update.message.from_user.first_name}! Let me get to work!'
    ...
    async def tele_handle_response(self, uname: str, text: str) -> str:
        ...
        if not self.started:
            return 'I\'d love to chat, but I haven\'t been started up yet, please wait!!'
