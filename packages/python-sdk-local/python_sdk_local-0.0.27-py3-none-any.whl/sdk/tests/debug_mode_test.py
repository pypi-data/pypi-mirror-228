...
from python_sdk_local.sdk.src.debug_mode import DebugMode
...
def test_debug_modeinit():

    #set env variable
    TEST_DEBUG_MODE_INIT_FUNCTION_NAME = 'test_debug_modeinit()'
    logger.start(TEST_DEBUG_MODE_INIT_FUNCTION_NAME)

    add_debug_file(EXAMPLE_DATA_1)

    DebugMode.init()
    debug_mode.DEBUG_EVERYTHING = False

    remove_debug_file()

    print(debug_mode.LOGGER_JSON)
    print(EXAMPLE_DATA_1)

    result = debug_mode.LOGGER_JSON == EXAMPLE_DATA_1
...
def test_is_logger_output_debug_info_1():
    ...
    result = DebugMode().is_logger_output('1', LoggerOutputEnum.Console.value, 501) == True
...
def test_is_logger_output_debug_mode_2():
    ...
    result1 = DebugMode.is_logger_output('2', LoggerOutputEnum.Logzio.value, 502) == True
    result2 = DebugMode.is_logger_output('2', LoggerOutputEnum.MySQLDatabase.value, 503) == True
    result3 = DebugMode.is_logger_output('2', LoggerOutputEnum.Console.value, 499) == True
...
def test_minimum_sevirity():
    ...
    result1 = DebugMode.is_logger_output('2', LoggerOutputEnum.Logzio.value, 501) == False
    result2 = DebugMode.is_logger_output('2', LoggerOutputEnum.Logzio.value, 502) == True
...
def test_debug_everything():
    ...
    DebugMode.init() # file DNE, so debug_everything = True

    result1 = DebugMode.is_logger_output('2', LoggerOutputEnum.Logzio.value, 501) == True
    print(result1)
    result2 = DebugMode.is_logger_output('2', LoggerOutputEnum.Logzio.value, 502) == True
    print(result2)
