...
// ===================================================================
// experimental functions to control
// the height of outer div based upon
// inner divs height
...
function adjustOuterHeight() {
        ...
        container.style.height = getMaxHeight(children) + "px";
      }
...
function    updateChartConfig(chartInstance, jsonPath, updatedValue){
    console.log("update charg config called");
	...
	throw new Error("cannot deal with non options path");
    }

    for(let i =3; i < keys.length; i++){
	console.log("now visiting ");
	console.log(keys[i]);
	console.log(temp)
	if(i == keys.length -1){
	    ...
	    if (updatedValue){
		console.log("on evalutation: updatedValue = ", updatedValue)
	    }
	}else{
	    console.log("access for key  ");
	    console.log(keys[i]);
	    temp = temp[keys[i]];
	    console.log(temp)
...
class JustpyCore {
	...
	handlePageUpdateEvent(msg) {
	    ...
	    console.log("set jpComponentBuilder.justpyComponents");
	...
	console.log("handleDiffPatchUpdate called");
	// console.log("diff directive: ",  msg.data);
	// let element = document.getElementById("/mylabel");
	// console.log ("captured element /mylabel: ", element);
