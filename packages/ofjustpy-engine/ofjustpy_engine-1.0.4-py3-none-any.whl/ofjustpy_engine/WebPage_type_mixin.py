...
class WebPageMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        # TODO: is events really required?
        self.events = []
        ...
        self.page_id = self.session_manager.session_id + ":" + self.staticCore.id
    ...
    async def update(self, websocket=None):
        ...
        try:
            ...
            # websocket_dict = self.websocket_dict
        except:
            return self
        ...
        if self.to_json_optimized:
            ...
            # print ("json_to_send = ", json_to_send)
    ...
    async def on_disconnect(self, websocket=None):
        print ("=================> Page disconnect CALLED <=============")
        self.is_active = False
        if self.is_cached == False:
            print("===> page-disconnect: page is inactive and not on cache: schedule for cleanup")
            self.session_manager.schedule_page_removal(self)
        else:
            print("===> page_disconnect: page is inactive and but on cache: cleanup postponed")
            pass
        pass

    def purge_page(self):
        #remove from AppDB.pageId_to_webpageInstance
