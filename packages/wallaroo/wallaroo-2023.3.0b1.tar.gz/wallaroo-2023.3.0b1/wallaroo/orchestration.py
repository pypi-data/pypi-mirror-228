...
class Orchestration(Object):
    ...
    def __init__(self, client: Client, data: Dict[str, Any], standalone=False) -> None:
        self.client = client
    ...
    def _fetch_attributes(self) -> Dict[str, Any]:
        return self._gql_client.execute(
            gql.gql(queries.named("orch/GetOrchestration")), {"id": self._id}
        )["orchestration_by_pk"]

    @staticmethod
    def list_orchestrations(client: Client) -> List[Orchestration]:
        from wallaroo.wallaroo_ml_ops_api_client.api.orchestration.orchestration_list import (
            sync,
        )
        from wallaroo.wallaroo_ml_ops_api_client.models.list_orchestrations_request import (
            ListOrchestrationsRequest,
        )

        res = sync(
            client=client.mlops(),
            json_body=ListOrchestrationsRequest.from_dict(
        ...
        return OrchestrationList(
            [Orchestration(client=client, data=v.to_dict()) for v in res]
        )
    ...
    def upload(
        ...
        from wallaroo.wallaroo_ml_ops_api_client.models.orchestration_upload_multipart_data import (
        ...
        u = OrchestrationUploadMultipartData(file, metadata)

        ret = sync(client=client.mlops(), multipart_data=u)

        if ret is None:
            raise OrchestrationUploadFailed("Internal service error")

        orch = Orchestration(client, dict({"id": ret.id}))
        orch._rehydrate()
        return orch
    ...
    def run_once(
        ...
        ret = sync_detailed(client=self.client.mlops(), json_body=oneshot)
    ...
    def run_scheduled(
        ...
        cronjob = CronJobJsonBody(
            name=name,
            timeout=timeout,
            schedule=schedule,
            orch_id=self.id(),
            json=CronJobJsonBodyJson.from_dict(json_args),
            workspace_id=self.workspace_id(),
            debug=debug,
        )
        ret = sync_detailed(client=self.client.mlops(), json_body=cronjob)
    ...
    #         debug=debug,
    #     )
    #     ret = sync_detailed(client=self.client.mlops(), json_body=network_service)
    ...
    def list_tasks(self):
        from .wallaroo_ml_ops_api_client.api.task.get_tasks_by_orch_sha import sync
        from .wallaroo_ml_ops_api_client.models.get_tasks_by_orch_sha_request import (
            GetTasksByOrchShaRequest,
        )
        from .wallaroo_ml_ops_api_client.models.get_tasks_by_orch_sha_response_200 import (
            GetTasksByOrchShaResponse200,
        )

        req = GetTasksByOrchShaRequest(self._sha)
        ret = sync(client=self.client.mlops(), json_body=req)

        if not isinstance(ret, GetTasksByOrchShaResponse200):
            # TODO: Better error
            raise Exception("Failed to find tasks for this orchestration.")

        return TaskList([Task(self.client, {"id": id}) for id in ret.ids])
