...
class Connector(Enum):
    Automatic = "Automatic"
    ServiceEngine = "zenonV6"
    SQL = "zenonSQL"
    DataStorage = "ServiceGridDataStorage"
...
class Server:
    ...
    password:str = None):
        ...
        if(analyzer_major_version >= 3):
            ...
            self._analyzer_major_version = analyzer_major_version
            self._metadb = metadb
    ...
    def read_MetaData_Archives(self)->pd.DataFrame:
        ...
        Returns
        --------
        version >= 11 --> DataFrame: [ID, PROJECT_ID, REFERENCE, VISUALNAME, DESCRIPTION, CYCLETIME LOTARCHIV, PARENT_ID, DESCRIPTION_TRANSLATABLE]
        version < 11 --> DataFrame: [ID, PROJECT_ID, REFERENCE, VISUALNAME, DESCRIPTION, CYCLETIME LOTARCHIV, PARENT_ID]
        """ 

        if(self._analyzer_major_version < 11):
            sqlCmd = f"""SELECT [ID],
                      [PROJECT_ID],
                      [REFERENCE],
                      [VISUALNAME],
    ...
    def read_MetaData_Equipments(self)->pd.DataFrame:
        ...
        Returns
        --------
        version >= 11 --> DataFrame: [ID, REFERENCE, VISUALNAME, DESCRIPTION, NESTINGLEVEL, PARENT_ID, MODEL_ID, VISUALNAME_TRANSLATABLE, DESCRIPTION_TRANSLATABLE]
        version < 11 --> DataFrame: [ID, REFERENCE, VISUALNAME, DESCRIPTION, NESTINGLEVEL, PARENT_ID, MODEL_ID]
        """   
        
        if(self._analyzer_major_version < 11):
            sqlCmd = f"SELECT [ID],[REFERENCE],[VISUALNAME],[DESCRIPTION],[NESTINGLEVEL],[PARENT_ID],[MODEL_ID] FROM [{self._metadb}].[dbo].[EQUIPMENT]"
        else:
            sqlCmd = f"SELECT [ID],[REFERENCE],[VISUALNAME],[DESCRIPTION],[NESTINGLEVEL],[PARENT_ID],[MODEL_ID],[VISUALNAME_TRANSLATABLE],[DESCRIPTION_TRANSLATABLE] FROM [{self._metadb}].[dbo].[EQUIPMENT]"
        try:
            return pd.read_sql(sqlCmd, self._cnxn)
        except:
    ...
    def read_MetaData_Projects(self)->pd.DataFrame:
        ...
        Returns
        --------
        version >= 11 --> DataFrame: [ID, REFERENCE, VISUALNAME, DESCRIPTION, SERVER, STANDBY]
        version < 11 --> DataFrame: [ID, REFERENCE, VISUALNAME, DESCRIPTION]
        """  
    ...
    def read_MetaData_Variables(self)->pd.DataFrame:
        ...
        Returns
        --------
        version >= 11 --> DataFrame: [ID, PROJECT_ID, REFERENCE, VISUALNAME, DESCRIPTION, MEANING, UNIT, VISUALNAME_TRANSLATABLE, DESCRIPTION_TRANSLATABLE, UNIT_TRANSLATABLE]
        version < 11 --> DataFrame: [ID, PROJECT_ID, REFERENCE, VISUALNAME, DESCRIPTION, MEANING, UNIT]
        """  
        if(self._analyzer_major_version < 11):
            sqlCmd = f"""SELECT [ID],
                        [PROJECT_ID],
                        ...
                        [MEANING],
                        [UNIT]
                        FROM [{self._metadb}].[dbo].[VARIABLE]"""
        else:
            sqlCmd = f"""SELECT [ID],
                        [PROJECT_ID],
                        [REFERENCE],
    ...
    def read_Metadata_VariablesInArchives(self, archive_references:list=None, project_references:list=None)-> pd.DataFrame:
    ...
    def read_Metadata_VariablesInEquipmentGroups(self, equipmentgroup_references:list=None, project_references:list=None)-> pd.DataFrame:
    ...
    def read_Metadata_ArchivesInEquipmentGroups(self, equipmentgroup_references:list=None, project_references:list=None)-> pd.DataFrame:
    ...
    def read_Metadata_ArchivesInProjects(self, project_references:list= None)-> pd.DataFrame:
    ...
    def read_MetaData_EventClasses(self)->pd.DataFrame:
        ...
        Returns
        --------
        version >= 11 --> DataFrame: [ID, REFERENCE, VISUALNAME, DESCRIPTION, VISUALNAME_TRANSLATABLE, DESCRIPTION_TRANSLATABLE]
        version < 11 --> DataFrame: [ID, REFERENCE, VISUALNAME, DESCRIPTION]
        """   
        ...
        if(self._analyzer_major_version < 11):
            ...
            sqlCmd = f"SELECT [ID],[REFERENCE],[VISUALNAME],[DESCRIPTION],[VISUALNAME_TRANSLATABLE],[DESCRIPTION_TRANSLATABLE] FROM [{self._metadb}].[dbo].[EVENTCLASS]"
        try:
            return pd.read_sql(sqlCmd, self._cnxn)
        except:
    ...
    def read_MetaData_EventGroups(self)->pd.DataFrame:
        ...
        if(self._analyzer_major_version < 11):
            ...
            sqlCmd = f"SELECT [ID],[REFERENCE],[VISUALNAME],[DESCRIPTION],[VISUALNAME_TRANSLATABLE],[DESCRIPTION_TRANSLATABLE] FROM [{self._metadb}].[dbo].[EVENTGROUP]"
        try:
            return pd.read_sql(sqlCmd, self._cnxn)
        except:
    ...
    connector:Enum = Connector.Automatic.value,
    variable_references = [], 
    ...
    show_visualnames:bool = False
        ...
        connector: `Enum`
            zenon Connectors: 'ServiceEngine', 'SQL', 'ServiceGridDataStorage', 'Automatic'
        ...
        """
        self._check_Connector(connector)

        variables = self._convert_variable_filter(variable_references)
    ...
    show_visualnames:bool = False
        ...
        variables = self._convert_variable_filter(variable_references)
        try:
            # Report Engine database has version 8 or higher. Here filtering on calcualtion type is directly possible
            if(self._version >= 8):
                if not  show_visualnames:
    ...
    show_visualnames:bool = False
        ...
        """Lists the current values of the selected variables from Service Engine. 
    ...
    archive_references:[]=None)-> pd.DataFrame:
        ...
        if archive_references == None:
            archives = self.read_Metadata_ArchivesInProjects(project_references=[project_reference])
    ...
    def read_Online_ContextList(self, project_reference:str, connector:Enum = Connector.Automatic.value)->pd.DataFrame:
    ...
    def _convert_variable_filter(self, current_list:list):
        if not current_list:
            return ""
        else:
            working = current_list.copy()
            if(self._analyzer_major_version >= 12):
                for i in range(len(working)):
                    working[i] = working[i].replace("&", "&a;").replace(",", "&c;")
            return ",".join(map(str, working))
    ...
    def _check_Connector(self, connector:Connector):
        if(self._analyzer_major_version < 11 and (connector == Connector.Automatic.value or connector == Connector.DataStorage.value)):
            raise RuntimeError("The Automatic and ServiceGridDataStorage connectors require a Report Engine with the version 11 or higher")  