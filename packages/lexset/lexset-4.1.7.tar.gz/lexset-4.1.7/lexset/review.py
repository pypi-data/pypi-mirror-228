...
class analysis:
    ...
    def plot_pixel_intensity_distribution(self, type = "Lexset"):
        ...
        if type == "Lexset":
            # Filter out the list to include only jpg, png, and tiff files
            valid_images = [img for img in all_images if os.path.splitext(img)[1].lower() in ['.jpg', '.png', '.tiff', '.tif'] and re.search(r'\.rgb_\d{4}\.png$', img)]
        else:
            valid_images = [img for img in all_images if os.path.splitext(img)[1].lower() in ['.jpg', '.png', '.tiff', '.tif']]
    ...
    def compare_ssim_distributions(self, compare_dir, target_size=(256, 256)):
        ...
        naming_convention_exists = any(re.search(r'\.rgb_\d{4}\.png$', img) for img in synthetic_images)
        
        if naming_convention_exists:
            # Use only images that follow the naming convention
            valid_images = [img for img in synthetic_images if re.search(r'\.rgb_\d{4}\.(jpg|png|tiff|tif|bmp)$', img, re.IGNORECASE)]
        else:
            # Use all images with valid extensions
            valid_images = [img for img in synthetic_images if os.path.splitext(img)[1].lower() in ['.jpg', '.png', '.tiff', '.tif', '.bmp']]
            
        synthetic_ssim_values = []
        
        for synthetic_image in valid_images:
            synthetic_image_path = os.path.join(self.dir, synthetic_image)
            real_image_path = os.path.join(compare_dir, real_images[np.random.randint(len(real_images))])
        ...
        # Save the SSIM values to a JSON file
