...
_PEP425_PYTHON_TAGS_IMPLEMENTATION = {
    'cp': 'cpython',
    'ip': 'IronPython',
    'pp': 'PyPy',
    'jy': 'Jython'
...
def find_eggs_for_plat_repo(brood_client, plat_repo, python_tags=None):
    ...
    if python_tags is None or len(python_tags) == 0:
        python_tags = brood_client.list_python_tags()

    for python_tag in sorted(python_tags):
...
def unpack_index(index, python_tags=None):
    ...
    for python_tag in python_tags:
        impl = _PEP425_PYTHON_TAGS_IMPLEMENTATION[python_tag[:2]]
        implementations[impl.lower()] = python_tag[2:]
    ...
    for meta in find_metadata(index):
        if implementations:
            meta_impl = meta['implementation'].lower()
            try:
                version = implementations[meta_impl]
            except KeyError:
                continue
            else:
                meta_version = meta['version'].replace('.', '')
                if not meta_version.startswith(version):
                    continue
...
def _validate_python_tag(context, param, value):
    for tag in value:
        if tag[:2] not in _PEP425_PYTHON_TAGS_IMPLEMENTATION:
            raise click.BadParameter(
                f"Supported PEP425 tags are cp, ip, pp or jy. Received: {tag}")
...
@click.option(
    '-t', '--python-tags', multiple=True,
    callback=_validate_python_tag,
    type=str,
    help="Python tags to export. Supported tags are `cpXX`, `ipXX`, `ppXX`, "
         "and `jyXX`.")
@click.argument(
...
@pass_brood_client
def export_db(
        brood_client, artefact_kinds, repositories, platforms, python_tags,
        target_dir):
    ...
    for platform_repo in iter_plat_repos(requested_repos, requested_platforms):
        ...
        for artefact_kind in artefact_kinds:
            ...
            if artefact_kind == 'runtimes':
                ...
                if runtimes_index:
                    rt_metadata = unpack_index(runtimes_index, python_tags)
                    msg = "Exporting runtimes from {0}/{1} for {2}".format(
                        org_name, repo_name, platform
                    )
                    if python_tags:
                        msg = "{0} and python-tags: {1}".format(
                            msg, ', '.join(python_tags)
                        )
                    click.echo(msg)
                    download_runtimes(
            ...
            if artefact_kind == 'eggs':
                eggs = find_eggs_for_plat_repo(
                    brood_client, platform_repo, python_tags)
                if len(eggs) > 0:
                    msg = "Exporting eggs from {0}/{1} for {2}".format(
                        org_name, repo_name, platform
                    )
                    if python_tags:
                        msg = "{0} and python-tags: {1}".format(
