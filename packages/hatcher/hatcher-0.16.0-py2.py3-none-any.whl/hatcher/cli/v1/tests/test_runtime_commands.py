...
class TestUploadRuntimeMain(MainTestingMixin, unittest.TestCase):
    ...
    def test_without_force(self):
        ...
        upload_url = '{host}{uri}'.format(
            host=self.host,
            uri=URLS_V1.data.runtimes.upload.format(
    ...
    def test_upload_verify_no_upstream(self):
        ...
        metadata_url = '{host}{uri}'.format(
            host=self.host,
            uri=URLS_V1.metadata.artefacts.runtimes.format(
                organization_name=self.organization,
                repository_name=self.repository,
                platform=self.platform,
                implementation='cpython',
...
class TestDeleteRuntimeMain(MainTestingMixin, unittest.TestCase):

    def setUp(self):
        MainTestingMixin.setUp(self)
        self.host = 'http://brood-dev.invalid'
        self.platform = "rh7_x86_64"
        self.initial_args = ['--api-version', '1',
                             '--url', 'brood-dev.invalid', 'runtimes',
                             'delete']
        self.final_args = [self.organization, self.repository, self.platform]
    ...
    def test_delete_runtime(self):
        # Given
        args = ['cpython', '3.8.17+1']
        runtime_url = '{host}{uri}'.format(
            host=self.host,
            uri=URLS_V1.data.runtimes.delete.format(
                organization_name=self.organization,
                repository_name=self.repository,
                platform=self.platform,
        ...
        responses.add(responses.DELETE, runtime_url, status=204)

        # When
        result = self.runner.invoke(
            main.hatcher,
            args=self.initial_args + ['-y'] + self.final_args + args,
        )

        # Then
        self.assertEqual(result.exit_code, 0)
        self.assertEqual(len(responses.calls), 1)
        runtime_delete_request, = responses.calls
        self.assertTrue(
            runtime_delete_request.request.url.startswith(runtime_url)
        )
