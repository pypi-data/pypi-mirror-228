...
class TestEggMetadataValidation(unittest.TestCase):
    ...
    def test_from_egg_supported_metadata_version(self):
        # Given
        name = 'good_egg'
        version = '1.0'
        build = 1
        ...
        # When
        egg_metadata = EggMetadata.from_file(target)

        # Then
        self.assertEquals(
            egg_metadata._egg_metadata, OkonomiyakiEggMetadata.from_egg(target)
        )
        self.assertEquals(egg_metadata.path, target)

    def test_from_egg_unsupported_metadata_minor_version(self):
        # Given
        name = 'bad_egg'
        version = '1.0'
        build = 1
        ...
        with self.assertRaisesRegexp(
                ...
                (r"Egg '{0}' uses an unsupported metadata version {1}. "
                 "Hatcher {2} supports only eggs up to {3}.".format(
                    ...
                    hatcher.__version__,
                    EggMetadata.MAX_METADATA_VERSION))):
            EggMetadata.from_file(target)

    def test_from_egg_unsupported_metadata_major_version(self):
        # Given
        name = 'bad_egg'
        version = '1.0'
        build = 1
