        ...
        if False not in list(check_bool_dict.values()):
            ...
            if response_ok == True:
                ...
                if price_adj == False:
                    ...
                    json1 = json.loads(response.content).replace("'", "\"").replace('[(', '\"[(').replace(')]',')]\"').replace('False','0').replace('True','1').replace('nan','NaN')
                ...
                if price_adj == True:
                    ...
                    qfq_df['index'] = qfq_df['index'].str.replace('-', '', regex=False)
                    df = pd.merge(left=df, left_on='date', right=qfq_df, right_on='index')

                    if df['qfq_factor'].isnull().values.any():
                        df['qfq_factor'] = df['qfq_factor'].fillna(method='ffill')
    ...
    def get_ccass_by_code(self, code, start_date, end_date):
        ...
        if False not in list(check_bool_dict.values()):
            ...
            if response_ok == True:
                ...
                df['total_non_ccass_participants_shares'] = df['total_issued_shares'] - df['total_ccass_participants_shares']
