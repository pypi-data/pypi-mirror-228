...
class FabricDataFrame(_SDataFrame):
    ...
    Parameters
    ----------
    data : numpy.ndarray, typing.Iterable, dict or pandas.DataFrame
    ...
    **kwargs : dict
    ...
    def _get_common_workspace_and_dataset(self) -> Tuple[Union[str, UUID], str]:
        ...
        num_workspace_names = df_column_metadata[MetadataKeys.WORKSPACE_NAME].nunique() if "workspace_name" in df_column_metadata else 0
        num_workspace_ids = df_column_metadata[MetadataKeys.WORKSPACE_ID].nunique() if "workspace_id" in df_column_metadata else 0
    ...
    def _map_metadata_to_table_column_tuples(self, columns: Optional[List[str]] = None) -> List[Tuple[str, str]]:
        """
        Converts specified columns (or all columns with metadata if columns=None) to tuples in the form (TableName, ColumnName).
        """
        from sempy.fabric import MetadataKeys
        ...
        for col, metadata in (self.column_metadata or dict()).items():
            if col in columns:
                tuple_list.append((metadata[MetadataKeys.TABLE], metadata[MetadataKeys.COLUMN]))
        return tuple_list
    ...
    def add_measure(
        ...
        *measures : list[str]
        ...
        if self.column_metadata is None:
            ...
            raise ValueError("Cannot join measures without column metadata or dataset. Provide a dataset name to enable auto-resolution of column names, set column metadata manually or retrieve a FabricDataFrame from a PowerBI dataset using read_table.")  # noqa: E501
        ...
        dataset_client = _get_or_create_workspace_client(workspace) \
            .get_dataset_client(dataset, use_xmla)  # type: ignore
        ...
        df_measure = dataset_client.evaluate_measure(measures[0], self._map_metadata_to_table_column_tuples(valid_columns), verbose=verbose)  # type: ignore
    ...
    @log
    def to_onelake(self, table: str, mode: Optional[str] = "error"):
        """
        ...
        ----------
        table : str
            The name of the table to write to OneLake.
        mode : str
