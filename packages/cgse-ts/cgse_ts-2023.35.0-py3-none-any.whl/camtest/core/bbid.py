...
bbid_yaml_filename = find_file(
    "bbid.yaml", in_dir="camtest/core", root=find_root(__file__, tests=("MKDOCS",))
)

bbid_values = Settings.load("BBID", filename=bbid_yaml_filename)


def get_bbid_for_func(func: Callable) -> int:
    """Returns the building block identifier for the given function"""
    if not isinstance(func, Callable):
        raise ValueError(f"The given argument should be a function or method.")
