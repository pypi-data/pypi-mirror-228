...
class Statistics:
    ...
    def stat_table(
        ...
        if columns is None:
            columns = df.keys()
        return pd.DataFrame({k: self.calculate(df[k]) for k in columns})
...
CATEGORICAL_STATISTICS = Statistics(
    [Size(), Size(True), Density(), Min(), Max(), Mode()]
)

if __name__ == "__main__":
    print(
        Extremum("count").calculate(pd.Series([(x + 7) ** 2 for x in range(-100, 100)]))
    )
    ...
    print(DEFAULT_STATISTICS.stat_table(df))
