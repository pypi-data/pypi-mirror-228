...
class SqliteDbPool:
    ...
    async def add(self, table: str, data: dict[str, Any], *, ignore: bool = False):
        async with self.write() as db:
            return await db.add(table, data, ignore=ignore)

    async def addList(self, table: str, dataList: list[dict[str, Any]], *, ignore: bool = False):
        async with self.write() as db:
            return await db.addList(table, dataList, ignore=ignore)
...
class _SqliteDbWrite(_SqliteDbRead):
    ...
    async def add(self, table: str, data: dict[str, Any], *, ignore: bool = False):
        ...
        async with self._db.execute(
            f'''
            INSERT {'OR IGNORE' if ignore else '' } INTO `{table}` ({fieldname_list})
            VALUES
...
class SqliteDbModel(BaseModel):
    ...
    async def add(self, ignore: bool = False):
        return await self._add(ignore=ignore)

    async def _add(self, exclude: set[str] = Null, include: set[str] = Null, *, ignore: bool = False):
    ...
    @classmethod
    async def addList(cls, modelList: Sequence[SqliteDbModel], *, ignore: bool = False):
        return await cls._addList(modelList, ignore=ignore)

    @classmethod
    async def _addList(cls, modelList: Sequence[SqliteDbModel], exclude: set[str] = Null, *, ignore: bool = False):
        if not modelList:
            return 0
