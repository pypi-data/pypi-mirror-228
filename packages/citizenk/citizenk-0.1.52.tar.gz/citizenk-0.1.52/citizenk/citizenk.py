...
class CitizenK(FastAPI):
    ...
    async def shutdown(self):
        ...
        for agent in self.websocket_agents.values():
            logger.debug("Shutting down websocket agents...")
            await agent.close()
        if self.main_consumer_loop_task is not None:
            logger.debug("Shutting down main consumer loops...")
            self.main_consumer_loop_task.cancel()
        if self.background_loop is not None:
            logger.debug("Shutting down background loop...")
    ...
    async def _main_consumer_loop(self):
        ...
        while True:
            ...
            try:
                consumed = await self.consumer_step()
                # Poll producer
                if self.producer is not None:
                    self.producer.poll()
