...
class KafkaAdapter:
    ...
    def get_all_broker_groups(self) -> list[str]:
        future = self.admin.list_consumer_groups(request_timeout=10)
        try:
    ...
    def get_group_members(self) -> dict[Tuple, str]:
        ...
        members = {}
        future_map = self.admin.describe_consumer_groups(groups, request_timeout=10)
        if len(future_map) == 0:
    ...
    def get_partitions_with_offsets(
        ...
        for topic in topics:
            ...
            metadata = self.connection.list_topics(topic, timeout=10)
    ...
    def delete_groups(self, groups: list[str]) -> int:
        ...
        future_map = self.admin.delete_consumer_groups(
            groups_to_delete, request_timeout=10
        )
        count = 0
    ...
    def delete_offset_for_group_api(
        ...
        if topic is None:
            return 0
        topics = self.admin.list_topics(topic, timeout=10).topics
        if len(topics) == 0:
