...
class Options:
    ...
    THRESHOLDS = (sys.argv[3],90, 90, 90, 90) # tuple([ 25*pow(2,i) for i in range(5) ])
...
def init(self):
    ...
    outh5 = os.path.split(self.HNAME)[-1].replace(".h5", f"_scale{bg_scale}_sparse.h5")
...
def dosegment( frm ):
    if dosegment.cache is None:
        ...
        if OPTIONS.MASK is None:
            msk = np.zeros( frm.shape, np.bool )
        else:
            msk = OPTIONS.MASK
