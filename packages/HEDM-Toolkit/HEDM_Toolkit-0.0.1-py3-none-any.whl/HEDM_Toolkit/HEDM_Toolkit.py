...
class FileConverter:
    ...
    def run_sparse_script(self):
        script_path = pkg_resources.resource_filename('hedm_pre', 'scripts/sparse_h5.py')
    ...
    def run_gen_flt_script(self):
        script_path = pkg_resources.resource_filename('hedm_pre', 'scripts/gen_flt.py')
        dx_file = pkg_resources.resource_filename('hedm_pre', 'data/imaged11_dx.edf')
        dy_file = pkg_resources.resource_filename('hedm_pre', 'data/imaged11_dy.edf')
        ...
        sparse_file = f"{base_name}_ilastik_proc_scale0_sparse.h5"
    ...
    def run_clean_flt_script(self):
        script_path = pkg_resources.resource_filename('hedm_pre', 'scripts/clean_flt.py')
        ...
        merge3D_tor = 1.6
        par_file = f"{base_name}.par"
        clean_flt_file = f"{base_name}_ilastik_proc_clean.flt"
    ...
    def run_ImageD11_indexing_script(self):
        script_path = pkg_resources.resource_filename('hedm_pre', 'scripts/indexing.py')
    ...
    def run_ImageD11_fitting_script(self, u_file, U_file, par_file, flt_file_name, t, omega_slop):
        script_path = pkg_resources.resource_filename('hedm_pre', 'scripts/fitting.py')
        cmd = [
            'python', script_path,
            '-u', u_file,
            '-U', U_file,
...
class ff_HEDM_process(FileConverter):
    def convert(self):
    ...
    for layer in range(params['layers']):
        ...
        # Call conversion function with the specific converter class you want to use
            if args.command == 'slice':
                ...
                run_conversion(ff_HEDM_process, **params)
