...
def SynchStockItemInfo(in_shCode='', gubun='0', updateInfoType='0', frProcCnt=1, toProcCnt=99999, allRedoInd=False): 
    ...
    if len(in_shCode) > 0:
        dfTemp = dfTemp[dfTemp['종목코드']==in_shCode]
    
    shcodeList = dfTemp.values.tolist()    
    print(dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), '전체 종목수 >> ', len(shcodeList))

    if frProcCnt > 1 or toProcCnt < len(shcodeList):
        shcodeList = shcodeList[max(0,frProcCnt-1) : min(len(shcodeList),toProcCnt)]
        print(dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), '작업대상(분할) >> ', len(shcodeList))

    if allRedoInd == True:
        workingList = shcodeList
    else:
        # 동일 일자로 기생성된 종목 제외
        workingList = []
    ...
    for i, item in enumerate(workingList):
        ...
        if updateInfoType == '0':
            while procInd == False:
                try:    
                    instXAQuery = ebest.GetTrData('t3320', gicode=shCode+'A', SLEEPTIME=3)
...
def SynchDailyStockTradeInfo(sDate, eDate, shCode='', gubun='0', periodKnd=''):
...
#             '' (저장된 데이터가 있는 경우 저장된 데이터 이후부터 eDate까지 동기화)
    ...
    for i, x in enumerate(getList):
        ...
        while procInd == False:
            try:    
                instXAQuery = ebest.GetTrDataOccurs('t8410', shcode=shcode, gubun='2', qrycnt=2000, sdate=x[1], edate=x[2], sujung='Y', SLEEPTIME=3)
...
def  SynchEbestMarketIndexInfo(sDate, eDate, symbol='', periodKnd='', maxProcCnt=9999):
    """
    periodKnd : 'ALL' (sDate 이전 데이터도 동기화) -> 최근에 상장된 주식들도 매번 작업 수행
                '' (저장된 데이터가 있는 경우 저장된 데이터 이후부터 eDate까지 동기화)
    ...
    for i, listVal in enumerate(getList):
        ...
        instXAQuery = ebest.GetTrDataOccurs('t3518', kind=kind, symbol=symbol, cnt=500, jgbn='0', 
                                            SLEEPTIME=3, SEARCHRANGE=[sDate, eDate])
