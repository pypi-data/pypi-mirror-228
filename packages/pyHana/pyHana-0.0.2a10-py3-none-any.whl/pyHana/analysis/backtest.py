...
def backtest_report(  dfStock = []    # backtest 대상 데이터(유효기간)
    ...
    # # # 기본 수익률(분석기간)
    basicEarnRatio = ( ( dfStock.iloc[len(dfStock)-1][pType] * 0.99785 - dfStock.iloc[0][pType] * 0.00015 )
    ...
    print('( 매수가 :', f"{dfStock.iloc[0][pType]:,}", '[' + dfStock.iloc[0]['일자'] + ']',        
          ', 매도가 :', f"{dfStock.iloc[len(dfStock)-1][pType]:,}", '[' + dfStock.iloc[len(dfStock)-1]['일자'] + '] )') 
