...
def getValidTradeInfo(shCode, sDt, tDt):
    ...
    df['종목코드'] = shCode
    df['종목명'] = code.GetStockItem(shCode)['종목명'].values[0]
    df = df[['종목코드', '종목명']+code.saveColumns["일별주가"]]      
    
    return df

# (국내외)일별 마켓지수 데이터와 일별 주가 정보간 거래일자 불일치 건 매핑을 위해 
# 일별 마켓지수 365일 데이터 생성 (누락된 일자는 전일자 기준으로 생성)
def makeFullDayData(df):
    ...
    return pd.DataFrame(resData, columns = ['Signal일자']+column_list)
...
def mergeStockSignal(dfStock, dfSignal, advance=0):
    """       
    advance : signal 발생일 이후 (advance) 영업일 이후 거래하도록 데이터 매핑 (default : 0)
    """    
    column_list = dfStock.columns.values.tolist() + dfSignal.columns.values.tolist()    
    idxStock = 0
    idxSignal = 0    
    mergeData = []
    
    # 선행일자 적용
    applyDt = dfSignal['Signal일자'].tolist()[advance:]
    if advance != 0:
    ...
    tradeDt = dfStock['일자'].tolist()
    dfStock = dfStock.values.tolist()
    dfSignal = dfSignal.values.tolist()
        
    while idxStock < len(tradeDt) and idxSignal < len(applyDt):
        
        if tradeDt[idxStock] < applyDt[idxSignal]:
            idxStock += 1
        elif tradeDt[idxStock] > applyDt[idxSignal]:
            idxSignal += 1        
        else:
            mergeData.append( dfStock[idxStock] + dfSignal[idxSignal] )                
            idxStock += 1
            idxSignal += 1

    dfRes = pd.DataFrame(mergeData, columns = column_list)
