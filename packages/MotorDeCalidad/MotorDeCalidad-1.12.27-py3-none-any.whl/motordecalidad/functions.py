...
MotorVersion = "1.12.27"
inform(f"Motor de Calidad Version Release {MotorVersion}")
cipher_suite = Fernet('j5uuSPUfbIN0CiBOvYceooGR5qu2bg64p1kY7ravNRw=')
...
def extractParamsFromJson(config):
    try:
        try:
            ...
            account_name = "adlseu2edthdev001"
            account_key = "T1RZsgj62zrRWcsYRW3QGr3+TEhtalj8o/fU3Zqmh4ef3TYxZw0P7+neqmgOPmbFOoVPZhLFT9GV+AStAj2YpA=="
            container_name = "rules"
            blob_name = f"configFiles/motor_dq/{tableName}"
...
def validateRules(object:DataFrame,rules:dict,registerAmount:int, entity: str, project:str,country: str,domain: str,subDomain: str,segment: str,area: str,error,dataDate:str,validData:str, Data,email,output,dfltDataDate):
    ...
    for code in rules:
        if rules[code].get(JsonParts.Fields) not in [0,["0"],"0"] :
            ...
            if code[0:3] == Rules.NullRule.code:
            ...
            elif code[0:3] == Rules.validateTimeInRange.code:
                print("Inicializando regla de validación de rango de fechas")
                column = rules[code].get(JsonParts.Fields)
                threshold:int = rules[code].get(JsonParts.Threshold)
                minRange = rules[code].get(JsonParts.MinRange)
                maxRange = rules[code].get(JsonParts.MaxRange)
                ...
                for field in column :
                    ...
                    if data[-One] > Zero:
                        ...
                        if write != "FALSE" :
                            errorData = errorData.union(errorTotal)
                    rulesData.append(data)
                    print("regla de validación de rango de fechas: %s segundos" % (time.time() - t))

            elif code[0:3] == Rules.validateConditional.code:
                print("Inicializando regla de condicional")
                threshold:int = rules[code].get(JsonParts.Threshold)
                ...
                if data[-One] > Zero:
                    ...
                    if write != "FALSE" :
                        errorData = errorData.union(errorTotal)
                rulesData.append(data)
                print("regla de validación condicional: %s segundos" % (time.time() - t))

            elif code[0:3] == Rules.validatePositionValue.code:
                print("Inicializando regla de validación en posición específica")
                columnName = rules[code].get(JsonParts.Fields)
                threshold:int = rules[code].get(JsonParts.Threshold)
                initialPosition:int = rules[code].get(JsonParts.initialPosition)
                finalPosition:int = rules[code].get(JsonParts.finalPosition)
                ...
                for field in columnName :
                    ...
                    if data[-One] > Zero:
                        ...
                        if write != "FALSE" :
                            errorData = errorData.union(errorTotal)
                    rulesData.append(data)
                    print("regla de validación en posición específica: %s segundos" % (time.time() - t))
            
            elif code[0:3] == Rules.validateEmail.code:
                print("Inicializando regla de validación de email")
                columnName = rules[code].get(JsonParts.Fields)
                threshold:int = rules[code].get(JsonParts.Threshold)
                referalData = rules[code].get(JsonParts.Input)
                ...
                for field in columnName :
                    ...
                    if data[-One] > Zero:
                        ...
                        if write != "FALSE" :
                            errorData = errorData.union(errorTotal)
                    rulesData.append(data)
                    print("regla de validación de Email: %s segundos" % (time.time() - t))
    ...
    try:
        ...
        print("Se omite la escritura del log")
