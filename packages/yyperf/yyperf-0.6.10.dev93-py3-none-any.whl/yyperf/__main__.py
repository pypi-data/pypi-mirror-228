...
from .web.handlers.page import (
    BaseHandler, DeviceConnectHandler, UiHandler,
...
from .web.handlers.perf_page import (DeviceListHandler, PackageListHandler, StartWDAHandler, CloseWDAHandler,
                                    AddOrGetDataHandler,DeleteOrUpdateDataHandler,
                                    DeviceAPPHandler, PerfCaptrueHandler, ExportDataHandler, DeviceAPPInstallHandler)
from .web.handlers.proxy import StaticProxyHandler
from .web.handlers.shell import PythonShellHandler
from .web.utils import current_ip
from .web.handlers.perf_page import engine
from .web.handlers.models import Base
...
def make_app(settings={}):
    application = tornado.web.Application(
        [
            ...
            (r"/api/v1/devices/install", DeviceAPPInstallHandler),
            (r"/api/v1/devices/([^/]+)/packagelist", PackageListHandler),
            (r"/ws/v1/captrue", PerfCaptrueHandler),
            (r"/api/v1/export/(.*)/(.*)", ExportDataHandler),
            (r"/api/v1/startWDA", StartWDAHandler),
            (r"/api/v1/close/wda", CloseWDAHandler),
            ...
            (r"/api/v1/data", AddOrGetDataHandler),
            (r"/api/v1/data/([^/]+)", DeleteOrUpdateDataHandler),
            (r"/quit", QuitHandler)
        ],
        **settings)
    return application
...
def remove_data_file():
    ...
    for dirs in dir_list:
        try:
            if dirs.endswith(".ipa") or dirs.endswith(".apk"):
