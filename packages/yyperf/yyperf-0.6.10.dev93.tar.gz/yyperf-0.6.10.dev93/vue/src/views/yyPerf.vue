<template>
  <div>
    ...
    <el-table :data="simple_perf_data" style="width: 100%;margin-bottom: 10px">
      <el-table-column v-for="(item, index) in perf_chart_data" v-if="item.show===true" :formatter="setAvgResult"
                       :key="'col'+index"
                       :label="item.title">
      </el-table-column>
    </el-table>
    <div style="margin-bottom: 10px; display: inline-flex; flex-wrap: wrap; justify-content: center">
      <template v-for="(item, index) in perf_chart_data">
        <el-card v-show="item.show" lazy v-loading="reloading" :key="'card_'+index"
                 :style="layoutCss">
          <!--            <highcharts :options="echart_object[index]" :constructor-type="'stockChart'"-->
          <!--                        :ref='"chart_"+index'-->
          <!--                        :style="layoutCss2"-->
          <!--            ></highcharts>-->
    ...
    >
      ...
      <el-card class="box-card" style="margin: 8px">
        ...
        <el-form>
          <el-form-item label="">
            <el-button @click="dialogVisible1 = true">显示采集历史</el-button>
          </el-form-item>
        </el-form>
      </el-card>
...
export default {
  ...
  methods: {
    ...
    getSelectData: function () {
      let key = []
      for (let i in this.perf_chart_data) {
        if (this.perf_chart_data[i].show) {
          key.push(i)
        }
      }
      return key
        ...
        if (data.hasOwnProperty('filepath')) {
          ...
          for (let index = 0; index < data.length; index++) {
            ...
            Object.keys(that.perf_chart_data).forEach(key => {
              if (that.perf_chart_data[key].show) {
