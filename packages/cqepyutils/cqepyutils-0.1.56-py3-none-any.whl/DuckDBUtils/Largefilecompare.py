...
def run_data_comparison(conn, src_path, trg_path, primary_keys, table_src='table_src', table_trg='table_trg'):
    ...
    # Execute the query without fetching all the results at once
    total_data_break_cursor = conn.execute(total_data_break_query)

    # Iterate through the results in chunks (e.g., 100000 rows at a time)
    chunk_size = 100000
    total_data_break_data = []  # Collect all the breaks here
    while True:
        chunk = total_data_break_cursor.fetchmany(chunk_size)
        if not chunk:
            break

        # Process the chunk of data here
        for row in chunk:
            src_data = row[:len(columns_src_names)]
            trg_data = row[len(columns_src_names):]

            # Determine if there is a data break
            for idx, col in enumerate(compare_columns):
                src_value = src_data[src_indices[idx]]
                trg_value = trg_data[trg_indices[idx]]
                if src_value != trg_value:
                    # Collect mismatch details
                    mismatched_columns = {key: src_data[columns_src_names.index(key)] for key in primary_keys}
                    mismatched_columns[f"{col}_Expected"] = src_value
                    mismatched_columns[f"{col}_Actual"] = trg_value
                    total_data_break_data.append(mismatched_columns)

    logging.info(f"Total Data Break Query took {time.time() - start_time} seconds")
    ...
    # Prepare the data for CSV
    total_data_break = total_data_break_data  # Use the collected break data
