...
def df_diff(source_file_path_name: str, target_file_path_name: str, delimiter: str = None, has_header: bool = True,
    ...
    dup_target_df['source'] = 'target'
    dup_source_df['source'] = 'source'
    ...
    # exec_summary_col = ['Summary', 'target', 'source', 'Mismatch']
    #
    # exec_summary_df = pd.DataFrame(columns=exec_summary_col)
    # exec_summary_df.loc[1] = ['Total_Records', total_target, total_source, total_mismatch]
    # exec_summary_df.loc[2] = ['Duplicates', total_target_dup, total_source_dup, 0]
    # exec_summary_df.loc[3] = ['Key_Mismatch', target_key_mismatch, source_key_mismatch, 0]
    #
    ...
    count_details = {
        ...
        'Total Data Breaks': 0,
        'Only in Source': len(source_df) - len(target_df),
        'Only in Target': len(target_df) - len(source_df)
    }

    # 'Total Key & Data Matched': len(cell_comp_df[cell_comp_df['Compare_Result'] == 'Match']),
    # 'Total Data Breaks': len(cell_comp_df[cell_comp_df['Compare_Result'] == 'Mismatch']),

    # Create the executive summary df
    exec_summary_col = ['Summary', 'Source Total', 'Target Total', 'Total Key Matched', 'Total Key & Data Matched',
                        'Total Data Breaks', 'Only in Source', 'Only in Target']

    exec_summary_df = pd.DataFrame(columns=exec_summary_col)
    for key, value in count_details.items():
        exec_summary_df = exec_summary_df.append({'Summary': key, 'Source Total': value}, ignore_index=True)
