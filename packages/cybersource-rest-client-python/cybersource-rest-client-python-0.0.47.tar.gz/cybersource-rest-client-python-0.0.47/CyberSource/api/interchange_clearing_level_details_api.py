...
class InterchangeClearingLevelDetailsApi(object):
    """
    ...
    def get_interchange_clearing_level_details(self, start_time, end_time, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_interchange_clearing_level_details(start_time, end_time, callback=callback_function)
        ...
        if kwargs.get('callback'):
            return self.get_interchange_clearing_level_details_with_http_info(start_time, end_time, **kwargs)
        else:
            (data) = self.get_interchange_clearing_level_details_with_http_info(start_time, end_time, **kwargs)
            return data

    def get_interchange_clearing_level_details_with_http_info(self, start_time, end_time, **kwargs):
        """
        Interchange Clearing Level data for an account or a merchant
        Interchange Clearing Level data for an account or a merchant
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_interchange_clearing_level_details_with_http_info(start_time, end_time, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method get_interchange_clearing_level_details" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        if 'organization_id' in params and len(params['organization_id']) > 32:
            if self.api_client.mconfig.log_config.enable_log:
                self.logger.error("InvalidArgumentException : Invalid value for parameter `organization_id` when calling `get_interchange_clearing_level_details`, length must be less than or equal to `32`")
            raise ValueError("Invalid value for parameter `organization_id` when calling `get_interchange_clearing_level_details`, length must be less than or equal to `32`")
        if 'organization_id' in params and len(params['organization_id']) < 1:
            if self.api_client.mconfig.log_config.enable_log:
                self.logger.error("InvalidArgumentException : Invalid value for parameter `organization_id` when calling `get_interchange_clearing_level_details`, length must be greater than or equal to `1`")
            raise ValueError("Invalid value for parameter `organization_id` when calling `get_interchange_clearing_level_details`, length must be greater than or equal to `1`")
        ...
        return self.api_client.call_api(f'/reporting/v3/interchange-clearing-level-details', 'GET',
                                        ...
                                        response_type='ReportingV3InterchangeClearingLevelDetailsGet200Response',
