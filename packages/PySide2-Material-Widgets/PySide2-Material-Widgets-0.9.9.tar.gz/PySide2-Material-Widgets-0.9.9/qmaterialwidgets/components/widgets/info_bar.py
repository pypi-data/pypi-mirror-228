...
from PySide2.QtWidgets import (QWidget, QFrame, QLabel, QHBoxLayout, QVBoxLayout, QGraphicsOpacityEffect,
                               QGraphicsDropShadowEffect)

from ...common.auto_wrap import TextWrap
from ...common.style_sheet import MaterialStyleSheet, themeColor, palette, argbToQColor
from ...common.icon import MaterialIconBase, Theme, isDarkTheme, drawIcon
from ...common.icon import FluentIcon as FIF
from ...common.color import translucent
from .button import TransparentToolButton


class InfoBarCloseButton(TransparentToolButton):
    """ Info bar close button """

    def __init__(self, infoIcon, parent):
        self.infoIcon = infoIcon
        super().__init__(parent)
        self.setIcon(FIF.CLOSE)

    def _hoverBackgroundColor(self):
        if not self._reverseColor():
            return translucent(palette.onSurfaceVariant, 20)

        return translucent(argbToQColor(palette.reverseScheme.onSurfaceVariant), 20)
    ...
    def _reverseColor(self):
        return isinstance(self.infoIcon, InfoBarIcon) and self.infoIcon == InfoBarIcon.SNACK



class InfoBarIcon(MaterialIconBase, Enum):
    """ Info bar icon """
    ...
    def path(self, theme=Theme.AUTO):
        ...
        return f':/qmaterialwidgets/images/info_bar/{self.value}_{color}.svg'
...
class InfoIconWidget(QWidget):
    ...
    def paintEvent(self, e):
        ...
        if self.icon not in [InfoBarIcon.INFORMATION, InfoBarIcon.SNACK]:
...
class InfoBar(QFrame):
    ...
    def __init__(self, icon: Union[InfoBarIcon, MaterialIconBase, QIcon, str], title: str, content: str,
        ...
        Parameters
        ----------
        icon: InfoBarIcon | MaterialIconBase | QIcon | str
        ...
        orient: Qt.Orientation
            the layout direction of info bar, use `Qt.Orientation.Horizontal` for short content
        ...
        self.closeButton = InfoBarCloseButton(icon, self)
    ...
    def __initWidget(self):
        ...
        self.shadowEffect.setColor(QColor(0, 0, 0, 100) if isDarkTheme() else QColor(0, 0, 0, 50))
    ...
    def __setQss(self):
        ...
        if isinstance(self.icon, Enum):
            ...
            self.titleLabel.setProperty('type', self.icon.value)
            self.contentLabel.setProperty('type', self.icon.value)

        MaterialStyleSheet.INFO_BAR.apply(self)
    ...
    @classmethod
    def snack(cls, title, content, orient=Qt.Orientation.Horizontal, isClosable=True, duration=1000,
             position=InfoBarPosition.TOP_RIGHT, parent=None):
        return cls.new(InfoBarIcon.SNACK, title, content, orient, isClosable, duration, position, parent)
