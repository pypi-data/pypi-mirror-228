...
Notes:
------
- Ensure the necessary dependencdelete_fileies are installed.
...
from .read_write_utils import write_to_file,read_from_file,delete_file
...
def get_stdout(result):
    """
    Capture and Store Standard Output from a Subprocess Result

    This line captures the standard output generated by a subprocess command and stores it in the variable 'stdout'.
    The variable 'result' should contain the result of a subprocess command, typically obtained by using functions
    from the 'subprocess' module. The underscore '_' is used as a placeholder to indicate that the standard error
    output is intentionally discarded in this context.

    Parameters:
    - result (subprocess.CompletedProcess): The result of a subprocess command execution.

    Returns:
    - stdout (bytes): The captured standard output from the subprocess.

    Note:
    The standard error output is ignored in this context and not stored.

    Example:
    result = subprocess.run(["ls", "-l"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

    print("Captured Output:", stdout.decode())
    """
...
def cmd_run(cmd: str, output_text: str = None,print_output:bool=False)) -> None:
    ...
    if print_output:
        output = read_from_file(output_text).strip()  # Read the entire output
...
def pexpect_cmd_with_args(command: str, child_runs: list, output_text: str = os.getcwd(),print_output:bool=False) -> int:
    ...
    write_to_file(file_path=get_output_text(),contents=output)
