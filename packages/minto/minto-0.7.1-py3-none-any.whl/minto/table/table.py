...
from minto.records.records import Record, get_pandas_dtypes, get_simple_dypes
...
class SchemaBasedTable:
    ...
    def insert(
        self, record: Record | pd.Series[Any] | list[Any] | tuple[Any, ...]
    ) -> None:
    ...
    def _validate_record(self, record: Record | pd.Series[Any] | list[Any]) -> Record:
        if isinstance(record, Record):
            obj = record.dict()
        elif isinstance(record, pd.Series):
            obj = record.to_dict()
        else:
            obj = {k: v for k, v in zip(self._schema, record)}
