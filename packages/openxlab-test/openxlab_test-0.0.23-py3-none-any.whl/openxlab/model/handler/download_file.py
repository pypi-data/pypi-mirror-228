...
def download_from_url(url, path=None, overwrite=False, file_name=None):
    ...
    try:
        ...
        river_pass_download_url = river_pass_url + '?url=' + url_encoded
        ...
        path_file = _download_to_local(river_pass_download_url, file_name=file_name, path=path,
                                       overwrite=overwrite, allow_cache=True, cache_path=os.path.join(os.path.expanduser("~"), '.cache', 'riverpass', url_hash))
