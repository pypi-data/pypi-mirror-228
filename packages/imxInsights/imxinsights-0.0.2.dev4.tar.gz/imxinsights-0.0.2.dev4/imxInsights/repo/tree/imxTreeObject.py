...
from imxInsights.utils.helpers import flatten_dict, get_file_path, get_yaml_dict
...
class ImxObject:
    ...
    def lookup_tree_from_root_element(start: ET.Element) -> list[ImxObject]:
        ...
        # todo if breaks if only situation, not sure if it trows exception if cant find version in attrs... situation should go one level uo.
        version: str = start.find("././../..").attrib["imxVersion"]
        config_file_path = get_file_path(__file__, "../../custom_puic_config.yaml")
        custom_puic_config = get_yaml_dict(config_file_path)
        if version == "1.2.4":
