...
class ChangeList:
    ...
    Args:
        ...
        props (list[Change]): A list of `Change` objects representing the changes made to the parent object's properties.
    ...
    def _ensure_status_matches_props(self):
        statuses = set((change.status for change in self.props))
    ...
    def to_dict(self, strip_attribute: bool = False, compact: Optional[bool] = True) -> dict[str, str]:
        """
        Return a dictionary representing the `ChangeList` object.

        Key is the property names, the values is the  change.
        ...
        """
        # if compact:
        return {ChangeList._key_name(change.key, strip_attribute): change.display_compact for change in self.props}
        # else:
...
class Change:
    ...
    def has_location(self) -> bool:
        """
        Determines if the property key is a location.
