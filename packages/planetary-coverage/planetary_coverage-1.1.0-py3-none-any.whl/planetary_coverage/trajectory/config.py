...
from ..spice.metakernel import MissingKernel, MissingKernelsRemote
...
class TourConfig:
    ...
    def __init__(self, mk=None, kernels=None,
                 spacecraft='JUICE', instrument='', target='Ganymede',
                 version='latest', kernels_dir=None, download_kernels=False,
                 remote_kernels=0, load_kernels=False, default_time_step='1 minute',
                 abcorr='NONE', exclude=None):
        # Properties
    ...
    def mk(self, args):
        ...
        self._version, self._kernels_dir, self._download_kernels, self._remote_kernels = \
            version, kernels_dir, download_kernels, remote_kernels
    ...
    def metakernel(self) -> Path:
        """Metakernel filename."""
        return self.mk.fname if self.mk else None  # pylint: disable=no-member
    ...
    def gaps(self, *refs):
        ...
        return EventsList([
            EventWindow('coverage-gap', t_start=start, t_end=stop)
    ...
    def _get_event(self, name):
        ...
        """
        if name in self.phases.keys():    # pylint: disable=C0201
            return self.phases[name]

        if name in self.timeline.keys():  # pylint: disable=C0201
    ...
    def new_tour(self, *, spacecraft=None, instrument=None, target=None):
        ...
        if not spacecraft and not instrument and not target:
            raise ValueError('You need to provide at least a `spacecraft`, '
    ...
    def flyby(self, approx_ca_date, *dt, alt_min=150_000):
        ...
        *dt: tuple(s), optional
        ...
        if isinstance(approx_ca_date, Event):
            ...
            return [
                self.flyby(event, *dt, alt_min=alt_min)
                for event in approx_ca_date
            ]
