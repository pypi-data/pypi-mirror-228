    ...
    **params
    ...
    if not tags:
        raise AttributeError(
            f'No version found in the cache for mission `{mission}`.') from None
    ...
    if not fname.exists():
        ...
        try:
            ...
            log_esa_api.warning(
                ...
                'Use local cache instead.', mk, tag)
...
def get_mk_offline(mission, mk='latest', version='latest'):
    ...
    if not mks:
        raise AttributeError(
            f'No metakernel found locally for mission `{mission}` '
    ...
    mks = [
        metakernel
        for metakernel in mks[::-1]
        if str(mk).lower() in metakernel.name
    ]

    if not mks:
        raise FileNotFoundError(
            f'`{mk}` metakernel at `{version}` is not in the cache.') from None
