...
def sub_obs_pt(time, observer, target, abcorr='NONE', method='NEAR POINT/ELLIPSOID'):
    ...
    if is_iter(time):
        return np.transpose([
            __sub_obs_pt(method, targ, t, frame, abcorr, obs)
            for t in time
        ])
...
def sc_state(time, spacecraft, target, abcorr='NONE'):
    ...
    if is_iter(time):
        return np.transpose([
            __state(targ, t, frame, abcorr, obs)
            for t in time
        ])
...
def attitude(time, observer, ref='J2000'):
    ...
    .. versionchanged:: 1.1.0
        ...
        See `issue #73
        <https://juigitlab.esac.esa.int/python/planetary-coverage/-/issues/73>`_
        for details.
    ...
    if is_iter(time):
        return np.moveaxis([
            __attitude(ref, frame, t) for t in time
        ], 0, -1)
...
def quaternions(attitude_matrix):
    ...
    if len(shape) < 2 or len(shape) > 3 or shape[:2] != (3, 3):
        raise ValueError(f'Input matrix shape: (3, 3) or (3, 3, N). {shape} provided')

    if len(shape) == 3:
        return np.transpose([
            __quaternions(m.tolist())
            for m in np.moveaxis(attitude_matrix, -1, 0)
        ])
...
def intersect_pt(time, observer, target, frame, ray, limb=False,
                 abcorr='NONE', corloc='TANGENT POINT', method='ELLIPSOID'):
    ...
    if is_iter(time) and np.ndim(ray) > 1:
        ...
        return np.transpose([
            __intersect_pt(method, targ, t, targ_frame, abcorr, corloc,
                           obs, frame, r, limb)
    ...
    if is_iter(time):
        return intersect_pt(time, observer, target, frame, [ray] * len(time),
                            limb=limb, abcorr=abcorr, corloc=corloc, method=method)

    if np.ndim(ray) > 1:
        return intersect_pt([time] * np.shape(ray)[0], observer, target, frame, ray,
                            limb=limb, abcorr=abcorr, corloc=corloc, method=method)

    return __intersect_pt(method, targ, time, targ_frame, abcorr, corloc,
                          obs, frame, ray, limb)


def __intersect_pt(method, target, time, targ_frame, abcorr, corloc,
                   obs, ref_frame, ray, limb):
    ...
    try:
        ...
        if limb:
            xyz, *_ = sp.tangpt(method, target, time, targ_frame, abcorr, corloc,
                                obs, ref_frame, ray)
            return xyz
...
def boresight_pt(time, observer, target, limb=False,
                 abcorr='NONE', corloc='TANGENT POINT', method='ELLIPSOID'):
    ...
    return intersect_pt(time, observer.spacecraft, target, str(observer.frame),
                        observer.boresight, limb=limb,
                        abcorr=abcorr, corloc=corloc, method=method)
...
def fov_pts(time, inst, target, limb=False, npt=24,
    ...
    return np.moveaxis([
        intersect_pt(time, inst.spacecraft, target, str(inst.frame), ray.copy(),
                     limb=limb, abcorr=abcorr, corloc=corloc, method=method)
        for ray in inst.rays(npt=npt)
    ], 0, -1)


def target_separation(time, observer, target_1, target_2, *,
                      shape_1='POINT', shape_2='POINT', abcorr='NONE'):
...
def local_time(time, lon, target, lon_type='PLANETOCENTRIC'):
    ...
    if is_iter(time) and is_iter(lon):
        ...
        return np.transpose([
            __local_time(t, targ_id, l, lon_type)
            for t, l in zip(time, rad_lon)
        ])
    ...
    return __local_time(time, targ_id, rad_lon, lon_type)
...
def true_anomaly(time, target, abcorr='NONE', frame='ECLIPJ2000'):
    ...
    if is_iter(time):
        return np.transpose([
            __true_anomaly(parent, t, frame, abcorr, targ, mu)
            for t in time
        ])
...
def groundtrack_velocity(target, state):
    ...
    .. versionchanged:: 1.1.0
        ...
        See `issue #35
        <https://juigitlab.esac.esa.int/python/planetary-coverage/-/issues/35>`_
        for details.
    ...
    if len(shape) < 1 or len(shape) > 2 or shape[0] != 6:
        raise ValueError(f'Input matrix shape: (6,) or (6, N). {shape} provided')

    if len(shape) == 2:
        return np.transpose([
            __gt_speed(s, re, rp, f)
            for s in np.transpose(state)
        ])
