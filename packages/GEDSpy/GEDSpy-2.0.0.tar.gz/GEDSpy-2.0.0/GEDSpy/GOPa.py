    ...
    for source in tqdm(os.listdir(path + '/tmp')):
        ...
        for int_id in tqdm(os.listdir(path + '/tmp/' + source)):
            if 'negative' not in int_id:
                with open('data/tmp/' + source + '/' + int_id, 'r',  encoding='utf-8') as file:
...
def update_downloading(path = _path_inside, path_in_use = _path_in_inside):
    ...
    text = 'Last GOPa update was done on ' + str(current_date)
...
def jbio_GOPa_prepare(path_in_use):
    ...
    GOPa_dis['relation_id'] = list(range(len(GOPa_dis['source'])))
    GOPa_dis['relation_id'] = [str(x) for x in GOPa_dis['relation_id']]
    GOPa_dis['relation_id'] = 'DIS:' + GOPa_dis['relation_id']
    GOPa_path = pd.concat([kegg, reactome])
    GOPa_path = GOPa_path.sort_values(by =['source', 'GOPa'])
    GOPa_path['relation_id'] = list(range(len(GOPa_path['source'])))
    GOPa_path['relation_id'] = [str(x) for x in GOPa_path['relation_id']]
    GOPa_path['relation_id'] = 'PA:' + GOPa_path['relation_id']
...
def gene_interactome_prepare(path_in_use):
    ...
    string = string[string['combined_score'] < np.percentile(string['combined_score'], 10)]
...
def GOPa_interactions_prepare(path_in_use):
    ...
    GOPa_dis = GOPa[GOPa['source'].isin(['DISEASE', 'VIRAL'])]
...
def update_from_sources(path = _path_inside, path_in_use = _path_in_inside):
...
def check_update_availability():
   ...
   """
    ...
    try:
        ...
        if get_latest_version('GEDSpy') != get_installed_version('GEDSpy'):
            print('\n')
        ...
        if species == 'human':
            ...
            for i in synonymes_list:
                for indx in gene_dictionary_q.index:
                    if gene_dictionary_q['synonymes'][indx] == gene_dictionary_q['synonymes'][indx] and i in gene_dictionary_q['synonymes'][indx]:
...
def GOPa_bar_plot(GOPa_data, p_val = 0.05, test = 'FISH', adj = 'FDR', n = 25, side = 'right', color = 'blue', width = 10, bar_width = 0.5, count_type = 'p_val', details = 0.75, omit = None):
    ...
    try:
        ...
        for nk, k in enumerate(sets.keys()):
                ...
                if k in ['PATHWAYS', 'GO-TERM'] and details != None:
                    tmp = tmp[tmp['ocr_n'] < np.quantile(tmp['ocr_n'], details)]
...
def GOPa_network_vis(GOPa_data, p_val = 0.05, test = 'FISH', adj = 'FDR', n_max = 10, list_of_terms = None, omit = None, details = 0.75, path = _path_tmp):
        ...
        if list_of_terms != None:
            ...
            for tr in list_of_terms:
                if True in list(GOPa_interactions['A'].isin([tr])):
                    ...
                    tmp = GOPa_interactions[(GOPa_interactions['A'].isin([tr])) | (GOPa_interactions['B'].isin([tr]))]
                    tmp2 = GOPa_interactions[GOPa_interactions['A'].isin(list(tmp['B']))]
                    ...
                    if len(B['n']) > n_max:
                        tmp = tmp[tmp['B'].isin(B['GOPa'][B['n'] >= int(B['n'][n_max])])]
                    ...
                    tmp2 = GOPa_interactions[GOPa_interactions['B'].isin(list(tmp['A']))]
                    tmp = pd.concat([tmp,tmp2])                    
                    ...
                    if len(A['n']) > n_max:
                        tmp = tmp[tmp['A'].isin(A['GOPa'][A['n'] >= int(A['n'][n_max])])]
                        interactions_df = pd.concat([interactions_df, tmp])
                    else:
                        interactions_df = pd.concat([interactions_df, tmp])
            ...
            for nk, k in enumerate(set(GOPa['source'])):
                ...
                if k in ['PATHWAYS', 'GO-TERM'] and details != None:
                    GOPa_interactions = GOPa_interactions[~GOPa_interactions['A'].isin(list(tmp['GOPa'][tmp['ocr_n'] > np.quantile(tmp['ocr_n'], details)]))]
                    GOPa_interactions = GOPa_interactions[~GOPa_interactions['B'].isin(list(tmp['GOPa'][tmp['ocr_n'] > np.quantile(tmp['ocr_n'], details)]))]


            A = Counter(list(GOPa_interactions['A']))
            ...
            if len(A['n']) > n_max:
               list_a = list(GOPa_interactions['A'][GOPa_interactions['A'].isin(A['GOPa'][A['n'] >= int(A['n'][n_max])])])
            else:
                list_a = list(GOPa_interactions['A'])

                
            B = Counter(list(GOPa_interactions['B']))
            ...
            if len(B['n']) > n_max:
               list_b = list(GOPa_interactions['B'][GOPa_interactions['B'].isin(B['GOPa'][B['n'] >= int(B['n'][n_max])])])
            else:
                list_b = list(GOPa_interactions['B'])
...
def create_netowrk_GOPa_static(network, width = 16, height = 10, font_size = 8):
...
def create_netowrk_gene_static(network, width = 12, height = 10, font_size = 8):
...
def show_go_term(GOPa_metadata, GO_id = None, GO_name = None, n_max = 10, omit = None, path = _path_tmp):
    ...
    try:
        ...
        try:
            ...
            GOPa_network = pd.concat([GOPa_is_a_ids, GOPa_part_of_ids, GOPa_has_part_ids])
            ...
            if len(GOPa_network) > 0:  
                ...
                for s in srcs:
                    for a_term in set(GOPa_network['A'][GOPa_network['source'].isin([s])]):
                        ...
                        if n_max > len(gopa_list['n']):
                            n_t = len(gopa_list['n']) - 1
                        else:
                            n_t = n_max
                            
                            
                        list_of_B_term = list_of_B_term + list(gopa_list['GOPa'][gopa_list['n'] >= int(gopa_list['n'][n_t])])
                        
                
        
                GOPa_network = GOPa_network[GOPa_network['A'].isin(list(GOPa['GOPa'][GOPa['relation_id'].isin(det_list)])) & GOPa_network['B'].isin(list_of_B_term)]
...
def create_netowrk_GO_static(network, width = 12, height = 10, font_size = 8):
...
def gene_interactions_network_vis(GOPa_data, n_min = 2, color = 'darkviolet', path = _path_tmp):
    ...
    try:
        ...
        if len(GOPa_gene_interactions) > 0:
            ...
            print('\n \n')
            print('Lack of GENE-INTERACTIONS')
...
def blood_level_markers_bar_plot(GOPa_data, side = 'right', color = 'red', experiment_type = 'MS', n_max = 10,  width = 10, bar_width = 0.5):
