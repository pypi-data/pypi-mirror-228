...
def gene_interactions_network_vis(GOPa_data, n_min = 2, details = 0.75, color = 'darkviolet', path = _path_tmp):
    ...
    Args:
       GOPa_data (dict) - raw GOPa_data from gopa_interaction_analysis function
       n_min (int) - minimal number of interactions for gene / protein. It is used mutual with arg 'details' depending on level of detail.
       details (float) - degree detail for display of genes/proteins interactions where 1 is the maximal value and 0.1 is minimal [0.1-1]. It is used mutual with arg 'n_min' depending on n_min degree.
    ...
    try:
        ...
        if len(GOPa_gene_interactions) > 0:
            ...
            if np.quantile(gopa_list['weight'], details) > n_min:
                gopa_list = gopa_list[gopa_list['weight'] >= np.quantile(gopa_list['weight'], details)]
            else:
    ...
    try:        
        ...
        if experiment_type.upper() == 'MS':
            ...
            if standarize == True:
                ...
                ax.set_xlabel('log(Concentration + 1)') 
            else:
