...
class Protein:
    ...
    def strip_ligands(self, destination = os.getcwd()):
        """
        Strips ligands from pdb file linedata
        
        This method generates a .pdb file without the ligand molecules in the specified directory.
        
        Parameters
        ----------
        destination : str
            Destination of the .pdb file. Defaults to the user's current directory.
            
        """
        data = [i for i in self.data if i[0:6] != 'HETATM']
        with open(destination + '/' + self.PDB + '.pdb') as f:
            for line in data:
                f.write(line)
    ...
    def pdbqt(self, destination = os.getcwd(), strip_ligands = False):
        ...
        try:
            pdbqtconv = 'obabel -ipdb ' + protein + '.pdb -opdbqt -O ' + protein + '.pdbqt'
            os.system(pdbqtconv)
            with open(protein + '.pdbqt', 'r') as f:
                    pass
        except:
            print('Conversion failed! is openbabel installed?')
