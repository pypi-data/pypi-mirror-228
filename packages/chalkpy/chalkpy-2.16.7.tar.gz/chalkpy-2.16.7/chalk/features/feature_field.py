...
from chalk.utils.inspect import ast_parse_lambda
...
class Feature(Generic[_TPrim, _TRich]):
    def __init__(
        ...
        last_for: Optional[Feature] = None,
    ):
        self.last = None
    ...
    def etl_offline_to_online(self) -> bool:
        if self._etl_offline_to_online is None:
            raise RuntimeError("ETL offline_to_online was never finalized. Is this feature in a features class?")
    ...
    def converter(self) -> FeatureConverter:
        ...
        if self.typ.is_document:
        ...
        elif self.typ.is_dataframe:
            # Don't explicitly get col.converter here to avoid infinite recursion
            column_features: Dict[str, Feature] = {col.root_fqn: col for col in underlying_typ.columns}

            def frame_encoder(x: _TRich) -> List[Dict[str, _TPrim]]:
            ...
            def frame_decoder(x: _TPrim):
                ...
                for row in x:
                    ...
                    results.append(self.typ.underlying(**rich_features))
                return DataFrame(results)

            self._decoder = self._decoder or frame_decoder
            self._encoder = self._encoder or frame_encoder
            self._pyarrow_dtype = self._pyarrow_dtype or rich_to_pyarrow(self.typ.parsed_annotation, self.root_fqn)

        elif self.typ.is_features_cls:
            # Don't explicitly get field.converter here to avoid infinite recursion
            field_features: Dict[str, Feature] = {field.root_fqn: field for field in underlying_typ.features}

            def feature_class_encoder(rich: _TRich) -> _TPrim:
            ...
            def feature_class_decoder(x: _TPrim) -> _TRich:
                ...
                return underlying_typ(**decoded_fields)

            self._decoder = feature_class_decoder
            self._encoder = feature_class_encoder
            self._pyarrow_dtype = self._pyarrow_dtype or rich_to_pyarrow(self.typ.parsed_annotation, self.root_fqn)
        ...
        if not isinstance(underlying_typ, type) and hasattr(underlying_typ, "_chalk_feature"):
            underlying_typ = underlying_typ._chalk_feature.typ.parsed_annotation
    ...
    def from_root_fqn(cls, root_fqn: str) -> Feature:
        ...
        while len(split_fqn) > 0:
            ...
            for x in features_cls.features:
                ...
                if x.name == feature_name or x.name == feature_name.lower():
    ...
    def join(self) -> Optional[Filter]:
        ...
        if self._join is not None:
            ...
            if not callable(self._join):
                return self._join
            join = self._validate_join(self._join)
            return join
        ...
        for f in self.features_cls.__chalk_features_raw__:
            if (
                hasattr(f.typ.parsed_annotation, "_chalk_feature")
                and self.typ.as_features_cls.namespace == f.typ.parsed_annotation._chalk_feature.namespace
            ):
                joins.append((f.name, Filter(lhs=f, operation="==", rhs=f.typ.parsed_annotation._chalk_feature)))
        if len(joins) == 0:
    ...
    def as_last(self) -> Feature:
        ...
        if last_feature is None:
            last_feature = Feature(
                ...
                default_version=self.version and self.version.default,
                primary=False,
...
"""
    ...
    def _validate_join(self, f: Callable):
        try:
            ast_parse_lambda(self._join)
        except TypeError:
            _logger.warning(
    ...
    def _validate_filter(self, filter: Filter):
        if (
            not isinstance(filter, Filter)
            or not isinstance(filter.lhs, Feature)
            or not isinstance(filter.rhs, Feature)
            or filter.operation != "=="
        ):
            raise TypeError(
                f"Join function for {self.root_fqn} is incorrectly configured. "
        ...
        if filter.lhs.converter.primitive_type != filter.rhs.converter.primitive_type:
            raise TypeError(
                f"Join function for {self.root_fqn} is incorrectly configured. "
                f"Ensure that both sides of the join are the same type (either int or str). "
                f"'{filter.lhs.root_fqn}' is of type {filter.lhs.typ.parsed_annotation}, and "
                f"'{filter.rhs.root_fqn}' is of type {filter.rhs.typ.parsed_annotation}. "
            )
        namespace_conditions = (
            filter.lhs.root_namespace == self.namespace, filter.rhs.root_namespace == self.namespace
        )
        features_are_simple = filter.lhs._is_scalar and filter.rhs._is_scalar
        if sum(namespace_conditions) != 1 or not features_are_simple:
            raise TypeError(
                ...
                f"Exactly one of the features in the join must be of the same root namespace as the has-one feature. "
                f"Ensure that the join is a lambda function that sets features from two feature classes as equal."
            )
