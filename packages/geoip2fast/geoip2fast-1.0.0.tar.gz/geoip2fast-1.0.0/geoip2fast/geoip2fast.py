...
# -*- coding: utf-8 -*-
"""
GeoIP2Fast - Version: v1.0.0 - 29/Aug/2023
...
import sys, os, socket, pickle, json
...
class GeoIPDetail(object):
    ...
    """    
    def __init__(self, ip, country_code="", country_name="", cidr="", elapsed_time=""):
        self.ip = ip
        self.country_code = country_code
        self.country_name = country_name
        self.cidr = cidr
        self.hostname = ""
        self.is_private = bool(self.country_code == GEOIP_ECCODE_PRIVATE_NETWORKS)
...
class GeoIP2Fast(object):    
    ...
    def _ip2int(self,ipaddr:str)->int:
        ...
        """    
        try:
            return unpack("!I", socket.inet_aton(ipaddr))[0]
        except Exception as ERR:
    ...
    def _load_data(self, gzip_data_file:str, verbose=False)->bool:
        ...
        try:
            try:
                ...
                try:
                    inputFile = gzipOpen(str(gzip_data_file),'rb')
                except Exception as ERR:
    ...
    @lru_cache(maxsize=DEFAULT_LRU_CACHE_SIZE, typed=True)
    def _get_last_ip(self,cidr):
        try:
            return int(ip_network(cidr)[-1])
        except:
            return 0
    ...
    def lookup(self,ipaddr:str)->GeoIPDetail:
        ...
        try:            
            ...
            ##──── IF YOU COMMENT THESE 3 LINES BELOW, YOU WILL GET MUCH MORE SPEED, BUT... YOUR SEARCH ACCURACY GOES DOWN TO ~99,5% ─────────
            ##──── You can comment these 3 lines and execute the file ./tests/accuracy_check.py to see how many wrong locations could ────────
            ##──── be given in 1000000 lookups and check the difference of speed. Just for testing. ──────────────────────────────────────────
            last_ip = self._get_last_ip(cidr)
            if iplong > last_ip:                
            ...
            try:
                ...
                pass
            ##──── SUCCESS! ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────
            return GeoIPDetail(ipaddr,country_code,country_name,cidr,elapsed_time='%.9f sec'%(perf_counter()-startTime))
            ##────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
        except Exception as ERR:
