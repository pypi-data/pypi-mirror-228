...
class NoticeApis(APIView):
    ...
    def notice_add(self, *args, request_params=None, user_info=None, **kwargs):
        if user_info is None:
            user_info = {}
        if request_params is None:
            request_params = {}
        request_params.setdefault("from_user_id", user_info.get("user_id"))
        data, err = NoticeServices.notice_add(notice_params=request_params)
        if err:
            return util_response(err=1001, msg=err)
        return util_response(data=data)
