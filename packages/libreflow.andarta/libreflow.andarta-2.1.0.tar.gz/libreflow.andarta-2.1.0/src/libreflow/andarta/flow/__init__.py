...
from .asset import Asset, AssetFamily, AssetType, AssetLibrary, AssetLibraryCollection
from .file import (
    TrackedFile,
    TrackedFolder,
    Revision,
    TrackedFolderRevision,
    UploadPlayblastToKitsu
)
...
class Project(BaseProject, flow.InjectionProvider):
    ...
    user            = flow.Child(UserProfile).ui(hidden=True)
    ...
    def _injection_provider(cls, slot_name, default_type):
        ...
        elif slot_name == 'libreflow.baseflow.file.UploadPlayblastToKitsu':
            return UploadPlayblastToKitsu
    ...
    def get_required_versions(self):
        ...
        versions.append(['libreflow.andarta', str(project_cur_version), str(project_req_version), project_needs_update])
