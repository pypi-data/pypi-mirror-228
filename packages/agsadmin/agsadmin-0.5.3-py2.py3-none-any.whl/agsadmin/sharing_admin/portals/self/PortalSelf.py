...
class PortalSelf(PortalEndpointBase):
    ...
    def assign_category_schema(self, category_schema):
        r = self._create_operation_request(self,
                                           "assignCategorySchema",
                                           method="POST",
                                           data={"categorySchema": category_schema})
    ...
    def delete_category_schema(self):
        r = self._create_operation_request(self, "deleteCategorySchema", method="POST")
        return send_session_request(self._session, r).json()

    def get_category_schema(self):
        r = self._create_operation_request(self, "categorySchema", method="GET")
        return send_session_request(self._session, r).json()
    ...
    def get_roles(self, start=1, num=10):
        r = self._create_operation_request(self, "roles", data={"start": start, "num": num, "returnPrivileges": True})
