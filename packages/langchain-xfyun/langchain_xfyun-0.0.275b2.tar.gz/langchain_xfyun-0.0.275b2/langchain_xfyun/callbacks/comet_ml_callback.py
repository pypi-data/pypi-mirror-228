...
from langchain_xfyun.callbacks.utils import (
...
from langchain_xfyun.schema import AgentAction, AgentFinish, Generation, LLMResult

LANGCHAIN_MODEL_NAME = "langchain_xfyun-model"
...
class CometCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(
        ...
        self.experiment.log_other("Created from", "langchain_xfyun")
        if tags:
        ...
        warning = (
            ...
            "based on updates to `langchain_xfyun`. Please report any issues to "
    ...
    def flush_tracker(
        ...
        task_type: Optional[str] = "inference",
        workspace: Optional[str] = None,
        project_name: Optional[str] = "comet-langchain_xfyun-demo",
        tags: Optional[Sequence] = None,
        name: Optional[str] = None,
        visualizations: Optional[List[str]] = None,
        complexity_metrics: bool = False,
        ...
        Args:
            ...
            langchain_asset: The langchain_xfyun asset to save.
    ...
    def _log_session(self, langchain_asset: Optional[Any] = None) -> None:
        try:
            ...
            self.experiment.log_table("langchain_xfyun-llm-session.csv", llm_session_df)
        except Exception:
            self.comet_ml.LOGGER.warning(
        ...
        try:
            metadata = {"langchain_version": str(langchain_xfyun.__version__)}
            # Log the langchain_xfyun low-level records as a JSON file directly
            self.experiment.log_asset_data(
                self.action_records, "langchain_xfyun-action_records.json", metadata=metadata
            )
        except Exception:
            self.comet_ml.LOGGER.warning(
                "Failed to log session data to Comet",
        ...
        for idx, (prompt, output) in enumerate(zip(prompts, outputs)):
            ...
            for visualization in self.visualizations:
                try:
                    ...
                    self.experiment.log_asset_data(
                        html,
                        name=f"langchain_xfyun-viz-{visualization}-{idx}.html",
