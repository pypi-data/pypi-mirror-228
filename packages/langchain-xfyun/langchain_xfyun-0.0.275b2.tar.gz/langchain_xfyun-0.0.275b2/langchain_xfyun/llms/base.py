...
from langchain_xfyun.load.dump import dumpd
from langchain_xfyun.prompts.base import StringPromptValue
from langchain_xfyun.prompts.chat import ChatPromptValue
from langchain_xfyun.pydantic_v1 import Field, root_validator, validator
...
from langchain_xfyun.schema.language_model import BaseLanguageModel, LanguageModelInput
from langchain_xfyun.schema.messages import AIMessage, BaseMessage, get_buffer_string
from langchain_xfyun.schema.output import GenerationChunk
from langchain_xfyun.schema.runnable import RunnableConfig

logger = logging.getLogger(__name__)


def _get_verbosity() -> bool:
    return langchain_xfyun.verbose
...
def get_prompts(
    ...
    for i, prompt in enumerate(prompts):
        if langchain_xfyun.llm_cache is not None:
            cache_val = langchain_xfyun.llm_cache.lookup(prompt, llm_string)
...
def update_cache(
    ...
    for i, result in enumerate(new_results.generations):
        ...
        if langchain_xfyun.llm_cache is not None:
            langchain_xfyun.llm_cache.update(prompt, llm_string, result)
...
class BaseLLM(BaseLanguageModel[str], ABC):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if langchain_xfyun.llm_cache is None or disregard_cache:
            if self.cache is not None and self.cache:
                raise ValueError(
                    "Asked to cache, but no cache found at `langchain_xfyun.cache`."
                )
            run_managers = [
