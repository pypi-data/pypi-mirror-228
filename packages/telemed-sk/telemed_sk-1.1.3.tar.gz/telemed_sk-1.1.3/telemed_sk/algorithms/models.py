...
DEFAULT_XGB_REG_PARAMS = {"learning_rate": 0.01,
                          "early_stopping_rounds": 100,
...
class XGBoost(BaseEstimator):
    ...
    def __init__(
        ...
        """
        self.params = DEFAULT_XGB_PARAMS
        if regression:
            self.params.update(DEFAULT_XGB_REG_PARAMS)
...
class XGBoostAutoreg(BaseEstimator):
    """Implements a Boosting model to perform both regression and classification"""
    ...
    def __init__(
        ...
        """
        Initializes an instance of the XGBoostAutoreg class with the specified parameters.
        ...
        cat_features : list[str], optional
            A list of the names of categorical features in the input data. Default is an empty list.
        lags : int, optional
            Number of lags to use when predicting the next value. Default is 7.

        Examples
        --------
        >>> model = XGBoostAutoreg(regression=True, cat_features=['A', 'B'], lags=5)
        """
        self.params = DEFAULT_XGB_PARAMS
        if regression:
            self.params.update(DEFAULT_XGB_REG_PARAMS)
    ...
    def fit(
        ...
        Examples
        --------
        >>> model = XGBoostAutoreg()
        >>> model.fit(X_train, y_train, X_val, y_val)
        """
        X_train = to_category(X_train, self.cat_features)
        X_val = to_category(X_val, self.cat_features)

        X_train = X_train.sort_index().asfreq("D")
        X_val = X_val.sort_index().asfreq("D")
        y_train = y_train.sort_index().asfreq("D")
        y_val = y_val.sort_index().asfreq("D")

        forecaster = ForecasterAutoregMultiSeries(regressor=xgb.XGBModel(), lags=self.lags)
        X_eval, y_eval, _, _ = forecaster.create_train_X_y(y_val, X_val)

        self.forecaster = ForecasterAutoregMultiSeries(
                        regressor = xgb.XGBModel(**self.params),
                        lags = self.lags,
                        fit_kwargs = {'eval_set': [(X_eval, y_eval)], 'verbose': False}
                    )
        self.forecaster.fit(y_train, exog=X_train)
    ...
    def refit(self, X_train: pd.DataFrame, y_train: pd.DataFrame) -> Self:
        """
        Refits the model with the specified training data.
        ...
        -------
        XGBoostAutoreg
            The refitted model object.

        Examples
        --------
        >>> model = XGBoostAutoreg().fit(X_train, y_train, X_val, y_val)
        >>> model.refit(X, y)
        """
        params = self.params.copy()
        params["n_estimators"] = self.forecaster.regressor.best_iteration
        params["early_stopping_rounds"] = None
        X_train = to_category(X_train, self.cat_features)
        X_train = X_train.sort_index().asfreq("D")
        y_train = y_train.sort_index().asfreq("D")
        ...
        self.forecaster = ForecasterAutoregMultiSeries(xgb.XGBModel(**params), lags=self.lags)
        self.forecaster.fit(y_train, exog=X_train)
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        """
        X = to_category(X, self.cat_features)
        X = X.sort_index().asfreq("D")
        start_exog = X.index[0]
        last_window = self.y[self.y.index<start_exog][-self.lags:]
        if last_window.shape[0] < self.lags:
            last_window = self.y[self.y.index>=start_exog][:self.lags]
            preds = self.forecaster.predict(steps=len(X)-self.lags,
                                            exog=X.iloc[self.lags:, :],
                                            last_window=last_window)
            preds = pd.concat([last_window, preds]).values
        else:
            preds = self.forecaster.predict(steps=len(X), exog=X, last_window=last_window).values
        if not self.regression:
            if len(preds.shape) == 1:
                preds = preds.reshape(-1, 1)
            preds = preds / np.sum(preds, axis=1, keepdims=True)
...
class BoostingReconcileModel:
    def __init__(
        ...
        autoregression_lags : int, optional
            Number of autoregressive lags to use when predicting the next value. Default is 0.
        ...
        if self.log_transform:
            self.target_transform = lambda x: np.log(x + 1)
            self.inverse_transform = lambda x: np.exp(x) - 1
        else:
        ...
        self.autoregression_lags = autoregression_lags

    def _construct_model(self, regression: bool) -> XGBoost | XGBoostAutoreg:
        if self.autoregression_lags > 0:
            return XGBoostAutoreg(regression=regression,
                                  cat_features=self.cat_features,
                                  lags=self.autoregression_lags)
        return XGBoost(regression=regression, cat_features=self.cat_features)
    ...
    @staticmethod
    def _get_hier_params(df: pd.DataFrame) -> dict[str, Any]:
    ...
    def fit(
        ...
        self._models: dict[str, XGBoost | XGBoostAutoreg] = {}
        ...
        for params in self.hier_params:
            ...
            for train, val in zip(train_data, val_data):
                ...
                if params["n_hier"] == 1:
                    ...
                    model = self._construct_model(regression=True)
                else:
                    id_pred = f"class_{id_pred}"
                    model = self._construct_model(regression=False)
    ...
    def predict(self, X_pred: pd.DataFrame) -> pd.DataFrame:
        ...
        for params in self.hier_params:
            ...
            for data in predict_data:
                ...
                if params["n_hier"] == 1:
                    y_pred = self._models[f"reg_{id_pred}"].predict(X).ravel()
    ...
    def _prepare_output(
        ...
        pi_upper: dict[str, np.ndarray],
        pi_lower: dict[str, np.ndarray],
    ) -> pd.DataFrame:
