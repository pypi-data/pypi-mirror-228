...
from pynubankasync import MockHttpClient
from pynubankasync.exception import NuMissingCreditCard, NuInvalidAuthenticationMethod
from pynubankasync.nubank import Nubank
from pynubankasync.utils.graphql import prepare_request_body
from pynubankasync.utils.mock_http import GHOSTFLAME_URL

cert_path = Path(__file__).parent / "fixtures" / "cert.cer"


async def test_authenticate_with_qr_code_succeeds():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_qr_code(
        "12345678912", "hunter12", "some-uuid"
    )
...
async def test_authenticate_with_cert():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)
...
async def test_authenticate_with_refresh_token():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_refresh_token("token", cert_path)
...
async def test_authenticate_with_cert_missing_credit_card():
    ...
    mock_client.add_mock_url(
        "https://mocked-proxy-url/api/proxy/magnitude_123", "", "proxy_events"
    )
    mock_client.add_mock_url(
        "https://mocked-proxy-url/api/token", "", "discovery_login_alternative"
    )
...
async def test_get_card_feed():
    ...
    feed = await nubank_client.get_card_feed()
    ...
    assert (
        events[0]["href"] == "nuapp://transaction/43e713a0-07b7-43bb-9700-8d7ad2d5eee6"
    )
    assert (
        events[0]["_links"]["self"]["href"]
        == "https://prod-s0-facade.nubank.com.br/api/transactions/43e713a0-07b7-43bb-9700-8d7ad2d5eee6"
    )


async def test_get_bills_missing_credit_card():
    mock_client = MockHttpClient()
    mock_client.remove_mock_url(("https://mocked-proxy-url/api/proxy/events_123", ""))
    mock_client.remove_mock_url(("https://mocked-proxy-url/api/token", ""))
...
async def test_get_bills():
    ...
    bills = await nubank_client.get_bills()

    assert len(bills) == 3
    assert (
        bills[2]["_links"]["self"]["href"]
...
async def test_get_bill_details():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_qr_code(
        "12345678912", "hunter12", "some-uuid"
    ...
    bill_response = await nubank_client.get_bill_details(bill_mock)
    ...
    assert (
        bill["line_items"][0]["href"]
        == "nuapp://transaction/abcde-fghi-jklmn-opqrst-uvxz"
    )
...
async def test_get_card_statements():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_qr_code(
        "12345678912", "hunter12", "some-uuid"
    ...
    statements = await nubank_client.get_card_statements()
...
async def test_get_account_balance():
    ...
    balance = await nubank_client.get_account_balance()

    assert balance == 127.33


async def test_get_account_feed():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)

    statements = await nubank_client.get_account_feed()
...
async def test_get_account_statements():
    ...
    statements = await nubank_client.get_account_statements()
...
async def test_get_account_investments_details():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)

    statements = await nubank_client.get_account_investments_details()
    ...
    assert (
        statements[0]["redeemedBalance"]["id"] == "abcdefgh-ijkl-mnop-qrst-uvwxyz0123"
    )
    ...
    assert (
        statements[2]["redeemedBalance"]["id"] == "sdfgehhdf-jkre-thre-nghh-kuvsnjue633"
    )


async def test_get_account_investments_yield():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)

    month_yield = await nubank_client.get_account_investments_yield()
...
async def test_get_customer():
    ...
    customer = await nubank_client.get_customer()
...
async def test_get_qr_code(monkeypatch):
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    uid, qr = await nubank_client.get_qr_code()

    assert uid != ""
    assert isinstance(qr, QRCode)


async def test_should_generate_boleto():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)

    assert (
        await nubank_client.create_boleto(200.50)
        == "123131321231231.2313212312.2131231.21332123"
    )


async def test_should_create_money_request():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)
...
async def test_should_fetch_pix_keys():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)
...
async def test_should_create_pix_money_request():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)

    keys_data = nubank_client.get_available_pix_keys()
    request = nubank_client.create_pix_payment_qrcode(
        "1231231232", 1232213.23, keys_data["keys"][0]
    )
...
async def test_should_revoke_certificate():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_qr_code(
        "12345678912", "hunter12", "some-uuid"
    ...
    await nubank_client.revoke_token()
...
async def test_should_return_none_if_isnt_pix_transaction():
    client = MockHttpClient()
    client.remove_mock_url(
        (GHOSTFLAME_URL, str(prepare_request_body("pix_receipt_screen")))
    )
    client.add_mock_url(
        GHOSTFLAME_URL,
        str(prepare_request_body("pix_receipt_screen")),
        "pix_receipt_screen_not_found",
    )
...
async def test_should_return_none_if_pix_transaction_doesnt_have_identifier():
    client = MockHttpClient()
    client.remove_mock_url(
        (GHOSTFLAME_URL, str(prepare_request_body("pix_receipt_screen")))
    )
...
async def test_should_retrieve_pix_identifier_for_pix_transaction():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)
...
async def test_get_card_statement_details():
    statement_mock = {
        "_links": {
    ...
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)
    statement_details = await nubank_client.get_card_statement_details(statement_mock)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "create_pix_payment_qrcode",
            {"account_id": "2", "amount": 123, "pix_key": {}},
        ),
        ("get_pix_identifier", {"transaction_id": "abc123"}),
    ],
)
async def test_nuconta_methods_should_fail_with_web_authentication(
    method_name, method_args
):
    nu = Nubank(client=MockHttpClient())
    await nu.start()
    await nu.authenticate_with_qr_code("1234", "hunter12", "uuid")
...
async def test_get_account_savings_balance():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)

    savings_balance = await nubank_client.get_account_savings_balance()
...
async def test_get_pix_details():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()
    await nubank_client.authenticate_with_cert("1234", "hunter12", cert_path)

    pix_details = await nubank_client.get_pix_details("IdentificadorPixAqui")
...
async def test_should_error_if_cert_not_provided():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()

    with pytest.raises(ValueError) as ex:
        await nubank_client.authenticate_with_cert("1234", "hunter12")

        assert ex is not None


async def test_should_authenticate_with_cert_data():
    nubank_client = Nubank(client=MockHttpClient())
    await nubank_client.start()

    await nubank_client.authenticate_with_cert(
        "1234", "hunter12", cert_data=b"SOME_CERT"
    )
