...
def settext(self, text: str, PosX: int, PosY: int, font: str = 'Segoe UI Semibold', size: int = 30, opx: int = 2):
    if pygame.display.get_active():
        ...
        self.fill(fuchsia)  # Transparent background
        lines = str(text).splitlines()
        for i, l in enumerate(lines):
            self.blit(render(l, font, opx), (PosX // 2, PosY // 2 + size * i))
        try:
            rect
        except NameError:
            rect_w = w
