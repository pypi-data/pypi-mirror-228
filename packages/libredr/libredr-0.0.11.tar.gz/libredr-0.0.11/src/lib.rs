...
impl PyLibreDR {
  ...
  pub fn py_ray_tracing_forward(
    ...
    let input_shape = ray.shape();
    assert!(input_shape.len() > 1, "py_ray_tracing_forward: ray should be at least 1D");
    let ray_len_input = if camera_space { 33 } else { 18 };
    assert_eq!(input_shape[0], ray_len_input,
      "py_ray_tracing_forward: ray channel: {}, expected: {}", input_shape[0], ray_len_input);
    let ray = ray.to_owned_array().into_shape([ray_len_input, input_shape[1..].iter().product()])?;
    ...
    let max_bounce:(usize, usize, usize, usize) = if let Ok(max_bounce_forward) = max_bounce.extract(py) {
      (max_bounce_forward, max_bounce_forward, 0, 0)
    } else if let Ok((max_bounce_forward, max_bounce_backward)) = max_bounce.extract(py) {
      (max_bounce_forward, max_bounce_backward, 0, 0)
    } else if let Ok((max_bounce_forward, max_bounce_backward, max_bounce_low_discrepancy)) = max_bounce.extract(py) {
      (max_bounce_forward, max_bounce_backward, max_bounce_low_discrepancy, 0)
    } else if let Ok(max_bounce) = max_bounce.extract(py) {
      max_bounce
    } else {
      bail!("py_ray_tracing_forward: invalid type in max_bounce")
    };
    let clip_near:(f32, f32, f32) = if let Ok(clip_near) = clip_near.extract(py) {
      (clip_near, clip_near, clip_near)
    } else if let Ok(clip_near) = clip_near.extract(py) {
      clip_near
    } else {
    ...
    let response = response.into_shape(output_shape)?;
    Ok(PyArrayDyn::from_owned_array(py, response).to_owned())
  }
  ...
  pub fn py_ray_tracing_backward(
    ...
    let input_shape = d_ray.shape();
    assert!(input_shape.len() > 1, "py_ray_tracing_backward: d_ray should be at least 1D");
    assert_eq!(input_shape[0], 3,
      "py_ray_tracing_backward: d_ray channel: {}, expected: {}", input_shape[0], 3);
    let d_ray = d_ray.to_owned_array().into_shape([3, input_shape[1..].iter().product()])?;
    let response = py.allow_threads(|| {
      let rt = RUNTIME.get().expect("Initialized in pymodule");
      rt.block_on(self.0.ray_tracing_backward(d_ray))
    })?;
    let d_texture = PyArray3::from_owned_array(py, response.0).to_owned();
    let d_envmap = PyArray4::from_owned_array(py, response.1).to_owned();
    let d_ray_texture = response.2.map(|x| -> Result<Py<PyArrayDyn<f32>>> {
      let mut output_shape = input_shape.to_owned();
      output_shape[0] = 14;
      Ok(PyArrayDyn::from_owned_array(py, x.into_shape(output_shape)?).to_owned())
    }).transpose()?;
...
/// * (default: 1)
#[pymodule]
pub fn libredr(_py: Python<'_>, module: &PyModule) -> PyResult<()> {
  init_static()?;
  module.add("__author__", "Bohan Yu <ybh1998@protonmail.com>")?;
  module.add("__version__", format!("LibreDR {}", common::build::CLAP_LONG_VERSION))?;
