...
impl LibreDR {
  ...
  pub async fn new(connect: String, unix: bool, tls: bool) -> Result<Self> {
    ...
    config.insert(String::from("connect"), connect);
    ...
    Ok(LibreDR {
      connection: Connection::from_config(&config).await?,
    })
  }
  ...
  /// # Arguments
  /// * `ray` - ray parameters
  ///   * if `camera_space` is `false` 18 * `n_pixels`
  ///     * including ray position 9 * `n_pixels`
  ///     * including ray direction 9 * `n_pixels`
  ///   * if `camera_space` is `true`, add another (1 + 14) channels
  ///     * including ray depth 1 * `n_pixels` (if depth <= 0, treat as hit miss)
  ///     * including ray material 14 * `n_pixels`
  ...
  ///   * render image 3 * `n_pixels`
  /// * if `camera_space` is `false`, add another
  ///   * ray texture coordinate 2 * `n_pixels`
  ///   * ray depth (Euclidean distance) 1 * `n_pixels`
  ///   * ray normal 3 * `n_pixels`
  ...
  pub async fn ray_tracing_forward(&mut self,
      ...
      requires_grad: bool,
      srand: i32,
      low_discrepancy: u32) -> Result<Array2<f32>> {
  ...
  ///
  /// # Arguments
  /// * `d_ray` - gradient of image 3 * `n_pixels` (must ensure same `n_pixels` as [`RequestRayTracingForward`])
  ///
  /// # Return
  /// Return shape will be,
  ...
  ///   * 3rd return value 14 * `n_pixels` (same shape as [`RequestRayTracingForward`])
  ///     * including d_ray_texture
  #[allow(clippy::too_many_arguments)]
  pub async fn ray_tracing_backward(&mut self, d_ray: Array2<f32>) ->
      Result<(Array3<f32>, Array4<f32>, Option<Array2<f32>>)> {
