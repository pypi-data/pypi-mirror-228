...
def save_to_db(account, total_cash, total_margin, summary_df, position_df):
    ...
    for symbol in symbol_list:
        ...
        sdf = summary_df[summary_df[option_summary.SYMBOL]==symbol]
        for i, rec in sdf.iterrows():
            uuid_value = rec[option_summary.UUID]
            symbol = rec[option_summary.SYMBOL]
            strategy = rec[option_summary.STRATEGY]
            credit = str(rec[option_summary.CREDIT])
            spread = rec[option_summary.SPREAD]
            open_price = rec[option_summary.OPEN_PRICE]
            exp_date = rec[option_summary.EXP_DATE]
            max_profit = rec[option_summary.MAX_PROFIT]
            max_loss = rec[option_summary.MAX_LOSS]
            margin = rec[option_summary.MARGIN]
            pnl = rec[option_summary.PNL]
            quantity = rec[option_summary.QUANTITY]
            status = rec[option_summary.STATUS]
            legs_desc = str(rec[option_summary.LEGS])

            win_prob = rec[option_summary.WIN_PROB]
            breakeven_h = rec[option_summary.BREAKEVEN_H]             
            breakeven_l = rec[option_summary.BREAKEVEN_L]

            fields = [uuid_value,symbol,strategy,credit,spread,\
    ...
    account.update_cash_position(total_cash)   
    
    account.update_margin_position(total_margin)
    
    account.db_conn.commit() 
        
def fidelity_reader(account, summary_file, position_file):

    logger = logging.getLogger(__name__)

    if account.get_optionSummary().shape[0] > 0:
    ...
    position_df = position_df[position_df['Symbol'].notna()]
    ...
    total_cash = (cash_df['Current Value'].replace( '[\$,)]','', regex=True ).replace( '[(]','-', regex=True ).astype(float)).sum()
    
    total_margin = summary_df['Margin Requirements'].astype(float).sum()

    sdf = account.get_optionSummary()
    ...
    for pid in pair_list:            
        ...
        sdf.at[sindex, option_summary.UUID] = uuid_value    
        sdf.at[sindex, option_summary.SYMBOL] = symbol
        ...
        sdf.at[sindex, option_summary.MARGIN] = margin    
        sdf.at[sindex, option_summary.STATUS] = asset.OPENED      
        ...
        if fidelity_strategy == 'Unpaired':                     
            ...
            sdf.at[sindex, option_summary.STRATEGY] = strategy.UNPAIRED         
            sdf.at[sindex, option_summary.CREDIT] = False
            sdf.at[sindex, option_summary.QUANTITY] = quantity
            sdf.at[sindex, option_summary.OPEN_PRICE] = open_price 
            ...
            sdf.at[sindex, option_summary.LEGS] = json.dumps([json.dumps(vars(leg))])        
            ...
            sdf.at[sindex, option_summary.WIN_PROB] = win_prob
            sdf.at[sindex, option_summary.BREAKEVEN_L] = round(open_price,2)            
            sdf.at[sindex, option_summary.BREAKEVEN_H] = np.nan            
            pindex += 1     
            ...
            sdf.at[sindex, option_summary.STRATEGY] = strategy.SHORT_PUT         
            sdf.at[sindex, option_summary.CREDIT] = True
            sdf.at[sindex, option_summary.QUANTITY] = quantity
            sdf.at[sindex, option_summary.OPEN_PRICE] = open_price 
            sdf.at[sindex, option_summary.MAX_PROFIT] = open_price
            sdf.at[sindex, option_summary.MAX_LOSS] = np.nan
            sdf.at[sindex, option_summary.PNL] = np.nan           

            otype, strike, exp_date = parse_expstrike(srow.iloc[0]['Expiration & Strike'])        
            sdf.at[sindex, option_summary.EXP_DATE] = exp_date          
            ...
            sdf.at[sindex, option_summary.WIN_PROB] = win_prob
            sdf.at[sindex, option_summary.BREAKEVEN_L] = breakeven_l            
            sdf.at[sindex, option_summary.BREAKEVEN_H] = np.nan   
            ...
            quantity = abs(srow.iloc[0]['Quantity'].astype(float))                
            open_price = float(srow.iloc[0]['Averaged Cost']) / (quantity*100)                      
            sdf.at[sindex, option_summary.STRATEGY] = strategy.LONG_CALL         
            sdf.at[sindex, option_summary.CREDIT] = False
            sdf.at[sindex, option_summary.QUANTITY] = quantity
            sdf.at[sindex, option_summary.OPEN_PRICE] = open_price 
            sdf.at[sindex, option_summary.MAX_PROFIT] = np.nan
            sdf.at[sindex, option_summary.MAX_LOSS] = open_price
            sdf.at[sindex, option_summary.PNL] = np.nan           
            ...
            sdf.at[sindex, option_summary.WIN_PROB] = win_prob
            sdf.at[sindex, option_summary.BREAKEVEN_L] = breakeven_l            
            sdf.at[sindex, option_summary.BREAKEVEN_H] = breakeven_h  
            ...
            sdf.at[sindex, option_summary.QUANTITY] = call_quantity     
            sdf.at[sindex, option_summary.OPEN_PRICE] = round(call_price,2)      
            sdf.at[sindex, option_summary.STRATEGY] = strategy.COVERED_CALL 
            ...
            sdf.at[sindex, option_summary.WIN_PROB] = win_prob
            sdf.at[sindex, option_summary.BREAKEVEN_L] = breakeven_h            
            sdf.at[sindex, option_summary.BREAKEVEN_H] = np.nan
            sdf.at[sindex, option_summary.CREDIT] = True 
            sdf.at[sindex, option_summary.EXP_DATE] = exp_date         
            ...
            sdf.at[sindex, option_summary.MAX_PROFIT] = round(max_profit,2)
            sdf.at[sindex, option_summary.MAX_LOSS] = round(max_loss,2)
            sdf.at[sindex, option_summary.PNL] = round(max_profit/max_loss,2)        
            ...
            sdf.at[sindex, option_summary.LEGS] = json.dumps([json.dumps(vars(stock_leg)), json.dumps(vars(call_leg))]) 
            ...
            sdf.at[sindex, option_summary.STRATEGY] = option_trader_strategy             
            sdf.at[sindex, option_summary.CREDIT] = credit
            sdf.at[sindex, option_summary.EXP_DATE] = exp_date          
            sdf.at[sindex, option_summary.QUANTITY] = quantity                  
            leg_id = 0            
            sdf.at[sindex, option_summary.LEGS] = [] 
            ...
            sdf.at[sindex, option_summary.LEGS]  = json.dumps(leg_desc)   
            ...
            sdf.at[sindex, option_summary.SPREAD] = spread    
            sdf.at[sindex, option_summary.OPEN_PRICE] = round(open_price,2)                  
            sdf.at[sindex, option_summary.MAX_PROFIT] = round(max_profit,2)
            sdf.at[sindex, option_summary.MAX_LOSS]   = round(max_loss,2)        
            sdf.at[sindex, option_summary.PNL] = round(pnl,2)               
            ...
            sdf.at[sindex, option_summary.WIN_PROB] = win_prob
            sdf.at[sindex, option_summary.BREAKEVEN_H] = round(breakeven_h,2)            
            sdf.at[sindex, option_summary.BREAKEVEN_H] = round(breakeven_l,2)
...
def import_fidelity_pofolio(user, profolio_file_name):
    ...
    for account_number in account_number_list:
        ...
        if os.path.exists(summary_file_path):
            ...
            fidelity_reader(account_obj, summary_file_path, position_file_path)

            rt = account_obj.get_default_runtime_config()
            rt.auto_trade = False
            account_obj.update_default_runtime_config(rt)

            sdf = account_obj.get_optionSummary()

            symbol_list = list(sdf[option_summary.SYMBOL].unique())              
...
if __name__ == '__main__':
    ...
    import_fidelity_pofolio(jihuang, 'Portfolio_Positions.csv')
