...
class Oscillo:
    ...
    def openDevice(device):
        ...
        print(f"serie : {serie}, model: {model}")
        if vendor == 'TEKTRONIX':
            if serie == 'TDS':
                return TDS(device, model = model)
            if serie == 'TBS':
                return TBS(device, model = model)
            raise NameError('Série {} de Tektronix non géré'.format(serie))
        if vendor == 'KEYSIGHT TECHNOLOGIES':
            if (serie == 'DSO-X'):
                return DSOX(device, model = model)
            raise NameError('Série {} de Keyseight non géré'.format(serie))
