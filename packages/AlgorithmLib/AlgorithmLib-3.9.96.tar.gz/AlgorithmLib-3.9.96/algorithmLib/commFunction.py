...
def vad_nn(speech):
    import algorithmLib
    modelpath = algorithmLib.__file__
    new_filename = "VAD_NN"
    dirname, filename = os.path.split(modelpath)
    ...
    curpath = os.getcwd()
    print(curpath)
    model, utils = torch.hub.load(repo_or_dir=new_path,
                                  model='silero_vad',
                                  source='local',
                                  force_reload=True,
                                  onnx=False)
    ...
    speech_section = [[a['start'],a['end']] for a in speech_timestamps]
