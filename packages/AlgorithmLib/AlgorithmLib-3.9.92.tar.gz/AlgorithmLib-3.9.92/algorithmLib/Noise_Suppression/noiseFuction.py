...
def cal_noise_Supp_by_ref(srcFile,testFile,speechSection):
    """
    :return:
    """
    FRAME_LEN = 9600
    ...
    for a in range(n_sengen):
        ...
        if (a * frame_shift) > 13 * fs and (a * frame_shift) + FRAME_LEN < startPoint*fs:
            std_sample_list.append(curdstlevel_real)
        if (a * frame_shift) > endPoint * fs and (a * frame_shift) + FRAME_LEN < 26.5*fs:
            std_sample_list.append(curdstlevel_real)
        if a > 1 and a <= 6:
            init_Level += curdiv
            init_cnt += 1
    ...
    if init_Level - between_speech_level < 2:
        if before_speech_floor < -60:
            converage = 0
...
if __name__ == '__main__':
    ...
    speech_cn = r'C:\Users\Administrator\Documents\我的POPO\Speech\01_Common_clean_files\speech_cn\agora\0\speech_cn.wav'
    testfile = r'C:\Users\Administrator\Documents\我的POPO\Speech\agoraTestCase_69_knock_0\agora_speech_cn\0\mixDstFile.wav'
    mixfile = r'C:\Users\Administrator\Documents\我的POPO\Speech\agoraTestCase_69_knock_0\agora_speech_cn\0\mixFile.wav'
    delay = cal_fine_delay_of_specific_section(reffile=speech_cn,testfile=testfile,outfile='out.wav',speech_section=([16.467,18.769],[19.6,21.2],[22,24.2]))
    dur = cal_noise_Supp_by_ref(srcFile=mixfile,testFile='out.wav',speechSection=([16.467,18.769],[19.6,21.2],[22,24.2]))
    print(dur)
    pass