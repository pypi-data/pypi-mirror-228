...
class Config:
  ...
  def __init__(self):
    ...
    if os.path.isfile(self.config_filename):
      logging.info(f"[+] Loading configuration file {self.config_filename}")
      self.conf.read(self.config_filename)
    else:
      logging.info(f"[+] No configuration file {self.config_filename}")
    self._check()
  ...
  def _check(self):
    ...
    logging.info("[+] Configuration check")
    conf_lines = []
  ...
  def register_secrets(self, context):
    logging.info("[+] Secrets registration (conf)")
    conf_lines = []
    conf_lines = [line.split(':')[1].strip() for line in self.doc.splitlines() if line.strip().startswith("@conf")]
    for line in conf_lines:
      try:
  ...
  def get_section(self, section):
    ...
    try:
      ...
      err_msg = f"Unknown section: [{section}]"
  ...
  def get(self, section, key, param_type='str'):
    ...
    try:
      ...
      err_msg = f"Unknown parameter: [{section}][{key}]"
      logging.critical(err_msg)
      raise ScrippyCoreError(err_msg) from err
    except configparser.NoSectionError as err:
      err_msg = f"Unknown section: [{section}]"
