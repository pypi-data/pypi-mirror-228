    ...
    def to_node(
        ...
        for c in select.find_all(exp.Column):
            ...
            if isinstance(source, Scope):
                ...
                if (c, subfield) in cols_seen:
                    continue
                cols_seen.add((c, subfield))
                node.downstream.append(Node(name=c.sql(), source=source, expression=source, subfield=subfield))
