...
def _parse_args(
    ...
    parser.add_argument(
        ...
        nargs="?" if config.demo else None,
        default=f"{config.output_dir}/sample.csv",
    )
...
def _generate_sample_csv(config: argparse.Namespace) -> None:
    ...
    _logger.debug("generate sample csv file: %s", csv_filename)
...
def _run(config: argparse.Namespace) -> None:
    ...
    _logger.debug("number of cpu: %d", multiprocessing.cpu_count())
