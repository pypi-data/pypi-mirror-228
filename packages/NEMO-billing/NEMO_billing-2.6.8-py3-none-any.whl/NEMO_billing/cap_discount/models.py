...
class CAPDiscountConfiguration(BaseModel):
    ...
    core_facility = models.ForeignKey(
        CoreFacility, null=True, blank=True, help_text="The core facility this CAP applies to", on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        facility = f"{self.core_facility.name}/" if self.core_facility else ""
        return f"CAP Discount for {facility}{self.rate_category.name}"

    class Meta:
        constraints = [
            UniqueConstraint(
                fields=["rate_category", "core_facility"],
                name="%(class)s_unique_rate_category_facility",
            ),
            UniqueConstraint(
                fields=["rate_category"],
                condition=Q(core_facility=None),
                name="%(class)s_unique_rate_category_null_facility",
            ),
        ]
...
class CAPDiscount(BaseModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Dict of project_id -> Decimal
        self.tmp_project_discounts: Dict[int, Decimal] = {}
