...
class TestCapAmount(TestCase):
    ...
    def test_cap_uniqueness(self):
        ...
        self.cap_configuration.core_facility = facility
        self.cap_configuration.save()
        # Create new config without facility
        new_config = self.cap_configuration
        new_config.core_facility = None
        new_config.pk = None
        # Save, no problems
        new_config.save()
        ...
        new_config.core_facility = facility
        with transaction.atomic():
            self.assertRaises(IntegrityError, new_config.save)
        # Set different facility, should be fine
        new_config.core_facility = CoreFacility.objects.create(name="facility_2")
        new_config.save()
