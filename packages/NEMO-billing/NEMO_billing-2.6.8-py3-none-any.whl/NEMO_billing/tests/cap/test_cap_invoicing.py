...
class TestCapInvoicing(TestCase):
    def setUp(self) -> None:
    ...
    def test_invoicing_by_project_not_supported(self):
        month = timezone.now().strftime("%B %Y")
        self.assertRaises(
            AttributeError,
            processor.generate_invoice_for_project,
            month,
            self.project.account,
            InvoiceConfiguration.first_or_default(),
            self.user,
        )
    ...
    def test_month_start_different_facility(self):
        ...
        new_cap_configuration = deepcopy(self.cap_configuration)
        new_cap_configuration.pk = None
        new_cap_configuration.core_facility = facility
