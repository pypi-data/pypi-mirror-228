...
from typing import Any, Callable, Generic, Literal, Mapping, Sequence, TypeVar, cast, get_args
...
class BaseTrainer(BaseObject[TrainerConfigT], Generic[TrainerConfigT, ModelT, TaskT]):
    ...
    def load_checkpoint(
        ...
        *,
        ...
        with Timer("loading checkpoint", spinner=True):
            ...
            def get_prefix(ckpt: dict[str, Any], prefix: str) -> dict[str, Any]:
                return {k[len(prefix) :]: v for k, v in ckpt.items() if k.startswith(prefix)}

            def as_key(s: str) -> Any:  # noqa: ANN401
                if re.match(r"\d+", s) is not None:
                    return int(s)
                return s

            def unflatten_dict(ckpt: dict[str, Tensor]) -> dict[str, Any]:
                unflattened_ckpt: dict[str, Any] = {}
                for k, v in ckpt.items():
                    c = unflattened_ckpt
                    parts = [as_key(ki) for ki in k.split(".")]
                    for ki in parts[:-1]:
                        if ki not in c:
                            c[ki] = {}
                        c = c[ki]
                    c[parts[-1]] = v
                return unflattened_ckpt
            ...
            if optims is not None:
                ...
                def get_optim_state_dict(ckpt: dict[str, Any]) -> dict[str, Any]:
                    return {
                        "state": unflatten_dict(get_prefix(ckpt, "state.")),
                        "param_groups": ckpt.get("optim.param_groups", []),
                    }

                if has_prefix("optim."):
                    if isinstance(optims, dict):
                        for name, optim in optims.items():
                            optim.load_state_dict(get_optim_state_dict(prefixed_state_dict(f"optim.{name}")))
                    else:
                        optims.load_state_dict(get_optim_state_dict(prefixed_state_dict("optim")))
                else:
                    logger.warning("Checkpoint does not contain an optimizer state dict")

            if lr_scheds is not None:

                def get_lr_sched_state_dict(ckpt: dict[str, Any]) -> dict[str, Any]:
                    return {
                        "state": unflatten_dict(get_prefix(ckpt, "state.")),
                        "param_groups": ckpt.get("lr.param_groups", []),
                    }

                if has_prefix("lr."):
                    if isinstance(lr_scheds, dict):
                        for name, lr_sched in lr_scheds.items():
                            lr_sched.load_state_dict(get_lr_sched_state_dict(prefixed_state_dict(f"lr.{name}")))
                    else:
                        lr_scheds.load_state_dict(get_lr_sched_state_dict(prefixed_state_dict("lr")))
                else:
    ...
    def save_checkpoint(
        ...
        with Timer("saving checkpoint", spinner=True):
            ...
            def get_tensor_dict_recursive(d: Any) -> dict[str, Tensor]:  # noqa: ANN401
                if isinstance(d, Mapping):
                    return {
                        f"{k}.{k2}" if k2 else f"{k}": v2
                        for k, v in d.items()
                        for k2, v2 in get_tensor_dict_recursive(v).items()
                    }

                if isinstance(d, Sequence):
                    return {
                        f"{i}.{k2}" if k2 else f"{i}": v2
                        for i, v in enumerate(d)
                        for k2, v2 in get_tensor_dict_recursive(v).items()
                    }

                return {"": d} if isinstance(d, Tensor) else {}

            def remove_tensors(d: Any) -> Any:  # noqa: ANN401:
                if isinstance(d, Mapping):
                    rd = {k: remove_tensors(v) for k, v in d.items()}
                    rd = {k: v for k, v in rd.items() if v is not None}
                    return None if len(rd) is None else rd
                if isinstance(d, Sequence):
                    rl = [remove_tensors(v) for v in d]
                    rl = [v for v in rl if v is not None]
                    return None if len(rl) == 0 else rl
                return None if isinstance(d, Tensor) else d

            def add_state_dict(key: str, sd: dict[str, Any]) -> None:
                sd_tensors = get_tensor_dict_recursive(sd)
                sd_metadata = remove_tensors(sd)
                state_dict.update(sd_tensors)
                metadata[key] = json.dumps(sd_metadata)
