...
"""
** Allows to combine probabilistic laws. **
-------------------------------------------
"""
...
def compute_cumhist(
    expr: typing.Union[Basic, numbers.Real, str], n_per_bars: numbers.Integral=3
) -> torch.Tensor:
    """
    ** Calculate the cumulative histogram of a law combination of reduced uniform probability. **
    ...
    n_per_bars : int
        The average number of samples by bars if theire is enouth memory.
    ...
    Examples
    --------
    >>> from cutcutcodec.core.optimisation.simplify.proba import compute_cumhist
    ...
    lins = torch.linspace(start, 1.0-start, nbr, dtype=torch.float32)
    lins = [lins] * len(symbs)
...
def inv_cumhist(min_: numbers.Real, max_: numbers.Real, cumhist: torch.Tensor) -> list:
    ...
    ** Reverse the histogram of the estimation of the proba repartition function. **
    ...
    --------
    >>> from cutcutcodec.core.optimisation.simplify.proba import compute_cumhist
    >>> from cutcutcodec.core.optimisation.simplify.proba import inv_cumhist
    >>> min_, max_, cum = compute_cumhist("1 + x1 + x2")
    ...
    index = index.to(dtype=torch.uint8, copy=False)
