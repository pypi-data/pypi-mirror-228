...
def _node_to_ast(
    ...
    * Input / Ouput pickalisable, no modification inplace for alow parralelisation.
    ...
    >>> filter_2 = FilterDelay(gen.out_streams, 10)
    >>> filter_1 = FilterDelay(filter_2.out_streams, 5)
    ...
    def get_container_output_1(filter_delay_1: Node) -> ContainerOutput:
        """** Creation of the node 'container_output_1'. **"""
        container_output_1 = ContainerOutput.__new__(ContainerOutput)
        container_output_1.setstate([filter_delay_1.out_streams[0]], state={})
        return container_output_1
    >>> func, modules = _node_to_ast("filter_delay_1", graph)
    >>> print(ast.unparse(func))
    def get_filter_delay_1(filter_delay_2: Node) -> FilterDelay:
        """** Creation of the node 'filter_delay_1'. **"""
        filter_delay_1 = FilterDelay.__new__(FilterDelay)
        filter_delay_1.setstate([filter_delay_2.out_streams[0]], state={'delay': '5'})
        return filter_delay_1
