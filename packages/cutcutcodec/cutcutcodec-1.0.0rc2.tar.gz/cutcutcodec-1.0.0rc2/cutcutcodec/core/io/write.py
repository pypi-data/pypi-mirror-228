...
class ContainerOutputFFMPEG(ContainerOutput):
    ...
    ** Allows you to write the output file to disk. **
    ...
    def __init__(self,
            ...
            For audio streams, here is the format to follow:
                {
                    ...
                    "bit_rate": int, # (optional) the flow in bytes/s (ex 128000)
                }
            For video streams, here is the format to follow:
                {
                    ...
                    "shape": list[int, int], # shape (height, width) of the frames (ex [1080, 1920])
        ...
        assert hasattr(streams_settings, "__iter__"), streams_settings.__class__.__name__
        ...
        for stream, settings in zip(in_streams, streams_settings):
            ...
            if stream.type == "video":
                ...
                assert hasattr(settings["shape"], "__iter__"), settings["shape"].__class__.__name__
                settings["shape"] = list(settings["shape"])
        ...
        with av.open(
            ...
            for stream, settings in zip(self.in_streams, self.streams_settings):
                if stream.type == "audio":
                    ...
                    stream_av = container_av.add_stream(settings["encodec"], rate)
                    stream_av.options = settings["options"] | {"strict": "experimental"}
                    streams_av.append(stream_av)
                    shapes.append(None)
                elif stream.type == "video":
                    stream_av = container_av.add_stream(settings["encodec"], settings["rate"])
                    stream_av.options = settings["options"] | {"strict": "experimental"}
                    stream_av.height, stream_av.width = settings["shape"]
                    stream_av.pix_fmt = "yuv420p"
                    streams_av.append(stream_av)
                    shapes.append(tuple(settings["shape"]))
                else:
            ...
            with tqdm.tqdm(
                ...
                for index, frame in scheduler(list(self.in_streams), rates, shapes=shapes):
                    if isinstance(frame, FrameAudio):
                        frame = frame_audio_to_av(frame)
                        stream_av = streams_av[index]
                    elif isinstance(frame, FrameVideo):
                        frame = frame_video_to_av(frame)
                        stream_av = streams_av[index]
                    else:
                        raise TypeError(
                            f"only audio and video frame are accepted, not {frame.type}"
                        )
...
def frame_audio_to_av(frame_audio: FrameAudio) -> av.audio.frame.AudioFrame:
    """
    ** Converts a FrameAudio cutcutcodec into a av frame for encodage. **
    ...
    --------
    >>> from cutcutcodec.core.classes.frame_audio import FrameAudio
    >>> from cutcutcodec.core.io.write import frame_audio_to_av
    >>>
    >>> frame_audio_to_av(FrameAudio(10, 48000, "mono", 1, 1024)) # doctest: +ELLIPSIS
    <av.AudioFrame 0, pts=480000, 1024 samples at 48000Hz, mono, flt at ...>
    >>> frame_audio_to_av(FrameAudio(10, 48000, "stereo", 2, 1024)) # doctest: +ELLIPSIS
    <av.AudioFrame 0, pts=480000, 1024 samples at 48000Hz, stereo, flt at ...>
    >>>
    """
    ...
    frame_av = av.audio.frame.AudioFrame.from_ndarray(
        ...
        format="flt",
        layout=frame_audio.profile.name,
    )
...
def frame_video_to_av(frame_video: FrameVideo) -> av.video.frame.VideoFrame:
    """
    ** Converts a FrameVideo cutcutcodec into a av frame for the encodage. **
    ...
    --------
    >>> from cutcutcodec.core.classes.frame_video import FrameVideo
    >>> from cutcutcodec.core.io.write import frame_video_to_av
    >>>
