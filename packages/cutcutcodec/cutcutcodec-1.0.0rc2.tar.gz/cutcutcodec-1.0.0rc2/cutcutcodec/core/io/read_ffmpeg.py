...
class ContainerInputFFMPEG(ContainerInput):
    ...
    ** Allows to decode a multimedia file with ffmpeg. **
    ...
    (720, 1280, 3)
    (360, 640, 3)
    FrameAudio(0, 2, 'stereo', [[        nan  0.18039602, -0.34764948]
                                [        nan -0.07235986,  0.07892987]])
    FrameAudio(0, 2, 'mono', [[        nan  0.06998454, -0.24758172]])
    >>>
    """
...
class _StreamVideoFFMPEG(StreamVideo, _StreamFFMPEGBase):
    ...
    def _snapshot(self, timestamp: Fraction, mask: torch.Tensor) -> torch.Tensor:
        ...
        if frame_np.shape[:2] != mask.shape: # cv2 two time faster than torch
            frame_np = resize_keep_ratio(frame_np, mask.shape)
