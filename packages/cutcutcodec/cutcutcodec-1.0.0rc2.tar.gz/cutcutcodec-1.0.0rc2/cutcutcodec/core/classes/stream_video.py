...
class StreamVideo(Stream):
    ...
    ** Representation of any video stream. **
    ...
    def _frame_to_float(frame: torch.Tensor) -> torch.Tensor:
        """
        ** Convert a video frame or a tensor into float32. **

        Warning, avoid copy as much as possible.
        """
        if frame.dtype.is_floating_point:
            return frame
        frame = frame.to(dtype=torch.float32)
        return torch.mul(frame, 1/255, out=frame)

    @staticmethod
    def _frame_to_uint8(frame: torch.Tensor) -> torch.Tensor:
        """
        ** Convert a video frame into an uint8 tensor. **

        Warning, avoid copy as much as possible.
        """
        if frame.dtype == torch.uint8:
            return frame
        frame = torch.mul(frame, 255.0, out=frame)
        frame = torch.clip(frame, 0.0, 255.0, out=frame)
        frame = torch.round(frame, out=frame)
        frame = frame.to(dtype=torch.uint8)
        return frame
    ...
    def snapshot(self,
        ...
        ** Extract the closest frame to the requested date. **
        ...
        if math.isnan(timestamp): # default transparent video frame
            ...
            frame = self._frame_to_uint8(frame)
