...
class EditFilterCut(EditBase):
    """
    ** Allows to view and modify the properties of a filter of type ``FilterCut``.
    """
    ...
    def init_limit(self, grid_layout, ref_span=0):
        """
        ** Displays and allows to modify the limits. **
        """
        class _Val:
        ...
        for i, limit in enumerate(limits):
            ...
            self.limit_textboxs[i].textChanged.connect(_Val(i, self))
        ...
        ** Check that the limit is correct and update all the limits. **
        ...
        if not text and limit_index >= 1 and limit_index + 1 == len(self.state["limits"]):
            ...
            if sorted(limits) != limits or len(set(limits)) != len(limits):
                ...
                return
        self.try_set_state(
            self.get_new_state({"limits": new_limits}), self.limit_textboxs[limit_index]
        )
