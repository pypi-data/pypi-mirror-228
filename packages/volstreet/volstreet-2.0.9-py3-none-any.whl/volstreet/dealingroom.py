...
from volstreet.exceptions import OptionModelInputError, ScripsLocationError
...
class PriceFeed(SmartWebSocketV2):
    def __init__(self, webhook_url=None, correlation_id="default"):
        ...
        self.midcp_index = Index("MIDCPNIFTY")  # Midcp temp fix
        ...
        new_price_dict.update(
            {
                ...
                "MIDCPNIFTY": {"ltp": self.midcp_index.fetch_ltp()},
            }
        )  # Finnifty/Midcp temp fix
    ...
    def add_options(self, *underlyings, range_of_strikes=10, expiries=None, mode=1):
        ...
        If expiries is a dictionary, then the respective expiries for each underlying are added.
            ...
            if symbol.startswith(index) and "CE" in symbol and expiry in symbol:
                ...
                if calc_iv:
                    ...
                    call_iv, put_iv, avg_iv = straddle_iv(
                        call_price, put_price, spot, strike, time_left_to_expiry
                    )
                    self.symbol_option_chains[index][expiry][strike][
                        "call_iv"
                    ] = call_iv
    ...
    def process_price_iv_log(
        ...
        self.iv_log[index][expiry][strike]["times"].append(currenttime())
...
class Option:
    def __init__(self, strike: int, option_type: str, underlying: str, expiry: str):
        ...
        self.underlying_symbol, self.underlying_token = fetch_symbol_token(
            self.underlying
        )
    ...
    def fetch_iv(self, S=None, t=None):
        S = S if S is not None else self.underlying_ltp()
        t = t if t is not None else time_to_expiry(self.expiry)
    ...
    def fetch_greeks(self):
        S = self.underlying_ltp()
        t = time_to_expiry(self.expiry)
        return bs.greeks(
            self.underlying_ltp(),
            self.strike,
            time_to_expiry(self.expiry),
            0.05,
            self.fetch_iv(S, t),
            self.option_type,
        )
    ...
    def ivs(self):
        return strangle_iv(
            *self.fetch_ltp(),
            ...
            self.call_strike,
            self.put_strike,
            timeleft=time_to_expiry(self.expiry),
        )
...
class Index:
    ...
    def set_future_symbol_tokens(self):
        if not self.future_symbol_tokens:
            for i in range(0, 3):
                try:
                    self.future_symbol_tokens[i] = fetch_symbol_token(
                        self.name, future=i
                    )
    ...
    def fetch_ltp(self, future=None):
        ...
        if self.name in ["FINNIFTY", "MIDCPNIFTY"]:  # Finnifty & Midcpnifty temp fix
            ...
            ltp = spot_price_from_future(
                ltp, self.spot_future_rate, time_to_expiry(self.fut_expiry)
            )
            ...
            ltp = fetchltp("NSE", self.symbol, self.token)
        return ltp
    ...
    def return_greeks_for_strikes(self, strike_range=4, expiry=None, option_type="CE"):
        if expiry is None:
            expiry = self.current_expiry
        underlying_price = self.fetch_ltp()
        atm_strike = findstrike(underlying_price, self.base)
        strikes = (
    ...
    def most_equal_strangle(
        ...
        last_notified_time = currenttime() - timedelta(minutes=6)
        while currenttime().time() < exit_time:
            ...
            if last_notified_time < currenttime() - timedelta(minutes=5):
                notifier(
                    f"Most equal strangle: {most_equal} with disparity {min_disparity} "
                    f"and prices {ltp_cache[most_equal.call_option]} and {ltp_cache[most_equal.put_option]}",
                    notification_url,
    ...
    def rollover_overnight_short_straddle(
        ...
        if weekend_in_expiry:  # far from expiry
            ...
            time_to_expiry(self.current_expiry, effective_time=True, in_days=True) < 1.5
    ...
    def intraday_strangle(
        ...
        def position_monitor(info_dict):
            ...
            def process_ctb(
                ...
                if method == "breakeven":
                    hedge_profit = total_price - hedge_total_ltp - self.base
                    return hedge_profit >= threshold_break_even

                elif method == "pct":
                    if (
                        total_price - (hedge_total_ltp + self.base)
            ...
            while not info_dict["trade_complete"]:
                ...
                call_iv, put_iv, avg_iv = strangle_iv(
                    ...
                    spot=spot_price,
                    timeleft=time_to_expiry(expiry),
                )
                ...
                if (
                    ...
                    and currenttime().time() < time(14, 15)
                ):
                    try:
        ...
        def justify_stop_loss(info_dict, side):
            ...
            except OptionModelInputError as e:
                estimated_movement = (
            ...
            if stop_loss_order_ids is None:  # If stop loss order ids are not provided
                ...
                if stop_loss_triggered:
                    ...
                    if (
                        ...
                        if price_increase > 1.5 and not stop_loss_justified:
                            message = (
                                f"{self.name} strangle {side} stop loss forced to trigger due to price increase. "
                                f"Price increase from stop loss price: {price_increase}"
                            )
                            notifier(message, notification_url)
    ...
    def intraday_trend(
        ...
        def get_futures_basis(traded_price):
            try:
                self.set_future_symbol_tokens()
                fut_1_price, fut_2_price = self.fetch_ltp(future=0), self.fetch_ltp(
                    future=1
                )
            except Exception as e:
                logger.error(f"Error in getting futures prices: {e}")
                notifier(f"Error in getting futures prices: {e}", notification_url)
        ...
        while entries < max_entries and currenttime().time() < exit_time:
            ...
            notifier(
                f"{self.name} trender {entries+1} exit price: {exit_price}, PNL Pct: {pnl_pct:0.2f}, "
                f"Traded basis change: {traded_basis_change}",
                notification_url,
            )
        ...
        notifier(
            f"{self.name} trender completed. Total PnL Pct: {combined_pnl_pct:0.2f}",
            notification_url,
        )
...
def fetch_symbol_token(
    ...
    if expiry is None and strike is None and option_type is None:  # Cash segment
        if (
            name in ["NIFTY", "BANKNIFTY"] and future is None
        ...
        elif name in ["FINNIFTY", "MIDCPNIFTY"] or isinstance(
            future, int
        ):  # Finnifty & Midcpnifty temp fix and futures
...
def get_strangle_indices_to_trade(*indices, safe_indices=None):
    ...
    if "MIDCPNIFTY" in closest_index_names:
        ...
        return get_strangle_indices_to_trade(
            *indices_without_midcp, safe_indices=safe_indices
        )
...
def indices_to_trade(nifty, bnf, finnifty, multi_before_weekend=False):  # delete this
    fin_exp_closer = time_to_expiry(
        finnifty.current_expiry, effective_time=True, in_days=True
    ) < time_to_expiry(nifty.current_expiry, effective_time=True, in_days=True)
...
def time_to_expiry(expiry, effective_time=False, in_days=False):
