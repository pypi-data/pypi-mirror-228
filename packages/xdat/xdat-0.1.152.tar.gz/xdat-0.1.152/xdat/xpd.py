...
def x_iter_groups(df, on, dropna=True, df_only=False, with_tqdm=False):
    if not on:
        yield df, [], ""
        return
    ...
    total = len(groups) if with_tqdm else None
    for group_keys, df_group in tqdm(groups, total=total, disable=not with_tqdm):
...
def x_replace(self, valid_vals=None, replace_vals=None, default=None):
...
def x_groupby(self, by, aggs, ignore_index=None, check_lost_keys=True, **kwargs):
    ...
    def check_lost_cols(df):
        ...
        for col in by:
            assert len(self[col].unique()) == len(df[col].unique()), f"{col}: {len(self[col].unique())} != {len(df[col].unique())}"
...
def x_calc_rank_num_series(sa):
    ...
    # Set the rank of null values to NaN
...
def x_calc_rank_num(self, key_col_name, score_col_name, ascending=True):
    ...
    """
    assert ascending is True, "Not yet implemented"
    if isinstance(key_col_name, str):
        key_col_name = [key_col_name]

    # sa_ranks = self.groupby(key_col_name)[score_col_name].rank(ascending=ascending, method='min')
    sa_ranks = self.groupby(key_col_name)[score_col_name].apply(lambda sa: x_calc_rank_num_series(sa))
    try:
    ...
    sa_ranks = sa_ranks.reset_index(level=level_num)
    sa_ranks = sa_ranks.sort_values(col_name)
    ranks = sa_ranks[score_col_name].values
    return ranks
