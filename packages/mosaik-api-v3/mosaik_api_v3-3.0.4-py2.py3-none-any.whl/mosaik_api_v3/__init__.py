...
def start_simulation(simulator, description='', extra_options=None):
    ...
    *extra_option* may be a list of options for `docopt <http://docopt.org/>`_
    ...
    """
    return asyncio.run(start_simulation_async(simulator, description, extra_options))

async def start_simulation_async(simulator, description='', extra_options=None):
    OK, ERR = 0, 1
    ...
    if inspect.isgeneratorfunction(func):
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                request = next(gen)
                while True:
                    # Send old request's result and get next request
                    request = gen.send(await request)
