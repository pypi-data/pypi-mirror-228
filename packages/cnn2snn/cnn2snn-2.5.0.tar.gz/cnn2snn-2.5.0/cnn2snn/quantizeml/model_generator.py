...
from .input_conv import InputConvBlockConverterV1, InputConvBlockConverterV2
...
def generate_qml_model(model):
    ...
    Args:
        model (:obj:`tf.keras.Model`): a model to convert.
    ...
    # TO DO: reset_buffer should be moved into record function in qml
    reset_buffers(model)
    ...
    if isinstance(blocks[0], (StemBlockConverter, InputConvBlockConverterV1,
                              InputConvBlockConverterV2)):
    ...
    for block in blocks:
        # The next check converts the potential layers block to its corresponding Akida layer
