...
from quantizeml.onnx_support.layers.base_layer import DOMAIN
...
class OnnxConverter:
    ...
    def __init__(self, node, model):
        assert node.domain == DOMAIN, f"Unrecognized {node.name}: it is not part of the domain."
        self._node = node
        self._model = model
    ...
    def input_shape(self):
        input_shape = onnx_graph_tools.get_tensor_shape(self.input_name, self._model.graph)
        # Remove batch size as akida requires a 3D input shape
        akida_shape = input_shape[1:]
    ...
    def input_dtype(self):
        return onnx_graph_tools.get_tensor_dtype(self.input_name, self._model.graph)
    ...
    @property
    def func(self):
        func = onnx_graph_tools.find_by_name(self._node.op_type, self._model.functions)
