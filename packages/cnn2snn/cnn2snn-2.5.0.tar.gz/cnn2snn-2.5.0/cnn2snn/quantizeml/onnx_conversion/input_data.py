...
@register_onnx_converter_target("InputData")
class InputDataOnnxConverter(OnnxConverter):
    ...
    def load_attributes(self, node):
        self.input_signed = not issubclass(self.input_dtype.type, np.unsignedinteger)
        if issubclass(self.input_dtype.type, np.integer):
            self.input_bits = np.iinfo(self.input_dtype).bits
        else:
    ...
    def _additional_checks(self):
        ...
        outbound_nodes = onnx_graph_tools.get_next_neighbor_nodes(self._node, self._model.graph)
        if len(outbound_nodes) != 1:
            raise RuntimeError(f"{self.name} must be connected to only one node.")

        # This layer is the only one that allows uint8 and int8 types
        if self.input_bits != 8 or not self.input_signed:
            raise ValueError(f"Expected an input dtype [int8] on {self.name} node.")
