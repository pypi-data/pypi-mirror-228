...
from .check_compatibility import (check_weight_types, check_attributes, check_if_squared,
                                  check_padding_compatibility, check_input_dtype)
...
def check_convolution_compatibility(converter, use_pad_value=False):
    ...
    """
    # Input constrains
    check_input_dtype(converter)

    # Weights constrains
    check_weight_types(converter,
                       weight_names=["W", "Scale", "Shift", "pads"],
                       expect_types=["int8", "int32", "int32", "int64"])

    # Zero point constrains
    if use_pad_value:
        check_weight_types(converter, ["x_pad_value"], expect_types=["uint8"])
        if converter.weights["x_pad_value"].size != 1:
            raise ValueError("Padding value must be a single value.")
    ...
    if np.any(converter.weights["Scale"] <= 0):
    ...
    check_padding_compatibility(converter)
