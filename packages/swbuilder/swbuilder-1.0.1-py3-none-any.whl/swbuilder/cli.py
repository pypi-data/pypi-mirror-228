...
def make_arg_parser():
    parser = argparse.ArgumentParser(
        description="NOTE: Docker environmental variables must be set.\n"
    ...
    bo.add_argument(
        "--name", type=str, help="Name for custom docker images (requires --requires)"
    )
    ...
    ca.add_argument(
        ...
        "`swbuilder foo bar --cache-repo docker.io/cache` will use "
    ...
    ca.add_argument("--no-cache", action="store_true", help="Rebuild every layer")
    ca.add_argument(
        "--bust-cache",
        action="append",
    ...
    ca.add_argument(
        ...
        "--clear-cache",
        action="store_true",
        help="Remove swbuilder's cache of files for `copy-from`.",
    )
    ca.add_argument(
        "--keep-build-tags",
        action="store_true",
    ...
    rt.add_argument(
        ...
        "`swbuilder hello-world -u quay.io/elvis` will tag the image "
        "as `quay.io/elvis/hello-world`. You can add a ':' to the end to "
        "image names into tags:\n "
        "`swbuilder -u quay.io/elvis/repo: hello-world` "
        "will create the "
    ...
    rt.add_argument(
        ...
        "For example: `swbuilder hello-world -u elvis/repo: -t 1.0` will create "
    ...
    hh.add_argument("--version", action="store_true", help="Print version and exit.")
    hh.add_argument(
        "--help-yaml",
        action="store_true",
...
[yet another image name]: ..."""
    ...
    print(
        textwrap.fill(
            "The idea is to write dockerfile commands for each specific "
