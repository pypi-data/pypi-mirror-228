...
from scoamp.utils.api_utils import load_auth
    ...
    def check_git_and_lfs(cls):
        ...
        try:
            if cls.git_version is None:
                ...
                cls.git_version = subprocess.run(["git", "--version"], stderr=subprocess.PIPE, stdout=subprocess.PIPE, encoding='utf-8', check=True).stdout.strip()
        ...
        try:
            if cls.lfs_version is None:
                ...
                cls.lfs_version = subprocess.run(["git-lfs", "--version"], stderr=subprocess.PIPE, stdout=subprocess.PIPE, encoding='utf-8', check=True).stdout.strip()
        except FileNotFoundError:
