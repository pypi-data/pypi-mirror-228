...
from .esi_oauth_native import esi_oauth_local
...
class ESITokens(object):
    ...
    def save(self) -> None:
        ...
        if os.path.exists(self.token_path) and os.stat(self.token_path).st_size:
            with open(self.token_path, "r") as all_tokens_fp:
                all_tokens = json.load(all_tokens_fp)
            all_tokens.update({self.clientId: tokens_list})
        else:
            all_tokens = {self.clientId: tokens_list}

        with open(self.token_path, "w") as all_tokens_fp:
    ...
    def __load_tokens(self) -> None:
        ...
        """
        if not os.path.exists(self.token_path) or not os.stat(self.token_path).st_size:
            return
