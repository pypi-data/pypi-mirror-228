...
from typing import TYPE_CHECKING, Callable, Coroutine, Dict, List, Tuple, Union
...
class InsertBuffer:
    ...
    Note:
        This buffer might contain expired database entries, 
        but it gurantees to return the **newest** entry of a given key.
        Also, upon flushing to database, 
        this buffer gurantees only the **newest** entry exists in the database after a flush.
    """
    ...
    def flush(self) -> None:
        ...
        self.db.commit()
        self.clear()
        logger.debug("Insertbuffer entries flushed")
    ...
    def select(self, key_hash) -> Tuple:
        ...
        """
        # Reverse order ensures the newest entry of the key_hash is returned
        for i in reversed(range(len(self.buffer))):
            if self.buffer[i][0][0] == key_hash:
...
class _DeleteHandler:
    ...
    This handler holds a list of datetimes,
    at which this handler deletes **ALL** cache lines that has expired.

    This handler only gurantees to delete expired cache lines no later than 5 minutes passed the expired time.
    It does not delete expired cache lines immediately at the ``expires`` timestamp.
    """
    ...
    def __init__(self, db: "ESIDBManager", table: str):
        ...
        db_dir = os.path.join(os.path.dirname(__file__), ".deleter")
