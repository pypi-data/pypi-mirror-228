...
def create_user_and_grant_privileges(config) -> None:
    ...
    Args:
        config (Callable): A callable that takes a
    ...
    Raises:
        psycopg2.Error: If there is an error connecting to PostgreSQL or
        executing queries.
    """
...
def create_database(config) -> None:
    ...
    Args:
        config (Callable): A callable that takes
        a configuration key as an argument
        and returns the corresponding configuration value.
    ...
    Raises:
        psycopg2.Error: If there is an error connecting to
