    ...
    if isinstance(model, PSF):
        if scaling_noise_ref is None:
            ...
            for i in range(n_image):
                centered_masked_noise_maps[i] = shift(deepcopy(masked_noise[i]), (
                - kwargs['kwargs_gaussian']['y0'][i],
                - kwargs['kwargs_gaussian']['x0'][i]), mode='constant', cval=np.nan, order=1)
            ...
            if masks is not None:
                if not np.all(masks[scaling_noise_ref, :, :] == 1.):
                    raise RuntimeError(
                        "Your reference star contain masked pixels, I cannot propagate the noise correctly. Use `scaling_noise_ref = None` or remove the mask.")
            noise_map = shift(deepcopy(noise_maps[scaling_noise_ref, :, :]), (
                - kwargs['kwargs_gaussian']['y0'][scaling_noise_ref],
                - kwargs['kwargs_gaussian']['x0'][scaling_noise_ref]), mode='constant', cval=np.nan, order=1) #x0 and y0 are inverted
        ...
        noise_map = np.nan_to_num(noise_map, nan=np.nanmedian(noise_map)) / np.sqrt(
            n_image)  # effective noise maps, scale as 1/sqrt(number of stars)
        ...
        deconv_kernel = model.get_gaussian(0, kwargs_centered, deepcopy(
            kwargs['kwargs_moffat']))  # get the gaussian kernel, will be normalised to 1 later
