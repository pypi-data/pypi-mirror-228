...
class Deconv(object):
    ...
    def model(self, kwargs):
        ...
        for i in range(self.epochs):
            ...
            row = jnp.hstack((row1, row2, row3, epochmean))
            rows.append(row)
        args = jnp.array(rows)
    ...
    def export(self, output_folder, kwargs_final, data, sigma_2, epoch=None, format='fits', header=None):
        """
        ...
        for i,e in enumerate(epoch_arr):
            ...
            if i == 0:
                save_fct(h, os.path.join(output_folder, 'background'), header=high_res_header)
...
def setup_model(data, sigma_2, s, xs, ys, subsampling_factor, initial_a=None, astrometric_bound=None,
                dithering_bound=None, convolution_method='scipy'):
    """
