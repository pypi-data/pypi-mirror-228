...
class PSF(object):
    ...
    def model(self, init=0, kwargs_moffat=None, kwargs_gaussian=None, kwargs_background=None, high_res=False):
        ...
        r = self.get_gaussian(init, kwargs_gaussian, kwargs_moffat)
        if high_res:
    ...
    def smart_guess(self, data, fixed_background=True, guess_method='barycenter', masks=None, offset_limit=None):
        """
        ...
        mean_flux = data.sum() / self.M
        ratio = jnp.array([data[i].sum() / mean_flux for i in range(self.M)])
        ...
        kwargs_up = {
            ...
            'kwargs_gaussian': {'a': list([jnp.inf for i in range(self.M)]),
                                'x0': list([offset_limit for i in range(self.M)]),
                                'y0': list([offset_limit for i in range(self.M)])
                                },
