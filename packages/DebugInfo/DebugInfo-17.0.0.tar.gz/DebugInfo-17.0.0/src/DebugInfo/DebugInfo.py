# -*- coding:UTF-8 -*-
# Copyright yaoyuan dou GNU GENERAL PUBLIC LICENSE, see LICENSE file.
...
class 分隔线模板:
    ...
    def 文本对齐(self, 方式: int or str = 对齐方式.居中对齐):
        """
        ...
        if isinstance(方式, 对齐方式):
            ...
            方式 = str(方式).strip()
            if len(方式) < 1:
                self.__提示对齐 = 对齐方式.居中对齐
            elif 方式[0] == 'l':
                self.__提示对齐 = 对齐方式.左对齐
            elif 方式[0] == 'c':
                self.__提示对齐 = 对齐方式.居中对齐
            elif 方式[0] == 'r':
                self.__提示对齐 = 对齐方式.右对齐
            else:
                self.__提示对齐 = 对齐方式.居中对齐
    ...
    def __str__(self) -> str:
        ...
        if self.__提示对齐 in [对齐方式.左对齐, 对齐方式.右对齐] or 提示文本显示宽度 < 1:
...
class 语义日期模板:
    ...
    def __init__(self,
                 ...
                 打印方法: _Callable[[str], any] = print):
        self.__目标日期: _datetime = 目标日期 if isinstance(目标日期, _datetime) else _datetime.now()
...
class 调试模板:
    ...
    def __init__(self,
                 ...
                 打印方法: callable = print):
        self.__调试状态: bool = False if not 调试状态 else 调试状态
    ...
    def __表格各列显示宽度表(self) -> tuple[list, list[int]]:
        ...
        for 行元素 in 展开的表格:
            ...
            for 列号 in range(总列数):
                if 列号 < 列数:
                    表格各列显示宽度表[列号] = max(显示宽度(行元素[列号], self.__特殊字符宽度字典), 表格各列显示宽度表[列号])
    ...
    def __表格分隔器(self, 填充字符: str = '-', 修饰方法: _Callable[[str], str] = None, 重复: bool = False, ) -> callable:
    ...
    def 执行位置(self, 位置: callable or str) -> None:
        """
        使用 self.__打印方法 打印一条消息,提示当前代码的运行位置,消息格式 '{}{}{}'.format(缩进, 位置提示符, 参数.__name__ + '开始执行')
        :param 位置: 需要进行提示的方法, 参数.__name__ 是该方法的名称,也可以直接使用字符串指定提示内容
        :return: None
        """
        if self.__调试状态:
            提示文本: str
    ...
    def 帮助文档(打印方法: _Callable[[str], None] = None) -> None:
        ...
        if not callable(打印方法):
            ...
            白板.添加一行('表格操作.添加分隔行', '通过 添加分隔行.__doc__ 查看详情', '|')
            ...
            白板.添加一行('表格操作.修改指定行', '通过 修改指定行.__doc__ 查看详情', '|')
