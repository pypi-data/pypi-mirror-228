...
with alchemite_apiclient.ApiClient(configuration) as api_client:
    ...
    try:
        # Find the outlying values in the model's training dataset
        api_response = api_instance.models_id_training_dataset_outliers_get(id)
        pprint(api_response)
    except alchemite_apiclient.ApiException as e:
        print("Exception when calling ModelsApi->models_id_training_dataset_outliers_get: %s\n" % e)
```
...
# **models_metadata_put**
> InlineResponse200 models_metadata_put()
...
with alchemite_apiclient.ApiClient(configuration) as api_client:
    ...
    model = Model(
        ...
        validation_method=ModelValidationMethods("none"),
    ) # Model | A JSON object containing the name and training method for the model.  The dataset ID of the dataset that will be used to train the model can be provided now if already uploaded or set later using POST /models/{id}/dataset.
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **model** | [**Model**](Model.md)| A JSON object containing the name and training method for the model.  The dataset ID of the dataset that will be used to train the model can be provided now if already uploaded or set later using POST /models/{id}/dataset. |
