...
def time_print(msg: str):
    # Print a timestamp along with a message
    print("\033[32m%s\033[0m %s" % (time.strftime('[%H:%M:%S]', time.localtime(time.time())), msg))

def count_result(in_bam:str, out_dir:str, ctg:str, quality:int):
...
def combine_txt(indir: str,beadsbacode: str):
    # Combine results from all contigs into a single file
    os.system("rm -rf %s/cell_count_detail.xls" % indir)
    os.system("find %s/temp/ -name \"*.txt\" | xargs cat  >> %s/cell_count_detail_raw.xls" % (indir, indir))
    cellcountRaw = pd.read_table(f"{indir}/cell_count_detail_raw.xls", sep='\t', header=None)
    selectbarcode = pd.read_table(beadsbacode,sep='\t', header=None)
    cellcountRaw.columns = ['BARCODE', 'GENE', 'UMI', 'Count']
    cellcountFilter = cellcountRaw[cellcountRaw['BARCODE'].isin(selectbarcode[0])]
    cellcountFilter.to_csv(f"{indir}/cell_count_detail.xls", index=False, header=False,sep='\t')
    os.system("rm -rf %s/cell_count_detail_raw.xls" % indir)
...
def fraction_reads(outdir:str, totalreads:int, threads:int = 4):
    """
    ...
    stats_df.to_csv(os.path.join(outdir,"saturation_cDNA.xls"),sep='\t')

def count_saturation(inbam,infile,beadsbacode,outdir,threads,quality):
    ...
    for ctg in ctg_list:
        pool1.apply_async(count_result,(inbam, resulttemp, ctg, quality,))
    pool1.close()
    pool1.join()
    time_print("combine txt")
    combine_txt(outdir,beadsbacode)
    shutil.rmtree(resulttemp)
    time_print("Analysis saturation")
    fraction_reads(outdir,totalReads,threads)
...
def parse_args():
    ...
    parser.add_argument('--inbam', 
        metavar='FILE', 
        type=str,
        help='input anno_decon_sorted.bam'
        )
    parser.add_argument(
        '--outdir',
        metavar='DIR',
    ...
    parser.add_argument(
        '--beadsbarcode',
        metavar='FILE',
        help='Need beadsbarcode information', 
        )
    parser.add_argument(
        '--file',
        metavar='FILE',
        help='Need reads information', 
        )
    parser.add_argument(
        '--quality',
        type=int, 
        default=20, 
        help='Minimal map quality to filter. Default is 20'
        )
    parser.add_argument(
        '--threads',
        metavar='INT',
...
if __name__=='__main__':
    ...
    outdir = args.outdir
    infile = args.file
    beadsbarcode = args.beadsbarcode
    quality = args.quality
    threads = args.threads
    count_saturation(inbam,infile,beadsbarcode,outdir,threads,quality)