...
from johnsnowlabs.utils.py_process import log_multi_run_status, str_to_file
...
def test_ipynb(
    file_path_or_url: Union[List[str], str],
    use_i_py=True,
    ...
    if not os.path.exists(file_path_or_url):
        ...
        if "http" and "//" in file_path_or_url:
            ...
            with urllib.request.urlopen(file_path_or_url) as response, open(
                file_name, "wb"
            ) as out_file:
            ...
            if not os.path.exists(
                johnsnowlabs.utils.testing.test_settings.workshop_cert_nb_folder
            ):
                # Clone repo
                cur_dir = os.getcwd()
                os.chdir(f"{johnsnowlabs.utils.testing.test_settings.tmp_notebook_dir}")
                os.system(
                    f"git clone {johnsnowlabs.utils.testing.test_settings.workshop_git} -b {branch}"
                )
                os.chdir(cur_dir)
            if "WORKSHOP-FIN" == file_path_or_url:
                return test_ipynb_folder(
                    johnsnowlabs.utils.testing.test_settings.workshop_fin_folder,
                    use_i_py=use_i_py,
            ...
            if "WORKSHOP-LEG" == file_path_or_url:
                return test_ipynb_folder(
                    johnsnowlabs.utils.testing.test_settings.workshop_leg_folder,
                    use_i_py=use_i_py,
                    model_cache_dir=model_cache_dir,
                    out_dir=out_dir,
                )
            if "WORKSHOP-MED" == file_path_or_url:
                return test_ipynb_folder(
                    johnsnowlabs.utils.testing.test_settings.workshop_med_folder,
                    use_i_py=use_i_py,
                    model_cache_dir=model_cache_dir,
                    out_dir=out_dir,
                )
            if "WORKSHOP-PUB" == file_path_or_url:
                return test_ipynb_folder(
                    johnsnowlabs.utils.testing.test_settings.workshop_pub_folder,
                    use_i_py=use_i_py,
                    model_cache_dir=model_cache_dir,
                    out_dir=out_dir,
                )
    ...
    if not os.path.isfile(file_path_or_url):
        raise ValueError(
            f"""Invalid target, must either be: 
...
def test_ipynb_folder(
    ...
    return test_list_of_ipynb(
        get_all_nb_in_local_folder(nb_folder),
        work_dir,
        log,
        model_cache_dir=model_cache_dir,
        out_dir=out_dir,
...
def execute_nb_as_nb(nb_path, out_file, work_dir=None):
    ...
    Path(
        johnsnowlabs.utils.testing.test_settings.testing_work_dir,
    ...
    try:
        ...
        print(
            f"🚨 Failure Executing notebook cell \n{err}",
        )
        nbformat_to_ipynb(out_file, nb)
    except Exception as err:
        exception_message = err
        print(
            f"🚨 Unknown Exception while trying to run Notebook  \n{err}",
        )
        nbformat_to_ipynb(out_file, nb)
        ...
        if "outputs" in cell:
            for cell_output in cell["outputs"]:
                if cell_output["output_type"] == "error":
                    bad_cells.append(
                        make_nbconvert_exec_log(
                            bad_cell_content=cell["source"],
...
def make_nbconvert_exec_log(
    bad_cell_content, error_output_type, error_name, error_value, error_traceback
):
