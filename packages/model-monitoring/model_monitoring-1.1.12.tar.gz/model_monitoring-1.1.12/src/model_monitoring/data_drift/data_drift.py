...
def psi_report(
    ...
    warning = pd.Series(
        [
            ...
            for i in report_df["common psi"].values
        ]
    )
...
def dd_red_yellow(df, missing_alert=False, missing_max_thresholds=10, missing_mid_thresholds=5):
    ...
    ret.loc[df.warning == "Red Alert", ["common psi", "warning"]] = "background-color: red"
    ret.loc[df.warning == "Yellow Alert", ["common psi", "warning"]] = "background-color: yellow"
    ret.loc[
        df["validity warning"].fillna("Missing").str.startswith("Red Alert"), "validity warning"
    ] = "background-color: red"
    ret.loc[
        df["validity warning"].fillna("Missing").str.startswith("Yellow Alert"), "validity warning"
    ] = "background-color: yellow"
    if missing_alert:
        ret.loc[df["drift(%) missing"] > missing_max_thresholds, "drift(%) missing"] = "background-color: red"
        ret.loc[
            df["drift(%) missing"] <= missing_max_thresholds and df["drift(%) missing"] > missing_mid_thresholds,
            "drift(%) missing",
        ] = "background-color: yellow"
