...
class EntityCache(BaseCache):
        ...
        if field_name_items and not is_checked:
            for rel_field in model._meta.related_objects:
                if rel_field.name == field_name_items[0]:
                    ...
                    raise ValueError(
                        WRONG_RELATED_FIELD_NAME_ERROR.format(full_field_name, field_name_items[0])
                    )
    ...
    def _prepare_entity_hash_tables(self):
        ...
        for entity in self._initial_entities:
            ...
            searching_field_values = (
                searching_field_values
                if check_is_iterable(searching_field_values)
                else (searching_field_values, )
            )
    ...
    def _filter_in(
        ...
        tmp_hash_table = {
            v: searching_hash_table[v]
            for v in value
            if v in searching_hash_table
        }
    ...
    def _filter_range(
        ...
        if value[0] == value[1]:
            ...
            # Если границы не совпадают с размерностью набора ключей, то требуется уточнение
                ...
                new_searching_hash_table = {
                    field_value: searching_hash_table[field_value]
                    for field_value in sliced_field_values
                }
        ...
        **kwargs,
        ...
        for attr, value_filter in kwargs.items():
            ...
            if field_name == PK:
                ...
                raise ValueError(
                    FILTER_FIELD_NAME_NOT_IN_SEARCHING_FIELDS_ERROR.format(', '.join(self._searching_fields), field_name)
                )
    ...
    def _get_entities_by_searching_hash_tables(
        ...
        entities = [
            self._initial_searching_hash_tables[self._model_pk_attname][entity_id]
    ...
    def get(
        ...
        **kwargs,
        ...
        tmp_filter_searching_hash_tables = (
            deepcopy(self._filter_searching_hash_tables)
            if self._filter_searching_hash_tables
            else {}
        )
    ...
    def values_list(
        ...
        *args,
        ...
        if not set(args).issubset(fields):
            ...
            raise RuntimeError(FIELDS_MUST_BE_FROM_ONLY_OR_VALUES_FIELDS_ERROR.format(', '.join(diff_fields), ))

        values_fields = [
            p.replace(LOOKUP_SEP, '.')
            for p in args
        ]
    ...
    def values(
        ...
        *args,
        ...
        Необходимо обратить внимание, что `pk` заменяется на атрибута первичного ключа модели.
        """
        if not args:
            raise ValueError(FIELDS_NOT_PASSED_ERROR)
        ...
        values = [
            dict(zip(args, entity))
            for entity in values_list_
        ]
