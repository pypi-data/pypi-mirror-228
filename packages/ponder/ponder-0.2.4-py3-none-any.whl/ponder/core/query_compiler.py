...
from ponder.common import NoCredentialsError
...
class DBMSQueryCompiler(BaseQueryCompiler):
    ...
    ##################################
    #
    # TODO: API Verification should move out of the DBMSQueryCompiler and
    # into the registry (unless there is some reason we can't do that).
    try:
        ...
        try:
            ...
            raise make_exception(
                ValueError,
                PonderError.QUERY_COMPILER_FOUND_API_KEY_WITH_INVALID_FORMAT,
                "Run `ponder reset && ponder login` and enter your API key.\n"
                "API key can be retrieved from app.ponder.io",
            )
    except (KeyError, FileNotFoundError):
        raise make_exception(
            NoCredentialsError,
            PonderError.QUERY_COMPILER_FOUND_NO_API_KEY,
    ...
    if len(_api_key) != 40:
        raise make_exception(
            ValueError,
            PonderError.QUERY_COMPILER_FOUND_API_KEY_WITH_WRONG_LENGTH,
            "Run `ponder reset && ponder login` and enter your API key.\n"
            "API key can be retrieved from app.ponder.io",
        )
    ...
    def _verify_api_key(key) -> dict:
        ...
        url = "https://api.ponder.io/api-key/verify"
    ...
    if "email" in verification_response:
        raise make_exception(
            ValueError,
            PonderError.QUERY_COMPILER_FOUND_BACKWARDS_API_KEY_WAS_VALID,
    ...
    if "email" not in verification_response or (
        ...
        raise make_exception(
            ValueError,
            PonderError.QUERY_COMPILER_FOUND_API_KEY_WAS_INVALID,
            "Invalid API Key. API Key can be retrieved from app.ponder.io",
        )

    # TODO(https://ponderdata.atlassian.net/browse/POND-1439): only allow the engines
    # that a user's Ponder account is authorized to use.
    del _verify_api_key, set_up, verification_response
    ...
    def to_dataframe(self, nan_as_null: bool = False, allow_copy: bool = True):
        raise make_exception(
            NotImplementedError,
            PonderError.TO_DATAFRAME_NOT_SUPPORTED,
            "Converting to dataframe exchange protocol is not supported yet",
        )
