        ...
        if operator in ["CORR", "COV"]:
            ...
            for other_col_id in range(len(self.columns)):
                ...
                if df is None:
                    ...
                    df = intermediate_df.concat(
                        axis=0, others=[df], how="outer", sort=False
                    )
            df = (
                df.from_labels(drop=False)
                .to_labels(
                    [
                        __PONDER_ROW_LABELS_COLUMN_NAME__,
                        __PONDER_AGG_OTHER_COL_ID__,
            ...
            df = df.mask(
                ...
                [__PONDER_ROW_LABELS_COLUMN_NAME__, __PONDER_AGG_OTHER_COL_NAME__]
            )
            return df
    ...
    def groupby(
        ...
        for other_col_id in other_col_ids:
            ...
            if df is None:
                ...
                df = intermediate_df.from_labels(drop=False).concat(
                    axis=0, others=[df], how="outer", sort=False
                )
        df = (
            df.to_labels(
