...
from .common import configure, init
...
def _verify(api_key, client_logger):
    ...
    api_key_error = (
        "API KEY is not valid. API KEY can be obtained from https://app.ponder.io"
    )
    # API KEY is not valid
    if len(api_key) != 40:
        raise ApiKeyNotValidError(api_key_error)
    # Verify API KEY is valid
    base_url = "https://api.ponder.io"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {api_key}",
    }
    verification = requests.patch(
        f"{base_url}/api-key/verify", headers=headers, params={"count": 1}
    )
    if (
