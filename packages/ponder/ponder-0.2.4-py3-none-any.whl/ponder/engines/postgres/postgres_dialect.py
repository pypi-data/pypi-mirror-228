...
class postgres_dialect(SQLDialect):
    pandas_type_to_postgres_type_map = {
        "bool": "BOOLEAN",
        "date": "DATETIME",
    ...
    def generate_copy_into_table_command(
        ...
        from_clause = ", ".join(
            [
                f"${column_index}"
                if (
                    self.pandas_type_to_postgres_type_map[str(column_type)] != "BOOLEAN"
                )
    ...
    def generate_select_count_star_statement(self, table):
        ...
        # PostgreSQL requires subselects to be named
        return f"SELECT COUNT(*) FROM ({formatted_table_name}) AS PONDER_INPUT;"
    ...
    def generate_autoincrement_type(self):
        return "bigint GENERATED ALWAYS AS IDENTITY (START WITH 0 MINVALUE 0)"
    ...
    def generate_create_table_command(
        ...
        try:
            columns_clause = ", ".join(
                [
                    ...
                    + " "
                    + self.pandas_type_to_postgres_type_map[str(column_type)]
        ...
        if is_temp or is_global_temp:
            ...
            if len(order_column_name or "") > 0:
                ...
                columns_clause += self.generate_autoincrement_type()
    ...
    def generate_create_table_for_sp_command(
        ...
        create_statement = f"CREATE TEMPORARY TABLE {table_name} ( "
        ...
        for column_name in column_names:
            if column_name in cast_to_map:
                if (
                    ...
                    db_type = self.pandas_type_to_postgres_type_map[
                    ...
                    if cast_from_type == "float" and db_type == "INT":
                        ...
                        column_expr = (
                            ...
                            + self.pandas_type_to_postgres_type_map[
                                cast_to_map[column_name]
                            ]
                        )
            else:
    ...
    def generate_cast_to_type_command(self, col, cast_type):
        return (
            ...
            f"{self.pandas_type_to_postgres_type_map[cast_type]})"
        )
