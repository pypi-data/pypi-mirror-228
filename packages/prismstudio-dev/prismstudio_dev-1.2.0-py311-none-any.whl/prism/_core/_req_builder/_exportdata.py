...
def retrieve_datafiles(datafile: Union[str, int]):
    ...
    res = requests.get(datalink, stream=True)
    if res.status_code >= 400:
        return _process_fileresponse(res, "data")

    content = download(res)
    ret_dict = _process_fileresponse(res, "exportdata", content, keep_dict=True, file_type='zip')
...
def rename_datafiles(old: Union[str, int], new: str):
    ...
    Examples
    --------
        >>> prism.rename_datafiles("test_single_q", "new_datafile")
        Data File renamed
        {'status': 'Success',
...
def delete_datafiles(datafile: Union[str, int]):
    ...
    Examples
    --------
        >>> prism.delete_datafiles("new_datafile")
        Data file deleted
        {'status': 'Success',
