# DB-API Helper funcs, you do not need to understand this
...
# def scope_exists(scope_name,host):
#     return len(list(filter(lambda x : x['name'] == scope_name,json.loads(list_scopes(host=host))['scopes']))) == 1
...
def query_endpoint(data, nlu_model_name, db_token, base_name=None):
    ...
    url = f"https://dbc-3d4c44aa-a512.cloud.databricks.com/serving-endpoints/{endpoint_name}/invocations"
    headers = {
...
def query_and_deploy_if_missing(
    ...
    """
    nlu_model: reference to nlu_model you want to query and deploy if its not already deployed
    ...
    if isinstance(nlu_model, str):
        ...
        raise Exception("Not supported model type =", type(nlu_model), nlu_model)
    return query_endpoint(
        query if is_json_query else query_to_json(query), nlu_model, db_token, base_name
    )
...
def deploy_nlu_model_as_endpoint(
    ...
    # SECRET_VALUE = os.environ['JOHNSNOWLABS_LICENSE_JSON']
    # Using seperate license for container
