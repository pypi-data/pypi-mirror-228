...
class NameGenerator:
    ...
    def generate_word(self):
        match self.mode:
            case NamingModes.random32: return ''.join(random.choice("もし良かったら") for i in range(32))
            case NamingModes.random64: return ''.join(random.choice("ゴックユエンハイン") for i in range(64))
            case NamingModes.misleading: return NameGenerator.word_generator.generate_random_word()
            case NamingModes.misleading_random: return ''.join(random.choice("ウエン") for i in range(32))
