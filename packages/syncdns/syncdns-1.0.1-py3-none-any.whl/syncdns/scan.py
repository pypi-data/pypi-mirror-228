...
from syncdns.utils.takeover import get_takeover_a_records, get_takeover_cnames
from syncdns.aws.ec2 import get_ec2_instances
from syncdns.utils.whitelist import parseWhitelistFile, check_whitelist
from syncdns.aws.route53 import get_route53_records
from syncdns.aws.eips import get_eips
from syncdns.aws.lb import get_load_balancers
from syncdns.utils.result import get_unused_records, print_results
from syncdns.cloudFlare.records import get_cloudflare_records
from syncdns.utils.filters import get_cnames, get_a_records, get_aws_cnames_map, get_elastic_ips_map, get_ec2_instances_from_cnames, get_load_balancers_from_cnames, filter_cnames, filter_ips_by_subnets, get_other_entries_from_cnames
from syncdns.utils.old_dns_entries import get_old_dns_entries, update_old_dns_entries
from syncdns.utils.constants import Result_keys
...
def send_webhook(slackwebhook, results):
    ...
    for key in results:
        if(len(results[key]) == 0):
            ...
            payload = "{}:".format(Result_keys[key])
            _ = webhook.send(text=payload)
            for record in results[key]:
                payload = "Name: {}  Record: {}".format(record["name"], record["records"])
                _ = webhook.send(text=payload)
...
def assume_role(role_arn):
    ...
    assumed_role_object = sts.assume_role(
        RoleArn=role_arn,
        RoleSessionName="syncdns-session"
    )
...
@click.option(
    ...
    help="Manually specify DNS records to check against. syncdns will check these IPs after checking retrieved DNS "
...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="Skip printing the ASCII art when starting up syncdns.",
)
@click.option(
    "--profile",
    default="",
    help="Specify a specific AWS profile to run syncdns on.",
)
@click.option(
    "--roles",
    required=False,
    type=click.Path(exists=True),
    help="Specify CSV filename with AWS account IDs to run syncdns on. Each account must have syncdns role "
         "assumable by syncdns ec2/lambda/whatever is running syncdns. Role name: syncdnsTargetAccountRole."
...
@click.option(
    ...
    help="Like --roles, but finds all organisation accounts automatically. The argument value should be ARN of a role "
         "with organizations:ListAccounts and organizations:DescribeAccount. Ec2/lambda/whatever is running syncdns"
...
def aws(
    ...
    if cloudflaretoken != "":
        ...
        cf_dns_records = get_cloudflare_records(cloudflaretoken, zones)
    ...
    aws_cnames_map = get_aws_cnames_map(ec2_instances, load_balancers)
    ...
    for region in aws_regions:
        ...
        for account_id in account_ids:
            ...
            role_arn = "arn:aws:iam::{0}:role/syncdnsTargetAccountRole".format(account_id)
            try:
    ...
    other_entries = get_other_entries_from_cnames(filtered_cname_records, ec2_instances, load_balancers)
