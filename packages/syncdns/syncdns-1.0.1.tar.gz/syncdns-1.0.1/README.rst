...
-  `syncdns <#syncdns>`__
   ...
   -  `Installing syncdns <#installing-syncdns>`__
   -  `Using syncdns <#using-syncdns>`__
...
syncdns obtains all the DNS records present in all of your AWS
accounts (Route53), and can optionally take in records via CSV input, or
via Cloudflare.

After these records are collected, syncdns iterates through all of
...
Installing syncdns is as simple as running:
``pip install syncdns``. The CLI tool will then be accessible via
the ``syncdns`` command.
...
::
   ...
   ❯ syncdns scan aws --help                                                                                                                                                                     
   Usage: syncdns scan aws [OPTIONS]

     Scan for dangling elastic IPs inside your AWS accounts.

   Options:
     --profile TEXT          Specify a specific AWS profile to run syncdns
                             on.
     ...
     --records PATH          Manually specify DNS records to check against.
                             syncdns will check these IPs after checking
...
Run syncdns with access to Cloudflare DNS records, send
...
::
   ...
   ❯ syncdns scan aws --cloudflaretoken APIKEY --slackwebhook https://hooks.slack.com/services/KEY --allregions

Run syncdns with a manually input list of subdomain A records (see
``records.csv`` in this repo for example file):

::

   ❯ syncdns scan aws --records records.csv
...
::
   ...
   ❯ syncdns scan aws --cloudflaretoken whougonnacall
...
6.  Set the name of the policy to be ``syncdnsPolicy``.
7.  Click ``Create Policy``.
8.  Go to
    https://console.aws.amazon.com/iam/home#/users$new?step=permissions&accessKey&userNames=syncdns&permissionType=policies
9.  Select ``syncdnsPolicy``.
...
syncdns can use roles instead of profiles which removes the need of
...
syncdnsTargetAccountRole role **named exactly like that** in every
...
The syncdns will be running in lambda/ec2/whatever with
``syncdns`` role - let’s call it syncdns master role. The master
role must be able to assume TargetAccountRoles.

.. code:: terraform

   resource "aws_iam_policy" "syncdns_target_account_roles" {
     name        = "syncdns_target_account_roles"
     path        = "/"
       ...
       "Statement" : [
         {
           ...
           "Resource" : ["arn:aws:iam::*:role/syncdnsTargetAccountRole"]
       }]
     })
...
Then, in the account that is running syncdns, attach following
policy to syncdns master role so it can assume the organisation
lookup role:
       ...
       "Statement" : [
         {
           "Effect" : "Allow",
           "Action" : "sts:AssumeRole",
           "Resource" : ["arn:aws:iam::ORG_LOOKUP_ACCOUNT_ID:role/ta-application-security-prd-syncdns-org-role"]
       }]
   }
...
If you want syncdns to pull in all the A records that you have set
...
(password manager). In order to use it with syncdns, pass it in via
...
5. Install syncdns by going to root of repo.
   ``pip3 install --editable .``
6. Make your changes and run the syncdns command.
