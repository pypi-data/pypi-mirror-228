...
def verify_gcp_networking(
    ...
    if is_private_service_cloud and not _vpc_has_proxy_only_subnet(
        factory, project_id, cloud_region, vpc
    ):
        logger.internal.warning(
...
def _vpc_has_proxy_only_subnet(
    factory: GoogleCloudClientFactory,
    project_id: str,
    cloud_region: str,
    vpc: compute_v1.types.compute.Network,
) -> bool:
    ...
    for subnet in subnets:
        if (
            subnet.network == vpc.self_link
            and subnet.purpose == PROXY_ONLY_SUBNET_PURPOSE
        ):
            return True
    return False
...
def verify_gcp_project(  # noqa: PLR0911
    ...
    response = (
        ...
        .batchGet(
            ...
            names=[
                f"projects/{project_id}/services/{api}" for api in GCP_REQUIRED_APIS
            ],
        )
        .execute()
    )
    ...
    try:
        enable_project_apis(factory, project_id, logger.internal)
...
def verify_firewall_policy(  # noqa: PLR0911, PLR0912
    factory: GoogleCloudClientFactory,
    resources: CreateCloudResourceGCP,
    project_id: str,
    cloud_region: str,
    ...
    if not firewall:
        try:
            ...
            logger.log_resource_not_found_error("Firewall Policy", firewall_policy)
            return False
...
def verify_cloud_storage(  # noqa: PLR0911, PLR0912
    ...
    if not any(
        ...
        logger.internal.warning(
            ...
            "If you are using the UI, please create the correct CORS rule for Anyscale according to https://docs.anyscale.com/user-guide/onboard/clouds/deploy-on-gcp#appendix-detailed-resource-requirements"
        )
        if strict:
            return False
...
def verify_memorystore(
    ...
    # 3 is STANDARD_HA
    # https://github.com/googleapis/google-cloud-python/blob/main/packages/google-cloud-redis/google/cloud/redis_v1/types/cloud_redis.py#L322
    if redis_instance.tier != 3:
        logger.internal.warning(
