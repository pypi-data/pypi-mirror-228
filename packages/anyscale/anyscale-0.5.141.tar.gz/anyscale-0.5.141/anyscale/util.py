...
TERMINATED_STATE_GRACE_PERIOD_SECONDS = 60
...
def _get_role(role_name: str, region: str) -> Optional[Boto3Resource]:
...
def _get_aws_efs_mount_target_ip(
    boto3_session: Any, efs_id: str, cloud_setup_logger: CloudSetupLogger
) -> Optional[str]:
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "FileSystemNotFound":
            return None
        cloud_setup_logger.log_resource_exception(
...
def _get_memorydb_cluster_config(
    ...
    try:
        ...
        return AWSMemoryDBClusterConfig(
            id=cluster["ARN"], endpoint=f"{endpoint['Address']}:{endpoint['Port']}",
        )
    except ClientError as e:
...
def populate_dict_with_workspace_config_if_exists(
    ...
    """
    If the service/job is submitted from the workspace, we populate values from the workspace
    ...
    if (
        ...
        # If the service/job configs are not specified, infer them from the workspace:
...
def filter_actions_from_policy_document(
    policy_document: Dict[Any, Any], action_filter: Callable[[Dict], bool] = None
) -> set:
...
def filter_actions_associated_with_role(
    ...
    role: Boto3Resource,
    action_filter: Callable[[Dict], bool] = None,
) -> Set[str]:
...
@contextmanager
def handle_and_log_lines_in_click_exception(
