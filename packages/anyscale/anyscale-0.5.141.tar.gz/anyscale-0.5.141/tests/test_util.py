...
def test_prepare_cloudformation_template():
    ...
    with patch("anyscale.util.get_availability_zones", new=Mock(return_value=mock_azs)):
        cfn_template = prepare_cloudformation_template(
            mock_region, mock_cfn_stack_name, mock_cloud_id, True
        )
    ...
    with open(
        f"{os.path.dirname(os.path.abspath(__file__))}/cloud_formation_template.txt"
    ) as f:
...
def test_gen_valid_name():
    NAME_VALIDATION_REGEX_PATTERN = r"^[A-Za-z0-9_-]+$"
...
@pytest.mark.parametrize(
    ("describe_cluster_response", "raise_exception",),
    [
        pytest.param({}, True),
        pytest.param({"Clusters": [{}]}, True),
        pytest.param(
            {
                "Clusters": [
                    {
...
def test_get_memorydb_cluster_config(describe_cluster_response, raise_exception):
    ...
    with patch("anyscale.util.boto3", new=mock_boto3):
        if raise_exception:
            ...
            assert cluster_config.endpoint == "mock_cluster_endpoint_address:6379"
