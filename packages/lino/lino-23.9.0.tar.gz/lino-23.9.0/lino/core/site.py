...
class Site(object):
    ...
    """The value to use as
    `upload_to
    <https://docs.djangoproject.com/en/4.1/ref/models/fields/#django.db.models.FileField.upload_to>`__
    ...
    ALLOWED_ATTRIBUTES['span'] = ['class', 'data-index', 'data-denotation-char',
        'data-link', 'data-title', 'data-value', 'contenteditable']
    ...
    logged.  It corrently works only for requests that are being processed by
    ...
    *within* the framework.
    ...
    See also :ref:`host.logging`.
    """
    auto_configure_logger_names = 'atelier django lino radicale'
    """
    ...
    def __init__(self, settings_globals=None, local_apps=[], **kwargs):
        ...
        if hasattr(self, 'allow_duplicate_cities'):
            raise ChangedAPI("allow_duplicate_cities is now a setting of the countries plugin")
    ...
    def setup_logging(self):
        ...
        # Ignore logging in redundant startup.
        ...
        if self.logger_filename and 'file' not in handlers:
            ...
            if logdir.is_dir():
                ...
                handlers['file'] = {
                    ...
                    'filename': str(logdir / self.logger_filename),
                    'encoding': 'UTF-8',
                    'formatter': 'verbose',
                }
                ...
                if self.use_linod:
                    handlers['root'] = {
                        'class': 'lino.core.logging.LinoSocketHandler',
                        'host': str(self.site_dir / 'log_sock'),
                        'port': None,
                        'level': level,
                        # 'filters': ['root']
                    }
        ...
        if self.use_linod:
            if 'file' in loggercfg['handlers']:
                loggercfg['handlers'].remove('file')
    ...
    def define_feature(self, name, enabled=False):
        if name in self.features:
            raise Exception("A feature %s already exists." % name)
    ...
    def has_feature(self, name):
        # if not name in self.features:
        #     raise Exception("Unknown feature %s." % name)
        return getattr(self.features, name, False)
    ...
    def get_requirements(self):
        ...
        if self.textfield_bleached:
            reqs.add("bleach")
        if self.has_feature('third_party_authentication'):
    ...
    def logger(self):
        """
        This must not be used before Django has done its logging config. For
...
site. :manage:`diag` is a command-line shortcut to this.
    ...
    def sorted_plugins(self):
        ...
        if self._sorted_plugins is None:
            ...
            for p in self.installed_plugins:
                if p.is_active() and p.needed_by is None:
                    # explicitly installed
                    self._sorted_plugins.append(p)
            for p in self.installed_plugins:
                if p.is_active() and p.needed_by is not None:
...
#                     raise ChangedAPI(msg)
    ...
    def get_middleware_classes(self):
        ...
        if self.has_feature('third_party_authentication') and has_socialauth:
    ...
    def get_main_html(self, ar, **context):
        ...
        return self.plugins.jinja.render_from_ar(
            ar, 'admin_main.html', **context)
                ...
                for user_type in UserTypes.objects():
                    for wf in self.kernel.web_front_ends:
                        count += with_user_profile(
                            user_type, wf.renderer.build_js_cache, force)
        if verbosity > 0:
            self.logger.info("%d lino*.js files have been built in %s seconds.",
    ...
    <https://docs.djangoproject.com/en/4.1/ref/request-response/#django.http.HttpRequest.build_absolute_uri>`__
...
signature as `django.core.mail.EmailMessage`.
    ...
    def get_plugin_setting(self, plugin_name, option_name, *default):
        ...
        if len(default) == 0:
            raise Exception(
                "Plugin {} is not installed and no default was provided".format(plugin_name))
        return default[0]
