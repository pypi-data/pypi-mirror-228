    ...
    def __init__(self, layout_handle, index, editor, **kw):
        ...
        if isinstance(editor, FieldElement):
            ...
            def fk_renderer(fld, name):
                ...
                if rpt is not None and rpt.detail_action is not None:
...
class ConstantElement(LayoutElement):
    ...
    def __init__(self, lh, fld, **kw):
        ...
        # self.text = text
        # print("20230313 ContantElement.__init__() ->", self.value.get('html'))
...
class FieldElement(LayoutElement):
    ...
    def __init__(self, layout_handle, field, hide_sum=False, **kw):
        ...
        #     print("20210629 Field '%s' in %s has no name!" %
        #                     (field, layout_handle))
        ...
        super().__init__(layout_handle, field.name, **kw)
...
class PreviewTextFieldElement(TextFieldElement):
    pass
...
class ForeignKeyElement(ComplexRemoteComboFieldElement):
    ...
    # def store_options(self, **kw):
    #     kw = super().store_options(**kw)
    #     if self.editable:
    #         # kw.update(paramNames=js_code(py2js({'start': 'start', 'limit': 'limit', 'mk': 'mk', 'mt': 'mt'})))
    #         kw.update(paramNames={'start': 'start', 'limit': 'limit', 'mk': 'mk', 'mt': 'mt'})
    #     return kw
    ...
    def get_field_options(self, **kw):
        ...
        if settings.SITE.kernel.web_front_ends[0].app_label == "react":
            options = dict(
                related_actor_id=actor.actor_id,
                allowBlank=kw.get('allowBlank', False))
            da = actor.default_action
            options.update(view_permission=bool(da) and da.get_view_permission(get_user_profile()))
            if (
                hasattr(self.field, 'model')
                and hasattr(self.field.model, f"create_{self.field.name}_choice")
            ):
                options.update(allowCreate=True)
            else:
        ...
        if not isinstance(self.layout_handle.layout, ColumnsLayout):
            ...
            if self.layout_handle.ui.renderer.extjs_version is not None:
                ...
                if a2 and not a2.get_view_permission(get_user_profile()):
                    a2 = None
...
class DisplayElement(FieldElement):
    ...
    def __init__(self, *args, **kw):
        ...
        fld = self.field
        self.preferred_height = fld.preferred_height
        self.preferred_width = fld.preferred_width
        if hasattr(fld, 'max_length') and fld.max_length is not None:  # might be explicit 0
            # if fld.max_length == 0:
            #     raise Exception("20230218 max_length is 0 {} {}".format(fld.__class__, fld))
            self.preferred_width = fld.max_length
...
class HtmlBoxElement(DisplayElement):
    ...
    def as_plain_html(self, ar, obj):
        # print("20210615 HtmlBoxElement.as_plain_html()", self, ar, ar.get_user(), obj)
        value = self.value_from_object(obj, ar)
...
class LightWeightContainer(HtmlBoxElement):
    """

    Renders plain html usually generated by `HtmlBox` fields.

    Note that this creates an automatic VirtualField that is special because it
    is created during :func:`create_layout_element` after the startup analysis.

    """

    def __init__(self, lh, slave, name, return_type_for_method, **kw):
        box = fields.HtmlBox(
            slave.label, name=name, help_text=slave.help_text,
            delayed_value=True)

        box._return_type_for_method = return_type_for_method
        fld = fields.VirtualField(box, return_type_for_method)
        ...
        #     print("20181121 SlaveSummaryPanel getter", slave)
        #     return slave.get_table_summary(*args, **kwargs)
        # fld.name = slave.__module__ + '_' + slave.__name__
        fld.model = lh.layout._datasource
        name = slave.actor_id
        # name = slave.actor_id.replace('.', '_')
        # fld.name = slave.actor_id.replace('.', '_')
        fld.model.add_virtual_field(name, fld)
        # actor.virtual_fields[fld.name] = fld
        # fld.lino_resolve_type()
        assert fld.preferred_width != 0
        assert box.preferred_width != 0
        super().__init__(lh, fld, **kw)
        assert self.preferred_width != 0


class SlaveSummaryPanel(LightWeightContainer):
    """
...
class StoryElement(LightWeightContainer):
    ...
    Used to display objects as plain html genereted by
    :meth:`Model.as_summary_row<lino.core.model.Model.as_summary_row>` from
    tables with `display_mode` DISPLAY_MODE_STORY.

    """
    def __init__(self, lh, slave, name, **kw):
        super().__init__(lh, slave, name, slave.get_table_story, **kw)

class ListElement(LightWeightContainer):
...
class Panel(Container):
    ...
    *all* its children are vflex (if vflex and non-vflex elements are
    ...
    def __init__(self, layout_handle, name, vertical, *elements, **kw):
        ...
        if w == 0:
            raise Exception("{} has preferred_width 0 ({} children: {})".format(
                self, "vertical" if self.vertical else "horizontal", self.elements))
...
_FIELD2ELEM = (
    ...
    (fields.PreviewTextField, PreviewTextFieldElement),
...
def get_htmlbox_element(lh, de, name, **kw):
    ...
    lh.add_data_elem(field)
    e = HtmlBoxElement(lh, field, **kw)
    e.add_requirements(*de.required_roles)
    return e

def get_summary_element(lh, de, name, **kw):
    e = SlaveSummaryPanel(lh, de, name, **kw)
    e.add_requirements(*de.required_roles)
    assert lh.layout._datasource is e.field.model
    lh.add_data_elem(e.field)
    return e
...
def create_layout_element(lh, name, **kw):
    ...
    if isinstance(de, GenericRel):
        de.primary_key = False
        de.verbose_name = None
        de.help_text = None
        lh.add_data_elem(de)
        return GenericRelElement(lh, de, **kw)
    ...
    if isinstance(de, type) and issubclass(de, tables.AbstractTable):
        ...
        if isinstance(lh.layout, FormLayout):
            ...
            if len(de.display_mode) > 1 and lh.ui.renderer.front_end.media_name == 'react':
