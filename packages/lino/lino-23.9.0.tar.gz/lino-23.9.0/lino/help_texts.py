...
every individual database object."""),
    ...
    'lino.mixins.Created' : _("""Adds a timestamp field which holds the creation time of every
individual database object."""),
    ...
    'lino.mixins.ProjectRelated' : _("""Mixin for models that are related to a "project",
i.e. to an object of the type given by your lino.core.site.Site.project_model."""),
    ...
    'lino.mixins.ProjectRelated.update_owned_instance' : _("""When a project-related object controls
...
resolved at startup to the model's class object."""),
    'lino.mixins.dupable.Dupable.on_analyze' : _("""Setup the dupable_word_model attribute.  This will be
called only on concrete subclasses."""),
    'lino.mixins.dupable.Dupable.dupable_matches_required' : _("""Return the minimum number of words that must sound alike before
...
qualification like "Dr." or "PhD"."""),
    ...
    'lino.mixins.human.Human.mf' : _("""Taking three parameters m, f and u of any type, returns one
of them depending on whether this Person is male, female or of
unknown gender."""),
    'lino.mixins.human.Human.get_last_name_prefix' : _("""May be used for handling special of titles (e.g. "Cardinal",
"Graf") which come before the last name (not before the first
name)."""),
    'lino.mixins.human.Human.get_full_name' : _("""Returns a one-line string composed of salutation,
first_name and last_name."""),
    'lino.mixins.human.Human.format_family_member' : _("""Used in humanlinks.LinksByHuman and in
...
field "Age"."""),
    ...
    'lino.mixins.human.Born.get_age' : _("""Return the age (in years) of this human.  See
...
fields which form a kind of editable timestamp field."""),
    ...
    'lino.mixins.periods.CombinedDateTime.set_datetime' : _("""Given a datetime value, update the two corresponding fields
FOO_date and FOO_time (where FOO is specified in name which
must be either "start" or "end")."""),
    'lino.mixins.periods.CombinedDateTime.get_datetime' : _("""Return a datetime value from the two corresponding
...
end_time."""),
    ...
    'lino.mixins.periods.DateRange' : _("""Mixin for models that represent a period defined by a start date and an end
date."""),
...
different views."""),
    ...
    'lino.mixins.ref.Referrable' : _("""Mixin for things that have a unique reference, i.e. an identifying
name used by humans to refer to an individual object."""),
    ...
    'lino.mixins.ref.Referrable.on_duplicate' : _("""Before saving a duplicated object for the first time, we must
change the ref in order to avoid an IntegrityError."""),
    ...
    'lino.mixins.ref.Referrable.quick_search_filter' : _("""Overrides the default behaviour defined in
...
the primary key."""),
    ...
    'lino.mixins.registrable.RegistrableState' : _("""Base class for the choices of the choicelist that defines the state
field of any Registrable."""),
    'lino.mixins.registrable.RegistrableState.is_editable' : _("""Whether the registrable object having this state should be
editable or not."""),
    'lino.mixins.registrable.Registrable' : _("""Base class to anything that may be "registered" and "deregistered", where
"registered" means "this object has been taken account of"."""),
    ...
    'lino.mixins.registrable.Registrable.get_registrable_fields' : _("""Return a list of the fields that are disabled when this is
registered (i.e. state is not editable)."""),
    ...
    'lino.mixins.registrable.Registrable.register' : _("""Register this object.  The base implementation just sets the state
to "registered"."""),
    'lino.mixins.registrable.Registrable.deregister' : _("""Deregister this object.  The base implementation just sets the
state to "draft"."""),
...
"sequence number"."""),
    ...
    'lino.mixins.sequenced.Sequenced.duplicate' : _("""Create a duplicate of this object and insert the new object
below this one."""),
...
column."""),
    'lino.mixins.sequenced.Sequenced.get_siblings' : _("""Return a Django Queryset with all siblings of this, or None if
this is a root element which cannot have any siblings."""),
    ...
    'lino.mixins.sequenced.Hierarchical.whole_tree' : _("""Returns a tuple of tuples.
The first item is the top most ancestor and the second item is a tuple
of all the children of the ancestor.
A children is wrappend inside another tuple where the first item is the
children and second item is None when the children has no grandchildren
otherwise the second item will be another tuple of grandchildren
and so on."""),
...
object."""),
    'lino.modlib.changes.ChangesByObject.master' : _("""alias of ContentType"""),
...
including those applied to "child" objects."""),
    'lino.modlib.changes.ChangesByMaster.master' : _("""alias of ContentType"""),
...
Jinja renderer."""),
    ...
    'lino.modlib.linod.Plugin' : _("""linod plugin"""),
    'lino.modlib.linod.Plugin.remove_after' : _("""Removes Job history older then the given value."""),
    ...
    'lino.modlib.memo.parser.Parser.add_suggester' : _("""trigger is a short text, usually one character, like "@" or "#",
which will trigger a list of autocomplete suggestions to pop up."""),
    ...
    'lino.modlib.memo.parser.Parser.register_django_model' : _("""Register the given string name as command for referring to
database rows of the given Django database model model."""),
    ...
    'lino.modlib.memo.parser.Parser.parse' : _("""Parse the given string src, replacing memo commands by their
result."""),
    ...
    'lino.modlib.system.Plugin.use_dashboard_layouts' : _("""Whether to use system.DashboardLayouts."""),
...
generating of "traditional" ("snail") mail addresses."""),
    'lino.utils.addressable.Addressable.address_person_lines' : _("""Yield one or more text lines, one for each line
of the person part."""),
    'lino.utils.addressable.Addressable.address_location_lines' : _("""Yield one or more text lines, one for each line
...
are separated by linesep, which defaults to a newline."""),
    'lino.utils.addressable.Addressable.get_address_html' : _("""Return the full postal address as a text containing simple html
markup with a single paragraph (<p>)."""),
    'lino.utils.addressable.Addressable.has_address' : _("""Checks whether the address is non empty and returns a boolean value."""),
...
using a HttpRequest to build the context."""),
    ...
    'lino.utils.cycler.Cycler' : _("""An iterator that loops over an iteration and starts back at the beginning
...
Checkers choicelist."""),
    'lino.modlib.checkdata.Checker.update_problems' : _("""Update the problem messages of this checker
...
user."""),
    'lino.modlib.users.User.calendar_sync_token' : _("""The next sync token for the user's calendar synchronization."""),
    'lino.modlib.linod.Procedure' : _("""Is basically a Job which contains a function func to run at
default interval given by every_unit and every_value."""),
    ...
    'lino.modlib.linod.Procedures' : _("""The choicelist that contains instances Procedure as choices."""),
    'lino.modlib.linod.LogLevels' : _("""A choicelist with the equivalent log levels from the logging module."""),
    'lino.modlib.linod.JobRule' : _("""Overrides the recurrent rule of a Procedure."""),
    'lino.modlib.linod.JobRule.name' : _("""Some name for this job rule."""),
    'lino.modlib.linod.JobRule.procedure' : _("""The pointer to an instance of Procedure."""),
    'lino.modlib.linod.JobRule.cancelled' : _("""Tells whether the task should be ignored."""),
    'lino.modlib.linod.JobRule.log_level' : _("""Tells what sort of messeages to put into the Job.message field."""),
    'lino.modlib.linod.JobRule.run' : _("""Performs a routine job."""),
    'lino.modlib.linod.Job' : _("""A historical record of a system task."""),
    'lino.modlib.linod.Job.start_datetime' : _("""Tells at what time exactly this job started."""),
    'lino.modlib.linod.Job.rule' : _("""A pointer to an instance of JobRule which describes the nature of the task."""),
    'lino.modlib.linod.Job.message' : _("""Stores information about the job, mostly logs."""),
    'lino.modlib.linod.JobRules' : _("""The default actor for the JobRule model."""),
    'lino.modlib.linod.Jobs' : _("""The default actor for the Jobs model."""),
    'lino.modlib.linod.tasks.Tasks' : _("""The base class that manages the system tasks."""),
    'lino.modlib.linod.tasks.Tasks.setup' : _("""Gathers all the jobs defined using dd.api.schedule_often() and dd.api.schedule_daily() decorator."""),
    'lino.modlib.linod.tasks.Tasks.run' : _("""Runs the system tasks which are due for the current time."""),
    'lino.modlib.linod.tasks.Tasks.status' : _("""Returns the status of all the tasks."""),
    'lino.modlib.linod.tasks.Tasks.create_rule' : _("""Creates a JobRule for a given
Procedure"""),
    'lino.modlib.linod.tasks.Tasks.cancel_rule' : _("""Sets Job.cancelled to True for a given rule."""),
    'lino.modlib.linod.SetupTasks' : _("""An action to adjust the database objects to the latest changes."""),
    'lino.modlib.linod.RunJob' : _("""An action to run a job immediately."""),
    'lino.modlib.search.SiteSearch' : _("""The virtual table used to implement site-wide searches using default Django
ORM filtering."""),
...
subclasses."""),
    'lino.modlib.gkfs.ContentTypes' : _("""Lino installs this as the default data view for
...
Combobox."""),
    ...
    'lino.modlib.memo.Mention' : _("""Django model to represent a mention, i.e. the fact that some memo text of
the owner points to some other database row."""),
    ...
    'lino.modlib.notify.Message.subject' : _("""The subject of this message. See Subject and body of a notification
message."""),
    'lino.modlib.notify.Message.body' : _("""The body of this message. See Subject and body of a notification
message."""),
    'lino.modlib.notify.Message.user' : _("""The recipient of this message. The site user to whom this
message is to be delivered."""),
    ...
    'lino.modlib.notify.Message.emit_notification' : _("""Emit a notification message to each the given recipients,
respecting their individual user preferences."""),
...
given mail_mode mm."""),
    ...
    'lino.modlib.notify.ChangeNotifier' : _("""Mixin for models that can emit notifications to a list of
...
This action is available on About."""),
    ...
    'lino.modlib.system.RecurrenceSet' : _("""Mixin for models that express a set of repeating ("recurrent")
events. Example usage can be found in specs.cal.automatic_events."""),
    ...
    'lino.modlib.system.RecurrenceSet.end_date' : _("""The end date of the first meeting to be generated.  Leave
this field empty if the meetings last less than one day."""),
    ...
    'lino.modlib.system.RecurrenceSet.positions' : _("""Space-separated list of one or several positions within the recurrency
cycle."""),
    'lino.modlib.system.RecurrenceSet.max_events' : _("""Maximum number of calendar entries to generate."""),
    'lino.modlib.system.RecurrenceSet.weekdays_text' : _("""A virtual field returning the textual formulation of the
weekdays where the recurrence occurs."""),
    'lino.modlib.system.Recurrencies' : _("""List of possible choices for a 'recurrency' field."""),
    'lino.modlib.system.Recurrencies.easter' : _("""Repeat events yearly, moving them together with the Easter
data of that year."""),
    ...
    'lino.modlib.system.DurationUnit.add_duration' : _("""Return a date or datetime obtained by adding value
...
Returns None is orig is empty."""),
    ...
    'lino.modlib.system.Genders' : _("""Defines the two possible choices "male" and "female" for the gender of a
person."""),
...
lino.mixins.periods.DateRange."""),
    'lino.modlib.system.TimeZones' : _("""Used by lino.modlib.users.User.time_zone and
...
uploaded file in a new browser window."""),
    'lino.modlib.uploads.Uploads' : _("""Base class for all data views of upload files."""),
...
It encapsulates some really basic functionality.
...
uploads of this type.  See also Shortcuts."""),
    ...
    'lino.modlib.uploads.UploadsByController' : _("""Shows the uploaded files associated to this
database object."""),
...
display the list of available users."""),
    ...
    'lino.modlib.users.User.username' : _("""Must be either unique or empty.  Users with an empty username cannot
sign in."""),
    'lino.modlib.users.User.user_type' : _("""The user type assigned to this user. User accounts having this
field empty cannot be used to sign in."""),
    'lino.modlib.users.User.partner' : _("""The partner record with additional contact information about
this user account."""),
    'lino.modlib.users.User.initials' : _("""The nickname or initials of this user. This does not need to be unique
but should be reasonably identifying."""),
    'lino.modlib.users.User.time_zone' : _("""The time zone Lino should use in time fields or when displaying
timestamps to this user."""),
    'lino.modlib.users.User.date_format' : _("""The date format to use when parsing or displaying dates."""),
    'lino.modlib.users.User.verification_code' : _("""A random string set for every new user. Used for
online registration.
When this is non empty, is_verified() returns False and the verify action is available."""),
    'lino.modlib.users.User.person' : _("""A virtual read-only field that returns the Person MTI child of the partner (if it
exists) and otherwise None."""),
    'lino.modlib.users.User.company' : _("""A virtual read-only field that returns the Company MTI child of the partner (if it
exists) and otherwise None."""),
...
or the username."""),
    'lino.modlib.users.User.get_row_permission' : _("""Only system managers may edit other users.
See also disabled_fields()."""),
...
action."""),
    ...
    'lino.modlib.users.ChangePassword.current' : _("""The current password. Leave empty if the user has no password yet. A
site manager doesn't need to specify this at all."""),
    ...
    'lino.modlib.users.SignIn' : _("""Open a window that asks for username and password and authenticates as this
user when submitted."""),
