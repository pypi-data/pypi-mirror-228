...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('-t', '--tolerate', action='store_true',
                            dest='tolerate', default=False,
                            help='Tolerate database errors.')
        parser.add_argument('-s', '--simulate', action='store_true',
                            dest='simulate', default=False,
                            help="Just simulate, don't actually write any data.")
...
''' % s)
        skip_models = {
            'contenttypes.ContentType', 'sessions.Session', 'checkdata.Message'
        }
        self.models = [m for m in sorted_models_list()
           if full_model_name(m) not in skip_models]
        ...
        #
        # if settings.SITE.is_installed('checkdata'):
        #     from django.contrib.sessions.models import Session
        #     M = settings.SITE.models.checkdata.Message
        #     self.models = [m for m in self.models if not issubclass(m, M)]
        ...
        for model in progress.bar(self.models):
            ...
            for i, filename, qs in chunks:
                ...
                if self.options['simulate']:
                    msg = ("The data of table {} has not been dumped because "
                       "--simulate was specified.").format(model._meta.db_table)
