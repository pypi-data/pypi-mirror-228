...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--remove-media', action='store_true',
                            dest='remove_media', default=False,
    ...
    def makemigrations(self, verbosity):
        for p in settings.SITE.installed_plugins:
            ...
            if p.app_name.startswith('lino') or makemigrations:
                ...
                call_command('makemigrations', p.app_name.split('.')[-1], interactive=False, verbosity=verbosity)
    ...
    def handle(self, *args, **options):
        ...
        remove_media = options.pop('remove_media')
        ...
        if interactive:
            ...
            if remove_media:
                msg += " and remove all files below {}".format(mroot)
            msg += ".\nAre you sure (y/n) ?"
            if not confirm(msg):
