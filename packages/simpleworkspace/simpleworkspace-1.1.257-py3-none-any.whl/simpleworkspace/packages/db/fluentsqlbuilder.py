...
class FluentSqlBuilder:
    def __init__(self, connection: MySQLConnection|MsSqlConnection|Sqlite3Connection):
        ...
        self._segments = []
        self._params = []
        self._connection_is_MSSQL = isinstance(connection, MsSqlConnection)  # Detect MSSQL connection
    ...
    def CreateTable(self, table_name: str, ignoreIfExists: bool = True):
        ...
        if(ignoreIfExists):
            self.Append(f"CREATE TABLE IF NOT EXISTS {table_name}")
        else:
            self.Append(f"CREATE TABLE {table_name}")
        return self
