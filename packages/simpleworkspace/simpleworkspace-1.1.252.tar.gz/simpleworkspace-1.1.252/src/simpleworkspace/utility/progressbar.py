...
from ..types.time import TimeEnum, TimeSpan
import threading
...
class ProgressBar:
    ...
    def __init__(self, iterable=None, total=None):
        ...
        self._CallbacksPerIncrementSettings = None
    ...
    def Increment(self, increment=1):
        ...
        with self._lock:
            ...
            if self._CallbacksPerIncrementSettings:
                for setting in self._CallbacksPerIncrementSettings:
    ...
    def AddCallbackAfterIncrements(self, increments: int, callback: Callable[[], any]):
        """
        Set a callback to be executed after x increments.
        :param increments: The number of increments after which the callback should be executed.
        :param callback: The user-specified function to be called after 'increments' number of increments. recieves in param1 the class instance itself
        """
        if self._CallbacksPerIncrementSettings is None:
            self._CallbacksPerIncrementSettings = []
        self._CallbacksPerIncrementSettings.append({"increments": increments, "callback": callback, "effective_increments": 0})
