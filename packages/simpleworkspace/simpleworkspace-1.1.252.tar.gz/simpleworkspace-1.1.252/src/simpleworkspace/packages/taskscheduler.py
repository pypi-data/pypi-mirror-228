...
class CommandTask(ITask):
    ...
    def __init__(self, interval:_TimeSpan, command:str|list[str]) -> None:
        ...
        super().__init__()
        self.task_id = md5(' '.join(command).encode()).hexdigest()[:16]
...
class TaskSchedulerManager:
    ...
    def _RunTaskEvent(self, task: ITask, eventFunc: _Callable):
        with self._lock_threading_activeTasks:
            self._threading_activeTasks.add(task.task_id)
        ...
        try:
            output = eventFunc()
            taskResult.output = f"Event {eventName}[{taskResult.ElapsedMS} MS]: {task.task_id}"
            if(output):
                taskResult.output += ", " + output
        except Exception as e:
            taskResult.error = f"Event {eventName} failed[{taskResult.ElapsedMS} MS]: {task.task_id}, Error: {_traceback.format_exc()}"
        
        with self._lock_threading_activeTasks:
            self._threading_activeTasks.remove(task.task_id)
        return taskResult
    ...
    def Run(self):
        ...
        while self._FLAG_RUN:
            tasksNeedingScheduleRun = _LINQ(tasksWithSchedules) \
                .Where(lambda task: task.task_id not in self._threading_activeTasks) \
