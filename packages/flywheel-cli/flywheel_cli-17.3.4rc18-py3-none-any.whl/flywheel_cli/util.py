...
def sanitize_filename(filename):
    ...
    The asterisk in t2*, t2 *, t2_* (case insensitive) will be changed to the word
    "star" prior to sanitization
    """
    # IMPORTANT
...
def sorted_container_nodes(containers):
    """Returns a sorted iterable of containers sorted by label or id (whatever is
    available)
...
def localize_timestamp(timezone=None):
    """Localize timestamp"""
    timezone = DEFAULT_TZ if timezone is None else timezone
    return timezone.replace(tzinfo=timezone)
...
def args_to_list(items):
    """Convert an argument into a list of arguments (by splitting each element on
    comma)"""
    result = []
...
def hrsize(size: int) -> str:
    ...
    for suffix in "KMGTPEZY":
        size_f /= 1024.0
        if size_f < 10.0:
            return f"{round(size_f, 1)}{suffix}B"
        if size_f < 1000.0:
            return f"{int(round(size_f, 0))}{suffix}B"
    return f"{int(round(size_f, 0))}YB"
...
class KeyWithOptions:
    ...
    If given a dictionary, pops <key> from the dictionary, and stores it as an
    attribute. Otherwise takes the provided value and stores it as an attribute
    """
...
def encode_json(obj: Any) -> Any:
    ...
    if isinstance(obj, datetime.datetime):
        if obj.tzinfo is None:
            obj = obj.replace(tzinfo=DEFAULT_TZ)
...
def merge_lists(list_a, list_b):
    """Merge lists 'list_a' and 'list_b', returning the result or None if the result is
    empty"""
...
class DicomUtils:
    ...
    def determine_session_label(self, context, dcm, uid, timestamp=None):
...
def parse_resolver_path(path: str) -> List[str]:
    ...
    file_path: List[str] = []
    if "/files/" in path:
        path, fp = path.split("/files/")
        file_path = ["files", fp]

    analysis_path: List[str] = []
    if "/analyses/" in path:
        path, ap = path.split("/analyses/")
        analysis_path = ["analyses", ap]

    paths: List[str] = path.split("/") or []
    paths = paths + analysis_path + file_path

    return [path_el for path_el in paths if path_el]
...
def get_sdk_client(api_key: str) -> flywheel.Client:
    ...
    log.debug(f"SDK Version: {flywheel.flywheel.SDK_VERSION}")
...
class SDKClient:
    ...
    def __init__(self, api_key: str):
        ...
        self._auth_info: Optional[models.FWAuth] = None
    ...
    def upload(self, cont_name, cont_id, filename, fileobj, metadata=None):
        ...
        log.debug(f"Uploading {filename} size {size} to {cont_name}/{cont_id}")
        key = f"{cont_name}{cont_id}"
        if not metadata:
            metadata = {}
        if not metadata.get("size") and (self.signed_url or self.multipart_signed_url):
            metadata["size"] = size
        ...
        if not signed_successful:
            if self.signed_url:
                ...
                raise errors.BaseError(
                    f"Could not upload {filename} to {cont_name}/{cont_id} using "
                    "signed upload"
                )
    ...
    def multipart_signed_url_upload(
        ...
        log.debug(f"Signed urls: {signed_urls}, part_size={part_size}")
        e_tags = []
        s3_upload_id = None
        for part_num, part_url in enumerate(signed_urls):
            log.debug(f"Upload to {part_url}")
            if not s3_upload_id:
                parsed = urlparse(part_url)
                upload_dict = parse_qs(parsed.query)
                if "uploadId" in upload_dict:
                    s3_upload_id = upload_dict["uploadId"][0]

            # Splitting file into parts with PartialReader
            remaining = metadata["size"] - part_size * part_num
            ...
            if not r.ok:
                ...
                return False
            etag_dict = r.headers
            if "ETag" in etag_dict:
                e_tags.append(etag_dict["ETag"].strip('"'))
        log.debug(f"Completing ticket {ticket}")
        if s3_upload_id and len(e_tags) != 0:
            self.call_api(
                core_url,
                "POST",
            ...
            self.call_api(core_url, "POST", query_params=[("ticket", ticket)])
        return True
    ...
    def create_upload_ticket(self, url, filename, metadata=None):
        ...
        log.debug(
            (
                "Requesting a signed url upload, url "
                f"{url} filename {filename} metadata {metadata}"
            )
        )
    ...
    def post_deid_log(self, payload) -> str:
        ...
        resp = self.session.post(
            ...
            json=payload,
            headers={"Authorization": detect_auth_header(self.api_key, self.host)},
        )
        return resp.json()["id"]
    ...
    def can_import_into(
        ...
        having_actions = set(self.get_user_actions(resolved_prj))
...
def create_unique_filename(path: str, path_list: List[str]) -> str:
    """Create unique filename by appending a counter to the end of file name if such
    file already exists in the given filename set"""
...
def detect_auth_header(key: str, host: Optional[str] = None):
    """
    Will detect if a key passed in, is a drone or device key and
    return the appropriate header.
    """
    key = key.rsplit(":", maxsplit=1)[-1]
    if len(key) == 57:
        return f"Bearer {key}"
    return f"scitran-user {host}:{key}"
