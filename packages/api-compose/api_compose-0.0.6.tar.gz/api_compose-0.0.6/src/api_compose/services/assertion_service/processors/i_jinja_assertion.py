...
from api_compose.services.composition_service.models.actions.inputs import JsonHttpActionInputModel
from api_compose.services.composition_service.models.actions.outputs import JsonHttpActionOutputModel
...
class InteractiveJinjaAssertion(cmd2.Cmd):
    ...
    def do_examples(self, arg):
        ...
        self.poutput(
            f"""\t\t render {{ output_body('{self.example_action_model.execution_id}', '$.field_one') }} == '1234'  """)
        self.poutput(f"""\t Assert Output Headers:""")
        self.poutput(
            f"""\t\t render {{ output_headers('{self.example_action_model.execution_id}', '$.Server') }} == 'Google Frontend'  """)
        self.poutput(f"""\t Assert Output Status Code:""")
        self.poutput(
            f"""\t\t render {{ output_status_code('{self.example_action_model.execution_id}' }} == 200  """)
    ...
    def do_actions(self, ns: argparse.Namespace):
        ...
        for idx, action_model in enumerate(self.jinja_context.action_models):
            ...
            if ns.input or ns.all:
                self.poutput(f"output={action_model.input.model_dump_json(indent=4)}")
