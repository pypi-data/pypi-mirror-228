...
def NP_FindActualWingHome(winghome):
    ...
    if app_dir and os.path.isdir(
        os.path.join(app_dir, 'Contents', 'Resources')
    ):
...
if 'WINGDB_ACTIVE' in os.environ and os.environ['WINGDB_ACTIVE'] != str(
    os.getpid()
):
...
if (
    ...
    try:
        ...
        very_verbose_log = os.environ.get(
            'WINGDB_LOGVERYVERBOSE', kLogVeryVerbose
        )
        if (
        ...
        security_token = os.environ.get(
            'WINGDB_SECURITYTOKEN', kSecurityToken
        )
        ...
        bootstrap_utils = NP_LoadModuleFromBootstrap(
            WINGHOME, 'bootstrap_utils'
        )
        winghome, user_settings = bootstrap_utils.NP_SetupWingHomeModule(
            WINGHOME
        )
        ...
        err = startdebug.CreateErrStream(
            netserver, logfile, very_verbose_log
        )
        ...
        startdebug.RestoreEnvironment(
            orig_sys_modules, orig_sys_path, orig_meta_path
        )
