...
logger = logging.getLogger('lino_xl.lib.google.mixins')
...
class GoogleSynchronized(dd.Model):
    ...
    if dd.is_installed('google'):
        google_id = dd.CharField(max_length=200, verbose_name=_('Google resource ID/name'), blank=True)
...
class GoogleCalendarSynchronized(GoogleSynchronized):
    ...
    if dd.is_installed('google'):
        ...
        def insert_or_update_into_google(self, crs, user=None, synchronizer=None):
            if user is None:
            ...
            if self.google_id:
                c = make_api_call(lambda: crs.get(calendarId=self.google_id))
                if c.get('timeZone') is not None:
                    body.pop('timeZone')
                make_api_call(lambda: crs.update(calendarId=self.google_id, body=body))
                if synchronizer is not None:
                    synchronizer.stats.calendars__out_update += 1
            else:
                c = make_api_call(lambda: crs.insert(body=body))
        ...
        @classmethod
        def sync_deleted_records(cls, synchronizer, resource):
            for obj in rt.models.google.DeletedEntry.objects.select_related('user').filter(
                    calendar=True, user=synchronizer.user):
                try:
                    make_api_call(
                ...
                if (cs := synchronizer.stats.calendars) % synchronizer.logging_frequency == 0:
                    logger.info(f"google.Synchronizer ({synchronizer.user}): synchronized {cs} calendars so far.")

        @classmethod
        def insert_or_update_google_calendar(cls, cal: dict, user=None, synchronizer=None):
            if user is None:
                assert synchronizer is not None
                user = synchronizer.user
            try:
            ...
            room = calendar.room_calendars.filter(name=calendar.name, calendar=calendar).order_by('pk').first()
            if room is None:
                room = Room(
                    name=cal.get("summary", ""),
    ...
    if dd.is_installed('google'):
        ...
        def synchronize_with_google(self, user=None) -> bool:
            if self.end_time is None and self.end_date is None:
                return False
            if not (calendar := self._get_calendar()):
                return False
            if user is not None:
        ...
        def sync_deleted_records(cls, synchronizer, resource):
            for obj in rt.models.google.DeletedEntry.objects.select_related('user').filter(
                    calendar=False, user=synchronizer.user):
                try:
                    make_api_call(
                        lambda: resource.delete(calendarId=obj.calendar_id, eventId=obj.event_id)
                    )
                except HttpError:
                    continue
                obj.delete()
                synchronizer.stats.events_deleted__out += 1
                if (evs := synchronizer.stats.events) % synchronizer.logging_frequency == 0:
                    logger.info(f"google.Synchronizer ({synchronizer.user}): synchronized {evs} events so far.")
        ...
        def get_outward_insert_update_queryset(cls, user):
            ...
            if not cal_cases:
                qs = qs.annotate(cal=models.Value(
                    settings.SITE.site_config.site_calendar.pk if settings.SITE.site_config.site_calendar else None,
                    output_field=dd.ForeignKey(rt.models.cal.Calendar)
                ))
            else:
                qs = qs.annotate(cal=models.Case(
                    *cal_cases,
                    default=settings.SITE.site_config.site_calendar.pk if settings.SITE.site_config.site_calendar else models.Value(None),
                    output_field=dd.ForeignKey(rt.models.cal.Calendar)
                ))
            return qs.filter(cal__isnull=False).annotate(sub=models.Case(
                models.When(
                    models.Exists(
                        rt.models.google.CalendarSubscription.objects.filter(
                    ...
                    then=rt.models.google.CalendarSubscription.objects.filter(
                        models.Q(access_role='writer') | models.Q(access_role='owner'),
                        calendar__pk=models.OuterRef("cal"),
                        user=user
                    ).values('pk')[:1]
                ...
                output_field=dd.ForeignKey(rt.models.google.CalendarSubscription, on_delete=models.SET_NULL, null=True, blank=True)
            )).filter(sub__isnull=False).annotate(edit=models.Case(
                models.When(
                    models.Q(modified__gte=rt.models.google.CalendarSubscription.objects.filter(pk=models.OuterRef("sub")).values("modified")[:1]),
                    then=models.Value(True)
                ),
                default=models.Value(False),
                output_field=models.BooleanField()
        ...
        def insert_or_update_into_google(self, ers, synchronizer=None):
            body = {
            ...
            if self.google_id:
                make_api_call(
                    lambda: ers.update(calendarId=cal.google_id, eventId=self.google_id, body=body)
                )
                if synchronizer is not None:
                    synchronizer.stats.events__out_update += 1
            else:
                e = make_api_call(
                    lambda: ers.insert(calendarId=cal.google_id, body=body)
                )
                if synchronizer is not None:
        ...
        @classmethod
        def insert_or_update_google_event(cls, event: dict, room, user=None, synchronizer=None):
            if user is None:
                assert synchronizer is not None
                user = synchronizer.user
            try:
...
class GoogleContactSynchronized(GoogleSynchronized):
    ...
    if dd.is_installed('google'):
        ...
        @classmethod
        def delete_google_contact(cls, contact, user=None, synchronizer=None):
            if user is None:
                assert synchronizer is not None
                user = synchronizer.user
            try:
                item = cls.objects.get(google_id=contact["resourceName"], user=user)
                item.delete()
                rt.models.google.DeletedContact.objects.filter(contact_id=contact["resourceName"], user=user).delete()
                if synchronizer is not None:
        ...
        @classmethod
        def sync_deleted_records(cls, synchronizer, resource):
            for obj in rt.models.google.DeletedContact.objects.select_related('user').filter(user=synchronizer.user):
                try:
                    make_api_call(
                        lambda: resource.deleteContact(resourceName=obj.contact_id)
                    )
                except HttpError:
                    continue
                obj.delete()
                synchronizer.stats.contacts_deleted__out += 1
                if (cns := synchronizer.stats.contacts) % synchronizer.logging_frequency == 0:
                    logger.info(f"google.Synchronizer ({synchronizer.user}): synchronized {cns} contacts so far.")

        @classmethod
        def get_outward_insert_update_queryset(cls, user=None):
            if user is not None:
                return cls.objects.filter(
                    modified__gte=rt.models.google.ContactSyncToken.objects.get(user=user).modified, user=user)
            return cls.objects.annotate(syncable=models.Case(
                models.When(
                    ~models.Exists(rt.models.google.ContactSyncToken.objects.filter(user=models.OuterRef("user"))),
                    then=models.Value(True)
                ),
                models.When(
                    models.Exists(
        ...
        def parse_street(cls, street_line, address):
            ...
            if address.street_prefix:
                pattern += "(" + (
                    p_street_prefix := "(?P<street_prefix>{address.street_prefix})"
            ...
            if m["addr1"] is None or m["street_prefix"] is None or m["street"] is None:
                ...
                for i, part in parts:
                    p_index = i
                    if match := re.match(r"^(?P<street_prefix>.*)?( )?" + p_street + "$", part):
                        if m["street_prefix"] is None:
                            m["street_prefix"] = match.groupdict()["street_prefix"]
                        break
                    elif match := re.match("^" + p_street_prefix + "?( )?" + "(P?<street>.*)?$", part):
                        if m["street"] is None:
        ...
        def insert_or_update_into_google(self, resource, synchronizer=None):
            if self.google_id:
                person = make_api_call(lambda: resource.get(
            ...
            name = [n for n in person.get("names", [{
                "metadata": {
                    "primary": True
                }
            }]) if n.get("metadata", {}).get("primary")][0]
            ...
            if bdate := contact.birth_date:
                birthdays = person.get("birthdays", [{"metadata": {"primary": True}}])
                birthday = [b for b in birthdays if b.get("metadata", {}).get("primary")][0]
            ...
            def dump_address(cont, addr={"metadata": {"primary": False}}):
                if cont.country is not None:
                    ...
                    addr["countryCode"] = cont.country.isocode or cont.country.iso3 or cont.country.short_code
            ...
            address = [a for a in addresses if a.get("metadata", {}).get("primary")][0]
            addresses.clear()
            ...
            for a in rt.models.addresses.Address.objects.filter(partner=contact, primary=False):
                addresses.append(dump_address(a))

            person["addresses"] = addresses

            phones = person.get("phoneNumbers", [{"metadata": {"primary": True}}])
            phone = [p for p in phones if p.get("metadata", {}).get("primary")][0]
            phones_dup = phones.copy()
            phones.clear()
            if "type" in phone:
                phone["value"] = getattr(contact, self.type_map.get(phone["type"], "phone"))
            else:
                if contact.phone:
            ...
            for phone in rt.models.phones.ContactDetail.objects.filter(
                ...
                partner=contact,
                detail_type__in=[
                    (phone_type := lambda t: rt.models.phones.ContactDetailTypes.get_by_name(t).value)("phone"),
                    phone_type("gsm"),
                    phone_type("fax")
                ]
            ):
                phn = [p for p in phones_dup if p.get("canonicalForm", p["value"]) == phone.value]
                if phn:
                    phn = phn[0]
                else:
                    phn = {
            ...
            emails = person.get("emailAddresses", [{"metadata": {"primary": True}}])
            email = [e for e in emails if e.get("metadata", {}).get("primary")][0]
            ...
            if self.google_id:
                make_api_call(lambda: resource.updateContact(resourceName=self.google_id, body=person))
                if synchronizer is not None:
                    synchronizer.stats.contacts__out_update += 1
            else:
                resp = make_api_call(lambda: resource.createContact(body=person))
                if synchronizer is not None:
        ...
        @classmethod
        def insert_or_update_google_contact(cls, contact, user=None, synchronizer=None):
            if user is None:
                assert synchronizer is not None
                user = synchronizer.user
            try:
            ...
            if cls.contacts_model is None:
                cls.contacts_model = dd.resolve_model(dd.plugins.google.contacts_model)
            ...
            for phone in contact.get("phoneNumbers", []):
                ...
                phones.append({
                    "primary": False,
                    "value": v,
                    "detail_type": rt.models.phones.ContactDetailTypes.get_by_name(t)
                })
            ...
            for address in contact.get("addresses", []):
                ...
                if (cc := address.get("countryCode")) is not None:
                    ...
                    country = rt.models.countries.Country.objects.filter(
                        models.Q(short_code=cc) | models.Q(isocode=cc) | models.Q(iso3=cc)
                    ).first()

                    if country is None:
                        raise Exception(f"No such country in the database {address['country']} ({address['countryCode']})")
            ...
            if person.pk is None:
                person.save_new_instance(person.get_default_table().request(user=user))
            else:
                person.save()
            ...
            for i, phone in enumerate(phones):
                p, __ = rt.models.phones.ContactDetail.objects.get_or_create(partner=person, **phone)
                phones[i] = p

            if phones:
                rt.models.phones.ContactDetail.objects.filter(
                    partner=person, primary=False
                ).exclude(pk__in=[p.pk for p in phones]).delete()

            for i, address in enumerate(addresses):
                a, __ = rt.models.addresses.Address.objects.get_or_create(partner=person, **address)
                addresses[i] = a

            if addresses:
                rt.models.addresses.Address.objects.filter(
                    partner=self.contact, primary=False
                ).exclude(pk__in=[a.pk for a in addresses]).delete()

            self.full_clean()
            if self.pk is None:
                self.save_new_instance(ar=self.get_default_table().request(user=user))
                if synchronizer is not None:
