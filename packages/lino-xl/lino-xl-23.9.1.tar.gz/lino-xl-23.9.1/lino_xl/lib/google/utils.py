...
import logging
logger = logging.getLogger('lino_xl.lib.google.utils')
...
def get_credentials(user):
    ...
    creds = Credentials(
        ...
        scopes=scps.split() if isinstance(scps := suser.extra_data['scopes'], str) else scps
    )
...
def make_api_call(make_request_fn, args=tuple(), kwargs=None, msg_fn=None, silent=False, _retry=0):
    ...
    try:
        ...
        if msg_fn is not None:
            logger.warning(msg_fn(e, *args, **kwargs))
