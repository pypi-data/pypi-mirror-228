...
def _validate_env_vars(raise_on_error=True):
...
def log_model(
    ...
    if _should_use_mlflowdbfs(run_root_artifact_uri):
        print('USING DBFS, ROOT', run_root_artifact_uri)
...
def _get_or_create_sparksession(model_path=None):
    ...
    if not _validate_env_vars(False) and model_path:
...
def _unpack_and_save_model(spark_model, dst):
    ...
    if isinstance(spark_model, PipelineModel):
        ...
        try:
            ...
            # "mlflowdbfs"
            print(f"Could not overwrite your model, use a different path for saving or remove {dst}")
