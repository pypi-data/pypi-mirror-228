    ...
    if _JOHNSNOWLABS_ENV_JSON_LICENSE_KEY not in os.environ:
        if not env_var_missing_logged:
            print(
                ...
                f"environment variable is not set. Ensure all dependencies are resolved. \n "
                f"You can set it as the raw license.json string from John Snow Labs"
            )
...
def get_default_pip_requirements():
    ...
    if _JOHNSNOWLABS_ENV_HEALTHCARE_SECRET not in os.environ:
        raise Exception(
            "You need to set the HEALTHCARE_SECRET environment variable set. Please contact John Snow Labs to get one")
    ...
    return [
        ...
        _SPARK_NLP_JSL_WHEEL_URI.format(secret=os.environ[_JOHNSNOWLABS_ENV_HEALTHCARE_SECRET]),
...
def log_model(
    ...
    elif is_local_uri(run_root_artifact_uri) or not _maybe_unpack_and_save_model(
            spark_model,
...
def _save_model_metadata(
    ...
    _save_jars_and_lic(dst_dir, store_license=store_license)


def _save_jars_and_lic(dst_dir, store_license=True):
...
def _unpack_and_save_model(spark_model, dst):
    ...
    try:
        ...
        print(f"Could save your model, use a different path for saving or remove {dst}. \n"
