        ...
        if self.sample_weight is not None:
            if self.pipe.steps[-1][0]=='XGB':
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(X=self.X_train, y_train=self.y_train, X_test=self.X_test, y_test=self.X_test, XGB__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags,rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train, X_test=self.X_test, y_test=self.X_test, LGBM__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags,rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train,  X_test=self.X_test, y_test=self.X_test, RF__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train, X_test=self.X_test, y_test=self.X_test, KNN__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train, X_test=self.X_test, y_test=self.X_test, LR__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train,  X_test=self.X_test, y_test=self.X_test, MLP__sample_weight=self.sample_weights)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train,  X_test=self.X_test, y_test=self.X_test, DT__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                    self._model.fit(self.X_train, self.y_train, DT__sample_weight=self.sample_weights)
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train,  X_test=self.X_test, y_test=self.X_test, SVR__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        self._model.fit(self.X_train, self.y_train, LF__sample_weight=self.sample_weights, X_prev = X_prev,columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags,rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                    else:
                        self._model.fit(self.X_train, self.y_train, X_test=self.X_test, y_test=self.X_test, LF__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        self._model.fit(self.X_train, self.y_train, LB__sample_weight=self.sample_weights, X_prev = X_prev,columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags,rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                    else:
                        self._model.fit(self.X_train, self.y_train, X_test=self.X_test, y_test=self.X_test, LB__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
            ...
            if self.params["Search"]=="Forecast":
                if self.y_test is None and self.X_test is None:
                    ...
                    self._model.fit(X=self.X_train, y=self.y_train, X_test=self.X_test, y_test=self.X_test, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
            else:
...
class Classification:
    ...
    def Voting_Classifier(self, models:list, ordinal_cat_cols=None, scoring='accuracy',
        ...
        if grid_params is not None and random_params is None and bayes_pbounds is None:
            assert all('Voting__'  in s for s in grid_params.keys() ), "Grid parameters must start with 'Voting__modelname__'"
        elif random_params is not None:
            assert all('Voting__' in s for s in random_params.keys()), "Random parameters must start with 'Voting__modelname__'"
        elif bayes_pbounds is not None:
            assert all('Voting__' in s for s in bayes_pbounds.keys() ), "Bayesian parameter bounds must start with 'Voting__modelname__'"
            assert all('Voting__' in s for s in bayes_int_params ), "Bayesian integer parameters must start with 'Voting__modelname__'"
...
class Regression:
    ...
    def RandomForest_Regressor(self,  ordinal_cat_cols=None, categories='auto', scoring='neg_mean_squared_error', criterion="friedman_mse",
            ...
            #Select Grid Search, Random Search or Bayesian Optimization
            select_searchcv=SelectSearchCV(self.X_train, self.y_train,self.X_test,self.y_test, pipe, scoring, sample_weight=sample_weight,cv=self.cv,n_jobs=n_jobs)
            select_searchcv.select_automatic(grid_params, random_params, random_n_iter, bayes_pbounds, bayes_int_params, bayes_n_iter)
    ...
    def LightGBM_Regressor(self, ordinal_cat_cols=None, categories='auto', scoring='neg_mean_squared_error',
            ...
            #Select Grid Search, Random Search or Bayesian Optimization
            select_searchcv=SelectSearchCV(self.X_train, self.y_train,self.X_test,self.y_test, self.X_test, self.y_test, pipe, scoring, sample_weight=sample_weight,cv=self.cv,n_jobs=n_jobs)
            select_searchcv.select_automatic(grid_params, random_params, random_n_iter, bayes_pbounds, bayes_int_params, bayes_n_iter)
            self.model=select_searchcv.fit(X_prev=self.X_prev, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags,rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
