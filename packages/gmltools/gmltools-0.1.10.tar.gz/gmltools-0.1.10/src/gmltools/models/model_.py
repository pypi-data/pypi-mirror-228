        ...
        if self.sample_weight is not None:
            if self.pipe.steps[-1][0]=='XGB':
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train, LGBM__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags,rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train, RF__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train,  KNN__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train,  LR__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train,  DT__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                    self._model.fit(self.X_train, self.y_train, DT__sample_weight=self.sample_weights)
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train,  LF__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
                else:
                ...
                if self.params["Search"]=="Forecast":
                    if self.y_test is None and self.X_test is None:
                        ...
                        self._model.fit(self.X_train, self.y_train, LB__sample_weight=self.sample_weights, X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
            ...
            if self.params["Search"]=="Forecast":
                if self.y_test is None and self.X_test is None:
                    ...
                    self._model.fit(X=self.X_train, y=self.y_train,  X_prev = X_prev, X_test=self.X_test, y_test=self.y_test, columns_lags=columns_lags, column_rolled_lags=column_rolled_lags,lags=lags, rolled_lags=rolled_lags, rolled_metrics=rolled_metrics, column_rolled_lags_initial=column_rolled_lags_initial)
            else:
