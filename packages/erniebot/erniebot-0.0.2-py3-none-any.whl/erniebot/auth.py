...
def build_auth_provider(config_dict: Dict[str, Any],
                        api_type: Union[str, APIType]) -> 'AuthProvider':
    if isinstance(api_type, str):
        api_type = convert_str_to_api_type(api_type)
    if api_type in (APIType.QIANFAN, APIType.YINIAN):
        return BCEAuthProvider(api_type, **config_dict)
    else:
...
class AuthProvider(object):
    ...
    def _get_cache_key(self) -> Tuple[Optional[str], Optional[str]]:
        raise NotImplementedError
    ...
    def _update_cache(self) -> str:
        token = _GlobalAuthCache().add_or_update_entry(
            self.api_type.name, self._cache_key, self._request_access_token)
        if token is None:
...
class BCEAuthProvider(AuthProvider):
    def __init__(self,
                 api_type: APIType,
    ...
    def _request_access_token(self) -> str:
        ...
        ak = self._cfg_dict['ak'] or ''
        sk = self._cfg_dict['sk'] or ''
        params = {
    ...
    def _get_cache_key(self) -> Tuple[Optional[str], Optional[str]]:
        return (self._cfg_dict['ak'], self._cfg_dict['sk'])
