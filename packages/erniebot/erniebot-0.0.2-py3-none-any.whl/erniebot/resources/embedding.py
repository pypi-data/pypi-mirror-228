...
class Embedding(EBResource, Creatable):
    ...
    _PREFIX: ClassVar[str] = 'embeddings'
    _MODEL_INFO_DICT: ClassVar[Dict[APIType, Dict[str, Any]]] = {
        APIType.QIANFAN: {
    ...
    def _prepare_create(self,
        ...
        # url
        if self.api_type in self._MODEL_INFO_DICT:
            model_info_dict = self._MODEL_INFO_DICT[self.api_type]
            if model not in model_info_dict:
                raise errors.InvalidArgumentError(
                    f"{repr(model)} is not a supported model.")
            url = f"/{self._PREFIX}/{model_info_dict[model]['suffix']}"
        else:
