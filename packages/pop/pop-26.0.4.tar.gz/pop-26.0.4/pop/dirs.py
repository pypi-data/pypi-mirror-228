...
class UniqueList(list):
    ...
    def __sub__(self, other):
        # This isn't normally defined on lists.
        new_list = UniqueList(self)
        for item in other:
            if item in new_list:
...
class UniqueDirList(UniqueList):
    def insert(self, index, item):
        ...
        if "DYNE" in context:
            ...
            for name, paths in context["DYNE"].items():
                ...
                if "CONFIG" in context:
                    dict_tools.update.update(ret[name]["CONFIG"], context["CONFIG"])
                if "CLI_CONFIG" in context:
                    dict_tools.update.update(
                        ret[name]["CLI_CONFIG"], context["CLI_CONFIG"]
                    )
                if "SUBCOMMANDS" in context:
                    dict_tools.update.update(
                        ret[name]["SUBCOMMANDS"], context["SUBCOMMANDS"]
                    )
                for path in paths:
