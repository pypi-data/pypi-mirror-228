...
class Hub:
    ...
    def __init__(self):
        ...
        # NOT be app-merged
        pypaths = ["pop.mods.pop"]
        if HAS_TESTING_LIBS:
            pypaths.append("pytest_pop.mods")
        self._subs["pop"] = Sub(self, "pop", pypath=pypaths)
        self._reverse_sub = None
        self._iter_ind = 0
        # Set up the conf OPT structure so it is always available
        self.OPT = self.pop.data.null_map()
        self._imports = {}
    ...
    def __setstate__(self, state: Dict):
        ...
        subs = state["subs"]
        opt = state["OPT"]
        reverse_sub = state["reverse_sub"]

        self._imports = {
            subname: importlib.import_module(modname)
            for subname, modname in state["imports"].items()
        }
        self.OPT = self.pop.data.imap(opt)
        self._restore_subs(subs, self)
        if reverse_sub:
            self.pop.sub.dynamic(
                sub=self,
                resolver=reverse_sub._resolver,
                context=reverse_sub._context,
                **reverse_sub._restoreable_state,
            )
        self._cache_reset()
    ...
    def _restore_subs(self, subs: Dict[str, "Sub"], root):
        ...
        for name, sub in subs.items():
            if sub._reverse_sub:
                self.pop.sub.dynamic(
                    resolver=sub._reverse_sub._resolver,
                    context=sub._reverse_sub._context,
                    sub=root,
                    **sub._reverse_sub._restoreable_state,
                )
            else:
    ...
    def __iter__(self) -> Iterator["Sub"]:
        def iterator(subs: Dict[str, Sub]):
            for sub in self._iter_subs:
                yield subs[sub]

        return iterator(self._subs)
    ...
    def _cache_attr(self, item, obj):
        setattr(self, item, obj)
        self._cached.add(item)

    def _cache_reset(self):
        # because we're not tracking aliases, removing anything from the sub
        # requires a flush of the cache.
        for item in getattr(self, "_cached", set()):
            delattr(self, item)
        self._cached = set()
    ...
    def __getattr__(self, item: str):
        ...
        if item in self._subs:
            self._cache_attr(item, self._subs[item])
            return self._subs[item]
        elif item in self._sub_alias:
            resolved = self._sub_alias[item]
            if resolved in self._subs:
                self._cache_attr(item, self._subs[resolved])
                return self._subs[resolved]
        elif item in self._imports:
            self._cache_attr(item, self._imports[item])
...
class Sub:
    ...
    def _prepare(self):
        ...
        if set(self._default_recursive_contracts) != set(
            getattr(self._root, "_default_recursive_contracts", [])
        ):
            # This feature is unused and untested as far as I can tell, but
            #  my fix for accidental double contracts would break this (see
            #  https://gitlab.com/vmware/pop/pop/-/merge_requests/173).
            for c in getattr(self._root, "_default_recursive_contracts", []):
                if c not in self._default_recursive_contracts:
                    self._default_recursive_contracts.append(c)

        self._recursive_contract_subs = []
        if self._recursive_contract_dirs:
            self._recursive_contract_dirs += getattr(
                self._root, "_recursive_contract_dirs", pop.dirs.UniqueDirList()
            )
            # child Subs look at the parent's _recursive_contract_dirs, so we can't modify it
            filtered_recursive_contract_dirs = (
                self._recursive_contract_dirs - self._contract_dirs
            )
            for i in range(len(filtered_recursive_contract_dirs)):
                self._recursive_contract_subs.append(
                    Sub(
                        hub=self._hub,
            ...
            self._recursive_contract_subs = getattr(
                self._root, "_recursive_contract_subs", pop.dirs.UniqueDirList()
            )
    ...
    def __getstate__(self):
        return dict(
            ...
            _subs=self._subs,
            _reverse_sub=self._reverse_sub,
            imports={subname: mod.__name__ for subname, mod in self._imports.items()},
        )
    ...
    def __setstate__(self, state: Dict):
        ...
        self._reverse_sub = state.pop("_reverse_sub")
    ...
    def _cache_reset(self):
        # because we're not tracking aliases, removing anything from the sub
        # requires a flush of the cache. Also, adding anything *may* have actually
        # overridden something, so reset the cache then, too.
        # This could be smarter, but loading on the hub is usually rare.
        for item in getattr(self, "_cached", set()):
            delattr(self, item)
        self._cached = set()
    ...
    def __getattr__(self, item: str):
        ...
        if item.startswith("_"):
            ...
            # Get the attribute from one sub higher up until the hub calls "ref.last"
            return getattr(self._root, f"{self._subname}.{item}")
        elif item in self._loaded:
            ret = self._loaded[item]
            if not isinstance(ret, pop.loader.LoadError):
                self._cache_attr(item, ret)
            ...
            resolved = self._sub_alias[item]
            if resolved in self._subs:
                self._cache_attr(item, resolved)
                return self._subs[resolved]
        elif item in self._imports:
            self._cache_attr(item, self._imports[item])
            return self._imports[item]
        ...
        if mod is None and self._reverse_sub:
            # This sub contains a reverse sub that could contain the result
            return self._reverse_sub[item]
    ...
    def __contains__(self, item: str):
        try:
            ret = getattr(self, item, None)
            if isinstance(ret, ReverseSub):
                return item in self._reverse_sub
    ...
    def __iter__(self) -> Iterator["Sub"]:
        ...
        def iterate(loaded):
            for mod_name in sorted(loaded.keys()):
                yield loaded[mod_name]

        return iterate(self._loaded)
    ...
    def _load_item(self, iface: str, bname: str):
        ...
        if iface not in self._scan:
            raise pop.exc.PopLoadError(f"Bad call to load item, no iface {iface}")
        if bname not in self._scan[iface]:
            raise pop.exc.PopLoadError(
                f"Bad call to load item, no bname {bname} in iface {iface}"
            )
    ...
    def _prep_mod(self, mod: ModuleType, iface: str, bname: str):
        ...
        if "__verify_order__" in dir(mod):
            self.__verify_order__ = pop.contract.Contracted(
                self._hub,
                contracts=[],
                func=mod.__verify_order__,
                ref=f"{self._subname}.{name}",
                name="__verify_order__",
            )
...
class ReverseSub(Sub):
    """
    A sub that discovers attributes available to it dynamically through a resolver
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param kwargs: Any kwargs that should be passed to the underlying sub
        """
        Sub.__init__(self, subname=subname, hub=hub, **kwargs)
        self._resolver = resolver
        self._context = context
        # self._loaded_all = True
        if refs is None:
            self._refs = tuple([*subname.split(".")])
        else:
            self._refs = tuple([*refs, *subname.split(".")])

        # Imitate the name attribute like a callable should
        self.__name__ = self._subname

    def __getstate__(self):
        sub = Sub.__getstate__(self)
        # These are all the args required to recreate the ReverseSub with hub.pop.sub.dynamic()
        sub["resolver"] = self._resolver
        sub["context"] = self._context
        # These are for when the reverse sub is wrapped directly
        sub["refs"] = self._refs
        sub["hub"] = self._hub
        return sub
    ...
    def __setstate__(self, state: Dict):
        """
        Sometimes a reverse sub will be called by proxy and can be restored by a hub.
        Sometimes it is called directly and the ReverseSub needs to have everything inside it.
        """
        self.__init__(
            hub=state.pop("hub"),
            subname=state["subname"],
            resolver=state.pop("resolver"),
            context=state.pop("context"),
            refs=state.pop("refs"),
        )
        Sub.__setstate__(self, state)

    def __getattr__(self, item: str):
        """
        Build a reference based on the namespace attrs until a reverse sub is called
        """
        if item.startswith("_"):
            return self.__getattribute__(item)
        ...
        return ReverseSub(
            ...
            static=self._static,
            contracts_pypath=self._contracts_pypath,
            contracts_static=self._recursive_contracts_static,
            default_contracts=self._default_contracts,
            virtual=self._virtual,
            dyne_name=self._dyne_name,
            omit_start=self._omit_start,
    ...
    def __next__(self):
        # Override the parent Sub's iterator
        raise StopIteration

    def __contains__(self, item):
        """
        Return true if the item's reference underneath this one is resolvable
        """
        try:
            if getattr(self, item)._resolve():
                return True
        except Exception:
            ...
        return False

    def _resolve(self) -> Callable or None:
        """
        Call the resolver and get back the external call represented by this sub's reference
        """
        ref = ".".join(self._refs)
        ret = self._resolver(ref, self._context)
        return ret
    ...
    def __call__(self, *args, **kwargs):
        ...
        if not func:
            ref = ".".join(self._refs)
            raise AttributeError(f"ReverseSub could not resolve the reference {ref}")
        if isinstance(func, pop.contract.Contracted):
            contracted_function = func
        else:
            contracted_function = pop.contract.create_contracted(
                hub=self._hub,
                contracts=[],
                func=func,
                ref=".".join(self._refs),
        ...
        # if the resolved func was a Contracted, it is located at 2 places on the hub.
        # so we don't want to permanently add recursive contracts, for when it is called
        # from the other location.

        # I haven't added these to a cache or optimized code execution because in general,
        # recursive contracts are used for API calls.
        contracts = [lmod for sub in self._contract_subs for lmod in sub]
        contracts.extend(
            [lmod for sub in self._recursive_contract_subs for lmod in sub]
        )
        regular_sub_contracts = []
        for contract in contracted_function.contracts:
            if contract not in contracts:
                regular_sub_contracts.append(contract)
        contracts = pop.contract._order(regular_sub_contracts + contracts)

        new_contracted_function = pop.contract.create_contracted(
            self._hub,
            contracts=contracts,
            func=contracted_function.func,
            ref=".".join(self._refs),
            name=contracted_function.__name__,
            implicit_hub=contracted_function.implicit_hub,
        )

        return new_contracted_function(*args, **kwargs)
