...
class Phrase(Model):
    txt: str = fields.CharField(4095, unique=True)  # todo remove unique
...
class Msg(Model):
    ...
    chat: fields.ForeignKeyRelation[Chat] = fields.ForeignKeyField("models.Chat", 'msgs')
    business: fields.ForeignKeyRelation[Business] = fields.ForeignKeyField('models.Business', 'msgs')
    business_id: int
    removed_at: datetime = DatetimeSecField(null=True)
    tgstat_id: int = fields.BigIntField(unique=True)
    created_at: datetime = DatetimeSecField(auto_now_add=True)
