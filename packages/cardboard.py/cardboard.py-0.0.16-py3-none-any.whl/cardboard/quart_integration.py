...
class QuartIntegration:
    ...
    """
    def __init__(self, app:Quart, cardboard:Cardboard|CardboardAsync, session_prefix:str="cardboard_", login_url:str=None, redirect_template:str=None):
        self.app:Quart = app
    ...
    def autologin(self, route_function):
        ...
        async def decorator(*args, **kwargs):
            ...
            if type(cache) == dict and round(time.time()) < cache.get("time")+60:
                result =  await route_function(self._constructAuthToken(cache.get("token"), session.get(self.session_expiry), session.get(self.session_refresh)), *args, **kwargs)
            ...
            if thetoken and concurrent.futures.ThreadPoolExecutor().submit(lambda: self.cbsync.check_token(thetoken)).result():
                ...
                if session.get(self.session_expiry) and session.get(self.session_refresh):
                    ...
                    try:
                        token = await self.cb.refresh_token(session.get(self.session_refresh))
                    except:
                        if self.redirect_template:
                            return await render_template(self.redirect_template, redirect=self.app_login_redirect)
                        return redirect(self.app_login_redirect)
                result = await route_function(token, *args, **kwargs)
                session[self.session_cache] = {"time": round(time.time()), "token": token.token}
                if session.get(self.session_rd):
                    a = session.pop(self.session_rd)
                    if self.redirect_template:
                        return await render_template(self.redirect_template, redirect=a)
            ...
            if code:
                ...
                response = await self.asyncpost(f"{self.cb._baseurl}/token", data=data, headers={"Content-Type": "application/x-www-form-urlencoded"})
    ...
    def optional_logged_in(self, route_function):
        ...
        async def decorator(*args, **kwargs):
            ...
            cache = session.get(self.session_cache)
            if type(cache) == dict and round(time.time()) < cache.get("time")+60:
                return await route_function(self._constructAuthToken(cache.get("token"), session.get(self.session_expiry), session.get(self.session_refresh)), *args, **kwargs)
            else:
                session[self.session_cache] = None
            if thetoken and concurrent.futures.ThreadPoolExecutor().submit(lambda: self.cbsync.check_token(thetoken)).result():
