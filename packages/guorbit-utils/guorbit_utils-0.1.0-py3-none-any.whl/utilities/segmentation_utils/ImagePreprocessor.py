...
class PreprocessingQueue:
    ...
    Parameters
    ----------
    :queue list[PreFunction]: list of PreFunctions to be applied
...
def generate_image_queue(seed: int = 0) -> PreprocessingQueue:
    """
...
def generate_mask_queue(seed: int = 0) -> PreprocessingQueue:
    """
...
def generate_default_queue(
    seed: int = 0,
) -> tuple[PreprocessingQueue, PreprocessingQueue]:
    """
...
def onehot_encode(masks, num_classes: int) -> tf.Tensor:
    """
...
def flatten(image:tf.Tensor, input_size: tuple[int, int], channels: int = 1) -> tf.Tensor:
    """flatten
...
def random_flip_up_down(image:tf.Tensor, seed: int = 0) -> tf.Tensor:
    """
...
def random_flip_left_right(image:tf.Tensor, seed: int = 0) -> tf.Tensor:
    """
