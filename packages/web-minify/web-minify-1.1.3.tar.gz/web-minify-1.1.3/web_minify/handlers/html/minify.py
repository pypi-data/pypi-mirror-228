...
def add_timestamp(html):
    uid = "web-minify-timestamp"
    return f"{html}\n<!-- {uid}{format(datetime.now().isoformat()[:-7].lower())} -->\n"


def html_minify(html, settings: dict):
    """Minify HTML main function.

    >>> html_minify(' <p  width="9" height="5"  > <!-- a --> b </p> c <br> ')
    '<p width=9 height=5 > b c <br>'
    """
    comments = settings.get("comments", False)
    ...
    # html = add_timestamp(html) if settings.get("timestamp", False) else html
    return True, html.strip() + "\n", None
