...
def remove_unnecessary_whitespace(css):
    ...
    def pseudoclasscolon(css):
        ...
        while match:
            css = "".join([css[: match.start()], match.group().replace(":", "___PSEUDOCLASSCOLON___"), css[match.end() :]])
            match = regex.search(css)
        return css
...
def condense_zero_units(css):
    ...
    return re.sub(r"([\s:])(0)(px|em|%|in|q|ch|cm|mm|pc|pt|ex|rem|s|ms|" r"deg|grad|rad|turn|vw|vh|vmin|vmax|fr)", r"\1\2", css)


def condense_multidimensional_zeros(css):
    """Replace `:0 0 0 0;`, `:0 0 0;` etc. with `:0;`."""
    return css.replace(":0 0 0 0;", ":0;").replace(":0 0 0;", ":0;").replace(":0 0;", ":0;").replace("background-position:0;", "background-position:0 0;").replace("transform-origin:0;", "transform-origin:0 0;")
...
def condense_hex_colors(css):
    ...
    regex = re.compile(r"""([^\"'=\s])(\s*)#([0-9a-f])([0-9a-f])([0-9a-f])""" r"""([0-9a-f])([0-9a-f])([0-9a-f])""", re.I | re.S)
    match = regex.search(css)
    while match:
...
def condense_std_named_colors(css):
    ...
    for color_name, color_hexa in iter(tuple({":aqua;": ":#0ff;", ":blue;": ":#00f;", ":fuchsia;": ":#f0f;", ":yellow;": ":#ff0;"}.items())):
...
def restore_needed_space(css):
    ...
    return css.replace("!important", " !important").replace("@media(", "@media (").replace("data:image/jpeg;base64,", "data:image/jpg;base64,").rstrip("\n;")  # !important  # media queries # jpeg > jpg
...
def add_timestamp(css):
    return f"{css}\n/* {format(datetime.datetime.now().isoformat()[:-7].lower())} */\n"


def css_minify(css, settings: dict):
    """Minify CSS main function."""
    css = remove_comments(css) if not settings.get("comments", False) else css
    css = sort_properties(css) if settings.get("sort") else css
    ...
    css = wrap_css_lines(css, 80) if settings.get("wrap", False) else css
    css = condense_semicolons(css)
    css = add_encoding(css) if settings.get("prefix", False) else css
    css = restore_needed_space(css)
    css = add_timestamp(css) if settings.get("timestamp", False) else css
    return css.strip(), None
