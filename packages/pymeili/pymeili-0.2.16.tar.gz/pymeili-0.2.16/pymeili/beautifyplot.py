...
try:
    ...
    def contourf(x, y, z, levels=10, cmap='28', contour=True, clabel=True,linewidths=1.5, color='fg', vmin='auto', vmax='auto', fontsize=fontsize.clabel, **kwargs):
        ...
        if clabel == True:
            ...
            for t in CL:
                t.set_fontproperties(fm.FontProperties(fname=fontpath, size=fontsize.sublabel))
    ...
    def hidespines(top=True, right=True, bottom=True, left=True):
        if top: plt.gca().spines['top'].set_color('none')
        if right: plt.gca().spines['right'].set_color('none')
        if bottom: plt.gca().spines['bottom'].set_color('none')
        if left: plt.gca().spines['left'].set_color('none')
    ...
    class initsubplots():
        ...
        # 其他
        def grid(self, b=None, which='major', axis='both', color='fg', linestyle=':', linewidth=0.5, **kwargs):
            if self.ndim == 1:
