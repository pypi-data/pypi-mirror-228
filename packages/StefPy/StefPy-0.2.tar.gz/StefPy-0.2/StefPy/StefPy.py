...
def Stef_Gmail(email):
    ...
    if response.json()['status'] == 'SUCCESS':
        return {'Status':'Available','Stef':'@Stef_Python'}
    else:
        return {'Status':'UnAvailable','Stef':'@Stef_Python'}
def Stef_Yahoo(email):
...
def CheckTik(email):
    ...
    if 'Sent successfully' in res:
        return {'Status':'OK','By':'@Stef_Python'}
    else :
        return {'Status':'FALSE','By':'@Stef_Python'}
def CheckInsta(email):
    ...
    data2 = {
        'uuid':ud,
        'password':'StefTools#Tele=@Stef_Python',
        'username':email,
        'device_id':ud,
        'from_reg':'false',
        '_csrftoken':'missing',
...
def InfoTik(user):
    try:
        ...
        return {'Result':'true','user':'true','name':i2,'id':i1,'bio':i3,'code-country':i4,'private':i5,'followers':i6,'following':i7,'likes':i8,'video':i9,'By':'@Stef_Python'}
    except IndexError :
        return {'result':'false','user':'false','By':'@Stef_Python'}
def InfoInsta(user):
    try:
        ...
        return {'result':'false','user':'false','Tele':'@Stef_Python'}
    ...
    try:
        ...
        return {'Result':'true','user':'true','name':f1,'id':f2,'followers':f3,'following':f4,'posts':f5,'date':f6,'reset':f7,'Tele':'@Stef_Python'}
    except KeyError:
        return {'Result':'true','user':'true','name':f1,'id':f2,'followers':f3,'following':f4,'posts':f5,'date':f6,'reset':'false','Tele':'@Stef_Python'}
lii = 0
pp = 0
def ListTik(user):
    global lii,pp
    ...
    for gt in rt:
    	try:
    		...
    		return{'status':'ok','users':rt,'By':'@Stef_Python'}
    	except IndexError:
    		exit()
def ListTik_Shear():
    ...
    for i in response['results']:
    	username = i['username']
    	return {'status':'ok','users':username}
