...
from .stream import KelvinStream, KelvinStreamConfig
...
def build_nested_map(msg: Message) -> dict[str, Any]:
    ...
    n = msg.resource.ns_string.split(".")
    ret = {}
    ref = ret
...
class KelvinClient:
    ...
    def __init__(self, config: KelvinStreamConfig = KelvinStreamConfig()) -> None:
        ...
        self.on_configuration: MessageCallbackType = None
    ...
    def _build_parameter_map(self, asset: bool, msg: Message):
    ...
    async def _process_message(self, msg: Message) -> None:
        ...
        if filters.is_configuration(msg):
            # todo: store configuration
            if self.on_configuration:
                await self.on_configuration(msg)
            return

        if filters.is_parameter(msg):
            # determine if it's an asset or app parameter and store it
            res = msg.resource
            asset = False if "param" in res.ns_id else True
            self._build_parameter_map(asset, msg)
            if asset:
                # self._asset_parameters[msg.resource][res.ns_string] = msg
                if self.on_asset_parameter:
                    await self.on_asset_parameter(msg)
                return
            else:
                # self._app_parameters[msg.resource][res.ns_string] = msg
    ...
    async def publish_message(self, msg: Message) -> bool:
        ...
        """
        try:
            return await self._stream.write(msg)
        except ConnectionError:
