...
class EmailsProcessor:
    ...
    def get_processed_json_email_body(json_string):
        ...
        if json_string.count('"') > 8:
            ...
            json_string = before_body_key + '"body": "' + body_value + '"' + after_body_value
        return json_string
    ...
    def get_email_without_name_signature(email) -> str:
        ...
        if bool(re.search(compiled_pattern, email)):
            ...
            end_of_email = " ".join(email.split("<br/>")[-4:])
            signature = NER(end_of_email)
            entities_to_remove = "".join([str(entity) for entity in signature.ents if entity.label_ in ["PERSON", "ORG"]])
            if entities_to_remove:
                for entity in entities_to_remove.split(" "):
                    email = "".join(email.rsplit(entity, 1))
        return email.replace("<br/><br/><br/>", "<br/><br/>")
