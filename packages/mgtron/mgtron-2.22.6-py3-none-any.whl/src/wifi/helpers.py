...
from src.wifi.scanning import convert_signal_to_rssi
...
def wifi_scan_jam(sender) -> None:
    ...
    loggei.info(
        f"bluetooth item value = {dpg.get_item_theme(item='mssn_bluetooth_scan')}"
    )
...
def delete_scan_aliases(alias_file="wifi_scan_aliases.yml") -> None:
    """Delete the wifi scan aliases for a clean rescan."""
    loggei.info("%s()", delete_scan_aliases.__name__)

    try:
        # Read the contents of the YAML file
        with open(alias_file, "r", encoding="utf-8") as read_file:
            alias_data = yaml.load(stream=read_file, Loader=yaml.FullLoader)

            _ = [dpg.delete_item(item=i) for i in alias_data]
            loggei.debug(msg="WiFi scan aliases deleted")

    except (yaml.parser.ParserError, FileNotFoundError):
        loggei.error(
            "%s() failed to read alias file", delete_scan_aliases.__name__
        )
    ...
    try:
        ...
        loggei.error(
            "%s() failed to delete alias file", delete_scan_aliases.__name__
        )
...
def scan_window() -> None:
    ...
    with dpg.window(
        ...
        _ = [loggei.info("convert scan result: %s", i) for i in new_data]
    ...
    with dpg.window(
        ...
        headers = ["SSID", "MAC", "CH", "FREQ", "RSSI", "SECURITY"]
...
def fill_scan_result(
    ...
    for i in converted_data:
        dpg.delete_item(item=i[1])  # Delete the alias
    ...
    for i in converted_data:
        ...
        try:
            dpg.add_button(
                tag=i[1],  # Causes issue on re-scan unless item deleted
                parent=129,
...
def sort_results(sender, app_data, user_data):
    ...
    match sender:
        ...
        case "SECURITY":
            scan_data = sorted(
                scan_data, key=lambda x: x.get("security"), reverse=order_query
            )
        case _:
...
def return_indicated_results() -> dict[str, dict[str, str]]:
    ...
    [
        loggei.warning("scan result Aliases selected: %s", i)
        for i in selected_items_dict
    ]
    ...
    while tracker != count:
        if tracker >= 1:
            ...
            chase_freqs: list[float] = [
                i.get("frequency") for i in user_data.values()
            ]
...
def wifi_kill_all() -> None:
    ...
    threaded_wifi_scan(
        wifi_action=send_to_n_cards, vital_data=freq_and_strength
    )
...
def send_to_n_cards(freq_and_strength: dict[int, float]) -> None:
    ...
    _ = [
        (
            ...
            loggei.warning(
                "Port: %s", port[0] if isinstance(port, list) else port
            ),
            # device_refresh() if i == 1 else None,
            # Since the list is immediately popped, put it back at the end
            port.append(first_port_popped)
            if first_port_popped not in port
            else None,
        )
...
def threaded_wifi_scan(
    ...
    with dpg.window(
        ...
        dpg.add_text(
            tag="scan_text", default_value=text_to_display, pos=(300, 265)
        )
