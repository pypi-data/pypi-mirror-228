...
def start_up_card_state() -> None:
    ...
    for i in range(1, 9):
        ...
        if int(dpg.get_value(item=f"power_{i}")) != int(
            card_stats["power"][i - 1]
        ):
            dpg.set_value(
                item=f"power_{i}_indicator",
        ...
        if int(dpg.get_value(item=f"freq_{i}")) != int(
            card_stats["freq"][i - 1]
        ):
            dpg.set_value(
        ...
        if int(dpg.get_value(item=f"bandwidth_{i}")) != int(
            card_stats["bw"][i - 1]
        ):
            dpg.set_value(
                item=f"bandwidth_{i}_indicator",
...
def get_device_path_from_serial_number() -> str:
    ...
    try:
        serial_number: str = str(dpg.get_value(item="device_indicator")).split(
            sep=":"
        )[1]
    except IndexError:
        loggey.error(
...
def callstack_helper(
    ...
    loggey.info(f"values: {values}")
    last_values = values[-8:]
    loggey.info(f"last_values: {last_values}")
    # Nested for loop to check if the exit command is in the last 8 values
    for i in last_values:
        loggey.debug(f"i in first for loop: {i}")
        for i in i:
            loggey.debug(f"i in second for loop: {i}")
            if i == "exit":
                loggey.debug("Condition triggered")
                loggey.debug("Function will now return")
                no_power()
                return
    dpg.bind_item_theme(
...
def auto_fill_freq(
    ...
    _ = [
        dpg.set_value(
            item=f"freq_{i}", value=freq_val + freq_constant * (i - 1)
        )
        for i in range(1, 9)
        if freq_constant
    ]
...
def device_finder(sender=None, app_data=None, user_data: int = int()) -> None:
    ...
    desired_device = [device for device in device if "Teensy" in device[0]]
    loggey.info(
        msg=f"Devices filtered check: {desired_device} |"
    ...
    for _i, dev in enumerate(teensy_info, start=1):
        ...
        if user_data == dev:
            ...
            _ = (
                dpg.bind_item_theme(
                    item=f"card_{_i}", theme=grn_btn_theme
                ),
                [
                    dpg.bind_item_theme(
                        item=f"card_{_j}",
                ...
                for _, card in enumerate(devices):
                    match card.split(sep="_")[-1] == parser["mgtron"][
                        ...
                        case True if len(card) > 1:
                            ...
                            loggey.debug(
                                "%s is assigned to %s",
                                card, parser['mgtron'][f'card_{dev_count}']
                            )
    ...
    match user_data:
        case 1:
            ...
            try:
                ...
                loggey.error("%s", err)
                dpg.bind_item_theme(
                    item=f"card_{user_data}", theme=red_btn_theme
                )
                return
...
def get_card_status() -> dict[str, list]:
    ...
    try:
        ...
        # Log the line number
        loggey.error(
            msg=f"Line number: {sys.exc_info()[-1].tb_lineno} - {err}"
        )
...
def no_power():
    ...
    current_power = [
        str(dpg.get_value(item=f"power_{i}")) for i in range(1, 9)
    ]
    ...
    for i in range(1, 9):
        _ = dpg.bind_item_theme(
            item=f"stats_{i}",
            theme=grey_btn_theme,
        ) if current_power[i - 1].isdigit() and current_power[
            i - 1
        ] == "0" else None
...
def numpad(sender):
    ...
    if dpg.is_item_focused(item="freq_1") and dpg.is_item_hovered(
        item="freq_1"
    ):
        # Open the numpad
        with dpg.window(
...
def launch_api_server(path: pathlib.Path) -> None:
    """Launch the rust WiFi server."""
    loggey.debug(msg=f"{launch_api_server.__name__}()")
    loggey.info(msg=f"Path: {path}")

    cmd = str(path)

    # Launch the WiFi server
    pid = os.system(cmd)  # returns the exit code in unix
    # print(f"\nExit code: {pid}\n")

    if pid != 0:
        loggey.warning(msg=f"API server is likely running: {pid}")

    loggey.info(msg="API server launched")


# BlueIO API
start_server = threading.Thread(
    target=launch_api_server, args=(f"{ROOT}/src/assets/blueio_rs", ),
)
...
# WiFi API
start_server = threading.Thread(
    target=launch_api_server, args=(f"{ROOT}/src/assets/wifi_rs", ),
)
start_server.daemon = True
start_server.start()
...
# MGTRON API
start_server = threading.Thread(
    target=launch_api_server, args=(f"{ROOT}/src/assets/api_rs", ),
)
start_server.daemon = True
start_server.start()
...
# GPS API
start_server = threading.Thread(
    target=launch_api_server, args=(f"{ROOT}/venv/bin/gps", ),
)
start_server.daemon = True
start_server.start()
