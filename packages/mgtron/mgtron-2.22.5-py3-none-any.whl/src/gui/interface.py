...
def post_request_structure(endpoint: str, body: str):
    ...
    logger.info("The body info: %s", body)
...
def output_post(endpoint, body):
    ...
    logger.debug("Content length: %s,", len(json.dumps(body)))
    return output
...
class Megatron:
    ...
    def change_freq(
        ...
        post_request_structure(
            endpoint="freq",
            body={"port": teensy_port, "channel": channel, "value": freq}
        )
    ...
    def change_bandwidth(
        ...
        post_request_structure(
            endpoint="bw",
            body={"port": teensy_port, "channel": channel, "value": percentage}
        )
