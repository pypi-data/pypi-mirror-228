...
def wifi_save(
    ...
    for _, data in enumerate(scan_data):
...
def wifi_load(
    ...
    """Load the WiFi data stored in the database."""
    ...
    _ = [
        loggey.warning(
            "received Scan data: %s", j
        ) for i in wifi_data for j in i.items()
    ]
...
def main():
    ...
    # Print the data
    print(f"{F.MAGENTA}Wifi scan data 4{R}: {wifi_scan_data_4}")
