...
class main_menu(TUIMenu):
    ...
    def __init__(self, path, service):
        ...
        self.cad_assemblies = self.__class__.cad_assemblies(path + ["cad_assemblies"], service)
        ...
        self.boundary = self.__class__.boundary(path + ["boundary"], service)
        self.diagnostics = self.__class__.diagnostics(path + ["diagnostics"], service)
        self.parallel = self.__class__.parallel(path + ["parallel"], service)
        self.file = self.__class__.file(path + ["file"], service)
        self.material_point = self.__class__.material_point(path + ["material_point"], service)
        self.display = self.__class__.display(path + ["display"], service)
        self.mesh = self.__class__.mesh(path + ["mesh"], service)
        self.size_functions = self.__class__.size_functions(path + ["size_functions"], service)
        self.preferences = self.__class__.preferences(path + ["preferences"], service)
        self.openmp_controls = self.__class__.openmp_controls(path + ["openmp_controls"], service)
        self.scoped_sizing = self.__class__.scoped_sizing(path + ["scoped_sizing"], service)
    ...
    class cad_assemblies(TUIMenu):
        ...
        def __init__(self, path, service):
            ...
            self.draw_options = self.__class__.draw_options(path + ["draw_options"], service)
            self.manage_state = self.__class__.manage_state(path + ["manage_state"], service)
            self.labels = self.__class__.labels(path + ["labels"], service)
            super().__init__(path, service)
    ...
    class objects(TUIMenu):
        ...
        def __init__(self, path, service):
            self.path = path
            self.service = service
            self.join_intersect = self.__class__.join_intersect(path + ["join_intersect"], service)
            self.fix_holes = self.__class__.fix_holes(path + ["fix_holes"], service)
            self.remove_gaps = self.__class__.remove_gaps(path + ["remove_gaps"], service)
            self.volumetric_regions = self.__class__.volumetric_regions(path + ["volumetric_regions"], service)
            self.cad_association = self.__class__.cad_association(path + ["cad_association"], service)
            self.wrap = self.__class__.wrap(path + ["wrap"], service)
            self.labels = self.__class__.labels(path + ["labels"], service)
            self.deprecated = self.__class__.deprecated(path + ["deprecated"], service)
            self.create_new_mesh_object = self.__class__.create_new_mesh_object(path + ["create_new_mesh_object"], service)
        ...
        class volumetric_regions(TUIMenu):
            ...
            def __init__(self, path, service):
                self.path = path
                self.service = service
                self.hexcore = self.__class__.hexcore(path + ["hexcore"], service)
                self.scoped_prism = self.__class__.scoped_prism(path + ["scoped_prism"], service)
                self.tet = self.__class__.tet(path + ["tet"], service)
                super().__init__(path, service)
            def rename(self, *args, **kwargs):
                """
                Renames the region.
            ...
            class hexcore(TUIMenu):
                ...
                class set(TUIMenu):
                    ...
                    def __init__(self, path, service):
                        self.path = path
                        self.service = service
                        self.outer_domain_params = self.__class__.outer_domain_params(path + ["outer_domain_params"], service)
                        super().__init__(path, service)
                    def smooth_interface(self, *args, **kwargs):
                        """
                        Enable smoothing of hexcore interface.
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/hexcore/set/smooth_interface").execute(*args, **kwargs)
                    ...
                    def maximum_subdivisions(self, *args, **kwargs):
                        """
                        Maximum number of subdivision sweeps.
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/hexcore/set/maximum_subdivisions").execute(*args, **kwargs)
                    ...
                    def island_thresholds(self, *args, **kwargs):
                        """
                        Maximum number of cells and volume fraction in islands, deleted while separating the cells by region.
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/hexcore/set/island_thresholds").execute(*args, **kwargs)
                    ...
                    def smooth_iterations(self, *args, **kwargs):
                        ...
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/hexcore/set/smooth_iterations").execute(*args, **kwargs)
                    ...
                    def smooth_relaxation(self, *args, **kwargs):
                        """
                        Smoothing under relaxation on hexcore interface.
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/hexcore/set/smooth_relaxation").execute(*args, **kwargs)
                    ...
                    def only_hexcore(self, *args, **kwargs):
                        """
                        Create hexcore and activate tet domain.
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/hexcore/set/only_hexcore").execute(*args, **kwargs)
                    ...
                    def keep_hex_tet_separate(self, *args, **kwargs):
                        """
                        Separate Hex and Tet cells.
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/hexcore/set/keep_hex_tet_separate").execute(*args, **kwargs)
            ...
            class scoped_prism(TUIMenu):
                ...
                class set(TUIMenu):
                    ...
                    def poly_keep_nlayer_prism_characteristics(self, *args, **kwargs):
                        """
                        Number of layers to maintain poly-prism characteristics.
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/scoped_prism/set/poly_keep_nlayer_prism_characteristics").execute(*args, **kwargs)
                    def list(self, *args, **kwargs):
                        """
            ...
            class tet(TUIMenu):
                ...
                class set(TUIMenu):
                    ...
                    def __init__(self, path, service):
                        self.path = path
                        self.service = service
                        self.adv_front_method = self.__class__.adv_front_method(path + ["adv_front_method"], service)
                        self.improve_mesh = self.__class__.improve_mesh(path + ["improve_mesh"], service)
                        self.tet_improve = self.__class__.tet_improve(path + ["tet_improve"], service)
                        self.remove_slivers = self.__class__.remove_slivers(path + ["remove_slivers"], service)
                        self.advanced = self.__class__.advanced(path + ["advanced"], service)
                        super().__init__(path, service)
                    ...
                    def merge_free_nodes(self, *args, **kwargs):
                        """
                        Automatically merge duplicate nodes?.
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/tet/set/merge_free_nodes").execute(*args, **kwargs)
                    def delete_unused_nodes(self, *args, **kwargs):
                        """
                        Automatically delete unused nodes?.
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/tet/set/delete_unused_nodes").execute(*args, **kwargs)
                    ...
                    def improve_surface_mesh(self, *args, **kwargs):
                        ...
                        """
                        return PyMenu(self.service, "/objects/volumetric_regions/tet/set/improve_surface_mesh").execute(*args, **kwargs)
                    def set_region_based_sizing(self, *args, **kwargs):
                        """
                        Set region based sizings.
                        """
                    ...
                    class adv_front_method(TUIMenu):
                        ...
                        def __init__(self, path, service):
                            self.path = path
                            self.service = service
                            self.skew_improve = self.__class__.skew_improve(path + ["skew_improve"], service)
                            super().__init__(path, service)
                    ...
                    class advanced(TUIMenu):
                        ...
                        def __init__(self, path, service):
                            ...
                            self.service = service
                            super().__init__(path, service)
                        def max_nodes(self, *args, **kwargs):
                            """
                            Set maximum number of nodes in mesh.
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/max_nodes").execute(*args, **kwargs)
                        def freeze_boundary_cells(self, *args, **kwargs):
                            """
                            Freeze boundary cells .
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/freeze_boundary_cells").execute(*args, **kwargs)
                        def report_max_unmeshed(self, *args, **kwargs):
                            """
                            Max number of unmeshed entities reported.
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/report_max_unmeshed").execute(*args, **kwargs)
                        def defaults(self, *args, **kwargs):
                            """
                            Calculate defaults.
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/defaults").execute(*args, **kwargs)
                        def max_cells(self, *args, **kwargs):
                            """
                            Set maximum number of cells in mesh.
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/max_cells").execute(*args, **kwargs)
                        def progress_reports(self, *args, **kwargs):
                            """
                            Set time between progress reports in seconds.
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/progress_reports").execute(*args, **kwargs)
                        def report_unmeshed_faces(self, *args, **kwargs):
                            """
                            Report unmeshed faces.
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/report_unmeshed_faces").execute(*args, **kwargs)
                        def node_tolerance(self, *args, **kwargs):
                            """
                            Set node-tolerance.
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/node_tolerance").execute(*args, **kwargs)
                        def sliver_size(self, *args, **kwargs):
                            """
                            Set sliver-size.
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/sliver_size").execute(*args, **kwargs)
                        def report_unmeshed_nodes(self, *args, **kwargs):
                            """
                            Report unmeshed nodes.
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/report_unmeshed_nodes").execute(*args, **kwargs)
                        def keep_virtual_entities(self, *args, **kwargs):
                            """
                            Toggle deletion of virtual entities after intialization.
                            """
                            return PyMenu(self.service, "/objects/volumetric_regions/tet/set/advanced/keep_virtual_entities").execute(*args, **kwargs)
        ...
        class labels(TUIMenu):
            ...
            def __init__(self, path, service):
                self.path = path
                self.service = service
                self.cavity = self.__class__.cavity(path + ["cavity"], service)
                super().__init__(path, service)
            def delete(self, *args, **kwargs):
                """
    ...
    class boundary(TUIMenu):
        ...
        def __init__(self, path, service):
            self.path = path
            self.service = service
            self.shell_boundary_layer = self.__class__.shell_boundary_layer(path + ["shell_boundary_layer"], service)
            self.manage = self.__class__.manage(path + ["manage"], service)
            self.refine = self.__class__.refine(path + ["refine"], service)
            self.separate = self.__class__.separate(path + ["separate"], service)
            self.modify = self.__class__.modify(path + ["modify"], service)
            self.boundary_conditions = self.__class__.boundary_conditions(path + ["boundary_conditions"], service)
            self.feature = self.__class__.feature(path + ["feature"], service)
            self.remesh = self.__class__.remesh(path + ["remesh"], service)
            self.improve = self.__class__.improve(path + ["improve"], service)
            super().__init__(path, service)
        def delete_unused_nodes(self, *args, **kwargs):
            """
        ...
        class shell_boundary_layer(TUIMenu):
            ...
            class controls(TUIMenu):
                ...
                def __init__(self, path, service):
                    self.path = path
                    self.service = service
                    self.zone_specific_growth = self.__class__.zone_specific_growth(path + ["zone_specific_growth"], service)
                    super().__init__(path, service)
        ...
        class manage(TUIMenu):
            ...
            def __init__(self, path, service):
                self.path = path
                self.service = service
                self.user_defined_groups = self.__class__.user_defined_groups(path + ["user_defined_groups"], service)
                super().__init__(path, service)
            def list(self, *args, **kwargs):
                """
        ...
        class refine(TUIMenu):
            ...
            def __init__(self, path, service):
                self.path = path
                self.service = service
                self.local_regions = self.__class__.local_regions(path + ["local_regions"], service)
                super().__init__(path, service)
        ...
        class modify(TUIMenu):
            ...
            def patch_options(self, *args, **kwargs):
                """
                Settings for Patching zone(s) by filling holes.
                """
                return PyMenu(self.service, "/boundary/modify/patch_options").execute(*args, **kwargs)
        ...
        class remesh(TUIMenu):
            ...
            class controls(TUIMenu):
                ...
                def __init__(self, path, service):
                    self.path = path
                    self.service = service
                    self.intersect = self.__class__.intersect(path + ["intersect"], service)
                    super().__init__(path, service)
    ...
    class diagnostics(TUIMenu):
        ...
        def __init__(self, path, service):
            self.path = path
            self.service = service
            self.quality = self.__class__.quality(path + ["quality"], service)
            self.face_connectivity = self.__class__.face_connectivity(path + ["face_connectivity"], service)
            super().__init__(path, service)
    ...
    class file(TUIMenu):
        ...
        def __init__(self, path, service):
            ...
            self.import_ = self.__class__.import_(path + ["import_"], service)
            self.checkpoint = self.__class__.checkpoint(path + ["checkpoint"], service)
            self.project_beta = self.__class__.project_beta(path + ["project_beta"], service)
            super().__init__(path, service)
        ...
        class import_(TUIMenu):
            ...
            def __init__(self, path, service):
                self.path = path
                self.service = service
                self.cad_options = self.__class__.cad_options(path + ["cad_options"], service)
                super().__init__(path, service)
        ...
        class project_beta(TUIMenu):
            ...
            def new(self, *args, **kwargs):
                ...
                Create New Project.
                """
                return PyMenu(self.service, "/file/project_beta/new").execute(*args, **kwargs)
            def archive(self, *args, **kwargs):
                """
                Archive Project.
                """
                return PyMenu(self.service, "/file/project_beta/archive").execute(*args, **kwargs)
            def open(self, *args, **kwargs):
                """
                Open project.
                """
                return PyMenu(self.service, "/file/project_beta/open").execute(*args, **kwargs)
            def save_as_copy(self, *args, **kwargs):
                """
                Save As Copy.
                """
                return PyMenu(self.service, "/file/project_beta/save_as_copy").execute(*args, **kwargs)
            def save(self, *args, **kwargs):
                """
                Save Project.
                """
                return PyMenu(self.service, "/file/project_beta/save").execute(*args, **kwargs)
            def save_as(self, *args, **kwargs):
                """
                Save As Project.
                """
                return PyMenu(self.service, "/file/project_beta/save_as").execute(*args, **kwargs)
    ...
    class display(TUIMenu):
        ...
        def __init__(self, path, service):
            self.path = path
            self.service = service
            self.xy_plot = self.__class__.xy_plot(path + ["xy_plot"], service)
            self.advanced_rendering = self.__class__.advanced_rendering(path + ["advanced_rendering"], service)
            self.zones = self.__class__.zones(path + ["zones"], service)
            self.set_grid = self.__class__.set_grid(path + ["set_grid"], service)
        ...
        def views(self, *args, **kwargs):
            ...
            """
            return PyMenu(self.service, "/display/views").execute(*args, **kwargs)
        def clear(self, *args, **kwargs):
            """
        ...
        class set(TUIMenu):
            ...
            def __init__(self, path, service):
                ...
                self.styles = self.__class__.styles(path + ["styles"], service)
                super().__init__(path, service)
            ...
            def windows(self, *args, **kwargs):
                ...
                """
                return PyMenu(self.service, "/display/set/windows").execute(*args, **kwargs)
            ...
            class picture(TUIMenu):
                ...
                def raytracer_image(self, *args, **kwargs):
                    ...
                    """
                    return PyMenu(self.service, "/display/set/picture/raytracer_image").execute(*args, **kwargs)
    ...
    class mesh(TUIMenu):
        ...
        def __init__(self, path, service):
            ...
            self.service = service
            self.separate = self.__class__.separate(path + ["separate"], service)
            self.auto_mesh_controls = self.__class__.auto_mesh_controls(path + ["auto_mesh_controls"], service)
            self.tet = self.__class__.tet(path + ["tet"], service)
            self.domains = self.__class__.domains(path + ["domains"], service)
            self.poly = self.__class__.poly(path + ["poly"], service)
            self.manage = self.__class__.manage(path + ["manage"], service)
            self.modify = self.__class__.modify(path + ["modify"], service)
            self.cartesian = self.__class__.cartesian(path + ["cartesian"], service)
            self.rapid_octree = self.__class__.rapid_octree(path + ["rapid_octree"], service)
            self.cell_zone_conditions = self.__class__.cell_zone_conditions(path + ["cell_zone_conditions"], service)
            self.pyramid = self.__class__.pyramid(path + ["pyramid"], service)
            self.hexcore = self.__class__.hexcore(path + ["hexcore"], service)
            self.cavity = self.__class__.cavity(path + ["cavity"], service)
            self.poly_hexcore = self.__class__.poly_hexcore(path + ["poly_hexcore"], service)
            self.non_conformals = self.__class__.non_conformals(path + ["non_conformals"], service)
            self.thin_volume_mesh = self.__class__.thin_volume_mesh(path + ["thin_volume_mesh"], service)
            self.scoped_prisms = self.__class__.scoped_prisms(path + ["scoped_prisms"], service)
            self.prism = self.__class__.prism(path + ["prism"], service)
            super().__init__(path, service)
        ...
        class separate(TUIMenu):
            ...
            def __init__(self, path, service):
                ...
                self.local_regions = self.__class__.local_regions(path + ["local_regions"], service)
                super().__init__(path, service)
            def separate_wedge_prisms(self, *args, **kwargs):
                """
                Separate wedge-prism cells from bulk.
                """
                return PyMenu(self.service, "/mesh/separate/separate_wedge_prisms").execute(*args, **kwargs)
            ...
            def separate_prisms_from_hex(self, *args, **kwargs):
                """
                Separate prism cells from hex.
                """
                return PyMenu(self.service, "/mesh/separate/separate_prisms_from_hex").execute(*args, **kwargs)
        ...
        class tet(TUIMenu):
            ...
            class controls(TUIMenu):
                ...
                def merge_free_nodes(self, *args, **kwargs):
                    ...
                    Automatically merge duplicate nodes?.
                    """
                    return PyMenu(self.service, "/mesh/tet/controls/merge_free_nodes").execute(*args, **kwargs)
                def skewness_method(self, *args, **kwargs):
                    """
                ...
                def improve_surface_mesh(self, *args, **kwargs):
                    ...
                    """
                    return PyMenu(self.service, "/mesh/tet/controls/improve_surface_mesh").execute(*args, **kwargs)
                def refine_method(self, *args, **kwargs):
                    """
                ...
                def delete_unused_nodes(self, *args, **kwargs):
                    ...
                    """
                    return PyMenu(self.service, "/mesh/tet/controls/delete_unused_nodes").execute(*args, **kwargs)
                def max_cell_volume(self, *args, **kwargs):
                    """
                ...
                class advanced(TUIMenu):
                    ...
                    def max_nodes(self, *args, **kwargs):
                        ...
                        Set maximum number of nodes in mesh.
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/max_nodes").execute(*args, **kwargs)
                    def freeze_boundary_cells(self, *args, **kwargs):
                        """
                        Freeze boundary cells .
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/freeze_boundary_cells").execute(*args, **kwargs)
                    def report_unmeshed_nodes(self, *args, **kwargs):
                        """
                        Report unmeshed nodes.
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/report_unmeshed_nodes").execute(*args, **kwargs)
                    def report_unmeshed_faces(self, *args, **kwargs):
                        """
                        Report unmeshed faces.
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/report_unmeshed_faces").execute(*args, **kwargs)
                    def keep_virtual_entities(self, *args, **kwargs):
                        """
                        Toggle deletion of virtual entities after intialization.
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/keep_virtual_entities").execute(*args, **kwargs)
                    def node_tolerance(self, *args, **kwargs):
                        """
                        Set node-tolerance.
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/node_tolerance").execute(*args, **kwargs)
                    def max_cells(self, *args, **kwargs):
                        """
                        Set maximum number of cells in mesh.
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/max_cells").execute(*args, **kwargs)
                    def progress_reports(self, *args, **kwargs):
                        """
                        Set time between progress reports in seconds.
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/progress_reports").execute(*args, **kwargs)
                    def report_max_unmeshed(self, *args, **kwargs):
                        """
                        Max number of unmeshed entities reported.
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/report_max_unmeshed").execute(*args, **kwargs)
                    def defaults(self, *args, **kwargs):
                        """
                        Calculate defaults.
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/defaults").execute(*args, **kwargs)
                    def sliver_size(self, *args, **kwargs):
                        """
                        Set sliver-size.
                        """
                        return PyMenu(self.service, "/mesh/tet/controls/advanced/sliver_size").execute(*args, **kwargs)
        ...
        class poly(TUIMenu):
            ...
            class controls(TUIMenu):
                ...
                def __init__(self, path, service):
                    self.path = path
                    self.service = service
                    self.smooth_controls = self.__class__.smooth_controls(path + ["smooth_controls"], service)
                    self.prism = self.__class__.prism(path + ["prism"], service)
                    super().__init__(path, service)
        ...
        class modify(TUIMenu):
            ...
            def delete_isolated_cells(self, *args, **kwargs):
                """
                Delete isolated cells.
                """
                return PyMenu(self.service, "/mesh/modify/delete_isolated_cells").execute(*args, **kwargs)
            def list_selections(self, *args, **kwargs):
                """
        ...
        class rapid_octree(TUIMenu):
            ...
            def __init__(self, path, service):
                self.path = path
                self.service = service
                self.refinement_regions = self.__class__.refinement_regions(path + ["refinement_regions"], service)
                self.boundary_layer_mesh_configuration = self.__class__.boundary_layer_mesh_configuration(path + ["boundary_layer_mesh_configuration"], service)
                self.advanced_meshing_options = self.__class__.advanced_meshing_options(path + ["advanced_meshing_options"], service)
                self.mesh_sizing = self.__class__.mesh_sizing(path + ["mesh_sizing"], service)
                super().__init__(path, service)
            ...
            def boundary_mesh_optimization(self, *args, **kwargs):
                """
                Enable optimization to improve orthogonality at the boundary mesh.
                """
                return PyMenu(self.service, "/mesh/rapid_octree/boundary_mesh_optimization").execute(*args, **kwargs)
            ...
            class refinement_regions(TUIMenu):
                ...
                def list_properties(self, *args, **kwargs):
                    ...
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/refinement_regions/list_properties").execute(*args, **kwargs)
                def add(self, *args, **kwargs):
                    """
                ...
                def edit(self, *args, **kwargs):
                    ...
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/refinement_regions/edit").execute(*args, **kwargs)
            ...
            class boundary_layer_mesh_configuration(TUIMenu):
                ...
                def delete_all(self, *args, **kwargs):
                    ...
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/boundary_layer_mesh_configuration/delete_all").execute(*args, **kwargs)
                def add(self, *args, **kwargs):
                    """
                    Add a boudnary layer definition.
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/boundary_layer_mesh_configuration/add").execute(*args, **kwargs)
                def list(self, *args, **kwargs):
                    """
                    Specify the number of prismatic layers for surface zones.
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/boundary_layer_mesh_configuration/list").execute(*args, **kwargs)
                def edit(self, *args, **kwargs):
                    """
                    Specify the number of prismatic layers for surface zones.
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/boundary_layer_mesh_configuration/edit").execute(*args, **kwargs)
                def delete(self, *args, **kwargs):
                    """
                    Specify the number of prismatic layers for surface zones.
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/boundary_layer_mesh_configuration/delete").execute(*args, **kwargs)
            ...
            class advanced_meshing_options(TUIMenu):
                ...
                def mesh_optimization_scheme(self, *args, **kwargs):
                    """
                    Set optimization scheme for boundary mesh optimization in projection meshing.
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/advanced_meshing_options/mesh_optimization_scheme").execute(*args, **kwargs)
                def target_bnd_face_warp(self, *args, **kwargs):
                    """
                    Set target face warpage for boundary faces in mesh (-1.0 - 1.0). Higher values are likely to increase pullback.
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/advanced_meshing_options/target_bnd_face_warp").execute(*args, **kwargs)
                def target_int_face_warp(self, *args, **kwargs):
                    """
                    Set target face warpage for interior faces in mesh (-1.0 - 1.0). Higher values are likely to increase pullback.
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/advanced_meshing_options/target_int_face_warp").execute(*args, **kwargs)
                def activate_projection_layer_improvement_algorithm(self, *args, **kwargs):
                    """
                    Force the projection algorithm to do smoothing expansion without prisms defined.
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/advanced_meshing_options/activate_projection_layer_improvement_algorithm").execute(*args, **kwargs)
                ...
                def smooth_mesh_coarsening(self, *args, **kwargs):
                    """
                    Activte smoother mesh coarseing algorightm. Will increase runtime!.
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/advanced_meshing_options/smooth_mesh_coarsening").execute(*args, **kwargs)
                def max_num_inflection_point_iter(self, *args, **kwargs):
                    """
                    Sets the maximum number of iterations for inflection point removal during projection front fix.
                    """
                    return PyMenu(self.service, "/mesh/rapid_octree/advanced_meshing_options/max_num_inflection_point_iter").execute(*args, **kwargs)
        ...
        class hexcore(TUIMenu):
            ...
            class controls(TUIMenu):
                ...
                def only_hexcore(self, *args, **kwargs):
                    ...
                    Create hexcore and activate tet domain.
                    """
                    return PyMenu(self.service, "/mesh/hexcore/controls/only_hexcore").execute(*args, **kwargs)
                def keep_hex_tet_separate(self, *args, **kwargs):
                    """
                    Separate Hex and Tet cells.
                    """
                    return PyMenu(self.service, "/mesh/hexcore/controls/keep_hex_tet_separate").execute(*args, **kwargs)
                def island_thresholds(self, *args, **kwargs):
                    """
                    Maximum number of cells and volume fraction in islands, deleted while separating the cells by region.
                    """
                    return PyMenu(self.service, "/mesh/hexcore/controls/island_thresholds").execute(*args, **kwargs)
                def maximum_cell_length(self, *args, **kwargs):
                    """
                ...
                def smooth_relaxation(self, *args, **kwargs):
                    ...
                    Smoothing under relaxation on hexcore interface.
                    """
                    return PyMenu(self.service, "/mesh/hexcore/controls/smooth_relaxation").execute(*args, **kwargs)
                def set_region_based_sizing(self, *args, **kwargs):
                    """
                ...
                def maximum_subdivisions(self, *args, **kwargs):
                    ...
                    Maximum number of subdivision sweeps.
                    """
                    return PyMenu(self.service, "/mesh/hexcore/controls/maximum_subdivisions").execute(*args, **kwargs)
                def octree_hexcore(self, *args, **kwargs):
                    """
                ...
                def smooth_iterations(self, *args, **kwargs):
                    ...
                    Number of smoothing iterations on hexcore interface.
                    """
                    return PyMenu(self.service, "/mesh/hexcore/controls/smooth_iterations").execute(*args, **kwargs)
                def buffer_layers(self, *args, **kwargs):
                    """
                ...
                def smooth_interface(self, *args, **kwargs):
                    ...
                    Enable smoothing of hexcore interface.
                    """
                    return PyMenu(self.service, "/mesh/hexcore/controls/smooth_interface").execute(*args, **kwargs)
                def skip_tet_refinement(self, *args, **kwargs):
                    """
        ...
        class scoped_prisms(TUIMenu):
            ...
            def poly_keep_nlayer_prism_characteristics(self, *args, **kwargs):
                ...
                Number of layers to maintain poly-prism characteristics.
                """
                return PyMenu(self.service, "/mesh/scoped_prisms/poly_keep_nlayer_prism_characteristics").execute(*args, **kwargs)
            def list(self, *args, **kwargs):
                """
        ...
        class prism(TUIMenu):
            ...
            def __init__(self, path, service):
                ...
                self.service = service
                self.improve = self.__class__.improve(path + ["improve"], service)
                self.post_ignore = self.__class__.post_ignore(path + ["post_ignore"], service)
                self.controls = self.__class__.controls(path + ["controls"], service)
                self.split = self.__class__.split(path + ["split"], service)
                super().__init__(path, service)
            ...
            class controls(TUIMenu):
                ...
                def __init__(self, path, service):
                    self.path = path
                    self.service = service
                    self.normal = self.__class__.normal(path + ["normal"], service)
                    self.improve = self.__class__.improve(path + ["improve"], service)
                    self.proximity = self.__class__.proximity(path + ["proximity"], service)
                    self.morph = self.__class__.morph(path + ["morph"], service)
                    self.zone_specific_growth = self.__class__.zone_specific_growth(path + ["zone_specific_growth"], service)
                    self.offset = self.__class__.offset(path + ["offset"], service)
                    self.adjacent_zone = self.__class__.adjacent_zone(path + ["adjacent_zone"], service)
                    self.post_ignore = self.__class__.post_ignore(path + ["post_ignore"], service)
                    super().__init__(path, service)
                ...
                def auto_separate_cells(self, *args, **kwargs):
                    """
                    Automatically separate prism cells extruded from multiple face threads?.
                    """
                    return PyMenu(self.service, "/mesh/prism/controls/auto_separate_cells").execute(*args, **kwargs)
                def grow_individually(self, *args, **kwargs):
                    """
                    Grow from multiple zones one-at-a-time?.
                    """
                    return PyMenu(self.service, "/mesh/prism/controls/grow_individually").execute(*args, **kwargs)
                ...
                class normal(TUIMenu):
                    ...
                    def smooth_iter(self, *args, **kwargs):
                        """
                        Maximum number of smoothing iterations for normal vectors.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/normal/smooth_iter").execute(*args, **kwargs)
                    ...
                    def converge_locally(self, *args, **kwargs):
                        """
                        ...
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/normal/converge_locally").execute(*args, **kwargs)
                    def smooth(self, *args, **kwargs):
                        """
                        Perform smoothing of normal advancement direction vectors?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/normal/smooth").execute(*args, **kwargs)
                    def normal_method(self, *args, **kwargs):
                        """
                        Method in which normal marching direction vectors are determined.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/normal/normal_method").execute(*args, **kwargs)
                    ...
                    def smooth_converged(self, *args, **kwargs):
                        """
                        Convergence criterion (in DEGREES) for angle changes
                        during normal smoothing.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/normal/smooth_converged").execute(*args, **kwargs)
                ...
                class improve(TUIMenu):
                    ...
                    def __init__(self, path, service):
                        ...
                        self.service = service
                        super().__init__(path, service)
                    def node_smooth_rings(self, *args, **kwargs):
                        """
                        ...
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/node_smooth_rings").execute(*args, **kwargs)
                    ...
                    def edge_smooth_angle(self, *args, **kwargs):
                        """
                        Skewness-driven edge smoothing is only allowed between cap faces whose normals
                        are within this angle.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/edge_smooth_angle").execute(*args, **kwargs)
                    def edge_smooth(self, *args, **kwargs):
                        """
                        Perform local smoothing of nodes on longest edges of skewed faces?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/edge_smooth").execute(*args, **kwargs)
                    def node_smooth_angle(self, *args, **kwargs):
                        """
                        Only manifolds having characteristic angles
                        within this many degrees of 180 will be smoothed.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/node_smooth_angle").execute(*args, **kwargs)
                    def node_smooth_iter(self, *args, **kwargs):
                        """
                        Maximum number of smoothing iterations for nodes on advancing layers.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/node_smooth_iter").execute(*args, **kwargs)
                    ...
                    def face_smooth_rings(self, *args, **kwargs):
                        """
                        No. of rings to propagate skewness during smoothing of cap faces.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/face_smooth_rings").execute(*args, **kwargs)
                    def smooth_improve_prism_cells(self, *args, **kwargs):
                        """
                    ...
                    def face_smooth(self, *args, **kwargs):
                        """
                        Perform face-driven smoothing to improve skewness?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/face_smooth").execute(*args, **kwargs)
                    ...
                    def edge_swap(self, *args, **kwargs):
                        """
                        Perform edge swapping to improve skewness?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/edge_swap").execute(*args, **kwargs)
                    def check_size(self, *args, **kwargs):
                        """
                        Check for negative cell volume?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/check_size").execute(*args, **kwargs)
                    ...
                    def node_smooth_converged(self, *args, **kwargs):
                        """
                        Convergence criterion for node smoothing.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/node_smooth_converged").execute(*args, **kwargs)
                    def node_smooth(self, *args, **kwargs):
                        """
                        Perform node-driven smoothing to improve skewness?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/node_smooth").execute(*args, **kwargs)
                    def face_smooth_converged(self, *args, **kwargs):
                        """
                        Convergence criteria for cap face smoothing.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/face_smooth_converged").execute(*args, **kwargs)
                    ...
                    def swap_smooth_skew(self, *args, **kwargs):
                        """
                        Only faces with skewness >= this value are swapped and/or smoothed.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/swap_smooth_skew").execute(*args, **kwargs)
                    ...
                    def node_smooth_local(self, *args, **kwargs):
                        """
                        Allow node-smoothing to converge locally (for large geometries).
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/node_smooth_local").execute(*args, **kwargs)
                    def identify_feature_line(self, *args, **kwargs):
                        """
                        Perform normal smoothing on feature lines?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/identify_feature_line").execute(*args, **kwargs)
                    ...
                    def layer_by_layer_smoothing(self, *args, **kwargs):
                        """
                        Perform normals and heights smoothing to improve cell quality/shape?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/layer_by_layer_smoothing").execute(*args, **kwargs)
                    def shrink_left_handed_cap(self, *args, **kwargs):
                        """
                        Shrink if required to get rid of left handed faces.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/shrink_left_handed_cap").execute(*args, **kwargs)
                    ...
                    def post_adjust_height(self, *args, **kwargs):
                        """
                        Perform prism height adjustment based on growth rate?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/improve/post_adjust_height").execute(*args, **kwargs)
                ...
                class proximity(TUIMenu):
                    ...
                    def smoothing_rate(self, *args, **kwargs):
                        """
                        Rate at which shrinkage is propagated in lateral direction.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/proximity/smoothing_rate").execute(*args, **kwargs)
                ...
                class offset(TUIMenu):
                    ...
                    def smooth_converged(self, *args, **kwargs):
                        ...
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/offset/smooth_converged").execute(*args, **kwargs)
                    ...
                    def smooth_iter(self, *args, **kwargs):
                        ...
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/offset/smooth_iter").execute(*args, **kwargs)
                    ...
                    def smooth(self, *args, **kwargs):
                        ...
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/offset/smooth").execute(*args, **kwargs)
                ...
                class adjacent_zone(TUIMenu):
                    ...
                    def __init__(self, path, service):
                        ...
                        self.service = service
                        super().__init__(path, service)
                    def project_iter(self, *args, **kwargs):
                        """
                        Maximum number of iterations for projection to adjacent zones.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/adjacent_zone/project_iter").execute(*args, **kwargs)
                    def project(self, *args, **kwargs):
                        """
                        Project outer nodes to adjacent planar zones?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/adjacent_zone/project").execute(*args, **kwargs)
                    def side_feature_align_angle(self, *args, **kwargs):
                        """
                        This angle (degrees) is used for aligning projected normals along a feature edge.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/adjacent_zone/side_feature_align_angle").execute(*args, **kwargs)
                    def side_topology_align_angle(self, *args, **kwargs):
                        """
                        ...
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/adjacent_zone/side_topology_align_angle").execute(*args, **kwargs)
                    def retriangulate_adjacent(self, *args, **kwargs):
                        """
                        Retriangulate planar zones along which prism layers are grown?.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/adjacent_zone/retriangulate_adjacent").execute(*args, **kwargs)
                    ...
                    def project_converged(self, *args, **kwargs):
                        """
                        Convergence criterion for projection to adjacent zones.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/adjacent_zone/project_converged").execute(*args, **kwargs)
                    def retri_feature_angle(self, *args, **kwargs):
                        """
                        This angle (degrees) is used for determining the feature edges on the face zones which are retirangulated.
                        """
                        return PyMenu(self.service, "/mesh/prism/controls/adjacent_zone/retri_feature_angle").execute(*args, **kwargs)
    ...
    class preferences(TUIMenu):
        ...
        class general(TUIMenu):
            ...
            def import_physics_volume_definitions(self, *args, **kwargs):
                ...
                .
                """
                return PyMenu(self.service, "/preferences/general/import_physics_volume_definitions").execute(*args, **kwargs)
            def skip_creation_of_groups_pointing_to_single_entity(self, *args, **kwargs):
                """
                .
                """
                return PyMenu(self.service, "/preferences/general/skip_creation_of_groups_pointing_to_single_entity").execute(*args, **kwargs)
        ...
        class mat_pro_app(TUIMenu):
            ...
            def __init__(self, path, service):
                self.path = path
                self.service = service
                self.check_expression = self.__class__.check_expression(path + ["check_expression"], service)
                super().__init__(path, service)
            ...
            class check_expression(TUIMenu):
                ...
                def cdot(self, *args, **kwargs):
                    ...
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/cdot").execute(*args, **kwargs)
                def velocities(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/velocities").execute(*args, **kwargs)
                def giesekus(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/giesekus").execute(*args, **kwargs)
                def time(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/time").execute(*args, **kwargs)
                def pressure(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/pressure").execute(*args, **kwargs)
                def coordinates(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/coordinates").execute(*args, **kwargs)
                def edot(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/edot").execute(*args, **kwargs)
                def gdot(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/gdot").execute(*args, **kwargs)
                def vorticity(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/vorticity").execute(*args, **kwargs)
                def dvv(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/dvv").execute(*args, **kwargs)
                def temperature(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/temperature").execute(*args, **kwargs)
                def species(self, *args, **kwargs):
                    """
                    .
                    """
                    return PyMenu(self.service, "/preferences/mat_pro_app/check_expression/species").execute(*args, **kwargs)
        ...
        class simulation(TUIMenu):
            ...
            def pdf_combustion_robust_numerics(self, *args, **kwargs):
                ...
                .
                """
                return PyMenu(self.service, "/preferences/simulation/pdf_combustion_robust_numerics").execute(*args, **kwargs)
            def flow_model(self, *args, **kwargs):
                """
                .
                """
        ...
        class graphics(TUIMenu):
            ...
            def backface_cull(self, *args, **kwargs):
                ...
                .
                """
                return PyMenu(self.service, "/preferences/graphics/backface_cull").execute(*args, **kwargs)
            ...
            def surface_general_displacement(self, *args, **kwargs):
                ...
                .
                """
                return PyMenu(self.service, "/preferences/graphics/surface_general_displacement").execute(*args, **kwargs)
            def double_buffering(self, *args, **kwargs):
                """
