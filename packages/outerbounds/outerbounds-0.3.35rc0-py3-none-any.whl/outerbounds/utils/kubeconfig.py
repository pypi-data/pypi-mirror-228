    ...
    new_user = {
        ...
        "user": {
            "exec": {
                ...
                "command": outerbounds_binary_path,
                "args": cmd_args,
                "env": None,
...
def use_context(context: str) -> None:
    """
    Set the current context in the Kubernetes configuration.

    Args:
        context (str): Name of the context to set as current.
    """
    config_path = os.path.expanduser(os.environ.get("KUBECONFIG", "~/.kube/config"))
    config = get_kube_config_or_init_default(config_path)
...
def validate_kubeconfig_fields_for_workstations(
    ...
    """
    if current_kubeconfig["current-context"] != "outerbounds-workstations":
        return False
