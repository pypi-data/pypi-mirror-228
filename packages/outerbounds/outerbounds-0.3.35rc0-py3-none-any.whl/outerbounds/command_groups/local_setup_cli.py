        ...
        if "OBP_AUTH_SERVER" in config:
            ...
            try:
                ...
                if e.response is not None and (
                    ...
                    narrator.warn(reason="Can't check validity of token")
                    check_status.update(
                        status=OuterboundsCommandStatus.WARN,
...
def check_workstation_api_accessible(narrator: Narrator) -> CommandStatus:
    ...
    try:
        ...
        if len(missing_keys) > 0:
            ...
            narrator.warn(reason=missing_keys_msg)
            check_status.update(
                status=OuterboundsCommandStatus.WARN,
