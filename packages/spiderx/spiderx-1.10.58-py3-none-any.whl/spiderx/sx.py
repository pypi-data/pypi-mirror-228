...
def cmd_subprocess_run(cmd:Union[str,list], 类型=1,timeout=None)->bool:
    ...
    if 类型 == 1:
        ...
        rt = subprocess.run(cmd,startupinfo=si,creationflags=subprocess.CREATE_NO_WINDOW,timeout=timeout)
    elif 类型 == 2:
        # 隐藏窗口 有输出
        si = subprocess.STARTUPINFO()
        si.dwFlags = subprocess.STARTF_USESHOWWINDOW
        ...
        #在新控制台窗口中启动子进程。
        rt = subprocess.run(cmd, creationflags=subprocess.CREATE_NEW_CONSOLE,timeout=timeout)
    elif 类型 == 4:
        # 不允许创建窗口
        #在后台启动子进程，不创建窗口显示
        rt = subprocess.run(cmd, creationflags=subprocess.CREATE_NO_WINDOW,timeout=timeout)
    else:
        # 不允许子窗口
        # 将子进程从父进程分离，使其成为一个独立的进程组。
        rt = subprocess.run(cmd, creationflags=subprocess.DETACHED_PROCESS)
    if rt.returncode==0:
        return True
    else:
        return False
...
def 提取m3u8List(字符串:str,倒叙=0,关键字='\.m3u8')->list():
    lst=[]
...
def zlib_解压_base64(base64str:str)->str:
    ...
    _data = base64.b64decode(base64str)
...
def jwt_base64_payload(jwted:str)->dict:
    ...
    return json.loads(base64.b64decode(jwted.split('.')[1]))
...
def m3u8DL_CLI(文件路径:str='test',网址:str="",headers:str or dict={},key:bytes=None,iv:bytes=None,cli_href:str="",ffmpeg_href:str="",cli_local:str="",ffmpeg_local:str="",istest:bool=False,options:list=[])->bool:
    ...
    ''' CREATE_NEW_CONSOLE：在新控制台窗口中启动子进程。 '''
    returncode=subprocess.run(commands,creationflags=subprocess.CREATE_NEW_CONSOLE).returncode
    if returncode==0:
        return True
    else:
        return False
...
class FFMPEG():
    ...
    def 合并视频(self,视频路径1, 视频路径2, 输出路径):
        ...
        #print(ffmpeg_command)
        subprocess.call(ffmpeg_command,stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    def mpeg(self,输入路径,输出路径):
        cmd = f'"{self.ffmpeg_path}" -i "{输入路径}" -y -c copy -bsf:a aac_adtstoasc "{输出路径}'
        self.执行(cmd, show=1)
...
class WINRAR():
    ...
    def 解压(self, 文件路径:str, 解压目录:str= '', timeout=None)->bool:
        ...
        try:
            ...
            si = subprocess.STARTUPINFO()
            si.dwFlags = subprocess.STARTF_USESHOWWINDOW
            if subprocess.call(command, startupinfo=si,timeout=timeout,creationflags=subprocess.CREATE_NO_WINDOW) == 0:
                return True
            else:
                return False
        except Exception as e:
