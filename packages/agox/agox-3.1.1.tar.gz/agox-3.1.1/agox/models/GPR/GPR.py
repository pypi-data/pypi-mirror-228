...
class GPR(ModelBaseClass, RayPoolUser):
    ...
    dynamic_attributes = ["alpha", "K_inv", "X", "K", "kernel", "Y", "mean_energy"]
        ...
        **kwargs
        ...
        if self.use_ray:
            ...
            if n_optimize is None:
                self.n_optimize = self.cpu_count
        else:
            if n_optimize is None:
                self.n_optimize = 1
    ...
    def predict_energy(
        self, atoms: Atoms, k: np.ndarray = None, x: np.ndarray = None, **kwargs
    ) -> float:
    ...
    @candidate_list_comprehension
    def predict_forces(
        self, atoms: Atoms, dk_dr: np.ndarray = None, **kwargs
    ) -> np.ndarray:
        """
    ...
    def converter(
        ...
        return {"x": x, "k": k, "k0": k0, "dk_dr": dk_dr}
    ...
    def _get_kernel_vector(self, k: np.ndarray = None, 
                        x: np.ndarray = None, 
                        atoms: Atoms = None) -> np.ndarray:
        """
    ...
    def _get_kernel_derivative(self, atoms: Atoms, 
            x: np.ndarray = None, 
            dX_dr: np.ndarray = None, 
            dk_dX: np.ndarray = None) -> np.ndarray:
        """
        ...
        if dX_dr is None:
            dX_dr = self.descriptor.get_feature_gradient(
                atoms
            )  # [centers, atoms, 3, features]

        # Get kernel derivatives:
        if dk_dX is None:
            dk_dX = np.array(
                [
                    self.kernel.get_feature_gradient(self.X, x[i : i + 1])
    ...
    def _train_model(self, training_data: List[Atoms]) -> None:
        ...
        if self.use_ray:
            self.pool_synchronize(
                attributes=["alpha", "K_inv", "kernel", "K", "mean_energy"], writer=self.writer
            )
        ...
        Y = np.expand_dims(
            np.array(
                [
                    ...
                    - sum(self.single_atom_energies[d.get_atomic_numbers()])
                    for d in data
                ]
            ),
            axis=1,
    ...
    def _postprocess_energy(self, atoms: Atoms, e_pred: float) -> float:
        ...
        return e_pred + prior + self.mean_energy + sae
