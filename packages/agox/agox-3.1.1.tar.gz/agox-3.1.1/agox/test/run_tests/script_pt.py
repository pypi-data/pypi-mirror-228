...
database = ConcurrentDatabase(
    filename=db_path,
    store_meta_information=True,
    worker_number=worker_index,
...
sampler = ParallelTemperingSampler(
    temperatures=temperatures, order=3, database=database, swap_order=5
)

rattle_generator = RattleGenerator(
    **environment.get_confinement(), environment=environment, sampler=sampler, order=1
)
