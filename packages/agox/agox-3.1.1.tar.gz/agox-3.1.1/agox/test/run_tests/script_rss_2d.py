...
random_generator = RandomGenerator(
    **environment.get_confinement(), environment=environment, order=1
)
...
assert (
    np.array([c.get_positions() for c in database.get_all_candidates()])[
        :, environment.get_missing_indices(), 2
    ]
    == confinement_corner[2]
).all(), "Not all positions are 2D"
