...
from agox.generators.complementary_energy.ce_calculators import (
    ComplementaryEnergyDistanceCalculator,
)
...
model = GPR(
    ...
    database=database,
    prior=Repulsive(),
    n_optimize=0, # Set to a multiple of number of cores for a real search.
)
...
lambs = [0.5, 1, 1.5]
rc = 10.0
desc = ExponentialDensity.from_species(["Au", "Ni"], lambs=lambs, rc=rc)
...
ce_generator = ComplementaryEnergyGenerator(
    ce_calc, desc, ce_attractors, **environment.get_confinement()
)
