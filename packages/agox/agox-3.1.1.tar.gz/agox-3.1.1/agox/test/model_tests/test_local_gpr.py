...
from agox.test.test_utils import (check_file_is_deleted, get_name,
                                  get_test_data, get_test_environment,
                                  label_dict_list, load_expected_data,
                                  save_expected_data, test_data_dicts)
...
def test_model(test_data_dict, update_kwargs, cmd_options):
    ...
    descriptor = SOAP(
        environment=environment, periodic=environment.get_template().pbc.any()
    )
