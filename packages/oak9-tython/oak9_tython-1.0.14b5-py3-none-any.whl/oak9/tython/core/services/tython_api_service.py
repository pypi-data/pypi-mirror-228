...
class TythonApiService:
    ...
    def __init__(self, config: Configuration):
        self.config = config
        self.package_version = pkg_resources.get_distribution("oak9_tython").version
        self.headers = {
            "X-TYTHON-VERSION": f"Python/{self.package_version}"
        }
    ...
    def get_default_environment(self):
        ...
        response = requests.get(url, auth=(self.config.org_id, self.config.api_key), headers=self.headers)
    ...
    def apply_findings(self, findings: list[Finding], request_id: str):
        ...
        response = requests.post(
            url, auth=(self.config.org_id, self.config.api_key), json=payload, headers=self.headers
        )
