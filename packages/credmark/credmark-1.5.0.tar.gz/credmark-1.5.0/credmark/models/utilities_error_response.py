...
T = TypeVar("T", bound="UtilitiesErrorResponse")


@attr.s(auto_attribs=True)
class UtilitiesErrorResponse:
    """
    Attributes:
        ...
        message (Union[List[str], str]): If an error response (non-200 status code), the message will be set with an
            error message. It can either be a string or a list of strings. Example: ['Invalid chain ID'].
    """
    ...
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        ...
        utilities_error_response = cls(
            status_code=status_code,
            error=error,
            message=message,
        )

        return utilities_error_response
