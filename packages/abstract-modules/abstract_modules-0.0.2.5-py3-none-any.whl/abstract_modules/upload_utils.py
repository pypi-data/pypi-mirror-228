...
def alias_events(event):
    ...
    if event == "-SAVE-":
        ...
        for each in values.keys():
            if "-ADD_" in each:
                if values[each] == True:
                    original_name = each[len("-ADD_"):-1]
                    if original_name not in alias:
                        alias["ALIAS"][original_name]=values[f'-ALIAS_{original_name}-']
                    approved_ignored_installs.append(values[f'-FOUND_{original_name}-'])
            if "-NEVER_" in each:
                if values[each] == True:
                    original_name = each[len("-NEVER_"):-1]
                    if original_name not in alias["NEVER"]:
                        alias["NEVER"].append(original_name)
            if "-DEFAULT_" in each:
                if values[each] == True:
                    original_name = each[len("-DEFAULT_"):-1]
...
def get_install_requires(setup_js, project_dir,project_name):
    ...
    if ignored:
        layout = []
        longest = get_longest_len(ignored)
        for i,each in enumerate(ignored):
            if each in alias["NEVER"] or each in alias["DEFAULT"]:
                del ignored[i]
    ...
    print(setup_js['install_requires'],'\n',install_requires_newsetup_js['install_requires'])
    if setup_js['install_requires'] != install_requires_new:
        ...
        layout =[
            [[ggf("T",args={"text":new_text})],
             [ggf(line,args={"default_text":install_requires_new,"key":"-DEFAULT_INSTALLS-","size":(len(new_text),math.ceil(default_length)),"disabled":True}),ggf("T",args={"text":"?"})],
