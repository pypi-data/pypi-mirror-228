...
def get_install_requires(setup_js, project_dir,project_name):
    ...
    if ignored:
        ...
        for each in list(ignored):
            layout.append([get_gui_fun("Checkbox",args={"text":each,"key":"-IGNORED_"+each+'-',"enable_events":False,"enable_events":True}),get_gui_fun("T",args={"text":"alias: -"}),get_push()
                           get_gui_fun("Input",args={"key":"-ALIAS_"+each+'-',"size":(int(len(each)*1.5),1)}),get_gui_fun("T",args={"text":"current version -"}),get_push()
                           get_gui_fun("Input",args={"key":"-FULL_RETURN_"+each+'-',"size":(int(len(each)*1.5),1)}),get_push()
                           get_gui_fun("T",args={"text":"original name: -"}),get_push()
