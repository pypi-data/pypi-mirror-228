...
def iac_parser(args, cache_directory):
    ...
    for framework in args.frameworks:
        ...
        framework_plugins_dir = os.path.join(cache_directory, CodeTypeSubParser.IAC, framework.upper())

        plugins_available = False
        for _, _, plugins in os.walk(framework_plugins_dir):
            if len(plugins) > 0:
                plugins_available = True

            for plugin in plugins:
                plugin_data = read_from_file(os.path.join(framework_plugins_dir, plugin))
                plugin_primary_resource_type = plugin_data["primary_resource_type"]

                results += evaluate_plugin_on_graph(plugin_data,
                                                    grouped_nodes_by_resource_type[plugin_primary_resource_type],
                                                    grouped_links_by_source, grouped_links_by_target,
                                                    framework, graph_nodes_by_id, cache_directory, args.include_ignored)

        if not plugins_available:
            LOGGER.info(f"No plugin available for {framework}")
...
def iac_pre_evaluation(cache_directory):
    ...
    if check_if_upsert_required(iac_config_data, global_config_data):
        ...
        download_iac_cache(iac_cache_dir_path, global_config_data["api_token"])
...
def iac_post_evaluation(args, results, cache_directory):
    ...
    if len(filtered_results) > 0:
        ...
        if args.pre_commit and not args.quiet and not args.verbose:
            LOGGER.warning("Please use --quiet/-q(recommended) or --verbose mode with pre-commit. By default, results are shown in quiet mode.")
            args.quiet = True
...
def download_iac_cache(iac_cache_dir_path, api_token):
...
def generate_issue_meta(node, plugin_data):
    blocks = node["block_name_"].split(".")
    return {
        "issue_meta": {
            "plugin_id": plugin_data["id"],
            "plugin_value": plugin_data["id"],
            "severity": plugin_data["severity"],
            "description": plugin_data["description"],
            "title": plugin_data["title"],
            "issue_message": plugin_data["issue_message"],
            "impact": plugin_data["impact"],
            "info_link": plugin_data["info_link"],
            "recommended_action": plugin_data["recommended_action"],
            "block_name": node["block_name_"],
            "file_path": node["file_path_"],
            "code_start_line": node["config_"][blocks[0]][blocks[1]]["__start_line__"],
            "code_end_line": node["config_"][blocks[0]][blocks[1]]["__end_line__"],
        },
        "internal": {
...
def print_issue_on_console(issues, quiet, verbose):
    ...
    for issue in issues:
        if quiet:
            print(LogColors.FAIL + f'[ISSUE]\tFound vulnerability inside {issue["block_name"]} for plugin id {issue["plugin_id"]}' + LogColors.ENDC)
        else:
