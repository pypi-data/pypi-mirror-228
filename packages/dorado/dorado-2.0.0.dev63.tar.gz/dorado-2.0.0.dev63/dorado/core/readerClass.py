...
class aico_reader(reader):
    ...
    def _multi_level_sub(self, subdir, stack_type):
        '''
        File doubling comes from mix directory type, and the fact
        that every type (apparently not lights tho) is regarded as 
        a mix directory due to non-stack_type directories and files 
        being present in the diread.
        '''
        ...
        if len(directories) == 0 & len(files) > 0:
            ...
            for d in directories:
                ...
                # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= #
                #
                # Check if previous and current list is same to prevent file doubling
                #
                # =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= #
    ...
    def _save_phi(self, cr, phi, save_series, save_base):
        ...
        if save_series:
            ...
            for p in range(len(fildat.data)):
                image = fildat.data[p]
                tstr  = str(Time(image.header['DATE-OBS'], format='fits', out_subfmt='ymdhms').value)
