...
class aicoPhot:
    ...
    def imarith(self, cr, filter, operator, operand, norm = False, norm_type = 0):
        '''
        ...
        norm_funcs = [np.mean, np.median, np.max, np.min, np.sum]
        if norm:
            norm_func = norm_funcs[norm_type]
            operand = operand / norm_func(operand)
        for i in range(len(series.data)):
            if norm:
                im = series.data[i].data / norm_func(series.data[i].data)
            else:
                im = series.data[i].data 

            if (operator == '+'):
                series.data[i].data = im  + operand
            elif (operator == '-'):
                series.data[i].data = im  - operand
            elif (operator == '/'):
                series.data[i].data = im  / operand
    ...
    def mkBase(self, cr, filter, sigClip = False, minmax = False, mmcmip = 0.1, mmcmap = 0.9):
        ...
        try:
            ...
            print('Couldn\'t force the base to uint16 dtype.')
        ...
        runm = (( time.time()  - start_time) // 60)
        runs = (((time.time()  - start_time) // 1 ) % 60)
        runt = str(int(runm))  + ':' + str(int(runs))
        print("--- Combination time : %s ---" % runt) 
        print('\n')
...
class dracoPhot:
    ...
    def apPhot(self, cr, filter, toid, search = True, read_date = None, star_im = None, log_out = True) :
        '''
        apPhot performs basic aperture photometry based on photutils.aperture_photometry on a target
        within stack. Target photometry can optionally be compared to a control target within the stack 
        via the differential photometry method.
        ...
        self.get_stars(cr, filter, toid, search, read_date, star_im = star_im)
    ...
    def get_field(self, cr, filter, toid, im = None):
        startTime = time.time()
        stack = Dorado.ceres[Dorado.ceres_keys[cr]].data[Dorado.ceres[Dorado.ceres_keys[cr]].filters[filter]]
        w = stack.wcs
        if im == None:
    ...
    def starSeeker(self, cr, filter, im = None):
        ...
        opt_img  = skie.rescale_intensity(limg, in_range=(low,high))
    ...
    def get_stars(self, cr, filter, toid, search, read_date, star_im = None):
        # add these back if needed :: , limit_Mag = 16, search_bounds = [30, 20]
        # ask if stars should be saved with  flag
        if search:
            gaia_stars = self.get_field(cr, filter, toid, im = star_im)
            s3, opt_img = self.starSeeker(cr, filter, im = star_im)
    ...
    def star_chart(self, stars, im, toid, w):
        ...
        try:
            ...
            print('Couldn\'t make a histogram.')
