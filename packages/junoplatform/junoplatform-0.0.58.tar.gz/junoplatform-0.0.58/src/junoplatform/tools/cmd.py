...
class CMDBase(object):
    def __init__(self):
        ...
        self.juno_cfg = {}
        try:
            self.juno_cfg = yaml.safe_load(
                open(self.juno_file, 'r', encoding='utf-8'))
        except:
            pass
...
@pass_base
@auth
def create(base: CMDBase, name, plant, module):
    ...
    try:
        ...
        with open(f"{name}/input.json", "w", encoding='utf-8') as f:
            json.dump(input, f, sort_keys=False)
...
def deploy_package(base: CMDBase, package_id: str, kind: int):
    ...
    if not package_id:
        try:
            ...
            logging.error(
                "must under project dir to run this command when no package_id provided(using project default)")
            exit(1)
    s = deploy_package(base, package_id, kind=0)
