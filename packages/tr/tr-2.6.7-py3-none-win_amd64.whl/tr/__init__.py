...
_dll_map = {"Linux":"libtr.so", "Windows": "libtr.dll", "Darwin": None}
_libc = ctypes.cdll.LoadLibrary(str(Path(__file__).parent.joinpath(_dll_map.get(platform.system()))))
...
class CRNN(object):
    ...
    def run(self, img):
        ...
        for i in range(outputs[0].shape[1]):
            val = float(outputs[0][0][i])
            idx = round(outputs[1][0][i])
            if idx <= 0: 
                pre_idx = idx
                continue
