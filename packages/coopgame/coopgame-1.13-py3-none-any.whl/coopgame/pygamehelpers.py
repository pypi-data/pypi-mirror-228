...
def draw_box(surface: pygame.Surface,
             ...
             vec_transform_getter: vec.VecTransformerGetter = None
             ):
    ...
    if anchor_draw_args:
        draw_points(
            ...
            vec_transformer_getter=vec_transform_getter
        )
    if corner_draw_args:
        draw_points(
            surface=surface,
...
def scaled_points_of_a_rect(rect,
                            vec_transformer_getter: vec.VecTransformerGetter) -> List[Vector2]:
    ''' get the rectangle object representing the grid position that was input'''
    return scaled_points(rect.Corners,
                         vec_transformer_getter=vec_transformer_getter)

def scaled_points(points: List[Vector2],
                  vec_transformer_getter: vec.VecTransformerGetter=None) -> List[Vector2]:
...
def viewport_point_on_plane(viewport_point: Vector2,
                            game_area_rect,
                            vec_transformer_getter:vec.VecTransformerGetter=None) -> vec.FloatVec:
