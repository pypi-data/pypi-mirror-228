...
class SQLConn:
    ...
    def __init__(self, server_creds, master=False, trusted_certificate=True):
    ...
    def open_read_connection_pl(self):
        """ Open a reading connection with the Server for a Polars connection
        :return: The opened connection
        """
        if 'windows.net' in self.server.lower():
            def_port = ':1433'
            if ':' in self.server:
                def_port = ''
            self.con_string_pl = 'mssql://' + self.user_name + ':' + parse.quote_plus(self.password) + \
                                 '@' + parse.quote_plus(self.server) + def_port + '/' + self.db_name + '?encrypt=true'
        else:
            if ':' not in self.server:
                raise ValueError("Please provide the port number to connect to the on-prem server")
            else:
                port_no = self.server.split(':')[-1]
                server_no = self.server.split(':')[0]
                if self.master:
                    self.con_string_pl = 'mssql://' + self.user_name + ':' + parse.quote_plus(self.password) + \
                                         '@' + parse.quote_plus(self.server) + ':' + port_no + \
                                         '/master?trusted_connection=true;encrypt=true'
                else:
                    self.con_string_pl = 'mssql://' + self.user_name + ':' + parse.quote_plus(self.password) + \
                                         '@' + parse.quote_plus(server_no) + ':' + port_no + \
                                         '/' + self.db_name + '?trusted_connection=true;encrypt=true'
    ...
    def query_pl(self, sql_query):
        """ Read data from SQL according to the sql_query or retrieve the first result from a stored procedure
        -----------------------------
        query_str = "SELECT * FROM %s" & table
        con_.query_pl(query_str)
        -----------------------------
        :param sql_query: Query to be sent to SQL
        :return: Polars DataFrame gathering the requested data
        """
        warn("This method doesn't work for python 3.11. "
             "We will look at implementing this when a new connectorx release is published",
             DeprecationWarning, stacklevel=2)

        self.open_read_connection_pl()
        data = None
        try:
            data = pl.read_sql(sql_query, self.con_string_pl)
        except ValueError:
            print(traceback.format_exc())
        return data
