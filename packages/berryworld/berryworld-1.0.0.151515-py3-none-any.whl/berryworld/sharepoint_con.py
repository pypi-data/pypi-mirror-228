...
class SharepointConnection:
    ...
    def __init__(self, env_name=None, api_version=None):
        ...
        try:
            ...
            self.client_id = os.environ.get(f"SHAREPOINT_CLIENT_ID_{self.env_name.upper()}")
            self.scopes = ['Sites.ReadWrite.All', 'Files.ReadWrite.All']
            self.organisation_id = os.environ.get(f"SHAREPOINT_ORG_{self.env_name.upper()}")
            self.username = os.environ.get(f"SHAREPOINT_USER_{self.env_name.upper()}")
            self.password = os.environ.get(f"SHAREPOINT_PASSWORD_{self.env_name.upper()}")
            self.site_id = os.environ.get(f"SHAREPOINT_SITE_ID_{self.env_name.upper()}")
            self.site_name = os.environ.get(f"SHAREPOINT_SITE_NAME_{self.env_name.upper()}")
    ...
    def list_items(self, path, file_type='All'):
        ...
        path = path.rstrip("/")
        try:
            folders = [pat for pat in os.path.split(path) if pat != '']
            try:
    ...
    def get_file_content(self, file_path):
        ...
        try:
            folders = [path for path in os.path.split(file_path) if path != '']
            if len(folders) < 2:
    ...
    def create_folder(self, folder_path):
        ...
        try:
            folders = [path for path in os.path.split(folder_path) if path != '']
            if len(folders) < 2:
                raise Exception("Please provide a valid full path to the file")
