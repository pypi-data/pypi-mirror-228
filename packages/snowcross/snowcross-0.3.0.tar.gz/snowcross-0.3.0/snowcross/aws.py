...
def run_workflow(name: str) -> str:
    ...
    """
    return _client("glue").start_workflow_run(Name=name).get("RunId")


def check_workflow_completed_state(name: str, run_id: str) -> bool:
    """Check if a workflow run has completed.
    ...
    Returns:
        bool: Workflow run completed.
    """
    workflow_run_resp = _client("glue").get_workflow_run(
        Name=name,
        RunId=run_id,
    )
