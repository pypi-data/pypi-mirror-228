...
def get_alt_override(node, filename, lang):
    mpath, module = node.info['directory'].rsplit('/', 1)
    if module in {'res', 'ir'}:
        mpath = OPJ(mpath, 'modules')
    alt_override_module = '%s_%s' % (
        'translation_override', lang)
    alt_file_name = '%s_%s.po' % (module, lang)
    alt_file = OPJ(mpath, alt_override_module, 'locale',
        'override', alt_file_name)
...
def load_translations(pool, node, languages):
    ...
    for filename in itertools.chain(
        ...
        alt_override = get_alt_override(node, filename, lang)
        if alt_override:
            lang2filenames[lang].append(alt_override)
