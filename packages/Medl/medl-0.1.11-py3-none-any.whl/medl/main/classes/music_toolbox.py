...
from medl.common import BaseLogger, SongData, PlaylistData, BaseInitializer
...
class MusicToolbox:
    ...
    def download(self, tracks: List[SongData]) -> List[Path]:
        self._check_init()
        (tracks, dled) = self._pre_download_handler.exec(tracks)

        paths = self._parallelizer.run_in_parallel(self._download_track, tracks)
        self._post_download_handler.exec(tracks)

        return [*(p for p in paths if p), *dled]

    def _get_tracks(self, query: str) -> List[SongData]:
        self._check_init()
        data = self._metadata_fetcher.fetch(query)

        if data is None:
            return []
        elif isinstance(data, SongData):
            return [data]
        else:
            return data.tracks

    def _download_track(self, track_data: SongData) -> Optional[Path]:
