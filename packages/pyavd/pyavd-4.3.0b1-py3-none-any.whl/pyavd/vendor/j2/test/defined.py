...
def defined(value, test_value=None, var_type=None, fail_action=None, var_name=None, run_tests=False):
    ...
    if isinstance(value, Undefined) or value is None:
        ...
        if str(fail_action).lower() == "warning":
            ...
            if var_name is not None:
                raise AristaAvdError(f"{var_name} was a {type(value).__name__} but we expected a {str(var_type).lower()}!")
            else:
                raise AristaAvdError(f"A variable was a {type(value).__name__} but we expected a {str(var_type).lower()}!")
        if run_tests:
            return False, display._warns
        return False
    else:
