# -*- coding: utf-8 -*-
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: qwak/deployment/deployment.proto
"""Generated protocol buffer code."""
from google.protobuf.internal import enum_type_wrapper
from google.protobuf import descriptor as _descriptor
from google.protobuf import descriptor_pool as _descriptor_pool
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()


from google.protobuf import timestamp_pb2 as google_dot_protobuf_dot_timestamp__pb2
from google.protobuf import wrappers_pb2 as google_dot_protobuf_dot_wrappers__pb2
from _qwak_proto.qwak.audience.v1 import audience_pb2 as qwak_dot_audience_dot_v1_dot_audience__pb2
from _qwak_proto.qwak.auto_scaling.v1 import auto_scaling_pb2 as qwak_dot_auto__scaling_dot_v1_dot_auto__scaling__pb2
from _qwak_proto.qwak.user_application.common.v0 import resources_pb2 as qwak_dot_user__application_dot_common_dot_v0_dot_resources__pb2


DESCRIPTOR = _descriptor_pool.Default().AddSerializedFile(b'\n qwak/deployment/deployment.proto\x12\x1aqwak.deployment.management\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x1egoogle/protobuf/wrappers.proto\x1a\x1fqwak/audience/v1/audience.proto\x1a\'qwak/auto_scaling/v1/auto_scaling.proto\x1a/qwak/user_application/common/v0/resources.proto\"\x9e\x01\n\x0eHostingService\x12>\n\tsagemaker\x18\x01 \x01(\x0b\x32%.qwak.deployment.management.SageMakerB\x02\x18\x01H\x00\x12\x45\n\x0fkube_deployment\x18\x02 \x01(\x0b\x32*.qwak.deployment.management.KubeDeploymentH\x00\x42\x05\n\x03val\"\xff\x03\n\x0eKubeDeployment\x12\x15\n\tpod_count\x18\x04 \x01(\x05\x42\x02\x18\x01\x12\x43\n\x0f\x64\x65ployment_size\x18\x05 \x01(\x0b\x32*.qwak.deployment.management.DeploymentSize\x12Z\n\x1b\x61\x64vanced_deployment_options\x18\x06 \x01(\x0b\x32\x35.qwak.deployment.management.AdvancedDeploymentOptions\x12L\n\x14kube_deployment_type\x18\x07 \x01(\x0e\x32..qwak.deployment.management.KubeDeploymentType\x12\x45\n\x10serving_strategy\x18\t \x01(\x0b\x32+.qwak.deployment.management.ServingStrategy\x12\x63\n\x15\x65nvironment_variables\x18\n \x03(\x0b\x32\x44.qwak.deployment.management.KubeDeployment.EnvironmentVariablesEntry\x1a;\n\x19\x45nvironmentVariablesEntry\x12\x0b\n\x03key\x18\x01 \x01(\t\x12\r\n\x05value\x18\x02 \x01(\t:\x02\x38\x01\"\xe8\x01\n\x0fServingStrategy\x12\x45\n\x0frealtime_config\x18\x01 \x01(\x0b\x32*.qwak.deployment.management.RealTimeConfigH\x00\x12\x41\n\rstream_config\x18\x02 \x01(\x0b\x32(.qwak.deployment.management.StreamConfigH\x00\x12?\n\x0c\x62\x61tch_config\x18\x03 \x01(\x0b\x32\'.qwak.deployment.management.BatchConfigH\x00\x42\n\n\x08Strategy\"\x99\x01\n\x0eRealTimeConfig\x12\x41\n\x0etraffic_config\x18\x01 \x01(\x0b\x32).qwak.deployment.management.TrafficConfig\x12\x44\n\x13\x61uto_scaling_config\x18\x02 \x01(\x0b\x32\'.qwak.auto_scaling.v1.AutoScalingConfig\"\xf9\x01\n\rTrafficConfig\x12\x1f\n\x17selected_variation_name\x18\x01 \x01(\t\x12\x39\n\nvariations\x18\x02 \x03(\x0b\x32%.qwak.deployment.management.Variation\x12\x46\n\x17\x61udience_routes_entries\x18\x03 \x03(\x0b\x32%.qwak.audience.v1.AudienceRoutesEntry\x12\x1a\n\x12\x66\x61llback_variation\x18\x04 \x01(\t\x12(\n selected_variation_protect_state\x18\x05 \x01(\x08\"F\n\x0cStreamConfig\x12\x36\n\x05kafka\x18\x01 \x01(\x0b\x32\'.qwak.deployment.management.KafkaConfig\"\r\n\x0b\x42\x61tchConfig\"\xaf\x05\n\x0bKafkaConfig\x12\x42\n\x08\x63onsumer\x18\x01 \x01(\x0b\x32\x30.qwak.deployment.management.KafkaConfig.Consumer\x12\x42\n\x08producer\x18\x02 \x01(\x0b\x32\x30.qwak.deployment.management.KafkaConfig.Producer\x12\x0f\n\x07workers\x18\x03 \x01(\x05\x1a\x99\x02\n\x08\x43onsumer\x12\x18\n\x10\x62ootstrap_server\x18\x01 \x03(\t\x12\r\n\x05topic\x18\x02 \x01(\t\x12\r\n\x05group\x18\x03 \x01(\t\x12\x0f\n\x07timeout\x18\x04 \x01(\x05\x12Y\n\x10\x61uto_offset_type\x18\x05 \x01(\x0e\x32?.qwak.deployment.management.KafkaConfig.Consumer.AutoOffSetType\x12\x16\n\x0emax_batch_size\x18\x06 \x01(\x05\x12\x18\n\x10max_poll_latency\x18\x07 \x01(\x01\"7\n\x0e\x41utoOffSetType\x12\x0b\n\x07UNKNOWN\x10\x00\x12\x0c\n\x08\x45\x41RLIEST\x10\x01\x12\n\n\x06LATEST\x10\x02\x1a\xea\x01\n\x08Producer\x12\x18\n\x10\x62ootstrap_server\x18\x01 \x03(\t\x12\r\n\x05topic\x18\x02 \x01(\t\x12Z\n\x10\x63ompression_type\x18\x03 \x01(\x0e\x32@.qwak.deployment.management.KafkaConfig.Producer.CompressionType\"Y\n\x0f\x43ompressionType\x12\x0b\n\x07UNKNOWN\x10\x00\x12\x10\n\x0cUNCOMPRESSED\x10\x01\x12\x08\n\x04GZIP\x10\x02\x12\n\n\x06SNAPPY\x10\x03\x12\x07\n\x03LZ4\x10\x04\x12\x08\n\x04ZSTD\x10\x05\":\n\tSageMaker\x12\x16\n\x0einstance_count\x18\x04 \x01(\x05\x12\x15\n\rinstance_type\x18\x05 \x01(\t\"\xc2\x02\n\x0e\x44\x65ploymentSize\x12\x16\n\x0enumber_of_pods\x18\x01 \x01(\x05\x12\x0f\n\x03\x63pu\x18\x02 \x01(\x02\x42\x02\x18\x01\x12\x19\n\rmemory_amount\x18\x03 \x01(\x05\x42\x02\x18\x01\x12@\n\x0cmemory_units\x18\x04 \x01(\x0e\x32&.qwak.deployment.management.MemoryUnitB\x02\x18\x01\x12H\n\rgpu_resources\x18\x05 \x01(\x0b\x32-.qwak.user_application.common.v0.GpuResourcesB\x02\x18\x01\x12`\n\x1c\x63lient_pod_compute_resources\x18\x06 \x01(\x0b\x32:.qwak.user_application.common.v0.ClientPodComputeResources\"Y\n\x0cGpuResources\x12\x35\n\x08gpu_type\x18\x01 \x01(\x0e\x32#.qwak.deployment.management.GpuType\x12\x12\n\ngpu_amount\x18\x02 \x01(\x05\"\x8c\x02\n\x19\x41\x64vancedDeploymentOptions\x12%\n\x1dnumber_of_http_server_workers\x18\x01 \x01(\x05\x12\x1f\n\x17http_request_timeout_ms\x18\x02 \x01(\x05\x12/\n\x0b\x64\x61\x65mon_mode\x18\x03 \x01(\x0b\x32\x1a.google.protobuf.BoolValue\x12\x1b\n\x13\x63ustom_iam_role_arn\x18\x04 \x01(\t\x12\x16\n\x0emax_batch_size\x18\x05 \x01(\x05\x12(\n deployment_process_timeout_limit\x18\x06 \x01(\x05\x12\x17\n\x0fpurchase_option\x18\x07 \x01(\t\"\xc9\x08\n\x11\x44\x65ploymentDetails\x12\x31\n\rlast_deployed\x18\x01 \x01(\x0b\x32\x1a.google.protobuf.Timestamp\x12\x10\n\x08\x62uild_id\x18\x02 \x01(\t\x12\x16\n\x0enumber_of_pods\x18\x03 \x01(\x05\x12\x14\n\x0c\x63pu_fraction\x18\x04 \x01(\x02\x12\x15\n\rmemory_amount\x18\x05 \x01(\x05\x12<\n\x0cmemory_units\x18\x06 \x01(\x0e\x32&.qwak.deployment.management.MemoryUnit\x12\x19\n\x11number_of_workers\x18\x07 \x01(\x05\x12\x1f\n\x17http_request_timeout_ms\x18\x08 \x01(\x05\x12L\n\x14kube_deployment_type\x18\t \x01(\x0e\x32..qwak.deployment.management.KubeDeploymentType\x12\x45\n\x10serving_strategy\x18\n \x01(\x0b\x32+.qwak.deployment.management.ServingStrategy\x12\x38\n\tvariation\x18\x0b \x01(\x0b\x32%.qwak.deployment.management.Variation\x12\x1b\n\x13\x63ustom_iam_role_arn\x18\x0c \x01(\t\x12\x16\n\x0emax_batch_size\x18\r \x01(\x05\x12:\n\x08gpu_type\x18\x0e \x01(\x0e\x32(.qwak.user_application.common.v0.GpuType\x12\x12\n\ngpu_amount\x18\x0f \x01(\x05\x12\x16\n\x0e\x65nvironment_id\x18\x10 \x01(\t\x12\x1a\n\x12\x61vailable_replicas\x18\x11 \x01(\x05\x12/\n\x0b\x64\x61\x65mon_mode\x18\x12 \x01(\x0b\x32\x1a.google.protobuf.BoolValue\x12(\n deployment_process_timeout_limit\x18\x13 \x01(\x05\x12\x17\n\x0fpurchase_option\x18\x14 \x01(\t\x12\x66\n\x15\x65nvironment_variables\x18\x15 \x03(\x0b\x32G.qwak.deployment.management.DeploymentDetails.EnvironmentVariablesEntry\x12\x35\n\x11undeployment_time\x18\x16 \x01(\x0b\x32\x1a.google.protobuf.Timestamp\x12\x41\n\x06status\x18\x17 \x01(\x0e\x32\x31.qwak.deployment.management.ModelDeploymentStatus\x12\x15\n\rdeployment_id\x18\x18 \x01(\t\x1a;\n\x19\x45nvironmentVariablesEntry\x12\x0b\n\x03key\x18\x01 \x01(\t\x12\r\n\x05value\x18\x02 \x01(\t:\x02\x38\x01\"\xb9\x01\n\x19RuntimeDeploymentSettings\x12Y\n\x11root_logger_level\x18\x01 \x01(\x0e\x32>.qwak.deployment.management.RuntimeDeploymentSettings.LogLevel\"A\n\x08LogLevel\x12\x0b\n\x07NOT_SET\x10\x00\x12\t\n\x05\x45RROR\x10\x01\x12\x08\n\x04WARN\x10\x02\x12\x08\n\x04INFO\x10\x03\x12\t\n\x05\x44\x45\x42UG\x10\x04\"\xee\x02\n\x0f\x44\x65ploymentBrief\x12\x10\n\x08\x62uild_id\x18\x01 \x01(\t\x12\x41\n\x06status\x18\x02 \x01(\x0e\x32\x31.qwak.deployment.management.ModelDeploymentStatus\x12\x13\n\x0b\x64\x65ployed_by\x18\x03 \x01(\t\x12\x1b\n\x13\x66\x61ilure_reason_code\x18\x04 \x01(\t\x12\x17\n\x0f\x66\x61ilure_message\x18\x05 \x01(\t\x12\x19\n\x11technical_details\x18\x06 \x01(\t\x12\x38\n\x14\x64\x65ployment_timestamp\x18\x07 \x01(\x0b\x32\x1a.google.protobuf.Timestamp\x12\x16\n\x0e\x65nvironment_id\x18\x08 \x01(\t\x12\x16\n\x0evariation_name\x18\t \x01(\t\x12\x1f\n\x17variation_protect_state\x18\n \x01(\x08\x12\x15\n\rdeployment_id\x18\x0b \x01(\t\"\xd7\x02\n\x1eKubernetesModelDeploymentState\x12\x10\n\x08model_id\x18\x01 \x01(\t\x12\x15\n\tbranch_id\x18\x02 \x01(\tB\x02\x18\x01\x12\x41\n\x06status\x18\x03 \x01(\x0e\x32\x31.qwak.deployment.management.ModelDeploymentStatus\x12\x41\n\x0estatus_details\x18\x04 \x01(\x0b\x32).qwak.deployment.management.StatusDetails\x12\x16\n\x0evariation_name\x18\x05 \x01(\t\x12\x15\n\rdeployment_id\x18\x06 \x01(\t\x12\x12\n\nmodel_uuid\x18\x07 \x01(\t\x12\x43\n\nmanaged_by\x18\x08 \x01(\x0e\x32/.qwak.deployment.management.DeploymentManagedBy\"\x9e\x02\n\rStatusDetails\x12R\n\x16pending_status_details\x18\x01 \x01(\x0b\x32\x30.qwak.deployment.management.PendingStatusDetailsH\x00\x12U\n\x16success_status_details\x18\x02 \x01(\x0b\x32\x33.qwak.deployment.management.SuccessfulStatusDetailsH\x00\x12P\n\x15\x66\x61iled_status_details\x18\x03 \x01(\x0b\x32/.qwak.deployment.management.FailedStatusDetailsH\x00\x42\x10\n\x0estatus_details\"\x9c\x01\n\x14PendingStatusDetails\x12\x41\n\x0cold_instance\x18\x01 \x01(\x0b\x32+.qwak.deployment.management.InstanceDetails\x12\x41\n\x0cnew_instance\x18\x02 \x01(\x0b\x32+.qwak.deployment.management.InstanceDetails\"`\n\x17SuccessfulStatusDetails\x12\x45\n\x10running_instance\x18\x01 \x01(\x0b\x32+.qwak.deployment.management.InstanceDetails\"\xe3\x01\n\x13\x46\x61iledStatusDetails\x12\x43\n\x0f\x66\x61ilure_details\x18\x01 \x01(\x0b\x32*.qwak.deployment.management.FailureDetails\x12\x41\n\x0cold_instance\x18\x02 \x01(\x0b\x32+.qwak.deployment.management.InstanceDetails\x12\x44\n\x0f\x66\x61iled_instance\x18\x03 \x01(\x0b\x32+.qwak.deployment.management.InstanceDetails\"\x91\x01\n\x0e\x46\x61ilureDetails\x12_\n\x1e\x64\x65ployment_failure_reason_code\x18\x01 \x01(\x0e\x32\x37.qwak.deployment.management.DeploymentFailureReasonCode\x12\x1e\n\x16\x66\x61ilure_reason_details\x18\x02 \x01(\t\"V\n\x0fInstanceDetails\x12\x15\n\rdeployment_id\x18\x01 \x01(\t\x12\x10\n\x08\x62uild_id\x18\x02 \x01(\t\x12\x1a\n\x12\x61vailable_replicas\x18\x03 \x01(\x05\"\x88\x01\n\tVariation\x12\x0c\n\x04name\x18\x01 \x01(\t\x12\x16\n\x0e\x65nvironment_id\x18\x03 \x01(\t\x12:\n\x07traffic\x18\x02 \x01(\x0b\x32\'.qwak.deployment.management.TrafficSpecH\x00\x12\x11\n\tprotected\x18\x04 \x01(\x08\x42\x06\n\x04spec\"4\n\x0bTrafficSpec\x12\x12\n\npercentage\x18\x01 \x01(\x05\x12\x11\n\tis_shadow\x18\x02 \x01(\x08\"]\n\x18\x46utureDeploymentSettings\x12\x41\n\x0etraffic_config\x18\x01 \x01(\x0b\x32).qwak.deployment.management.TrafficConfig\"\xb6\x01\n\x1c\x45nvironmentDeploymentMessage\x12\x10\n\x08model_id\x18\x01 \x01(\t\x12\x15\n\tbranch_id\x18\x02 \x01(\tB\x02\x18\x01\x12\x10\n\x08\x62uild_id\x18\x03 \x01(\t\x12\x43\n\x0fhosting_service\x18\x04 \x01(\x0b\x32*.qwak.deployment.management.HostingService\x12\x16\n\nmodel_uuid\x18\x05 \x01(\tB\x02\x18\x01\"\x92\x01\n\"EnvironmentDeploymentResultMessage\x12\x1b\n\x13\x64\x65ployment_named_id\x18\x01 \x01(\t\x12\x41\n\x06status\x18\x02 \x01(\x0e\x32\x31.qwak.deployment.management.ModelDeploymentStatus\x12\x0c\n\x04info\x18\x03 \x01(\t\"\x90\x02\n\x1e\x45nvironmentUndeploymentMessage\x12\x10\n\x08model_id\x18\x01 \x01(\t\x12\x15\n\tbranch_id\x18\x02 \x01(\tB\x02\x18\x01\x12V\n\x14hosting_service_type\x18\x03 \x01(\x0e\x32\x38.qwak.deployment.management.DeploymentHostingServiceType\x12\x41\n\x0etraffic_config\x18\x04 \x01(\x0b\x32).qwak.deployment.management.TrafficConfig\x12\x16\n\x0evariation_name\x18\x05 \x01(\t\x12\x12\n\nmodel_uuid\x18\x06 \x01(\t\"w\n$EnvironmentUndeploymentResultMessage\x12\x41\n\x06status\x18\x01 \x01(\x0e\x32\x31.qwak.deployment.management.ModelDeploymentStatus\x12\x0c\n\x04info\x18\x02 \x01(\t\"\xd6\x01\n+EnvironmentRuntimeDeploymentSettingsMessage\x12\x10\n\x08model_id\x18\x01 \x01(\t\x12\x15\n\tbranch_id\x18\x02 \x01(\tB\x02\x18\x01\x12R\n\x13\x64\x65ployment_settings\x18\x03 \x01(\x0b\x32\x35.qwak.deployment.management.RuntimeDeploymentSettings\x12\x16\n\x0evariation_name\x18\x04 \x01(\t\x12\x12\n\nmodel_uuid\x18\x05 \x01(\t\"G\n1EnvironmentRuntimeDeploymentSettingsResultMessage\x12\x12\n\nerror_info\x18\x01 \x01(\t\"4\n$EnvironmentApplyModelTrafficResponse\x12\x0c\n\x04info\x18\x01 \x01(\t\"\xce\x01\n#EnvironmentDeploymentDetailsMessage\x12J\n\x13\x64\x65ployments_details\x18\x01 \x03(\x0b\x32-.qwak.deployment.management.DeploymentDetails\x12?\n\x10\x61udiences_routes\x18\x02 \x03(\x0b\x32%.qwak.audience.v1.AudienceRoutesEntry\x12\x1a\n\x12\x66\x61llback_variation\x18\x03 \x01(\t\"g\n#EnvironmentDeploymentHistoryMessage\x12@\n\x0b\x64\x65ployments\x18\x01 \x03(\x0b\x32+.qwak.deployment.management.DeploymentBrief\"V\n\x19\x45nvironmentTrafficMessage\x12\x39\n\nvariations\x18\x01 \x03(\x0b\x32%.qwak.deployment.management.Variation\"\x9e\x02\n BuildDeploymentEnvironmentStatus\x12\x87\x01\n\x1f\x65nvironment_to_deployment_brief\x18\x01 \x03(\x0b\x32^.qwak.deployment.management.BuildDeploymentEnvironmentStatus.EnvironmentToDeploymentBriefEntry\x1ap\n!EnvironmentToDeploymentBriefEntry\x12\x0b\n\x03key\x18\x01 \x01(\t\x12:\n\x05value\x18\x02 \x01(\x0b\x32+.qwak.deployment.management.DeploymentBrief:\x02\x38\x01\"@\n\x15\x42uildDeploymentStatus\x12\x12\n\nbuild_uuid\x18\x01 \x01(\t\x12\x13\n\x0bis_deployed\x18\x02 \x01(\x08*\x85\x03\n\x15ModelDeploymentStatus\x12\x16\n\x12INVALID_DEPLOYMENT\x10\x00\x12\x19\n\x15INITIATING_DEPLOYMENT\x10\x01\x12 \n\x1c\x46\x41ILED_INITIATING_DEPLOYMENT\x10\x08\x12\x16\n\x12PENDING_DEPLOYMENT\x10\x02\x12\x19\n\x15SUCCESSFUL_DEPLOYMENT\x10\x03\x12\x15\n\x11\x46\x41ILED_DEPLOYMENT\x10\x04\x12\x1b\n\x17INITIATING_UNDEPLOYMENT\x10\t\x12\x18\n\x14PENDING_UNDEPLOYMENT\x10\n\x12\x1b\n\x17SUCCESSFUL_UNDEPLOYMENT\x10\x05\x12\x17\n\x13\x46\x41ILED_UNDEPLOYMENT\x10\x06\x12\t\n\x05UNSET\x10\x07\x12\x19\n\x15\x41LL_BUILDS_UNDEPLOYED\x10\x0b\x12\x18\n\x14\x43\x41NCELLED_DEPLOYMENT\x10\x0c\x12 \n\x1cINITIATING_CANCEL_DEPLOYMENT\x10\r*O\n\x1c\x44\x65ploymentHostingServiceType\x12\x0b\n\x07INVALID\x10\x00\x12\r\n\tSAGEMAKER\x10\x01\x12\x13\n\x0fKUBE_DEPLOYMENT\x10\x02*F\n\x12KubeDeploymentType\x12\r\n\tUNDEFINED\x10\x00\x12\n\n\x06ONLINE\x10\x01\x12\n\n\x06STREAM\x10\x02\x12\t\n\x05\x42\x41TCH\x10\x03*l\n\x07GpuType\x12\x0f\n\x0bINVALID_GPU\x10\x00\x12\x0e\n\nNVIDIA_K80\x10\x01\x12\x0f\n\x0bNVIDIA_V100\x10\x02\x12\x0f\n\x0bNVIDIA_A100\x10\x03\x12\r\n\tNVIDIA_T4\x10\x04\x12\x0f\n\x0bNVIDIA_A10G\x10\x05*+\n\nMemoryUnit\x12\x0b\n\x07UNKNOWN\x10\x00\x12\x07\n\x03MIB\x10\x01\x12\x07\n\x03GIB\x10\x02*\x83\x01\n\x13\x44\x65ploymentManagedBy\x12!\n\x1d\x44\x45PLOYMENT_MANAGED_BY_INVALID\x10\x00\x12&\n\"DEPLOYMENT_MANAGED_BY_KUBE_CAPTAIN\x10\x01\x12!\n\x1d\x44\x45PLOYMENT_MANAGED_BY_ADMIRAL\x10\x02*\x93\x02\n\x1b\x44\x65ploymentFailureReasonCode\x12\x18\n\x14INVALID_FAILURE_CODE\x10\x00\x12\x12\n\x0eUNKNOWN_REASON\x10\x01\x12\x1e\n\x1aNO_REPLICA_SETS_PODS_FOUND\x10\x02\x12\x1d\n\x19MODEL_CONTAINER_NOT_FOUND\x10\x03\x12\x11\n\rUNSCHEDULABLE\x10\x04\x12\x1a\n\x16\x44OCKER_IMAGE_NOT_FOUND\x10\x05\x12\x19\n\x15MEMORY_LIMIT_EXCEEDED\x10\x06\x12\x0e\n\nCRASH_LOOP\x10\x07\x12-\n)CONTAINER_FAIL_TO_LOAD_FOR_UNKNOWN_REASON\x10\x08\x42\x45\n%com.qwak.ai.management.deployment.apiP\x01Z\x1aqwak/deployment;deploymentb\x06proto3')

_MODELDEPLOYMENTSTATUS = DESCRIPTOR.enum_types_by_name['ModelDeploymentStatus']
ModelDeploymentStatus = enum_type_wrapper.EnumTypeWrapper(_MODELDEPLOYMENTSTATUS)
_DEPLOYMENTHOSTINGSERVICETYPE = DESCRIPTOR.enum_types_by_name['DeploymentHostingServiceType']
DeploymentHostingServiceType = enum_type_wrapper.EnumTypeWrapper(_DEPLOYMENTHOSTINGSERVICETYPE)
_KUBEDEPLOYMENTTYPE = DESCRIPTOR.enum_types_by_name['KubeDeploymentType']
KubeDeploymentType = enum_type_wrapper.EnumTypeWrapper(_KUBEDEPLOYMENTTYPE)
_GPUTYPE = DESCRIPTOR.enum_types_by_name['GpuType']
GpuType = enum_type_wrapper.EnumTypeWrapper(_GPUTYPE)
_MEMORYUNIT = DESCRIPTOR.enum_types_by_name['MemoryUnit']
MemoryUnit = enum_type_wrapper.EnumTypeWrapper(_MEMORYUNIT)
_DEPLOYMENTMANAGEDBY = DESCRIPTOR.enum_types_by_name['DeploymentManagedBy']
DeploymentManagedBy = enum_type_wrapper.EnumTypeWrapper(_DEPLOYMENTMANAGEDBY)
_DEPLOYMENTFAILUREREASONCODE = DESCRIPTOR.enum_types_by_name['DeploymentFailureReasonCode']
DeploymentFailureReasonCode = enum_type_wrapper.EnumTypeWrapper(_DEPLOYMENTFAILUREREASONCODE)
INVALID_DEPLOYMENT = 0
INITIATING_DEPLOYMENT = 1
FAILED_INITIATING_DEPLOYMENT = 8
PENDING_DEPLOYMENT = 2
SUCCESSFUL_DEPLOYMENT = 3
FAILED_DEPLOYMENT = 4
INITIATING_UNDEPLOYMENT = 9
PENDING_UNDEPLOYMENT = 10
SUCCESSFUL_UNDEPLOYMENT = 5
FAILED_UNDEPLOYMENT = 6
UNSET = 7
ALL_BUILDS_UNDEPLOYED = 11
CANCELLED_DEPLOYMENT = 12
INITIATING_CANCEL_DEPLOYMENT = 13
INVALID = 0
SAGEMAKER = 1
KUBE_DEPLOYMENT = 2
UNDEFINED = 0
ONLINE = 1
STREAM = 2
BATCH = 3
INVALID_GPU = 0
NVIDIA_K80 = 1
NVIDIA_V100 = 2
NVIDIA_A100 = 3
NVIDIA_T4 = 4
NVIDIA_A10G = 5
UNKNOWN = 0
MIB = 1
GIB = 2
DEPLOYMENT_MANAGED_BY_INVALID = 0
DEPLOYMENT_MANAGED_BY_KUBE_CAPTAIN = 1
DEPLOYMENT_MANAGED_BY_ADMIRAL = 2
INVALID_FAILURE_CODE = 0
UNKNOWN_REASON = 1
NO_REPLICA_SETS_PODS_FOUND = 2
MODEL_CONTAINER_NOT_FOUND = 3
UNSCHEDULABLE = 4
DOCKER_IMAGE_NOT_FOUND = 5
MEMORY_LIMIT_EXCEEDED = 6
CRASH_LOOP = 7
CONTAINER_FAIL_TO_LOAD_FOR_UNKNOWN_REASON = 8


_HOSTINGSERVICE = DESCRIPTOR.message_types_by_name['HostingService']
_KUBEDEPLOYMENT = DESCRIPTOR.message_types_by_name['KubeDeployment']
_KUBEDEPLOYMENT_ENVIRONMENTVARIABLESENTRY = _KUBEDEPLOYMENT.nested_types_by_name['EnvironmentVariablesEntry']
_SERVINGSTRATEGY = DESCRIPTOR.message_types_by_name['ServingStrategy']
_REALTIMECONFIG = DESCRIPTOR.message_types_by_name['RealTimeConfig']
_TRAFFICCONFIG = DESCRIPTOR.message_types_by_name['TrafficConfig']
_STREAMCONFIG = DESCRIPTOR.message_types_by_name['StreamConfig']
_BATCHCONFIG = DESCRIPTOR.message_types_by_name['BatchConfig']
_KAFKACONFIG = DESCRIPTOR.message_types_by_name['KafkaConfig']
_KAFKACONFIG_CONSUMER = _KAFKACONFIG.nested_types_by_name['Consumer']
_KAFKACONFIG_PRODUCER = _KAFKACONFIG.nested_types_by_name['Producer']
_SAGEMAKER = DESCRIPTOR.message_types_by_name['SageMaker']
_DEPLOYMENTSIZE = DESCRIPTOR.message_types_by_name['DeploymentSize']
_GPURESOURCES = DESCRIPTOR.message_types_by_name['GpuResources']
_ADVANCEDDEPLOYMENTOPTIONS = DESCRIPTOR.message_types_by_name['AdvancedDeploymentOptions']
_DEPLOYMENTDETAILS = DESCRIPTOR.message_types_by_name['DeploymentDetails']
_DEPLOYMENTDETAILS_ENVIRONMENTVARIABLESENTRY = _DEPLOYMENTDETAILS.nested_types_by_name['EnvironmentVariablesEntry']
_RUNTIMEDEPLOYMENTSETTINGS = DESCRIPTOR.message_types_by_name['RuntimeDeploymentSettings']
_DEPLOYMENTBRIEF = DESCRIPTOR.message_types_by_name['DeploymentBrief']
_KUBERNETESMODELDEPLOYMENTSTATE = DESCRIPTOR.message_types_by_name['KubernetesModelDeploymentState']
_STATUSDETAILS = DESCRIPTOR.message_types_by_name['StatusDetails']
_PENDINGSTATUSDETAILS = DESCRIPTOR.message_types_by_name['PendingStatusDetails']
_SUCCESSFULSTATUSDETAILS = DESCRIPTOR.message_types_by_name['SuccessfulStatusDetails']
_FAILEDSTATUSDETAILS = DESCRIPTOR.message_types_by_name['FailedStatusDetails']
_FAILUREDETAILS = DESCRIPTOR.message_types_by_name['FailureDetails']
_INSTANCEDETAILS = DESCRIPTOR.message_types_by_name['InstanceDetails']
_VARIATION = DESCRIPTOR.message_types_by_name['Variation']
_TRAFFICSPEC = DESCRIPTOR.message_types_by_name['TrafficSpec']
_FUTUREDEPLOYMENTSETTINGS = DESCRIPTOR.message_types_by_name['FutureDeploymentSettings']
_ENVIRONMENTDEPLOYMENTMESSAGE = DESCRIPTOR.message_types_by_name['EnvironmentDeploymentMessage']
_ENVIRONMENTDEPLOYMENTRESULTMESSAGE = DESCRIPTOR.message_types_by_name['EnvironmentDeploymentResultMessage']
_ENVIRONMENTUNDEPLOYMENTMESSAGE = DESCRIPTOR.message_types_by_name['EnvironmentUndeploymentMessage']
_ENVIRONMENTUNDEPLOYMENTRESULTMESSAGE = DESCRIPTOR.message_types_by_name['EnvironmentUndeploymentResultMessage']
_ENVIRONMENTRUNTIMEDEPLOYMENTSETTINGSMESSAGE = DESCRIPTOR.message_types_by_name['EnvironmentRuntimeDeploymentSettingsMessage']
_ENVIRONMENTRUNTIMEDEPLOYMENTSETTINGSRESULTMESSAGE = DESCRIPTOR.message_types_by_name['EnvironmentRuntimeDeploymentSettingsResultMessage']
_ENVIRONMENTAPPLYMODELTRAFFICRESPONSE = DESCRIPTOR.message_types_by_name['EnvironmentApplyModelTrafficResponse']
_ENVIRONMENTDEPLOYMENTDETAILSMESSAGE = DESCRIPTOR.message_types_by_name['EnvironmentDeploymentDetailsMessage']
_ENVIRONMENTDEPLOYMENTHISTORYMESSAGE = DESCRIPTOR.message_types_by_name['EnvironmentDeploymentHistoryMessage']
_ENVIRONMENTTRAFFICMESSAGE = DESCRIPTOR.message_types_by_name['EnvironmentTrafficMessage']
_BUILDDEPLOYMENTENVIRONMENTSTATUS = DESCRIPTOR.message_types_by_name['BuildDeploymentEnvironmentStatus']
_BUILDDEPLOYMENTENVIRONMENTSTATUS_ENVIRONMENTTODEPLOYMENTBRIEFENTRY = _BUILDDEPLOYMENTENVIRONMENTSTATUS.nested_types_by_name['EnvironmentToDeploymentBriefEntry']
_BUILDDEPLOYMENTSTATUS = DESCRIPTOR.message_types_by_name['BuildDeploymentStatus']
_KAFKACONFIG_CONSUMER_AUTOOFFSETTYPE = _KAFKACONFIG_CONSUMER.enum_types_by_name['AutoOffSetType']
_KAFKACONFIG_PRODUCER_COMPRESSIONTYPE = _KAFKACONFIG_PRODUCER.enum_types_by_name['CompressionType']
_RUNTIMEDEPLOYMENTSETTINGS_LOGLEVEL = _RUNTIMEDEPLOYMENTSETTINGS.enum_types_by_name['LogLevel']
HostingService = _reflection.GeneratedProtocolMessageType('HostingService', (_message.Message,), {
  'DESCRIPTOR' : _HOSTINGSERVICE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.HostingService)
  })
_sym_db.RegisterMessage(HostingService)

KubeDeployment = _reflection.GeneratedProtocolMessageType('KubeDeployment', (_message.Message,), {

  'EnvironmentVariablesEntry' : _reflection.GeneratedProtocolMessageType('EnvironmentVariablesEntry', (_message.Message,), {
    'DESCRIPTOR' : _KUBEDEPLOYMENT_ENVIRONMENTVARIABLESENTRY,
    '__module__' : 'qwak.deployment.deployment_pb2'
    # @@protoc_insertion_point(class_scope:qwak.deployment.management.KubeDeployment.EnvironmentVariablesEntry)
    })
  ,
  'DESCRIPTOR' : _KUBEDEPLOYMENT,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.KubeDeployment)
  })
_sym_db.RegisterMessage(KubeDeployment)
_sym_db.RegisterMessage(KubeDeployment.EnvironmentVariablesEntry)

ServingStrategy = _reflection.GeneratedProtocolMessageType('ServingStrategy', (_message.Message,), {
  'DESCRIPTOR' : _SERVINGSTRATEGY,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.ServingStrategy)
  })
_sym_db.RegisterMessage(ServingStrategy)

RealTimeConfig = _reflection.GeneratedProtocolMessageType('RealTimeConfig', (_message.Message,), {
  'DESCRIPTOR' : _REALTIMECONFIG,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.RealTimeConfig)
  })
_sym_db.RegisterMessage(RealTimeConfig)

TrafficConfig = _reflection.GeneratedProtocolMessageType('TrafficConfig', (_message.Message,), {
  'DESCRIPTOR' : _TRAFFICCONFIG,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.TrafficConfig)
  })
_sym_db.RegisterMessage(TrafficConfig)

StreamConfig = _reflection.GeneratedProtocolMessageType('StreamConfig', (_message.Message,), {
  'DESCRIPTOR' : _STREAMCONFIG,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.StreamConfig)
  })
_sym_db.RegisterMessage(StreamConfig)

BatchConfig = _reflection.GeneratedProtocolMessageType('BatchConfig', (_message.Message,), {
  'DESCRIPTOR' : _BATCHCONFIG,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.BatchConfig)
  })
_sym_db.RegisterMessage(BatchConfig)

KafkaConfig = _reflection.GeneratedProtocolMessageType('KafkaConfig', (_message.Message,), {

  'Consumer' : _reflection.GeneratedProtocolMessageType('Consumer', (_message.Message,), {
    'DESCRIPTOR' : _KAFKACONFIG_CONSUMER,
    '__module__' : 'qwak.deployment.deployment_pb2'
    # @@protoc_insertion_point(class_scope:qwak.deployment.management.KafkaConfig.Consumer)
    })
  ,

  'Producer' : _reflection.GeneratedProtocolMessageType('Producer', (_message.Message,), {
    'DESCRIPTOR' : _KAFKACONFIG_PRODUCER,
    '__module__' : 'qwak.deployment.deployment_pb2'
    # @@protoc_insertion_point(class_scope:qwak.deployment.management.KafkaConfig.Producer)
    })
  ,
  'DESCRIPTOR' : _KAFKACONFIG,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.KafkaConfig)
  })
_sym_db.RegisterMessage(KafkaConfig)
_sym_db.RegisterMessage(KafkaConfig.Consumer)
_sym_db.RegisterMessage(KafkaConfig.Producer)

SageMaker = _reflection.GeneratedProtocolMessageType('SageMaker', (_message.Message,), {
  'DESCRIPTOR' : _SAGEMAKER,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.SageMaker)
  })
_sym_db.RegisterMessage(SageMaker)

DeploymentSize = _reflection.GeneratedProtocolMessageType('DeploymentSize', (_message.Message,), {
  'DESCRIPTOR' : _DEPLOYMENTSIZE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.DeploymentSize)
  })
_sym_db.RegisterMessage(DeploymentSize)

GpuResources = _reflection.GeneratedProtocolMessageType('GpuResources', (_message.Message,), {
  'DESCRIPTOR' : _GPURESOURCES,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.GpuResources)
  })
_sym_db.RegisterMessage(GpuResources)

AdvancedDeploymentOptions = _reflection.GeneratedProtocolMessageType('AdvancedDeploymentOptions', (_message.Message,), {
  'DESCRIPTOR' : _ADVANCEDDEPLOYMENTOPTIONS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.AdvancedDeploymentOptions)
  })
_sym_db.RegisterMessage(AdvancedDeploymentOptions)

DeploymentDetails = _reflection.GeneratedProtocolMessageType('DeploymentDetails', (_message.Message,), {

  'EnvironmentVariablesEntry' : _reflection.GeneratedProtocolMessageType('EnvironmentVariablesEntry', (_message.Message,), {
    'DESCRIPTOR' : _DEPLOYMENTDETAILS_ENVIRONMENTVARIABLESENTRY,
    '__module__' : 'qwak.deployment.deployment_pb2'
    # @@protoc_insertion_point(class_scope:qwak.deployment.management.DeploymentDetails.EnvironmentVariablesEntry)
    })
  ,
  'DESCRIPTOR' : _DEPLOYMENTDETAILS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.DeploymentDetails)
  })
_sym_db.RegisterMessage(DeploymentDetails)
_sym_db.RegisterMessage(DeploymentDetails.EnvironmentVariablesEntry)

RuntimeDeploymentSettings = _reflection.GeneratedProtocolMessageType('RuntimeDeploymentSettings', (_message.Message,), {
  'DESCRIPTOR' : _RUNTIMEDEPLOYMENTSETTINGS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.RuntimeDeploymentSettings)
  })
_sym_db.RegisterMessage(RuntimeDeploymentSettings)

DeploymentBrief = _reflection.GeneratedProtocolMessageType('DeploymentBrief', (_message.Message,), {
  'DESCRIPTOR' : _DEPLOYMENTBRIEF,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.DeploymentBrief)
  })
_sym_db.RegisterMessage(DeploymentBrief)

KubernetesModelDeploymentState = _reflection.GeneratedProtocolMessageType('KubernetesModelDeploymentState', (_message.Message,), {
  'DESCRIPTOR' : _KUBERNETESMODELDEPLOYMENTSTATE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.KubernetesModelDeploymentState)
  })
_sym_db.RegisterMessage(KubernetesModelDeploymentState)

StatusDetails = _reflection.GeneratedProtocolMessageType('StatusDetails', (_message.Message,), {
  'DESCRIPTOR' : _STATUSDETAILS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.StatusDetails)
  })
_sym_db.RegisterMessage(StatusDetails)

PendingStatusDetails = _reflection.GeneratedProtocolMessageType('PendingStatusDetails', (_message.Message,), {
  'DESCRIPTOR' : _PENDINGSTATUSDETAILS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.PendingStatusDetails)
  })
_sym_db.RegisterMessage(PendingStatusDetails)

SuccessfulStatusDetails = _reflection.GeneratedProtocolMessageType('SuccessfulStatusDetails', (_message.Message,), {
  'DESCRIPTOR' : _SUCCESSFULSTATUSDETAILS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.SuccessfulStatusDetails)
  })
_sym_db.RegisterMessage(SuccessfulStatusDetails)

FailedStatusDetails = _reflection.GeneratedProtocolMessageType('FailedStatusDetails', (_message.Message,), {
  'DESCRIPTOR' : _FAILEDSTATUSDETAILS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.FailedStatusDetails)
  })
_sym_db.RegisterMessage(FailedStatusDetails)

FailureDetails = _reflection.GeneratedProtocolMessageType('FailureDetails', (_message.Message,), {
  'DESCRIPTOR' : _FAILUREDETAILS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.FailureDetails)
  })
_sym_db.RegisterMessage(FailureDetails)

InstanceDetails = _reflection.GeneratedProtocolMessageType('InstanceDetails', (_message.Message,), {
  'DESCRIPTOR' : _INSTANCEDETAILS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.InstanceDetails)
  })
_sym_db.RegisterMessage(InstanceDetails)

Variation = _reflection.GeneratedProtocolMessageType('Variation', (_message.Message,), {
  'DESCRIPTOR' : _VARIATION,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.Variation)
  })
_sym_db.RegisterMessage(Variation)

TrafficSpec = _reflection.GeneratedProtocolMessageType('TrafficSpec', (_message.Message,), {
  'DESCRIPTOR' : _TRAFFICSPEC,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.TrafficSpec)
  })
_sym_db.RegisterMessage(TrafficSpec)

FutureDeploymentSettings = _reflection.GeneratedProtocolMessageType('FutureDeploymentSettings', (_message.Message,), {
  'DESCRIPTOR' : _FUTUREDEPLOYMENTSETTINGS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.FutureDeploymentSettings)
  })
_sym_db.RegisterMessage(FutureDeploymentSettings)

EnvironmentDeploymentMessage = _reflection.GeneratedProtocolMessageType('EnvironmentDeploymentMessage', (_message.Message,), {
  'DESCRIPTOR' : _ENVIRONMENTDEPLOYMENTMESSAGE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.EnvironmentDeploymentMessage)
  })
_sym_db.RegisterMessage(EnvironmentDeploymentMessage)

EnvironmentDeploymentResultMessage = _reflection.GeneratedProtocolMessageType('EnvironmentDeploymentResultMessage', (_message.Message,), {
  'DESCRIPTOR' : _ENVIRONMENTDEPLOYMENTRESULTMESSAGE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.EnvironmentDeploymentResultMessage)
  })
_sym_db.RegisterMessage(EnvironmentDeploymentResultMessage)

EnvironmentUndeploymentMessage = _reflection.GeneratedProtocolMessageType('EnvironmentUndeploymentMessage', (_message.Message,), {
  'DESCRIPTOR' : _ENVIRONMENTUNDEPLOYMENTMESSAGE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.EnvironmentUndeploymentMessage)
  })
_sym_db.RegisterMessage(EnvironmentUndeploymentMessage)

EnvironmentUndeploymentResultMessage = _reflection.GeneratedProtocolMessageType('EnvironmentUndeploymentResultMessage', (_message.Message,), {
  'DESCRIPTOR' : _ENVIRONMENTUNDEPLOYMENTRESULTMESSAGE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.EnvironmentUndeploymentResultMessage)
  })
_sym_db.RegisterMessage(EnvironmentUndeploymentResultMessage)

EnvironmentRuntimeDeploymentSettingsMessage = _reflection.GeneratedProtocolMessageType('EnvironmentRuntimeDeploymentSettingsMessage', (_message.Message,), {
  'DESCRIPTOR' : _ENVIRONMENTRUNTIMEDEPLOYMENTSETTINGSMESSAGE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.EnvironmentRuntimeDeploymentSettingsMessage)
  })
_sym_db.RegisterMessage(EnvironmentRuntimeDeploymentSettingsMessage)

EnvironmentRuntimeDeploymentSettingsResultMessage = _reflection.GeneratedProtocolMessageType('EnvironmentRuntimeDeploymentSettingsResultMessage', (_message.Message,), {
  'DESCRIPTOR' : _ENVIRONMENTRUNTIMEDEPLOYMENTSETTINGSRESULTMESSAGE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.EnvironmentRuntimeDeploymentSettingsResultMessage)
  })
_sym_db.RegisterMessage(EnvironmentRuntimeDeploymentSettingsResultMessage)

EnvironmentApplyModelTrafficResponse = _reflection.GeneratedProtocolMessageType('EnvironmentApplyModelTrafficResponse', (_message.Message,), {
  'DESCRIPTOR' : _ENVIRONMENTAPPLYMODELTRAFFICRESPONSE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.EnvironmentApplyModelTrafficResponse)
  })
_sym_db.RegisterMessage(EnvironmentApplyModelTrafficResponse)

EnvironmentDeploymentDetailsMessage = _reflection.GeneratedProtocolMessageType('EnvironmentDeploymentDetailsMessage', (_message.Message,), {
  'DESCRIPTOR' : _ENVIRONMENTDEPLOYMENTDETAILSMESSAGE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.EnvironmentDeploymentDetailsMessage)
  })
_sym_db.RegisterMessage(EnvironmentDeploymentDetailsMessage)

EnvironmentDeploymentHistoryMessage = _reflection.GeneratedProtocolMessageType('EnvironmentDeploymentHistoryMessage', (_message.Message,), {
  'DESCRIPTOR' : _ENVIRONMENTDEPLOYMENTHISTORYMESSAGE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.EnvironmentDeploymentHistoryMessage)
  })
_sym_db.RegisterMessage(EnvironmentDeploymentHistoryMessage)

EnvironmentTrafficMessage = _reflection.GeneratedProtocolMessageType('EnvironmentTrafficMessage', (_message.Message,), {
  'DESCRIPTOR' : _ENVIRONMENTTRAFFICMESSAGE,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.EnvironmentTrafficMessage)
  })
_sym_db.RegisterMessage(EnvironmentTrafficMessage)

BuildDeploymentEnvironmentStatus = _reflection.GeneratedProtocolMessageType('BuildDeploymentEnvironmentStatus', (_message.Message,), {

  'EnvironmentToDeploymentBriefEntry' : _reflection.GeneratedProtocolMessageType('EnvironmentToDeploymentBriefEntry', (_message.Message,), {
    'DESCRIPTOR' : _BUILDDEPLOYMENTENVIRONMENTSTATUS_ENVIRONMENTTODEPLOYMENTBRIEFENTRY,
    '__module__' : 'qwak.deployment.deployment_pb2'
    # @@protoc_insertion_point(class_scope:qwak.deployment.management.BuildDeploymentEnvironmentStatus.EnvironmentToDeploymentBriefEntry)
    })
  ,
  'DESCRIPTOR' : _BUILDDEPLOYMENTENVIRONMENTSTATUS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.BuildDeploymentEnvironmentStatus)
  })
_sym_db.RegisterMessage(BuildDeploymentEnvironmentStatus)
_sym_db.RegisterMessage(BuildDeploymentEnvironmentStatus.EnvironmentToDeploymentBriefEntry)

BuildDeploymentStatus = _reflection.GeneratedProtocolMessageType('BuildDeploymentStatus', (_message.Message,), {
  'DESCRIPTOR' : _BUILDDEPLOYMENTSTATUS,
  '__module__' : 'qwak.deployment.deployment_pb2'
  # @@protoc_insertion_point(class_scope:qwak.deployment.management.BuildDeploymentStatus)
  })
_sym_db.RegisterMessage(BuildDeploymentStatus)

if _descriptor._USE_C_DESCRIPTORS == False:

  DESCRIPTOR._options = None
  DESCRIPTOR._serialized_options = b'\n%com.qwak.ai.management.deployment.apiP\001Z\032qwak/deployment;deployment'
  _HOSTINGSERVICE.fields_by_name['sagemaker']._options = None
  _HOSTINGSERVICE.fields_by_name['sagemaker']._serialized_options = b'\030\001'
  _KUBEDEPLOYMENT_ENVIRONMENTVARIABLESENTRY._options = None
  _KUBEDEPLOYMENT_ENVIRONMENTVARIABLESENTRY._serialized_options = b'8\001'
  _KUBEDEPLOYMENT.fields_by_name['pod_count']._options = None
  _KUBEDEPLOYMENT.fields_by_name['pod_count']._serialized_options = b'\030\001'
  _DEPLOYMENTSIZE.fields_by_name['cpu']._options = None
  _DEPLOYMENTSIZE.fields_by_name['cpu']._serialized_options = b'\030\001'
  _DEPLOYMENTSIZE.fields_by_name['memory_amount']._options = None
  _DEPLOYMENTSIZE.fields_by_name['memory_amount']._serialized_options = b'\030\001'
  _DEPLOYMENTSIZE.fields_by_name['memory_units']._options = None
  _DEPLOYMENTSIZE.fields_by_name['memory_units']._serialized_options = b'\030\001'
  _DEPLOYMENTSIZE.fields_by_name['gpu_resources']._options = None
  _DEPLOYMENTSIZE.fields_by_name['gpu_resources']._serialized_options = b'\030\001'
  _DEPLOYMENTDETAILS_ENVIRONMENTVARIABLESENTRY._options = None
  _DEPLOYMENTDETAILS_ENVIRONMENTVARIABLESENTRY._serialized_options = b'8\001'
  _KUBERNETESMODELDEPLOYMENTSTATE.fields_by_name['branch_id']._options = None
  _KUBERNETESMODELDEPLOYMENTSTATE.fields_by_name['branch_id']._serialized_options = b'\030\001'
  _ENVIRONMENTDEPLOYMENTMESSAGE.fields_by_name['branch_id']._options = None
  _ENVIRONMENTDEPLOYMENTMESSAGE.fields_by_name['branch_id']._serialized_options = b'\030\001'
  _ENVIRONMENTDEPLOYMENTMESSAGE.fields_by_name['model_uuid']._options = None
  _ENVIRONMENTDEPLOYMENTMESSAGE.fields_by_name['model_uuid']._serialized_options = b'\030\001'
  _ENVIRONMENTUNDEPLOYMENTMESSAGE.fields_by_name['branch_id']._options = None
  _ENVIRONMENTUNDEPLOYMENTMESSAGE.fields_by_name['branch_id']._serialized_options = b'\030\001'
  _ENVIRONMENTRUNTIMEDEPLOYMENTSETTINGSMESSAGE.fields_by_name['branch_id']._options = None
  _ENVIRONMENTRUNTIMEDEPLOYMENTSETTINGSMESSAGE.fields_by_name['branch_id']._serialized_options = b'\030\001'
  _BUILDDEPLOYMENTENVIRONMENTSTATUS_ENVIRONMENTTODEPLOYMENTBRIEFENTRY._options = None
  _BUILDDEPLOYMENTENVIRONMENTSTATUS_ENVIRONMENTTODEPLOYMENTBRIEFENTRY._serialized_options = b'8\001'
  _MODELDEPLOYMENTSTATUS._serialized_start=8229
  _MODELDEPLOYMENTSTATUS._serialized_end=8618
  _DEPLOYMENTHOSTINGSERVICETYPE._serialized_start=8620
  _DEPLOYMENTHOSTINGSERVICETYPE._serialized_end=8699
  _KUBEDEPLOYMENTTYPE._serialized_start=8701
  _KUBEDEPLOYMENTTYPE._serialized_end=8771
  _GPUTYPE._serialized_start=8773
  _GPUTYPE._serialized_end=8881
  _MEMORYUNIT._serialized_start=8883
  _MEMORYUNIT._serialized_end=8926
  _DEPLOYMENTMANAGEDBY._serialized_start=8929
  _DEPLOYMENTMANAGEDBY._serialized_end=9060
  _DEPLOYMENTFAILUREREASONCODE._serialized_start=9063
  _DEPLOYMENTFAILUREREASONCODE._serialized_end=9338
  _HOSTINGSERVICE._serialized_start=253
  _HOSTINGSERVICE._serialized_end=411
  _KUBEDEPLOYMENT._serialized_start=414
  _KUBEDEPLOYMENT._serialized_end=925
  _KUBEDEPLOYMENT_ENVIRONMENTVARIABLESENTRY._serialized_start=866
  _KUBEDEPLOYMENT_ENVIRONMENTVARIABLESENTRY._serialized_end=925
  _SERVINGSTRATEGY._serialized_start=928
  _SERVINGSTRATEGY._serialized_end=1160
  _REALTIMECONFIG._serialized_start=1163
  _REALTIMECONFIG._serialized_end=1316
  _TRAFFICCONFIG._serialized_start=1319
  _TRAFFICCONFIG._serialized_end=1568
  _STREAMCONFIG._serialized_start=1570
  _STREAMCONFIG._serialized_end=1640
  _BATCHCONFIG._serialized_start=1642
  _BATCHCONFIG._serialized_end=1655
  _KAFKACONFIG._serialized_start=1658
  _KAFKACONFIG._serialized_end=2345
  _KAFKACONFIG_CONSUMER._serialized_start=1827
  _KAFKACONFIG_CONSUMER._serialized_end=2108
  _KAFKACONFIG_CONSUMER_AUTOOFFSETTYPE._serialized_start=2053
  _KAFKACONFIG_CONSUMER_AUTOOFFSETTYPE._serialized_end=2108
  _KAFKACONFIG_PRODUCER._serialized_start=2111
  _KAFKACONFIG_PRODUCER._serialized_end=2345
  _KAFKACONFIG_PRODUCER_COMPRESSIONTYPE._serialized_start=2256
  _KAFKACONFIG_PRODUCER_COMPRESSIONTYPE._serialized_end=2345
  _SAGEMAKER._serialized_start=2347
  _SAGEMAKER._serialized_end=2405
  _DEPLOYMENTSIZE._serialized_start=2408
  _DEPLOYMENTSIZE._serialized_end=2730
  _GPURESOURCES._serialized_start=2732
  _GPURESOURCES._serialized_end=2821
  _ADVANCEDDEPLOYMENTOPTIONS._serialized_start=2824
  _ADVANCEDDEPLOYMENTOPTIONS._serialized_end=3092
  _DEPLOYMENTDETAILS._serialized_start=3095
  _DEPLOYMENTDETAILS._serialized_end=4192
  _DEPLOYMENTDETAILS_ENVIRONMENTVARIABLESENTRY._serialized_start=866
  _DEPLOYMENTDETAILS_ENVIRONMENTVARIABLESENTRY._serialized_end=925
  _RUNTIMEDEPLOYMENTSETTINGS._serialized_start=4195
  _RUNTIMEDEPLOYMENTSETTINGS._serialized_end=4380
  _RUNTIMEDEPLOYMENTSETTINGS_LOGLEVEL._serialized_start=4315
  _RUNTIMEDEPLOYMENTSETTINGS_LOGLEVEL._serialized_end=4380
  _DEPLOYMENTBRIEF._serialized_start=4383
  _DEPLOYMENTBRIEF._serialized_end=4749
  _KUBERNETESMODELDEPLOYMENTSTATE._serialized_start=4752
  _KUBERNETESMODELDEPLOYMENTSTATE._serialized_end=5095
  _STATUSDETAILS._serialized_start=5098
  _STATUSDETAILS._serialized_end=5384
  _PENDINGSTATUSDETAILS._serialized_start=5387
  _PENDINGSTATUSDETAILS._serialized_end=5543
  _SUCCESSFULSTATUSDETAILS._serialized_start=5545
  _SUCCESSFULSTATUSDETAILS._serialized_end=5641
  _FAILEDSTATUSDETAILS._serialized_start=5644
  _FAILEDSTATUSDETAILS._serialized_end=5871
  _FAILUREDETAILS._serialized_start=5874
  _FAILUREDETAILS._serialized_end=6019
  _INSTANCEDETAILS._serialized_start=6021
  _INSTANCEDETAILS._serialized_end=6107
  _VARIATION._serialized_start=6110
  _VARIATION._serialized_end=6246
  _TRAFFICSPEC._serialized_start=6248
  _TRAFFICSPEC._serialized_end=6300
  _FUTUREDEPLOYMENTSETTINGS._serialized_start=6302
  _FUTUREDEPLOYMENTSETTINGS._serialized_end=6395
  _ENVIRONMENTDEPLOYMENTMESSAGE._serialized_start=6398
  _ENVIRONMENTDEPLOYMENTMESSAGE._serialized_end=6580
  _ENVIRONMENTDEPLOYMENTRESULTMESSAGE._serialized_start=6583
  _ENVIRONMENTDEPLOYMENTRESULTMESSAGE._serialized_end=6729
  _ENVIRONMENTUNDEPLOYMENTMESSAGE._serialized_start=6732
  _ENVIRONMENTUNDEPLOYMENTMESSAGE._serialized_end=7004
  _ENVIRONMENTUNDEPLOYMENTRESULTMESSAGE._serialized_start=7006
  _ENVIRONMENTUNDEPLOYMENTRESULTMESSAGE._serialized_end=7125
  _ENVIRONMENTRUNTIMEDEPLOYMENTSETTINGSMESSAGE._serialized_start=7128
  _ENVIRONMENTRUNTIMEDEPLOYMENTSETTINGSMESSAGE._serialized_end=7342
  _ENVIRONMENTRUNTIMEDEPLOYMENTSETTINGSRESULTMESSAGE._serialized_start=7344
  _ENVIRONMENTRUNTIMEDEPLOYMENTSETTINGSRESULTMESSAGE._serialized_end=7415
  _ENVIRONMENTAPPLYMODELTRAFFICRESPONSE._serialized_start=7417
  _ENVIRONMENTAPPLYMODELTRAFFICRESPONSE._serialized_end=7469
  _ENVIRONMENTDEPLOYMENTDETAILSMESSAGE._serialized_start=7472
  _ENVIRONMENTDEPLOYMENTDETAILSMESSAGE._serialized_end=7678
  _ENVIRONMENTDEPLOYMENTHISTORYMESSAGE._serialized_start=7680
  _ENVIRONMENTDEPLOYMENTHISTORYMESSAGE._serialized_end=7783
  _ENVIRONMENTTRAFFICMESSAGE._serialized_start=7785
  _ENVIRONMENTTRAFFICMESSAGE._serialized_end=7871
  _BUILDDEPLOYMENTENVIRONMENTSTATUS._serialized_start=7874
  _BUILDDEPLOYMENTENVIRONMENTSTATUS._serialized_end=8160
  _BUILDDEPLOYMENTENVIRONMENTSTATUS_ENVIRONMENTTODEPLOYMENTBRIEFENTRY._serialized_start=8048
  _BUILDDEPLOYMENTENVIRONMENTSTATUS_ENVIRONMENTTODEPLOYMENTBRIEFENTRY._serialized_end=8160
  _BUILDDEPLOYMENTSTATUS._serialized_start=8162
  _BUILDDEPLOYMENTSTATUS._serialized_end=8226
# @@protoc_insertion_point(module_scope)
