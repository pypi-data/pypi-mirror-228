...
from .transforms_utils import get_layers, get_layer_index, update_inbound, get_layers_by_type
...
def _find_batchnorms_to_fold(model):
    ...
    # Get all BatchNormalization layers present in the model
    bns = get_layers_by_type(model, BatchNormalization)

    # Find valid "layer -> BN" candidates
    for bn in bns:
        # Find a supported inbound layer
        inbound = bn.inbound_nodes[0].inbound_layers
        if isinstance(inbound, supported_layers):
            ...
            map_prev_layer_to_bn[inbound] = bn
