...
def stub_indices(string: str, inclusive=True) -> Tuple[int, int]:
...
def validate(template: str, lang: Language, replacements: Mapping[str, str]):
    ...
    all_stubbles = flatten([stubs(line=line, lang=lang) for line in all_lines])
...
def replace(line: str, lang: Language, replacements: Mapping[str, str]) -> str:
    stubs_in_line = stubs(line=line, lang=lang)
    remaining_line = line
    for stub in stubs_in_line:
        start, end = stub_indices(string=line, inclusive=True)
...
def populate(template: str, lang: Language, replacements: Dict[str, str]) -> str:
    ...
    for line in all_lines:
        if is_stubble_line(line=line, lang=lang):
            replaced = replace(line=line, lang=lang, replacements=replacements)
            stripped = strip(line=replaced, lang=lang)
            new_line = whitespace(line=line, lang=lang) + stripped
        else:
            new_line = line
