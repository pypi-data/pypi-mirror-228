...
class ExitCode:
    ...
    def to_python_code(self) -> str:
        ...
        # _indent = ' ' * 4
        # return '\n'.join([f"class {self.label}(Exception):",
        #                   f"{_indent}def __init__(self, detail, kill=True):",
        #                   f'{_indent * 2}logging.error("""{self.id}:{self.desc}. """ + detail)',
        #                   f"{_indent * 2}if kill:",
        #                   f"{_indent * 3}exit({self.id})"])
...
def compile_exitcodes(csv_path, outfile=None):
    ...
    #     logging.debug('Looking for default Exit Codes declaration file (~/artsem/config/exitcodes.csv')
    ...
    with open(csv_path, 'r', newline='') as csvfile:
        ...
        for row in reader:
            lcount += 1
            try:
                # err = {'id': int(row['id'].strip()), 'label': row['label'].strip(), 'desc': row['desc'].strip()}
