...
if __name__ == '__main__':
    ...
    if _args.publish is not None:
        if len(_args.publish) > 1:
            logging.error(f"More than one version specified. Please use only one version at a time")
            exit(1)
        elif len(_args.publish) == 0:
            _args.publish = auto_increment_version(get_current_version())
        else:
            _args.publish = _args.publish[0] if len(_args.publish) > 1 else auto_increment_version(get_current_version())
            logging.info(f"Manual versioning ({get_current_version()} -> {_args.publish})")

        if validate_version(_args.publish):
            run_cmd(f"{__interpreter__} -m hatch version {_args.publish}")
        else:
            logging.error(f"Invalid syntax version ({_args.publish}). Allowed [x.]x, where x are integers")
