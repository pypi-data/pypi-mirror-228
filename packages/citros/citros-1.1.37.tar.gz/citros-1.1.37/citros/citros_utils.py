...
class citros_utils():
    ...
    def is_ssh_agent_running(self):
        ...
        if plat == 'Windows':
            ...
            pgrep_result = subprocess.run(['pgrep', 'ssh-agent'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def start_ssh_agent(self):
        try:
            ...
            if plat == 'Windows':
                if not os.geteuid() == 0:
                    raise ValueError("Please run the script as administrator.")
                ...
                # script_text = ""
                # with self.get_script_path('ssh_agent_on_login.sh') as script:
                #     script_text = script.read_text()

                # bash_profile_path = Path('~/.bashrc').expanduser()
                # zsh_profile_path = Path('~/.zprofile').expanduser()
                # with open(bash_profile_path, 'a') as f:
                #     f.write(script_text)
                # if zsh_profile_path.exists():
                #     with open(zsh_profile_path, 'a') as f:
                #         f.write(script_text)
    ...
    def get_schema_path(self, schema_file):
        return importlib_resources.files('data.schemas').joinpath(schema_file)


    def get_default_file_path(self, default):
        return importlib_resources.files('data.defaults').joinpath(default)
    

    def get_script_path(self, script_file):
        return importlib_resources.files('data.scripts').joinpath(script_file)
