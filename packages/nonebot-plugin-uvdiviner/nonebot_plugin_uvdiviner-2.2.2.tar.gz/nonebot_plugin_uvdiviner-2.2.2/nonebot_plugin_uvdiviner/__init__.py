...
from .utils import version as __version__, init, reset_limit, minus_limit, get_limit, has_group, has_user, set_user, get_groups, get_users
...
helpcommand = on_startswith((".help divine", ".h divine", ".help div", ".h div"), priority=2, block=True)
...
if initialized:
    ...
    async def dqchandler(matcher: Matcher, event: Event):
        ...
        await matcher.send(f"占卜快速检定结果: {result}")
        logger.info(f"占卜快速检定结果: {result}")
