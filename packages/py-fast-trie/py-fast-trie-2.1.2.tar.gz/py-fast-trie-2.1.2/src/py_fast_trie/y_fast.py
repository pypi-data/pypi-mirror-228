...
class YFastTrie:
    ...
    def _calculate_representative(value: int, max_length: int) -> int:
        ...
        """
        result = min(
            max_length * (value // max_length) + (-1 % max_length), 2**max_length - 1
        )
        return cast(int, result)

    @staticmethod
    def _merge_subtrees(
        left_tree: SortedList, right_tree: SortedList, max_size: int
    ) -> tuple[SortedList, Optional[SortedList]]:
        """
    ...
    @staticmethod
    def _split_subtree(
        tree: SortedList, max_length: int
    ) -> tuple[SortedList, SortedList]:
        """
        ...
        median = tree.bisect_right(
            YFastTrie._calculate_representative(tree[len(tree) // 2], max_length)
        )
    ...
    def _get_value_subtree(
        self, value: int, create_subtree: bool = False
        ...
        if self._count == 0:
            ...
            rep_node = self._partitions._min
        else:
            # As the X-fast trie looks for strict successors,
            # if the value being searched for is a representative,
            # the wrong representative will be returned if the one
            # being searched for is not the largest,
            # and no representative will be returned at all if it is;
        ...
        if rep_node is None:
            ...
            # Every representative in the X-fast trie
            # should have a corresponding SortedList;
    ...
    def insert(self, value: int | bytes) -> None:
        ...
        if len(subtree) > self._max_subtree_size:
            ...
            self._partitions -= cast(int, rep_node.value)
    ...
    def predecessor(self, value: int | bytes) -> Optional[int]:
        ...
        if min(subtree) >= value:
            subtree = self._subtrees[cast(TrieNode, rep_node.left).value]
    ...
    def remove(self, value: int | bytes) -> None:
        ...
        if len(subtree) == 0:
            ...
            if rep_node.left is not None:
                left_rep = rep_node.left
                right_rep = rep_node
            else:
                left_rep = rep_node
                right_rep = cast(TrieNode, rep_node.right)
            ...
            self._partitions -= cast(int, left_rep.value)
            self._partitions -= cast(int, right_rep.value)

            # In with the new
            tree: SortedList
            for tree in filter(
                None, self._merge_subtrees(left_tree, right_tree, 2 * self._maxlen)
            ):
                rep = self._calculate_representative(max(tree), self._maxlen)
                self._partitions += rep
                self._subtrees[rep] = tree
    ...
    def successor(self, value: int | bytes) -> Optional[int]:
        ...
        if max(subtree) <= value:
            subtree = self._subtrees[cast(TrieNode, rep_node.right).value]
    ...
    def __iadd__(self, value: int | bytes) -> YFastTrie:
        self.insert(value)
        return self

    def __isub__(self, value: int | bytes) -> YFastTrie:
        self.remove(value)
        return self

    def __iter__(self) -> Iterable[int]:
        for rep in cast(Iterable[int], sorted(self._subtrees)):
            yield from self._subtrees[rep]
