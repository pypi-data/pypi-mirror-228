...
class H2OGAMParams(HasIgnoredCols, HasBetaConstraints, HasGamCols, Params):
    ...
    splinesNonNegative = Param(
        ...
        H2OTypeConverters.toNullableListBoolean())
    ...
    bs = Param(
        Params._dummy(),
        "bs",
        """Basis function type for each gam predictors, 0 for cr, 1 for thin plate regression with knots, 2 for monotone I-splines, 3 for NBSplineTypeI M-splines (refer to doc here: https://h2oai.atlassian.net/browse/PUBDEV-8835).  If specified, must be the same size as gam_columns""",
        H2OTypeConverters.toNullableListInt())
    ...
    def getStoreKnotLocations(self):
        return self.getOrDefault(self.storeKnotLocations)

    def getNumKnots(self):
        return self.getOrDefault(self.numKnots)

    def getSplineOrders(self):
        return self.getOrDefault(self.splineOrders)

    def getSplinesNonNegative(self):
        return self.getOrDefault(self.splinesNonNegative)
    ...
    def getStandardizeTpGamCols(self):
        return self.getOrDefault(self.standardizeTpGamCols)

    def getScaleTpPenaltyMat(self):
        return self.getOrDefault(self.scaleTpPenaltyMat)
    ...
    def setStoreKnotLocations(self, value):
        return self._set(storeKnotLocations=value)

    def setNumKnots(self, value):
        return self._set(numKnots=value)

    def setSplineOrders(self, value):
        return self._set(splineOrders=value)

    def setSplinesNonNegative(self, value):
        return self._set(splinesNonNegative=value)
    ...
    def setStandardizeTpGamCols(self, value):
        return self._set(standardizeTpGamCols=value)

    def setScaleTpPenaltyMat(self, value):
        return self._set(scaleTpPenaltyMat=value)
