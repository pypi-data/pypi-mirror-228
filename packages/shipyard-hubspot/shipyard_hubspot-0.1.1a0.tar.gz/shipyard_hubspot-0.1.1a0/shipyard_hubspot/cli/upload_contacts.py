...
def main():
    ...
    try:
        import_job_id = client.import_contact_data(import_name=args.import_name, filename=args.filename,
                                                   import_operations=args.import_operations,
                                                   file_format=args.file_format).get("id")
        while import_state := client.get_import_status(import_job_id).get("state") not in {"COMPLETE", "FAILED",
                                                                                           "CANCELED", None}:
