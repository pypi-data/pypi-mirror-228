...
def evaluate():
    ...
    if (
        ...
        if uname[-1] == "Microsoft":
            console.print(f"[bold][yellow]You are using Ubuntu in WSL")
            result["wsl"] = True
        else:
            console.print(f"[bold][yellow]You are using in standalone Ubuntu")
            result["wsl"] = False
    else:
    ...
    with console.status("[bold green]Gathering data...") as status:
        ...
        if risc_flag:
            ...
            console.print(f"Check riscv-gcc-compiler is not installed.")
