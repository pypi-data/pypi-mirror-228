...
@click.option("--no_generate", is_flag=True, help="Do not generate result file.")
def evaluate(year, theme, task, verbose, no_generate):
    ...
    if not no_generate:
        with open(
            f"result-{year}-{theme}-{task}-{datetime.now().strftime('%Y%m%d')}.json",
            "w",
            encoding="utf-8",
        ) as file:
