...
def tf_callback(msg):
    ...
    for transform in msg.transforms:
        if transform.header.frame_id == 'base_link' and transform.child_frame_id in base_link_to_frames:
...
def get_joint_states(console):
    ...
    try:
        ...
        while rclpy.ok() and cb_count < 10 and (node.get_clock().now() - start_time) < timeout_duration:
...
def evaluate():
    ...
    with console.status("[bold green]Gathering data...") as status:
        ...
        encrypted_msg = cryptocode.encrypt(str(result), cl_secret_pw)
        result_msg["encrypted_msg"] = encrypted_msg
        result_msg["generate"] = True
