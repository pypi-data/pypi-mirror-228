...
from pywallex.abstract import WallexClient
from pywallex.utils import calculate_sign
from pywallex.schemas import (
...
class Exchange:
    ...
    async def _post_request(self, path, data, calc_sign=True):
        ...
        response = await self.wallex_client.client.post(path, data=data)
        try:
    ...
    async def _get_request(self, path: str):
        response = await self.wallex_client.client.get(path)
        return response.json()
