...
if not os.path.exists(RUN_MODEL_SAVE_PATH):
    os.makedirs(RUN_MODEL_SAVE_PATH)
if not PREDICT_MODEL_SAVE_PATH:
    PREDICT_MODEL_SAVE_PATH = os.path.join(HOME_CACHE_PATH, "models")
if not os.path.exists(PREDICT_MODEL_SAVE_PATH):
    os.makedirs(PREDICT_MODEL_SAVE_PATH)
...
if not os.path.exists(CUSTOM_CACHE_PATH):
    ...
    with open(_init_file, "w") as f:
        f.write("#")
sys.path.append(CUSTOM_CACHE_PATH)
