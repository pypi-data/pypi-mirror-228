...
def history2Convergence():
    if it.cycle%1000 == 0:
        it.command(f"history export 1 file 'disp-x.his' truncate skip 200 vs step")
        it.command(f"history export 3 file 'disp-z.his' truncate skip 200 vs step")
    # print("the callback function is called")
    # col_1 = ["step","disp_x"]
    # col_3 = ["step","disp_z"]
    # plt.ion()
    ...
    # if it.cycle - cycle_Foundation == interval:
        ...
        # line1, = ax.plot(it.cycle, disp_x.disp_x)
    # elif (it.cycle - cycle_Foundation)%interval == 0:
        # it.command(f"history export 1 file 'disp-x.his' truncate skip {interval} vs step")
        # it.command(f"history export 3 file 'disp-z.his' truncate skip {interval} vs step")
        # disp_x = pd.read_table(f'{prj_dir}/disp-x.his',sep='  ',skiprows=2,names=col_1)
        # disp_z = pd.read_table(f'{prj_dir}/disp-z.his',sep='  ',skiprows=2,names=col_3)
        # updated_disp_x = disp_x.disp_x
        # line1.set_xdata(it.cycle)
...
def loading_procedure(soil_layers,load_final,load_matrix_final,prj_dir):
    ...
    # cycle_Foundation = it.cycle
    # interval = 200
    # it.set_callback("history2Convergence", -1)
