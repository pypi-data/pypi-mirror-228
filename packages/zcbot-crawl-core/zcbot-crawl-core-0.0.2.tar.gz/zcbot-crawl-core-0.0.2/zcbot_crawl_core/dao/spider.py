...
def get_batch_spider(spider_id: str) -> Optional[BatchSpider]:
    row = Mongo().get('zcbot_batch_spider', {'spiderId': spider_id, 'enable': 1})
    if not row:
        return None
...
# 获取流式采集爬虫配置
def get_stream_spider_redis_key(plat_code, task_type):
    return Mongo().get('zcbot_stream_spider', {'_id': _build_stream_spider_key(task_type, plat_code)})
