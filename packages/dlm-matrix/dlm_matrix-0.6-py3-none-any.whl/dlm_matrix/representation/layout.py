...
class MessageLayout(Relationship):
    ...
    def _calculate_coordinates(
        ...
        depth: int,
        mapping: object,
        scaling: str = ScalingType.LINEAR_COMBINATION,
        alpha_scale: float = 1,
    ...
    def calculate_weighted_z_coord(
        ...
        """
        Calculate a weighted z-coordinate based on normalized similarity scores and temporal weights.

        Args:
            normalized_scores (List[float]): List of normalized similarity scores, expected to be in the range [0, 1].
            temporal_weights (np.ndarray): Numpy array of temporal weights, expected to have the same length as normalized_scores.

        Returns:
            float: The weighted z-coordinate calculated as the sum of the product of each normalized score and its corresponding temporal weight.

        Raises:
            ValueError: If the lengths of normalized_scores and temporal_weights do not match.
        """

        if len(normalized_scores) != len(temporal_weights):
            raise ValueError(
                "Lengths of normalized_scores and temporal_weights must match."
            )

        # Calculate the weighted z-coordinate
        weighted_z_coord = np.dot(normalized_scores, temporal_weights)
