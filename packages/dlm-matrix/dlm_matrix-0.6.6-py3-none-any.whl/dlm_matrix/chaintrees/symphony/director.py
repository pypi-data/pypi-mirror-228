...
from dlm_matrix.base import Content, Coordinate
from dlm_matrix.utils import split_string_to_parts, log_handler
from .technique import SynthesisTechniqueDirector
from dlm_matrix.embedding.utils import semantic_similarity_cross_entropy_adjusted
...
# Assume these classes and methods are imported:
# ChainDirector, SynthesisTechniqueDirector, semantic_similarity_cross_entropy_adjusted, ChainInstruction, log_handler
from typing import List, Optional, Any
...
class ReplyChainDirector(ChainDirector):
    ...
    def is_similar_to_previous_prompts(self, new_prompt: str) -> bool:
        for prev_prompt in self.previous_prompts:
            if (
                semantic_similarity_cross_entropy_adjusted(new_prompt, prev_prompt)
                > 0.8
            ):
                return True
        return False
    ...
    def construct(
        ...
        if self.is_similar_to_previous_prompts(prompt):
