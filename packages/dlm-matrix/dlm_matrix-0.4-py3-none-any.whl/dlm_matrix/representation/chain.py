...
class ChainRepresentation(CoordinateRepresentation):
    ...
    def _hierarchical_layout(
        self, positions: np.ndarray, hierarchy: Dict[str, List[str]]
    ):
        tree = nx.DiGraph()
        ...
        pos = {}
        depth = nx.shortest_path_length(tree)
        max_depth = max(depth.values())
        for node, (x, y) in zip(self.graph.nodes, positions):
            path = nx.shortest_path(tree, source=node)
            d = depth[node]
            x += (d / max_depth) * np.cos((np.pi / 2) * (len(path) - 1) / max_depth)
            y += (d / max_depth) * np.sin((np.pi / 2) * (len(path) - 1) / max_depth)
            pos[node] = (x, y)
        return np.array([pos[node] for node in self.graph.nodes])

    def hierarchical_layout(self, hierarchy: Dict[str, List[str]]):
        """
        Returns a hierarchical layout of the graph.


        Implmetation input:
            hierarchy: A dictionary mapping parent nodes to lists of child nodes.

        Returns:


        """
        positions = nx.spring_layout(self.graph, seed=0)
        return self._hierarchical_layout(positions, hierarchy)
