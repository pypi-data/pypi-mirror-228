...
import json
from config import BaseTreeGeneratorConfig
...
if __name__ == "__main__":
    config = TreeGeneratorConfigFactory.create(
        content_generation_algorithm="random",
        content_length_distribution="uniform",
        ...
        min_depth=2,
        max_depth=4,
        content_characters=string.ascii_letters + string.digits + string.punctuation,
    )
    generator = TreeGeneratorFactory.create(config)
    trees = generator.generate_trees(["a", "b", "c"], ["content", "structure"])
    # prnt json.dumps(trees, indent=4)
