...
class ChatOpenAI(BaseChatModel):
    ...
    def get_num_tokens(
        ...
        if last_user_msg:
            try:
                similarity_score = semantic_similarity_cross_entropy_adjusted(
                    text, last_user_msg
                )
                adjusted_token_count = int(
                    base_token_count * (1 + similarity_weight * similarity_score)
                )
    ...
    def _truncate_conversation_history(
        ...
        for message in reversed(adjusted_tokens):
            if tokens_so_far + message["tokens"] <= min(target_tokens, self.max_tokens):
