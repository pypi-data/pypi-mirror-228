...
class CoordinateRepresentation(Representation):
    ...
    def _procces_coordnates(
        ...
        **kwargs: Dict[str, Union[str, float]],
        ...
        (
            embeddings,
            message_ids,
        ) = self.handler.semantic_similarity_model.generate_embeddings(
            self.message_dict
        )
    ...
    def get_chain_document_parameters(
        ...
        doc_dict = {
            ...
            "embedding": embedding_data[2].tolist(),
    ...
    def create_prompt_response_df(self) -> pd.DataFrame:
        ...
        def extract_coordinates(coordinate) -> list:
            ...
            # Assuming the coordinate_str is a string representation of a list/tuple
        ...
        all_texts = list(set(prompts + responses))
        all_encoded_texts = (
            self.handler.semantic_similarity_model.encode_texts_in_batches(all_texts)
        )
        encoding_lookup = {
            text: encoding for text, encoding in zip(all_texts, all_encoded_texts)
        }

        # Explicitly convert the embeddings to lists of floats
        prompt_encodings = [list(map(float, encoding_lookup[p])) for p in prompts]
        response_encodings = [list(map(float, encoding_lookup[r])) for r in responses]
