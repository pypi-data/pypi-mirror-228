...
class ChainRepresentation(CoordinateRepresentation):
    def __init__(
        ...
        semantic_similarity_model: Optional[SemanticSimilarity] = None,
        message_dict: Optional[Dict[str, Union[Coordinate, Any]]] = None,
        conversation_dict: Optional[Dict[str, Union[Coordinate, Any]]] = None,
    ):
        ...
        if api_key:
            self.semantic_similarity_model = SemanticSimilarity(api_key=api_key)
        elif semantic_similarity_model:
    ...
    def encode_texts(
        self, prompts: List[str], responses: List[str], use_openai: bool = False
        ...
        - prompts (List[str]): List of prompts.
        - responses (List[str]): List of responses.
        - use_openai (bool, optional): Flag to use OpenAI API for encoding. Defaults to False.
        ...
        unique_texts = list(collections.OrderedDict.fromkeys(prompts + responses).keys())
        
        # Encode these unique texts
        if use_openai:
            print("Using OpenAI API for encoding")
            all_encoded_texts = self.semantic_similarity_model._embed_text_batch(unique_texts)
        else:
            all_encoded_texts = self.handler.spatial_similarity_model.encode_texts_in_batches(unique_texts)
    ...
    def create_prompt_response_df(
        self, pre_computed_embeddings: bool = True, use_openai: bool = False
    ) -> pd.DataFrame:
        """
        ...
        if not pre_computed_embeddings:
            prompt_encodings, response_encodings = self.encode_texts(
                prompts, responses, use_openai=use_openai
            )
        ...
        relationship_df = pd.DataFrame(
            {
                ...
                "title": self.conversation.conversation.title,
            }
        )
