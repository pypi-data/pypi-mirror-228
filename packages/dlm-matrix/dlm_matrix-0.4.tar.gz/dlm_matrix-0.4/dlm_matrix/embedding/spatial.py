...
class SpatialSimilarity:
    ...
    def _apply_clustering_common(self, main_df, umap_embeddings, clustering_func):
        ...
        # Save main_df to a CSV file
        main_df.to_csv("embeddings_main_df.csv", index=False)

        # Create a DataFrame for 3D results
        columns_for_result3d = [
            "x",
            "y",
            "z",
            "content",
        ...
        result3d = pd.DataFrame(
            data=np.hstack(
                [
                    umap_embeddings,
                    main_df[["text", "author", "message_id"]].values,
                    labels[:, np.newaxis],
                ]
            ),
            columns=columns_for_result3d,
        )
