...
class ChainHandler(BaseModel):
    ...
    def serialize_tree_docs(self, tree_docs: Any) -> Union[str, None]:
        """Converts tree documents containing NumPy arrays to a JSON-serializable format."""
        try:
            json_tree_docs = json.dumps(
                tree_docs,
                default=lambda x: x.tolist() if isinstance(x, np.ndarray) else x,
            )
            return json_tree_docs
        except Exception as e:
