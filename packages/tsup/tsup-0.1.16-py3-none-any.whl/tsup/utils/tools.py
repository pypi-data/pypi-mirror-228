...
def make_insert_tdengine(
    conn, table, data, auto_update=False, update_columns=(), insert_ignore=False
):
    ...
    if update_columns:
        ...
        sql = "insert or {} into `{table}` ({keys}) values ({values})".format(
            "ignore" if insert_ignore else "replace",
            table=table,
            keys=keys,
            values=values,
        )
        if update_columns_:
            sql += " on duplicate key update {}".format(update_columns_)
...
def make_update_tdengine(conn, table, data, condition):
    key_values = []
    ...
    sql = "update `{table}` set {key_values} where {condition}".format(
        table=table, key_values=key_values, condition=condition
    )
    conn.execute(sql)


def make_batch_tdengine(
    conn, table, datas, auto_update=False, update_columns=(), update_columns_value=()
):
    if not datas:
        return
    ...
    if update_columns:
        ...
        sql = "insert or replace into `{table}` ({keys}) values ({values_placeholder})".format(
            table=table, keys=keys, values_placeholder=values_placeholder
        )
    else:
        sql = "insert or ignore into `{table}` ({keys}) values ({values_placeholder})".format(
            table=table, keys=keys, values_placeholder=values_placeholder
        )
...
def make_insert_sqlite(
    ...
    if update_columns:
        ...
        if update_columns_:
            sql += " on conflict do update set {}".format(update_columns_)
...
def make_update_sqlite(table, data, condition):
    key_values = []
...
def make_batch_sqlite(
    ...
    if update_columns:
        ...
        sql = "insert into `{table}` ({keys}) values ({values_placeholder}) on conflict ({keys}) do update set {update_columns}".format(
            table=table,
            keys=keys,
            values_placeholder=values_placeholder,
            update_columns=update_columns_,
