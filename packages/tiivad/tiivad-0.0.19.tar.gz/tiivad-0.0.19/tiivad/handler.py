...
def run_test(check_type, checks, component, kwargs, test_status, test_type):
    ...
    if test_type in STATIC_TESTS_ONE | STATIC_TESTS_MANY:
        ...
        if ea.raised_exception() and isinstance(ea.exception, OutOfInputsError):
            test_status = TestResult.FAIL
            checks.append(
                check_result(test_type, test_status, "Programm küsis rohkem sisendeid, kui testil oli anda"))
        ...
        check_lists = [
            ...
            # Funktsiooni tagastusväärtus ja muudetud väärtus
            kwargs.get("return_value_checks", []) + kwargs.get("param_value_checks", []),
            # Objekti olek
