...
class MultiTasks:
    def __init__(self, threads=10, pool=type_thread):
        self.pool = pool
    ...
    def execute_task(self, print_percent=True):
        with self.pool(max_workers=self.threads, initializer=init,
            ...
            if print_percent:
                iter_object = tqdm(self.features.items(),desc='Multi Tasks Progress')
            else:
                iter_object = self.features.items()
            for k, feature in iter_object:
