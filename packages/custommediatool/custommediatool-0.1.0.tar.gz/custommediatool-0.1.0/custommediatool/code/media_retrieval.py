...
news_api_ai = os.environ.get("newsapiai")
er = EventRegistry(apiKey=news_api_ai, allowUseOfArchive=False)
...
def get_companies_news(company, nmax=10, company_wiki=""):
    """
...
def get_dredging_news(target_query, bims_oppr="", nmax=10):
    """
...
def get_media_news(queries=[], bims_queries=[]):
    ...
    for company in comapnies:
        companies_news_dataframes.append(
            get_companies_news(company, 20, companies_wiki.get(company))
        )
    ...
    for query in gen_dredg_queries:
        general_dredg_news_dataframes.append(get_dredging_news(query, 20))
    ...
    if len(bims_queries) > 0:
        ...
        for query in bims_queries:
            bims_projects_news_dataframes.append(get_dredging_news(query[0], query[1], 20))
