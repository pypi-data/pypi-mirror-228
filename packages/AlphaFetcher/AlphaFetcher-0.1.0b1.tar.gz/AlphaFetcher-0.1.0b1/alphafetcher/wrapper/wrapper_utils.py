			...
			if multithread:
				with ThreadPoolExecutor(max_workers=workers) as executor:
					for uniprot_access in self.uniprot_access_list:
						executor.submit(self._fetch_single_metadata, uniprot_access, alphafold_api_base, pbar)
			else:
				for uniprot_access in self.uniprot_access_list:
			...
			if multithread:
				with ThreadPoolExecutor(max_workers=workers) as executor:
					for uniprot_access in valid_uniprots:
						executor.submit(self._download_single_protein, uniprot_access, pdb, cif, bcif, pae_image,
						                pae_data, pbar)
			else:
				for uniprot_access in valid_uniprots:
