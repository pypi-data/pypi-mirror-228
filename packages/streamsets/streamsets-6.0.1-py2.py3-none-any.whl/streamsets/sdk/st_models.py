...
from .utils import format_log, get_attribute, get_params, pipeline_json_encoder, SeekableList
...
class Stage:
    ...
    def add_output(self, *other_stages, event_lane=False, output_lane_index=None):
        ...
        if non_stages:
            raise TypeError('{} instances cannot be passed to \'>>\'. Only Stage(s) type can be passed to \'>>\'.'
...
class StageLibrary:
    ...
    def __init__(self, stage_library, repository_manifest):
        ...
        if self.id == 'not-specified':
            self.id, self.version = re.match(r'(streamsets-transformer-.+-lib)-(.+).tgz',
