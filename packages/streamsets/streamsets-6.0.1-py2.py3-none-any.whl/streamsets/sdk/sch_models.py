...
from . import sch_api
from .constants import SNOWFLAKE_EXECUTOR_TYPE, SNOWFLAKE_STAGE_RENAME_ALIASES
from .exceptions import InternalServerError, TopologyIssuesError
from .models import Configuration
from .sdc import DataCollector as SdcDataCollector
from .sdc_api import ApiClient as SdcApiClient
from .sdc_models import (PipelineBuilder as SdcPipelineBuilder, PipelineMetrics as SdcPipelineMetrics,
                         Stage as SdcStage, Batch as SdcBatch)
...
from .utils import (DEFAULT_PROVISIONING_SPEC, MutableKwargs, SeekableList, TRANSFORMER_DEFAULT_EXECUTION_MODE, Version,
                    build_tag_from_raw_tag, format_sch_log, get_library_directory_name, get_params, get_attribute,
                    get_topology_nodes, METERING_METRIC_LOOKUP, reversed_dict, set_acl, top_objects_by_metering_metric,
...
class ACLPermissionBuilder():
    ...
    def build(self, subject_id, subject_type, actions):
        ...
        try:
            ...
            raise ValueError('Subject ID {} does not exist in this organization.'
...
class _SchStage:
    """
        Args:
            ...
            supported_connection_types (:obj:`list`, optional): List of :obj:`str` instances of supported connection
                types. Default: ``None``

    """
    def __init__(self, supported_connection_types=None, *args, **kwargs):
    ...
    def use_connection(self, *connections):
        ...
        if connection.connection_type not in self._supported_connection_types:
            raise ValueError('Stage {} does not support connections of type {}.'
                             ' Supported connection types are {}'.format(self.stage_name,
...
class SchSdcStage(_SchStage, SdcStage):
    pass


class SchStStage(_SchStage, StStage):
    pass
...
class PipelineBuilder(SdcPipelineBuilder):
    ...
    def add_stage(self, label=None, name=None, type=None, library=None):
        ...
        # Store a stage's supported connection types to avoid users adding unsupported connections to a stage
        stage_json_definition = next(item for item in self._definitions['stages']
                                     if item['name'] == stage_instance['stageName'])
        supported_connection_types = ([config['connectionType'] for config in stage_json_definition['configDefinitions']
                                       if config['fieldName'] == 'connectionSelection' and config['connectionType']])
    ...
    def remove_stage(self, stage):
        ...
        if self != stage._pipeline:
            raise ValueError("Stage '{}' does not belong to the PipelineBuilder '{}'. Please pass a valid"
                             " stage that has already been added to the PipelineBuilder.".format(stage, self))

        # Find the stage within the PipelineBuilder
        stage_instance = next((stage_instance for stage_instance in self._pipeline[self._config_key]['stages']
                               if stage_instance['instanceName'] == stage.instance_name), None)

        if stage_instance:
            # Remove stage from PipelineBuilder
            self._pipeline[self._config_key]['stages'] = [pipeline_stage for pipeline_stage in
            ...
            raise ValueError("Stage '{}' does not exist in the stages of PipelineBuilder '{}'. Please pass a valid"
                             " stage that has already been added to the PipelineBuilder.".format(stage, self))

    def build(self, title='Pipeline', labels=None, build_from_imported=False, **kwargs):
        """Build the pipeline.
        ...
        if build_from_imported:
            pipeline = Pipeline(pipeline=self._sch_pipeline,
                                builder=self,
    ...
    def import_pipeline(self, pipeline, commit_id_regeneration=True, regenerate_id=True,  **kwargs):
        ...
        if isinstance(pipeline, dict):
            warnings.warn('A new import_pipeline method has been created to allow for a pipeline object to be passed'
...
class StPipelineBuilder(StPipelineBuilder):
    ...
    def build(self, title='Pipeline', build_from_imported=False, **kwargs):
        """Build the pipeline.
...
class Pipeline(BaseModel):
            ...
            if not self._data_internal['libraryDefinitions']:
                try:
                    if self.executor_type == SNOWFLAKE_EXECUTOR_TYPE:
                        self._data_internal['libraryDefinitions'] = (self._control_hub.api_client
                                                                     .get_pipelines_definitions(SNOWFLAKE_EXECUTOR_TYPE)
                                                                     .response.json())
                    else:
                        authoring_engine = self._control_hub.engines.get(id=self.sdc_id)
                        self._data_internal['libraryDefinitions'] = (authoring_engine._instance.api_client
                                                                     .get_definitions())
                except (ValueError, InternalServerError) as e:
                    if self.executor_type == SNOWFLAKE_EXECUTOR_TYPE:
                        error_message = (
                            'Editing pipeline {} is not supported when the snowpark engine is not accessible,'
                            ' please check connectivity to the StreamSets Platform or contact StreamSets Support.'
                            .format(self.pipeline_id))
                    else:
                        error_message = ('Editing pipeline {} is not supported when the engine is not accessible,'
                                         ' please check if the engine {} is running and try again.'
                                         .format(self.pipeline_id, self.sdc_id))
                    raise ValueError(error_message) from e

        # if provided library_definition is a string, convert it first before setting it
        if isinstance(self._data_internal['libraryDefinitions'], str):
            self._data_internal['libraryDefinitions'] = json.loads(self._data_internal['libraryDefinitions'])
        return self._data_internal['libraryDefinitions']

    @_library_definitions.setter
    def _library_definitions(self, library_definition):
        # if provided library_definition is a string, convert it first before setting it
        if isinstance(library_definition, str):
            self._data_internal['libraryDefinitions'] = json.loads(library_definition)
        else:
    ...
    def configuration(self):
        ...
        return Configuration(configuration=self._pipeline_definition['configuration'], id_to_remap=mapping)
    ...
    def stages(self):
        ...
        stage_class = _SchSdcStage if executor_type in ['COLLECTOR', 'SNOWPARK'] else _SchStStage
        pipeline_builder = SdcPipelineBuilder if executor_type in ['COLLECTOR', 'SNOWPARK'] else StPipelineBuilder

        all_stages = {}
        if self._library_definitions:
            all_stages = pipeline_builder._generate_all_stages(self._library_definitions)
        ...
        for stage in self._pipeline_definition['stages']:
            ...
            extra_args = {}
            if self._library_definitions:
                stage_definition = next((stage_def for stage_def in self._library_definitions['stages']
                                        if stage_def['name'] == stage['stageName']), {})

                variable_output_drive = stage_definition.get('outputStreamsDrivenByConfig',
                                                             None) if stage_definition else None
                if variable_output_drive == 'lanePredicates':
                    extra_args.update({'variable_output_drive': variable_output_drive})
            stage_list.append(all_stages.get(stage['stageName'],
                                             stage_class)(stage=stage, pipeline=self, label=stage['stageName'],
                                                          **extra_args))
        return stage_list
    ...
    def remove_stages(self, *stages):
        ...
        instance_names_to_remove = {stage.instance_name: stage for stage in stages
                                    if stage._pipeline.pipeline_id == self.pipeline_id}

        # If no valid stages have been provided, raise ValueError
        if not instance_names_to_remove:
            raise ValueError('No valid stages have been provided. Please provide stages that exist within the Pipeline'
                             ' {}.'.format(self))

        # Remove stages from the Pipeline
        self._pipeline_definition_internal['stages'] = [pipeline_stage for pipeline_stage in
                                                        self._pipeline_definition_internal['stages'] if
                                                        pipeline_stage['instanceName'] not in instance_names_to_remove]
    ...
    def error_stage(self):
        ...
        return (all_stages.get(error_stage['stageName'],
                               stage_class)(stage=error_stage, pipeline=self, label=error_stage['stageName'])
    ...
    def stats_aggregator_stage(self):
        ...
        return (all_stages.get(stats_aggregator_stage['stageName'],
                               stage_class)(stage=stats_aggregator_stage,
                                            pipeline=self,
                                            label=stats_aggregator_stage['stageName'])
    ...
    def add_stage(self, label=None, name=None, type=None, library=None):
        ...
        # Create pipeline_builder if Pipeline was not created via the SDK, else use pipeline's pipeline_builder instance
        try:
            if self.executor_type == SNOWFLAKE_EXECUTOR_TYPE:
                pipeline_builder = (self._control_hub
                                    .get_pipeline_builder(engine_type=executor_type.get(self.executor_type)))
            else:
                pipeline_builder = (self._control_hub
                                    .get_pipeline_builder(engine_type=executor_type.get(self.executor_type),
                                                          engine_id=self._data['sdcId'])
                                    if not self._builder else self._builder)
        ...
        # downstream of the get_pipeline_builder method and throws an InternalServerError
            ...
            raise ValueError('Editing pipeline {} is not supported when the engine is not accessible,'
                             ' please check if the engine is running and try again.'.format(self.pipeline_id)) from e

        # Setting so that pipeline_builder has the complete list of current stages and so that we can create a uniquely
        # labeled stage instance and don't default to stage_instance_1.
        # i.e. we label it stage_instance_n if n-1 stage_instances exist
        pipeline_builder._pipeline[pipeline_builder._config_key]['stages'] = self._pipeline_definition['stages']
...
class TopologyBuilder:
    ...
    def add_job(self, job):
        ...
        for node in nodes:
            ...
            if ('stageIcons' in library_definitions and library_definitions['stageIcons'] and
                icon_key in library_definitions['stageIcons']):
...
class Deployment(BaseModel):
    ...
    def engine_instances(self):
        warnings.warn('engine_instances is no longer supported, and will be deprecated in future releases.'
                      ' Instead please use desired_instances', DeprecationWarning)
...
class SelfManagedDeployment(Deployment):
    ...
    def install_script(self, install_mechanism='DEFAULT'):
        ...
        if not hasattr(self.InstallMechanism, install_mechanism.upper()):
            ...
            raise ValueError("Invalid installation mode. "
                             "Please specify one of the following: {}.".format(valid_mechanism))
...
class EC2Deployment(Deployment):
    ...
    """
    _ATTRIBUTES_TO_REMAP = {'ec2_instance_type': 'instanceType',
...
class GCEDeployment(Deployment):
    ...
    def is_complete(self):
        """Checks if all required fields are set in this deployment."""
        return all([self.region, self.zone, self.desired_instances >= 0, self.machine_type,
                    self.instance_service_account])
...
class SAQLSearchBuilder:
    ...
    def add_filter(self, property_name='name', property_operator='contains', property_value='',
        ...
        if property_name not in available_property_names:
            raise ValueError('{} is not an accepted property name value, the available values for {} are: {}'
                             .format(property_name, self._saql_search['type'], available_property_names))
        available_operators = SAQLSearchBuilder.OPERATOR_VALUES
        if property_operator not in available_operators:
            raise ValueError('{} is not an accepted operator value, the available values are: {}'
                             .format(property_operator, available_operators))
