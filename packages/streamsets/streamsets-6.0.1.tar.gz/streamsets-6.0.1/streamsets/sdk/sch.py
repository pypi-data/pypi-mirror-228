...
from .constants import SDC_EXECUTOR_TYPE, SNOWFLAKE_EXECUTOR_TYPE, SNOWFLAKE_ENGINE_ID, TRANSFORMER_EXECUTOR_TYPE
...
from .sch_models import (ActionAudits, AdminTool, Alert, ApiCredentialBuilder, ApiCredentials,
                         ...
                         Connections, DataSlaBuilder, DeploymentBuilder, DeploymentEngineConfigurations, Deployments,
                         DraftRuns, Engine, Engines, EnvironmentBuilder, Environments,
                         EC2Deployment, GCEDeployment, GCPEnvironment, GroupBuilder, Groups,
                         Job, JobBuilder, Jobs, KubernetesDeployment, KubernetesEnvironment, LegacyDeploymentBuilder,
                         LegacyDeployments, LoginAudits, MeteringUsage, NotImplementedEnvironment,
                         NotImplementedDeployment, OrganizationBuilder, Organizations, Pipeline,
                         ...
                         ReportDefinitions, SAQLSearch, SAQLSearchBuilder, SAQLSearches,
                         ScheduledTaskBuilder, ScheduledTask, ScheduledTasks, SelfManagedDeployment,
                         SelfManagedEnvironment, StPipelineBuilder, SubscriptionAudit, SubscriptionBuilder,
...
DEFAULT_SESSION_ATTRIBUTES = {'verify': True}
...
DEFAULT_USER_ROLES = ['connection:user', 'csp:deployment-manager', 'csp:environment-manager', 'datacollector:admin',
                      'jobrunner:operator', 'notification:user', 'org-user', 'pipelinestore:pipelineEditor',
                      'provisioning:operator', 'sla:editor', 'timeseries:reader', 'topology:editor', 'user']
...
class ControlHub:
    ...
    def __init__(self,
        ...
        self._roles.update({'datacollector:admin': 'Engine Administrator',
    ...
    def _get_aster_api_client(self):
        ...
        # a ControlHub object.
        aster_api_client = aster_api.ApiClient(server_url=self._aster_url or DEFAULT_ASTER_SERVER_URL,
                                               sch_auth_token=self.token,
                                               sch_credential_id=self.credential_id,
                                               session_attributes=DEFAULT_SESSION_ATTRIBUTES)
    ...
    def organization_global_configuration(self):
        ...
        ID_TO_REMAP = {'Timestamp of the contract expiration': 'Timestamp of the contract expiration.'
    ...
    def get_pipeline_builder(self, engine_type, engine_id=None, fragment=False):
        ...
        """
        if engine_type in {'data_collector', 'transformer'}:
            if not engine_id:
                raise ValueError("Pipelines of type 'data_collector' or 'transformer' require an engine id.")
            authoring_engine = self.engines.get(id=engine_id)
    ...
    def publish_pipeline(self, pipeline, commit_message='New pipeline', draft=False):
        ...
        if execution_mode == SNOWFLAKE_EXECUTOR_TYPE:
            ...
            try:
                ...
                raise Exception('The authoring engine for this pipeline does not exist in Control Hub. Please update '
                                'the authoring engine via pipeline.sdc_id') from ex
    ...
    def invite_user(self, user):
        ...
        # Second, retrieve the user entry in DataOps and update the roles/groups selected during UserBuilder.build()
        sch_user_id = '{}@{}'.format(aster_user['data']['schUser'], self.organization)
        sch_user_json = self.api_client.get_user(org_id=self.organization,
                                                 user_id=sch_user_id).response.json()
        sch_user_json.update({'roles': user._data['roles'], 'groups': user._data['groups']})
        update_user_command = self.api_client.update_user(org_id=self.organization,
                                                          user_id=sch_user_id,
                                                          body=sch_user_json)
    ...
    def activate_user(self, *users):
        ...
        aster_api_client = self._get_aster_api_client()
        aster_user_ids = [aster_api_client.get_org_users(search='schUser=={}'.format(user_id)
                                                         ).response.json()['data']['content'][0]['id']
    ...
    def assign_administrator_role(self, user):
        ...
        aster_api_client = self._get_aster_api_client()
        aster_user_json = aster_api_client.get_org_users(search='schUser=={}'.format(user_id)
                                                         ).response.json()['data']['content'][0]
    ...
    def deactivate_engine(self, engine):
        ...
        """
        logger.info('Deactivating engine component from organization %s with component id %s ...',
                    self.organization, engine.id)
        self.api_client.deactivate_components(org_id=self.organization,
                                              components_json=[engine.id])
    ...
    def activate_engine(self, engine):
        ...
        Args:
            engine (:py:class:`streamsets.sdk.sch_models.Engine`): Engine instance.
        """
        logger.info('Activating engine component from organization %s with component id %s ...',
                    self.organization, engine.id)
    ...
    def delete_and_unregister_engine(self, engine):
        ...
        """
        logger.info('Deactivating engine component from organization %s with component id %s ...',
                    engine.organization, engine.id)
        self.api_client.deactivate_components(org_id=self.organization,
                                              components_json=[engine.id])
        logger.info('Deleting engine component from organization %s with component id %s ...',
                    engine.organization, engine.id)
    ...
    def update_engine_labels(self, engine):
        ...
        """
        logger.info('Updating engine %s with labels %s ...',
                    engine.id, engine.labels)
        return Engine(engine=self.api_client.update_sdc_labels(data_collector_id=engine.id,
                                                               data_collector_json=engine._data
                                                               ).response.json(), control_hub=self)
    ...
    def update_engine_resource_thresholds(self, engine, max_cpu_load=None, max_memory_used=None,
    ...
    def add_api_credential(self, api_credential):
        ...
        if not api_credential._request:
            raise AttributeError('Malformed API Credential: the _request attribute is missing. Please verify that the '
    ...
    def delete_job(self, *jobs):
        ...
        """
        job_ids = [job.job_id for job in jobs]
        job_template_ids = [job.job_id for job in jobs if job.job_template]
        if job_template_ids:
            self.api_client.cascade_delete_job_template(job_template_ids)
            job_ids = list(set(job_ids) - set(job_template_ids))
    ...
    def start_job_template(self, job_template, name=None, description=None, attach_to_template=True,
                           ...
                           wait_for_data_collectors=False, inherit_permissions=False, wait=True, asynchronous=False):
        ...
        jobs_response = (self.api_client.create_and_start_job_instances(
                            job_template.job_id, start_job_template_json, inherit_permissions) if not asynchronous else
                         self.api_client.create_and_start_job_instances_async(
    ...
    def validate_pipeline(self, pipeline):
        ...
        if pipeline.executor_type != 'SNOWPARK':
            ...
            if self.use_websocket_tunneling:
                tunneling_instance_id = self.api_client.get_tunneling_instance_id(pipeline.sdc_id
                                                                                  ).response.json()['instanceId']
                validate_command = (self.api_client.validate_engine_pipeline(engine_id=pipeline.sdc_id,
                                                                             pipeline_id=engine_pipeline_id,
                                                                             tunneling_instance_id=tunneling_instance_id
                                                                             )
                                    .wait_for_validate(engine_id=pipeline.sdc_id,
                                                       tunneling_instance_id=tunneling_instance_id))
            else:
            ...
            validate_command = (self.api_client.validate_snowflake_pipeline(engine_pipeline_id, timeout=500000)
                                .wait_for_validate())
    ...
    def run_pipeline_preview(self, pipeline, batches=1, batch_size=10, skip_targets=True,
                             skip_lifecycle_events=True, end_stage=None, only_schema=None,
                             timeout=120000, test_origin=False, read_policy=None,
                             write_policy=None, executor=None, **kwargs):
    ...
    def run_snowflake_pipeline_preview(self, pipeline_id, rev=0, batches=1, batch_size=10, skip_targets=True,
                                       end_stage=None, only_schema=None, timeout=2000, test_origin=False,
        ...
        preview_command = self.api_client.run_snowflake_pipeline_preview(pipeline_id, rev, batches, batch_size,
                                                                         skip_targets, end_stage, only_schema, timeout,
                                                                         test_origin, stage_outputs_to_override_json)
        if kwargs.get('wait', True):
    ...
    def test_pipeline_run(self, pipeline, reset_origin=False, parameters=None):
        ...
        update_rules_command = (executor_instance.api_client
                                .update_pipeline_rules(pipeline_id=executor_pipeline_id, pipeline=pipeline_rules))
    ...
    def publish_scheduled_task(self, task):
        ...
        Returns:
            An instance of :py:class:`streamsets.sdk.sch_api.Command`.
        """
        warnings.warn('The publish_scheduled_task method has been deprecated and will be removed in a future release. '
                      'Please use the add_scheduled_task method instead.', DeprecationWarning)
    ...
    def add_scheduled_task(self, task):
        ...
        if not returned_data['response']['success']:
            if returned_data['response']['messages']:
                ...
                raise Exception(f'Could not published Scheduled Task {repr(task)} due to Error: {error_message}')
            else:
                raise Exception(f'Could not published Scheduled Task {repr(task)}')
    ...
    def update_scheduled_task(self, task):
        ...
        Returns:
            An instance of py:class:`streamsets.sdk.sch_api.Command`.
        """
        update_task_command = self.api_client.update_scheduled_task(data={'data': task._data},
                                                                    id=task.id,
                                                                    api_version=2)
    ...
    def _validate_and_execute_scheduled_tasks_action(self, action, *scheduled_tasks):
        ...
        Args:
            action (:py:class:`streamsets.sdk.sch_models.ScheduledTask.ScheduledTaskActions`): An enum of the action to be performed
            *scheduled_tasks:  One or more instances of :py:class:`streamsets.sdk.sch_models.ScheduledTask`
        Returns:
            An instance of:py:class:`streamsets.sdk.sch_api.Command`.
        """

        if not isinstance(action, ScheduledTask.ScheduledTaskActions):
            raise ValueError('Action not permitted on scheduled task, should be of type ScheduledTask.ScheduledTaskActions')
        ...
        response_command = self.api_client.perform_bulk_action_on_scheduled_tasks(list(scheduled_tasks_map.keys()),
                                                                                  action.value,
                                                                                  api_version=2)
        ...
        if scheduled_tasks_map:
            logger.warning('Scheduled tasks with ids {} were not found on Control Hub'
                           .format(', '.join(scheduled_tasks_map.keys())))
    ...
    def pause_scheduled_tasks(self, *scheduled_tasks):
        ...
        return self._validate_and_execute_scheduled_tasks_action(ScheduledTask.ScheduledTaskActions.PAUSE, *scheduled_tasks)
    ...
    def resume_scheduled_tasks(self, *scheduled_tasks):
        ...
        return self._validate_and_execute_scheduled_tasks_action(ScheduledTask.ScheduledTaskActions.RESUME, *scheduled_tasks)
    ...
    def kill_scheduled_tasks(self, *scheduled_tasks):
        ...
        return self._validate_and_execute_scheduled_tasks_action(ScheduledTask.ScheduledTaskActions.KILL, *scheduled_tasks)
    ...
    def delete_scheduled_tasks(self, *scheduled_tasks):
        ...
        return self._validate_and_execute_scheduled_tasks_action(ScheduledTask.ScheduledTaskActions.DELETE, *scheduled_tasks)
    ...
    def import_topologies(self, archive, import_number_of_instances=False, import_labels=False,
        ...
        """
        topologies = self.api_client.import_topologies(topologies_file=archive,
    ...
    def activate_environment(self, *environments, timeout_sec=DEFAULT_WAIT_FOR_STATUS_TIMEOUT):
        ...
        """
        environments = [environment for environment in environments if
                        not isinstance(environment, NotImplementedEnvironment)]
        if not environments:
    ...
    def add_deployment(self, deployment):
        ...
        if engine_type == 'DC':
            processed_stage_libs = ['streamsets-datacollector-{}-lib:{}'.format(lib, engine_version)
                                    for lib in stage_libs]
        elif engine_type == 'TF':
            processed_stage_libs = ['streamsets-spark-{}-lib:{}'.format(lib, engine_version) for lib in stage_libs]
        ...
        if isinstance(deployment, SelfManagedDeployment):
            ...
            fields = ['useSpotInstances', 'kubernetesLabels', 'memoryRequest', 'cpuRequest', 'memoryLimit', 'cpuLimit',
                      'yaml', 'advancedMode', 'hpa', 'hpaMinReplicas', 'hpaMaxReplicas',
    ...
    def get_deployment_builder(self, deployment_type='SELF'):
        ...
        if deployment_type == 'SELF':
            ...
            kubernetes_definition = {'kubernetesLabels': {'type': 'string'},
                                        ...
                                        'hpa': {'type': 'boolean'},
                                        'hpaMinReplicas': {'type': 'integer',
                                                           'format': 'int32'},
                                        'hpaMaxReplicas': {'type': 'integer',
                                                           'format': 'int32'},
                                        'hpaTargetCPUUtilizationPercentage': {'type': 'integer',
                                                                              'format': 'int32'}}
    ...
    def wait_for_job_status(self, job, status, timeout_sec=DEFAULT_WAIT_FOR_STATUS_TIMEOUT):
        ...
        def failure(timeout):
            raise TimeoutError('Timed out after {} seconds waiting for Job `{}` to reach status {}'.format(
    ...
    def wait_for_job_metrics_record_count(self, job, count, timeout_sec=DEFAULT_WAIT_FOR_METRIC_TIMEOUT):
        ...
        def failure(timeout):
            ...
            raise TimeoutError('Timed out after {} seconds waiting for Job metric `{}` to become {}'.format(
                timeout, metric, count
            ))
    ...
    def wait_for_job_metric(self, job, metric, value, timeout_sec=DEFAULT_WAIT_FOR_METRIC_TIMEOUT):
        ...
        def failure(timeout):
            raise TimeoutError('Job realtime summary {} did not reach value {} after {} s.'.format(
    ...
    def get_saql_search_builder(self, saql_search_type, mode='BASIC'):
        ...
        """
        if (not SAQLSearch.PipelineType.has_value(saql_search_type) and
           not SAQLSearch.JobType.has_value(saql_search_type)):
            raise (ValueError('{} is an invalid saql_search_type, the accepted values are: '
                              'PIPELINE, FRAGMENT, JOB_INSTANCE, JOB_TEMPLATE and JOB_DRAFT_RUN.'
                              .format(saql_search_type)))

        if not SAQLSearch.ModeType.has_value(mode):
            raise (ValueError('{} is an invalid mode value, the accepted values are: BASIC and ADVANCED.'
                              .format(mode)))
    ...
    def save_saql_search(self, saql_search):
        ...
        if not isinstance(saql_search, SAQLSearch):
            raise TypeError("saql_search must be an instance of streamsets.sdk.sch_models.SAQLSearch."
