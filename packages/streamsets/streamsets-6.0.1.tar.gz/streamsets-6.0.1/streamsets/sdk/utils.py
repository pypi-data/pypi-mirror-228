...
def get_decoded_jwt(token):
    ...
    return json.loads(base64.b64decode('{}{}'.format(token_payload, padding)))
...
def get_params(parameters, exclusions=None):
    ...
    return {camelize(arg.rstrip('_'), uppercase_first_letter=False): value
...
def update_acl_permissions(api_client, resource_type, permission):
    ...
    function_call_mapping = {'JOB': api_client.update_job_permissions,
                             ...
                             'LEGACY_DEPLOYMENT': api_client.update_legacy_deployment_permissions,
                             'DEPLOYMENT': api_client.update_deployment_permissions,
                             'ENVIRONMENT': api_client.update_environment_permissions,
...
def set_acl(api_client, resource_type, resource_id, acl_json):
    ...
    function_call_mapping = {'JOB': api_client.set_job_acl,
                             ...
                             'LEGACY_DEPLOYMENT': api_client.set_legacy_deployment_acl,
...
def top_objects_by_metering_metric(metering_data, object_type, metric):
    ...
    return OrderedDict([(key, value) for key, value in sorted(metric_by_object.items(),
...
def get_library_directory_name(lib, engine_type):
    ...
    if engine_type in ['TF', 'TRANSFORMER']:
        return ('streamsets-transformer-{}-lib'.format(lib)
                if 'credentialstore' in lib else 'streamsets-spark-{}-lib'.format(lib))
