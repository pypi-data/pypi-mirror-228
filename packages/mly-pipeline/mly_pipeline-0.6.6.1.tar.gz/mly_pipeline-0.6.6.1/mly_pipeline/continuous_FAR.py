...
def subGeneration(dataSet           # The DataSet
    ...
    out_name = dataSet.split('_')[0]+"_"+str(batchNumber)+"_"+str(len(out))
    #logger.debug(f"Batchnumber, batches, output name,{batchNumber}, {batches}, {out_name}")
    out.save(config["falseAlarmRates"]+"/temp/"+out_name)
...
def condorGeneration( batches = config['far_config']['batches']     # Number of parallel subscripts scripts to run 
    ...
    # to make sure that we will not generate dublicate files for the FAR
    ...
    for agroup, afile in zip(all_files_groups,all_files):
        ...
        # If the file group is already in hourly groups, and there is not 
        # any file in the inference output files waiting to be merged with the hourly,
    ...
    while (1):
        ...
        for file in listOfInputs: 
            ...
            while (jobsAndDagsRunning >= batches*(parallelGenerations-1)) or filesInTemp >= parallelGenerations*batches:
                ...
                heldJobs = int(subprocess.check_output("condor_q -hold -autoformat RequestMemory | wc -l",shell=True))

                if heldJobs > 0 and releaseJobsTrials < 14 and releaseJobsTrials%2 !=0:
                    os.system("condor_release --all")
                    releaseJobsTrials += 1
                    logger.info(f"Held Jobs: {heldJobs}, release trials: {releaseJobsTrials}")
                    
                    time.sleep(60)

                elif heldJobs > 0 and releaseJobsTrials <= 14 and releaseJobsTrials%2==0:

                    condor_memory = int(3*2048 + int(releaseJobsTrials/2)*2048)

                    logger.info(f"Held Jobs: {heldJobs},release trials ({releaseJobsTrials}) exceded, updating memory to {condor_memory}")
                    os.system(f"condor_qedit {config['user_name']} -constraint 'JobStatus == 5' RequestMemory {condor_memory}")
                    os.system("condor_release --all")
                    releaseJobsTrials += 1

                    time.sleep(60)
                
                elif heldJobs > 0 and releaseJobsTrials > 14:
                    os.system("condor_release --all")
                    logger.info(f"Held Jobs: {heldJobs}, release trials: {releaseJobsTrials}")
                    logger.warning(f"Memory is currently set to : {condor_memory} but it is still not enough. Update your search parameters to be less memory heavy!")
...
if __name__ == "__main__":
    ...
    print(mode)
    if mode == 'generation' or mode == 'sub_generation':
