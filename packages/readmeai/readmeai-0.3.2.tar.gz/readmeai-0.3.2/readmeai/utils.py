...
def clone_repository(repo_path: str, temp_dir: Path) -> None:
    ...
    try:
        git.Repo.clone_from(
            repo_path, temp_dir, depth=1, git_executable=git_exec_path
        )
    ...
    if platform.system() != "Windows":
        permissions = Path(temp_dir).stat().st_mode & 0o777
        if permissions != 0o755:
            raise ValueError(
                "Error: file permissions of cloned repository must be set to 0o755."
            )
...
def find_git_executable() -> Optional[str]:
    ...
    if platform.system() == "Windows":
        default_windows_path = "C:\\Program Files\\Git\\cmd\\git.EXE"
        if Path(default_windows_path).exists():
            return default_windows_path

    try:
        return subprocess.check_output(["which", "git"], text=True).strip()
