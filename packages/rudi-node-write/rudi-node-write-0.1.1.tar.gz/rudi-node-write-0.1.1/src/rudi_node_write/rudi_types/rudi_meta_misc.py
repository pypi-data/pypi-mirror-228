...
from rudi_node_write.rudi_types.rudi_const import check_rudi_version
...
from rudi_node_write.utils.typing_utils import get_type_name, check_is_int


class RudiDatasetSize(Serializable):
    def __init__(self, number_of_records: int = None, number_of_fields: int = None):
        self.number_of_records = check_is_int(number_of_records, accept_none=True, accept_castable=True)
        self.number_of_fields = check_is_int(number_of_fields, accept_none=True, accept_castable=True)
...
class RudiDataTemporalSpread(Serializable):
    def __init__(self, start_date: str, end_date: str = None):
        self.start_date = Date.from_json(start_date)
        if end_date is None:
            self.end_date = None
        else:
            ...
            if self.start_date > self.end_date:
                raise ValueError(
                    f"Temporal spread implies start date is older than end date, got '{start_date}' > '" f"{end_date}'"
                )
...
class RudiMetadataInfo(Serializable):
    def __init__(
        ...
        metadata_dates: RudiDates = None,
        metadata_provider: RudiOrganization = None,
...
if __name__ == "__main__":  # pragma: no cover
    ...
    log_d(tests, "RudiDatasetSize(4,5)", RudiDatasetSize(4, "5"))
    log_d(
        tests,
    ...
    log_d(tests, "RudiDataTemporalSpread()", RudiDataTemporalSpread(now_iso()))
    log_d(
        tests,
