...
from rudi_node_write.rudi_types.rudi_geo import RudiGeography
from rudi_node_write.rudi_types.rudi_licence import RudiAccessCondition, RudiLicence
from rudi_node_write.rudi_types.rudi_media import (
    RudiMedia,
    RudiMediaService,
    RudiMediaFile,
)
from rudi_node_write.rudi_types.rudi_meta_misc import (
    RudiMetadataInfo,
    RudiDatasetSize,
...
def normalize_theme(theme: str | list) -> str:
    ...
    return normalized_theme.strip() if normalized_theme else theme.strip()
...
class RudiMetadata(Serializable):
    def __init__(
        ...
        local_id: str = None,
        doi: str = None,
        resource_languages: list[Language] | Language = None,
        temporal_spread: RudiDataTemporalSpread = None,
        dataset_size: RudiDatasetSize = None,
        geography: RudiGeography = None,
        access_condition: RudiAccessCondition = None,
        ...
        self.synopsis: RudiDictionaryEntryList = check_type(synopsis, RudiDictionaryEntryList)
        ...
        # ---------- Optional parameters ----------
        self.local_id = check_is_string(local_id, accept_none=True)
        self.doi = check_is_string(doi, accept_none=True)

        self.resource_languages = (
            None
            if resource_languages is None
            else [check_is_literal(resource_languages, RECOGNIZED_LANGUAGES)]
            if is_string(resource_languages)
            else [check_is_literal(lang, RECOGNIZED_LANGUAGES) for lang in resource_languages]
        )

        self.temporal_spread = check_type(temporal_spread, RudiDataTemporalSpread, accept_none=True)
        self.geography = check_type(geography, RudiGeography, accept_none=True)
        self.dataset_size = check_type(dataset_size, RudiDatasetSize, accept_none=True)

        self.collection_tag = check_is_string(collection_tag, accept_none=True)
        self.metadata_source = check_is_string(metadata_source, accept_none=True)
    ...
    def from_json(o: dict):
        ...
        synopsis = RudiDictionaryEntryList.from_json(check_has_key(o, "synopsis"))
        summary = RudiDictionaryEntryList.from_json(check_has_key(o, "summary"))

        theme = check_is_string(check_has_key(o, "theme"))
        keywords = ensure_is_str_list(check_has_key(o, "keywords"))

        producer = RudiOrganization.from_json(check_has_key(o, "producer"))
        ...
        if isinstance(contact_list, list):
            ...
            raise TypeError(
                f"incorrect type for contacts attribute, expected 'list', got '" f"{get_type_name(contact_list)}'"
            )

        available_formats = [RudiMedia.from_json(media) for media in check_has_key(o, "available_formats")]
        ...
        collection_tag = check_is_string(o.get("collection_tag"), accept_none=True)
    ...
    def get_number_of_records(self) -> int | None:
        return self.dataset_size.number_of_records if is_def(self.dataset_size) else None
...
if __name__ == "__main__":  # pragma: no cover
    ...
    rudi_meta2.metadata_info.metadata_dates.validated = now_iso()
    log_d(tests, "RudiMetadata.__eq__", rudi_meta != rudi_meta2)
