...
class CStoichiometryHashLibrary(StoichiometryHashLibrary):
    def match(self, cstoichiometry):
        ...
        for t in index_tests:
            ...
            tstoichiometry = dict(
                map(
                    lambda x: ((x[0][0], replace[x[0][1]]), x[1]),
                    tstoichiometry.items(),
                )
            )
                ...
                for hash_type_lib in self.all_hashes:
                    if hash_val in self.all_hashes[hash_type_lib]:
                        for rxn_id_match in self.all_hashes[hash_type_lib][hash_val]:
                            ...
                            match[rxn_id_match].add(
                                (hash_type, hash_type_lib, hash_val, replace_str)
                            )
...
def multi_hasher(stoichiometry, exclude=[]):
    ...
    s_x = dict(
        filter(lambda x: x[0][0] not in exclude, s.items())
    )  # exclude compounds in exclude
    s_rev_x = dict(
        map(lambda x: (x[0], -1 * x[1]), s_x.items())
    )  # reverse exclude stoichiometry
...
def single_hasher(stoichiometry, ex1=[]):
    ...
    hashes = {"std": std, "rev": rev, "no_h_std": h_std, "no_h_rev": h_rev}
    return hashes
...
def single_hasher2(s, ex1=[]):
    ...
    remove_stoich_val_std = dict(
        map(lambda x: (x[0], x[1] / math.fabs(x[1])), s_copy.items())
    )
    remove_stoich_val_rev = dict(
        map(lambda x: (x[0], -1 * x[1]), remove_stoich_val_std.items())
    )
...
def hasher(s, ex1=[]):
    ...
    hashes = {"std": std, "rev": rev, "h_std": h_std, "h_rev": h_rev}
    return hashes
...
class MapValidator:
    ...
    def expand(self, annotation_id1, annotation_id2, namespace):
        for e in self.mapping:
            if (
                annotation_id1 in self.mapping[e]
                and namespace in self.mapping[e][annotation_id1]
            ):
                for i in self.mapping[e][annotation_id1][namespace]:
                    iref = ExternalReference(i, namespace)
                    if iref in self.mapping and annotation_id2 in self.mapping[iref]:
    ...
    def matcher(a, b):
        if a == b:
            return {"value": "MATCH", "data": set(a)}
        return {"value": "FAIL", "data": (set(a), set(b))}
    ...
    def compare(self, annotation_id1, annotation_id2, namespace, matcher=None):
        ...
        for e in self.mapping:
            if (
                annotation_id1 in self.mapping[e]
                and annotation_id2 in self.mapping[e]
                and namespace in self.mapping[e][annotation_id1]
                and namespace in self.mapping[e][annotation_id2]
            ):
...
class ModelSEEDMapper:
    def __init__(self, ms, s_hash, m_hash, cpd_to_seed):
        ...
        for rxn_id in ms.reactions:
            ...
            if not rxn.is_obsolete and not rxn.is_transport:
                if len(cmps) == 1:
                    stoichiometry = dict(
                        map(lambda x: (x[0][0], x[1]), cstoichiometry.items())
                    )
    ...
    def match(self, model_rxn):
        ...
        if len(cmps) == 1:
            ...
            stoichiometry = dict(
                map(lambda x: (self.remap_compound(x[0]), x[1]), stoichiometry.items())
            )
            ...
            cstoichiometry = dict(
                map(
                    lambda x: ((self.remap_compound(x[0][0]), x[0][1]), x[1]),
                    cstoichiometry.items(),
                )
            )
