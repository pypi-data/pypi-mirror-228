...
def build_graph_from_dict(dict_data, modelseed_local):
    g = nx.DiGraph()
...
class HierarchicalOntology:
    def __init__(self, g, hlib, ontology=None, universal_cpds=None):
        ...
        if g == None:
            ...
            self.g_bridge, self.bridges = HierarchicalOntology.generate_bridge_edges(
                self.g
            )
            self.t_map = HierarchicalOntology.build_translate_map(
                self.g_bridge, universal_cpds, "seed.compound"
            )
        else:
            self.g = g
            self.g_bridge, self.bridges = HierarchicalOntology.generate_bridge_edges(
        ...
        for e in self.g:
            self.g_compounds.add(e[0])
            self.g_compounds.add(e[1])
        self.hlib = hlib
    ...
    def generate_reaction_ontology_old(
        self, rxn, hash_f, all_hashes, test_single_swaps=True
    ):
        result = {}
