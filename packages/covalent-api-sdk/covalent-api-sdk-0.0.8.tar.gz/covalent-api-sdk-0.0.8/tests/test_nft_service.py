...
class TestNftService:
    ...
    @pytest.mark.asyncio
    async def test_success(self, client: Client):
        async for res in client.nft_service.get_chain_collections("eth-mainnet"):
            assert res is not None
        
    @pytest.mark.asyncio
    async def test_unsupported_chains(self, client: Client):
        async for res in client.nft_service.get_chain_collections("fantom-mainnet"):
            assert res == "250/fantom-mainnet chain not supported, currently supports: 1/eth-mainnet 56/bsc-mainnet 137/matic-mainnet 10/optimism-mainnet 42161/arbitrum-mainnet 8453/base-mainnet "
    ...
    @pytest.mark.asyncio
    async def test_success_for_token_Ids_for_contract_metadata(self, client: Client):
    ...
    @pytest.mark.asyncio
    async def test_no_metadata_for_token_Ids_for_contract_metadata(self, client: Client):
    ...
    def test_unsupported_address_traits_collection(self, client: Client):
        ...
        assert res.error == True
        assert res.data == None
        assert res.error_message == "250/fantom-mainnet chain not supported, currently supports: 1/eth-mainnet 56/bsc-mainnet 137/matic-mainnet 10/optimism-mainnet 42161/arbitrum-mainnet 8453/base-mainnet "
        assert res.error_code == 400
