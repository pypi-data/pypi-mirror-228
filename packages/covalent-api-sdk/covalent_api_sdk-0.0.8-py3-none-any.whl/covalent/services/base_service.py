...
from .util.api_helper import check_and_modify_response, chains, paginate_endpoint, quotes, user_agent, Response
...
class BaseService:
    ...
    async def get_block_heights(self, chain_name: chains, start_date: datetime, end_date: datetime, page_size: Optional[int] = None, page_number: Optional[int] = None) -> AsyncIterable[Block]:
        """
        Parameters:
        ...
        while not success:
            try:
                ...
                async for response in paginate_endpoint(f"https://api.covalenthq.com/v1/{chain_name}/block_v2/{start_date}/{end_date}/", self.__api_key, url_params, Block):
                    yield response
    ...
    async def get_log_events_by_address(self, chain_name: chains, contract_address: str, starting_block: Optional[int] = None, ending_block: Optional[str] = None, page_size: Optional[int] = None, page_number: Optional[int] = None) -> AsyncIterable[LogEvent]:
        """
        Parameters:
        ...
        while not success:
            try:
                ...
                async for response in paginate_endpoint(f"https://api.covalenthq.com/v1/{chain_name}/events/address/{contract_address}/", self.__api_key, url_params, LogEvent):
                    yield response
    ...
    async def get_log_events_by_topic_hash(self, chain_name: chains, topic_hash: str, starting_block: Optional[int] = None, ending_block: Optional[str] = None, secondary_topics: Optional[str] = None, page_size: Optional[int] = None, page_number: Optional[int] = None) -> AsyncIterable[LogEvent]:
        """
        Parameters:
        ...
        while not success:
            try:
                ...
                async for response in paginate_endpoint(f"https://api.covalenthq.com/v1/{chain_name}/events/topics/{topic_hash}/", self.__api_key, url_params, LogEvent):
                    yield response
    ...
    def get_address_activity(self, contract_address: str, testnets: Optional[bool] = None) -> Response[ChainActivityResponse]:
        """
        Parameters:
        ...
        while not success:
            try:
                ...
                response = requests.get(f"https://api.covalenthq.com/v1/address/{contract_address}/activity/", params=url_params, headers={
                    "Authorization": f"Bearer {self.__api_key}",
                    "X-Requested-With": user_agent
                })
