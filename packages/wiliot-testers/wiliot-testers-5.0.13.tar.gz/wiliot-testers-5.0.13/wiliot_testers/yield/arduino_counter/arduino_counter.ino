...
unsigned long time_btwn_trigges_ms = 0;
...
void loop(){
  if (Serial.available()>0)
  {
    ...
    {
      time_btwn_trigges_ms = incoming_data.substring(TIME_CMD_SIZE).toInt();
      Serial.print("Max movement time was set to ");
      Serial.print(time_btwn_trigges_ms);
      Serial.println("[sec]");
      time_btwn_trigges_ms = time_btwn_trigges_ms * 1000;
    }
  ...
  if (cur_gpio_state != last_gpio_state)
  {
    if (cur_gpio_state == true)
    {
      ...
      if (pulse > 1)
      {
        if (millis() - trigger_time > time_btwn_trigges_ms)
        {
          Serial.print(pulse); 
