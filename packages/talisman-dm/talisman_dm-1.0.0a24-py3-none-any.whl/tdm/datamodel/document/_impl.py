...
def _filter_elements(base_type: Type[_D]):
    def decorator(f: Callable[_P, 'TalismanDocumentImpl']):
...
class TalismanDocumentImpl(TalismanDocument, ViewContainer):
    ...
    def with_roots(self, nodes: Iterable[Union[str, AbstractNode]]) -> Self:
        return self._replace(structure=self._structure.as_roots(map(_to_id, nodes)))

    def without_structure(self, structure: Dict[Union[str, AbstractNode], Iterable[Union[str, AbstractNode]]]) -> Self:
