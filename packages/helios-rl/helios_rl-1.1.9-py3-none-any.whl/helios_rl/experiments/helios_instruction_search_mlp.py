...
class InstructionSimilarityFeedback(nn.Module):
    def __init__(self, input_size, hidden_size, lr=0.2) -> None:
        super(InstructionSimilarityFeedback, self).__init__()
        if (type(hidden_size) == int):
            self.hidden = nn.Linear(input_size, hidden_size)
            self.last_hidden_size = hidden_size
        else:
            layers = [nn.Linear(input_size, hidden_size)] + [nn.Linear(hidden_size[i - 1], hidden_size[i]) for i in range(1, len(hidden_size))]
            for i in range(1, len(layers)):
                layers.insert(i, nn.Tanh())
            self.hidden = nn.Sequential(*layers)
            self.last_hidden_size = hidden_size[-1]

        self.out = nn.Linear(self.last_hidden_size, input_size)
    ...
    def forward(self, state, instruction):
        ...
        len_instr = instruction.shape[0]
        s = torch.tile(state, (len_instr, 1))
        r = instruction.repeat_interleave(len_state, dim=0)
        
        return self.cos(self.hidden(r), s).mean()
...
class HeliosSearch:
    def __init__(self, Config:dict, LocalConfig:dict, Environment, 
        ...
        self.instr_feedback_layer: InstructionSimilarityFeedback = None    
    ...
    def search(self, action_cap:int=5, re_search_override:bool=False, simulated_instr_goal:any=None):
        ...
        for n, agent_type in enumerate(self.setup_info['agent_select']): 
            ...
            for p in range(0, self.num_plans):
                print("---")
                print("Plan Number = ", p)
                sample_env = self.env(train_setup_info)
                print("Environment Init Position: ", sample_env.start_obs)
                # New: user input here
                instructions, instr_descriptions = self.helios_input.user_input()
                    ...
                    for i,instr in enumerate(instructions):
                        ...
                        if (instruction in self.instruction_results):
                            if (agent_type+'_'+adapter) in self.instruction_results[instruction]:
                                ...
                                self.instr_feedback_layer = InstructionSimilarityFeedback(instruction_vector.shape[-1], 100).to(device)
                        ...
                        self.instr_feedback_layer.train()
                        # ---------------------------
                        search_count=0
                        while not sub_goal:
                            ...
                            if (max_sim<-1)|(sub_goal is None):#|(max_sim>1):
                                ...
                                sub_goal_sim = self.cos(sub_goal_t, instruction_vector)
                                loss = self.instr_feedback_layer.loss_func(max_sim, sub_goal_sim)
                                

                                self.instr_feedback_layer.eval()
                                new_sim = self.instr_feedback_layer(t_state, instruction_vector)
                                print("--- Change in sim results with reinforcement of correct state match =", sim_delta)
                                print("--- New Sim Value = ",  sim)
                                self.instr_feedback_layer.train()
                                
                                if (feedback.lower() == 'y')|(feedback.lower() == 'yes'):
                                    loss = self.instr_feedback_layer.loss_func(max_sim, 1.0)
                                    loss.backward()
                                    self.instr_feedback_layer.optimizer.step()

                                    # Average sim across each sentence in instruction vs state
                                    self.instr_feedback_layer.eval()
                                    new_sim = self.instr_feedback_layer(sub_goal_t, instruction_vector)
                                    sim_delta = new_sim - max_sim
                                    ...
                                    new_sim = self.instr_feedback_layer(sub_goal_t, instruction_vector)
                                    sim_delta = new_sim - max_sim
                                    print("--- Change in sim results with NEGATIVE reinforcement of correct state match =", sim_delta)
                                    print("--- New Sim Value = ",  sim)
                                    self.instr_feedback_layer.train()
                                    sub_goal = None
