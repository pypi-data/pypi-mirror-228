import pathlib
from typing import Any, Union, Iterable, Dict, Sequence, Iterable, Type, IO, List, Optional
...
class GraiYamlSerializer:
    ...
    def load(stream: Union[str, Path, IO]) -> Union[Dict, List[Dict]]:
        ...
        """
        if isinstance(stream, Path) or (isinstance(stream, str) and os.path.exists(stream)):
