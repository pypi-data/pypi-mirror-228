...
class RootWindow(tk.Tk, mixin.ContainerWidget):
    ...
    def start_eventloops(self, **kwargs) -> None:
        ...
        for elID, el in self.get_eventloops(**kwargs):
            logger.debug('Starting "%s" tName=%s', elID, el.tName)
            el.start()
    ...
    def __process_event_bus(self) -> None:
        ...
        if __debug__:
            for elID in self.__event_bus:
                assert isinstance(elID, str), f'Invalid EventLoop {elID!r} @ EventBus: Use keys from `self.eL`'
            unknown_elID = set(self.__event_bus) - set(self.eL)
        ...
        if count_common > 0:
            logger_eventbus.debug('* eventloop="ALL": %d common events', count_common)
            # Make sure all eventloops are defined
            for elID in set(self.eL) - set(typing.cast(typing.Iterable[str], self.__event_bus)):
                ...
                if wcallback is not None and wcallback.callback == self.use_eventbus:
                    logger_eventbus.debug('  * eventloop=%s: register, uses Event Bus', elID)
                    # Implement "defaultdict"
                    self.__event_bus[elID] = defaultdict(list)
                else:
                    logger_eventbus.debug('  * eventloop=%s: skipping, no `use_eventbus`', elID)
        for elID, elEvents in self.__event_bus.items():
            assert elID is not None
            assert isinstance(elID, str)
            assert elID in self.eL
            ...
            if __debug__:
                ...
                assert count_actual == count_all, f'eventloop={elID} event={event}: unmatched count actual={count_actual} spec={count_all}'
