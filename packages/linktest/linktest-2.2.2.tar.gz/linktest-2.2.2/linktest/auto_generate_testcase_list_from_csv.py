...
def generate_testcase_list_by_csv(file_name, rows, is_ui_testcase, tag_name=''):
    ...
    with open(file_name.split(".")[0] + "_case_list_auto_generated_by_csv.py", "a") as f:
        ...
        print(
            f"------ Generated {len(rows)} testcases based on the Python file and its associated CSV file: " + file_name.split(".")[0] + ".py")
                ...
                if file_name.split(".")[-1] == "py" and file_name != "__init__.py" and\
                    ...
                    with open(PROJECT_INFO.project_path + os.sep + directory_path + os.sep + file_name.split(".")[
                        ...
                        for line in py_file_obj.readlines():
                            # todo: Here, also verify whether the parameters in `run_test()` correspond to column names existing in the associated CSV file.
                            if re_func.check_python_method_for_linktest(line):
                                has_run_test_flag = True
                                break

                    if not has_run_test_flag:
                        print("Python 文件:%s 格式不符合linktest框架要求! 不会自动生成对应的testcase文件, 若需查看帮助信息, 请输入以下命令：python3 run.py --help csv" % ((PROJECT_INFO.project_path + os.sep + file_full_name).split(".")[0] + ".py"))
                        print("The format of the Python file: %s does not comply with the requirements of the linktest framework! The corresponding testcase file will not be automatically generated. If you need to view help information, please enter the following command: python3 run.py --help csv" % ((PROJECT_INFO.project_path + os.sep + file_full_name).split(".")[0] + ".py"))
                        continue
