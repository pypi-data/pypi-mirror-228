...
1. settings/__init__.py is used to configure the following items for linktest:
    ...
    - SHOW_LOG_IN_CONSOLE (determines whether to show execution logs; default value is True)
...
1. Set testcase_id in run_test(), e.g:
  ...
  There are two ways to set the tag for test cases:
    1. tag = "smoke, nightly"  # set tag as a string (tag names separated by ","); ensure all test cases have the correct tag name to avoid typos (e.g., "nighty" instead of "nightly")
    2. tag = [Tags.Smoke, Tags.Nightly]  # recommended! set tag as a list (tag names defined in settings/testcase_tags.py)
    Note:
        1. If the tag contains "ignore", the test case will be added only to ignore_testcase_list (even if it contains other tags)
        2. Tag names are not case-sensitive (e.g., tag = "smoke" is the same as tag = "SMOKE")
        3. If the tag name contains a ".", it will be replaced with an "_" (e.g., 'link.test' becomes 'link_test')
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                executing_testcase.logger.info("---------------- Test Framework: linktest==%s" % linktest.__version__ + os.linesep)
                executing_testcase.logger.info(
