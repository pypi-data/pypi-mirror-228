# coding: utf-8

"""
    Gradient AI API

    Interface for interacting with Gradient AI.  # noqa: E501

    The version of the OpenAPI document: 1.0.0
    Contact: support@gradient.ai
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""


from __future__ import annotations
import pprint
import re  # noqa: F401
import json


from typing import Optional
from pydantic import BaseModel, Field, constr
from gradientai.openapi.client.models.fine_tune_model_request_body_samples_inner_fine_tuning_parameters import FineTuneModelRequestBodySamplesInnerFineTuningParameters

class FineTuneModelRequestBodySamplesInner(BaseModel):
    """
    FineTuneModelRequestBodySamplesInner
    """
    fine_tuning_parameters: Optional[FineTuneModelRequestBodySamplesInnerFineTuningParameters] = Field(None, alias="fineTuningParameters")
    inputs: constr(strict=True, min_length=1) = Field(...)
    additional_properties: Dict[str, Any] = {}
    __properties = ["fineTuningParameters", "inputs"]

    class Config:
        """Pydantic configuration"""
        allow_population_by_field_name = True
        validate_assignment = True

    def to_str(self) -> str:
        """Returns the string representation of the model using alias"""
        return pprint.pformat(self.dict(by_alias=True))

    def to_json(self) -> str:
        """Returns the JSON representation of the model using alias"""
        return json.dumps(self.to_dict())

    @classmethod
    def from_json(cls, json_str: str) -> FineTuneModelRequestBodySamplesInner:
        """Create an instance of FineTuneModelRequestBodySamplesInner from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    def to_dict(self):
        """Returns the dictionary representation of the model using alias"""
        _dict = self.dict(by_alias=True,
                          exclude={
                            "additional_properties"
                          },
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of fine_tuning_parameters
        if self.fine_tuning_parameters:
            _dict['fineTuningParameters'] = self.fine_tuning_parameters.to_dict()
        # puts key-value pairs in additional_properties in the top level
        if self.additional_properties is not None:
            for _key, _value in self.additional_properties.items():
                _dict[_key] = _value

        # set to None if fine_tuning_parameters (nullable) is None
        # and __fields_set__ contains the field
        if self.fine_tuning_parameters is None and "fine_tuning_parameters" in self.__fields_set__:
            _dict['fineTuningParameters'] = None

        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> FineTuneModelRequestBodySamplesInner:
        """Create an instance of FineTuneModelRequestBodySamplesInner from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FineTuneModelRequestBodySamplesInner.parse_obj(obj)

        _obj = FineTuneModelRequestBodySamplesInner.parse_obj({
            "fine_tuning_parameters": FineTuneModelRequestBodySamplesInnerFineTuningParameters.from_dict(obj.get("fineTuningParameters")) if obj.get("fineTuningParameters") is not None else None,
            "inputs": obj.get("inputs")
        })
        # store additional fields in additional_properties
        for _key in obj.keys():
            if _key not in cls.__properties:
                _obj.additional_properties[_key] = obj.get(_key)

        return _obj

