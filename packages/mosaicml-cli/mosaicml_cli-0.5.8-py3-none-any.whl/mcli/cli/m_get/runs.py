...
class RunDisplayItem(MCLIDisplayItem):
    ...
    @classmethod
    def from_run(cls, run: Run, include_ids: bool = False, expanded: bool = False) -> List[MCLIDisplayItem]:
        run_rows = []
...
class MCLIRunDisplay(MCLIGetDisplay):
    ...
    def __init__(self, models: List[Run], include_ids: bool = False, expanded: bool = False):
    ...
    def __iter__(self) -> Generator[MCLIDisplayItem, None, None]:
        for model in self.models:
            items = RunDisplayItem.from_run(model, include_ids=self.include_ids, expanded=self.expanded)
            for item in items:
...
def cli_get_runs(
    ...
    **kwargs,
    ...
    display = MCLIRunDisplay(runs, include_ids=include_ids, expanded=expanded)
