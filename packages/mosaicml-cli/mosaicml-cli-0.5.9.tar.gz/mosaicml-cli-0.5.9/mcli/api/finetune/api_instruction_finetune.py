...
def instruction_finetune(model: str,
    ...
    Args:
        ...
        train_data_path: The name of a Hugging Face dataset with the train split (e.g. ``mosaicml/dolly_hhrlhf/train``)
            or the full remote location path of your training data (e.g. ``s3://my-bucket/my-data.jsonl``). The training 
            data should be formatted with each row containing a ``prompt`` and ``response`` field.
        ...
        eval_data_path: The name of a Hugging Face dataset with the test split (e.g. ``mosaicml/dolly_hhrlhf/test``)
            or the remote location of your evaluation data (e.g. ``s3://my-bucket/my-data.jsonl``). The evaluation data
            should be formatted with each row containing a ``prompt`` and ``response`` field. Default is ``None``.
        ...
        context_length: The maximum sequence length to use. This will be used to truncate any data that is too long. 
            ...
            length beyond each model's default. The default context length of each supported model is as follows:
            ``mosaicml/mpt-7b-8k``: 8192, ``mosaicml/mpt-30b``: 8192, ``mosaicml/mpt-7b``: 2048.
