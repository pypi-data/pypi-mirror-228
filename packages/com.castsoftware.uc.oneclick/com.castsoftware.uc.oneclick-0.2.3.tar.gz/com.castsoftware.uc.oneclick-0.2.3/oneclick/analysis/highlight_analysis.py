...
class HLAnalysis(Analysis):
    ...
    def run(cls, config:Config):
        ...
        rest = HLRestCall(hl_base_url=f'{config.hl_url}/WS2',hl_user=config.hl_user,hl_pswd=config.hl_password,hl_instance=config.hl_instance)
        ...
        try:
            ...
            for appl in config.application:
                ...
                if hl_status == '' or hl_status.startswith('Error'):
                    ...
                    proc = None
                    cls._log.info(f'Highlight analysis is already complete, skipping step ...')
