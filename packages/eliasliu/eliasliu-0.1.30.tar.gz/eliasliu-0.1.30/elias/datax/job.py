...
def job_dic(origin_table_name,origin_hosts,target_table_name,target_hosts,channel=6,record=0,percentage=0.02):
    job = {
        "job": {
            "setting": {
                "speed": {
            ...
            "content": [{
                "reader": dr.reader(origin_table_name,origin_hosts)["reader"],
...
def datax_json_config(origin_table_name,origin_hosts,target_table_name,target_hosts,channel=6,record=0,percentage=0.02):
    try:
        logger.info("开始生成json文件内容……" )
        job = job_dic(origin_table_name,origin_hosts,target_table_name,target_hosts,channel,record,percentage)
