...
def maxcompute_reader(origin_table_name,origin_hosts = u.all_hosts('bi_data_warehouse')):
...
def mysql_reader(origin_table_name,origin_hosts = u.all_hosts('bi_data_warehouse')):
    from elias.datax import auto_create_table as act 
    df_schema = act.schema_read(origin_table_name, origin_hosts)
...
def reader(origin_table_name,origin_hosts = u.all_hosts('bi_data_warehouse')):
    
    
    if origin_hosts['dbtype'] == 'maxcompute':
        dic = maxcompute_reader(origin_table_name,origin_hosts)
        return dic
    
    elif origin_hosts['dbtype'] == 'mysql':
        dic = mysql_reader(origin_table_name,origin_hosts)
        return dic
