...
public:
    ...
    static py::list computerBasicAttributes(ComponentTree *tree){
		...
		const int n = tree->getNumNodes();
		double *attrs = new double[n * numAttribute];
		/*
		0 - area
		1 - volume
		2 - level
		...
		*/
	    ...
	    AttributeComputedIncrementally::computerAttribute(tree->getRoot(),
						...
						[&attrs, n, &xmax, &ymax, &xmin, &ymin, &m10, &m01, &m11, &m20, &m02](NodeCT* node) -> void {
							...
							double area = attrs[node->getIndex()];
							double volume = attrs[node->getIndex() + n];
							double width = xmax[node->getIndex()] - xmin[node->getIndex()] + 1;	
							double height = ymax[node->getIndex()] - ymin[node->getIndex()] + 1;	
		...
		AttributeComputedIncrementally::computerAttribute(tree->getRoot(),
			[&attrs, n,  &m10, &m01, &m11, &m20, &m02, numCols](NodeCT* node) -> void {
				...
				double levelMean = attrs[node->getIndex() + (n*3) ];
				double xCentroid = m10[node->getIndex()] / attrs[node->getIndex()];
				double yCentroid = m01[node->getIndex()] / attrs[node->getIndex()];		
				for(int p: node->getCNPs()) {
					int x = p % numCols;
					int y = p / numCols;
			...
			[&attrs, n, &m10, &m01, &m11, &m20, &m02](NodeCT* node) -> void {
				double sumDiffLevel2 = attrs[node->getIndex() + (n*4) ];
				double area = attrs[node->getIndex()];
				attrs[node->getIndex() + (n*4) ] = sumDiffLevel2 / area; //variance level

				double moment20 = attrs[node->getIndex()+(n*9)];
				double moment02 = attrs[node->getIndex()+(n*10)];
				double moment11 = attrs[node->getIndex()+(n*11)];
				
				if(area > 1){
					auto normMoment = [area](double moment, int p, int q){ return moment / std::pow( area, (p + q + 2.0) / 2.0); };
					double a1 = moment20 + moment02 + std::sqrt( std::pow(moment20 - moment02, 2) + 4 * std::pow(moment11, 2));
					double a2 = moment20 + moment02 - std::sqrt( std::pow(moment20 - moment02, 2) + 4 * std::pow(moment11, 2));
