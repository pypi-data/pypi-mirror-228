...
class AttributeFilters{
    ...
    py::array_t<double> filteringBySubtractiveScoreRule(std::vector<double> criterion);

    static void filteringBySubtractiveScoreRule(ComponentTree *tree, std::vector<double> prob, double *imgOutput){
        double mapLevel[tree->getNumNodes()];
        ...
        for(NodeCT* node: tree->getListNodes()){
            if(node->getParent() != nullptr){ 
                ...
                mapLevel[node->getIndex()] = mapLevel[node->getParent()->getIndex()] + (h * prob[node->getIndex()]);
            }
