...
from azureml.rag.utils.tokens import token_length_function
...
def get_langchain_splitter(file_extension: str, arguments: dict) -> TextSplitter:
    ...
    if file_extension == ".py":
        ...
        return PythonCodeTextSplitter.from_tiktoken_encoder(**{**arguments, "disallowed_special": (), "allowed_special": "all"})
    ...
    if file_extension == ".txt" or file_extension in formats_to_treat_as_txt_once_loaded:
        ...
        return TokenTextSplitter(**{**arguments, "disallowed_special": (), "allowed_special": "all"})
        ...
        if use_rcts:
            ...
            return MarkdownTextSplitter.from_tiktoken_encoder(**{**arguments, "disallowed_special": (), "allowed_special": "all"})
        else:
            return MarkdownHeaderSplitter.from_tiktoken_encoder(remove_hyperlinks=True, remove_images=True, **{**arguments, "disallowed_special": (), "allowed_special": "all"})
    else:
