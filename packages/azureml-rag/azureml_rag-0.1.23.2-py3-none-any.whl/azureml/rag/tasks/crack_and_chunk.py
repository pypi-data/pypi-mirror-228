...
def generate_file_name(output_title_chunk:str, chunked_document: ChunkedDocument, file_extension: str) -> str:
    file_name = chunked_document.source.filename.replace("\\", "_").replace("/", "_")
    return Path(output_title_chunk) / f"Chunks_{file_name}.{file_extension}"
...
def custom_loading(python_file_path: str, ext_loaders, ext_splitters):
    ...
    for _, obj in inspect.getmembers(module):
        ...
        if inspect.isclass(obj) and issubclass(obj, BaseDocumentLoader) and obj != BaseDocumentLoader:
...
def main(args, logger, activity_logger):
    def filter_and_log_extensions(sources: Iterator[DocumentSource], allowed_extensions=SUPPORTED_EXTENSIONS) -> Iterator[DocumentSource]:
        ...
        for source in sources:
            ...
            if allowed_extensions is not None:
                if source.path.suffix.lower() not in allowed_extensions:
                    skipped_files += 1
                    ext_skipped = skipped_extensions.get(source.path.suffix.lower(), 0)
                    skipped_extensions[source.path.suffix.lower()] = ext_skipped + 1
                    logger.debug(f'Filtering out extension "{source.path.suffix.lower()}" source: {source.filename}')
                    continue
            ext_kept = kept_extension.get(source.path.suffix.lower(), 0)
            kept_extension[source.path.suffix.lower()] = ext_kept + 1
        ...
        if total_files == 0:
            raise Exception(f"No files found in input path using glob {args.input_glob}")
    ...
    if args.custom_loader:
        logger.info(f"Loading custom loader(s) from {args.custom_loader}", extra={"print": True})
    ...
    for chunked_document in chunked_documents:
        ...
        if args.output_format == "csv":
            write_chunks_to_csv(
                ...
                generate_file_name(args.output_title_chunk, chunked_document, "csv"))
        elif args.output_format == "jsonl":
            write_chunks_to_csv(
                chunks_to_dataframe(chunked_document.chunks),
                generate_file_name(args.output_title_chunk, chunked_document, "jsonl"))
