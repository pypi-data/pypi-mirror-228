        ...
        with track_activity(logger, "init.load_embeddings_container") as activity_logger:
            ...
            try:
                embeddings_container_dir_name = str(max([dir for dir in pathlib.Path(
                    embeddings_container_path).glob("*") if dir.is_dir() and dir.name != os.environ["AZUREML_RUN_ID"]], key=os.path.getmtime).name)
            except Exception as e:
                activity_logger.warn("Failed to get latest folder from embeddings_container.")
                logger.warn(
                    f"failed to get latest folder from {embeddings_container_path} with {e}.")
                pass

            if embeddings_container_dir_name is not None:
                logger.info(
                    f"loading from previous embeddings from {embeddings_container_dir_name} in {embeddings_container_path}")
                try:
                    embeddings_container = EmbeddingsContainer.load(
                        embeddings_container_dir_name, embeddings_container_path)
                    if hasattr(activity_logger, "activity_info"):
