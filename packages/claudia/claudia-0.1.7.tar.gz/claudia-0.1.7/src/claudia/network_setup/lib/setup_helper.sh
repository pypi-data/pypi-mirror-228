...
build_rippled() {
  ...
  if [ "${l_build_rippled_opt}" = "true" ]; then
    ...
    time_now="$(date +%Y%m%d_%H%M%S)"
    conan export external/snappy snappy/1.1.9@ > "${LOG_DIR}/${time_now}_rippled_snappy.log" 2>&1
...
docker_install_rippled() {
  ...
  if [ "${l_install_rippled_opt}" = "true" ]; then
      ...
      docker build -t ${RIPPLED_NODE_CONTAINER_NAME} -f "${RIPPLED_NODE_DOCKER_FILE_INSTALL_MODE}" . > /dev/null 2>&1
      exit_on_error $?
      detect_update_install_mode "${install_mode}"
      echo "Check help to start network"
  fi
}
...
start_sidechain() {
  ...
  if [ "${l_start_sidechain_opt}" = "true" ]; then
    if [ "${MODE}" = "${build}" ]; then
      ...
      echo "Please switch to ${build} mode. Check help to switch"
      exit 1
    fi
  fi
}
