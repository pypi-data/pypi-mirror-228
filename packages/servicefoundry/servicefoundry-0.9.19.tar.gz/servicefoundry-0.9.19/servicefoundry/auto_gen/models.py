...
#   timestamp: 2023-08-30T12:19:44+00:00
...
class BaseAutoscaling(BaseModel):
    ...
    max_replicas: conint(ge=1, le=100) = Field(
        ...,
...
class CodeserverImage(BaseModel):
    """
    +usage=Codeserver with persistent environment (Python 3.8.10)
    """
...
class ModelLibrary(str, Enum):
    """
    +label=Model Library
    +usage=Which library to use to load the model. Should be either transformers or sentence-transformers
...
class HuggingfaceModelHub(BaseModel):
    ...
    model_library: ModelLibrary = Field(
        "transformers",
        description="+label=Model Library\n+usage=Which library to use to load the model. Should be either transformers or sentence-transformers\n+placeholder=transformers",
    )
...
class Helm(BaseModel):
    ...
    source: Union[HelmRepo, OCIRepo] = Field(
...
class Notebook(BaseModel):
    ...
    image: Union[TruefoundryImageBase, TruefoundryImageFull, CodeserverImage] = Field(
    ...
    home_directory_size: conint(ge=5, le=64000) = Field(
        10,
        description="+label=Home Directory Size in GB (Persistent)\n+usage=Size of the home directory for the notebook (Persistent Storage)\n+sort=6",
    )
    resources: Optional[Resources] = None
    env: Optional[Dict[str, str]] = Field(
        None,
...
class Volume(BaseModel):
    ...
    storage_class: str = Field(..., description="+label=Storage Class\n+sort=2")
...
class Service(BaseService):
    ...
    replicas: Union[confloat(ge=0.0, le=100.0), ServiceAutoscaling] = Field(
        ...,
        description="+label=Replicas\n+usage=Replicas of service you want to run\n+icon=fa-clone\n+sort=4",
    )
...
class AsyncService(BaseService):
    ...
    replicas: Union[confloat(ge=0.0, le=100.0), AsyncServiceAutoscaling] = Field(
        ...,
        description="+label=Replicas\n+usage=Replicas of service you want to run\n+icon=fa-clone",
    )
...
class Application(BaseModel):
    __root__: Union[Service, AsyncService, Job, ModelDeployment, Notebook, Helm, Volume]
