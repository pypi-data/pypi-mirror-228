...
class HoneyHiveLangChainTracer(BaseTracer, ABC):
    ...
    def _convert_agent_run_to_log(
        ...
        return Log(
            ...
            created_at=datetime.datetime.fromtimestamp(run.end_time.timestamp()).isoformat(),
            duration=duration,
            metadata=metadata,
        )
...
class Log(BaseModel):
    ...
    duration: Optional[float] = Field(
        title="Duration",
        description="How long in seconds the trace step took.",
    )
