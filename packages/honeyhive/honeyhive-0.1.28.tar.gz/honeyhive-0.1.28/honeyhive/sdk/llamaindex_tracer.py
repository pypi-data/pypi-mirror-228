...
from honeyhive.sdk.langchain_tracer import Log, Config
...
class HoneyHiveLlamaIndexTracer(BaseCallbackHandler):
    ...
    if os.getenv("HONEYHIVE_URL"):
        _base_url = os.getenv("HONEYHIVE_URL")
    else:
        _base_url: str = "https://api.honeyhive.ai"
    if os.getenv("HONEYHIVE_API_KEY"):
        _headers["Authorization"] = "Bearer " + os.getenv("HONEYHIVE_API_KEY")
    ...
    def __init__(
        ...
        event_ends_to_ignore = (
            event_ends_to_ignore if event_ends_to_ignore else []
        )
        super().__init__(
        ...
        if project is None and os.getenv("HONEYHIVE_PROJECT_NAME"):
            self.project = os.getenv("HONEYHIVE_PROJECT_NAME")
        self.source = source
    ...
    def log_trace(self) -> None:
        """Log the trace tree to HoneyHive."""
        try:
    ...
    def _convert_event_pair_to_log(
        ...
        start_time_ms, end_time_ms, end_time = self._get_time_in_ms(event_pair)
    ...
    def _get_time_in_ms(self, event_pair: List[CBEvent]) -> Tuple[int, int]:
        ...
        end_time_in_ms = int(
            (end_time - datetime(1970, 1, 1)).total_seconds() * 1000
        )
