...
class Client:
    ...
    # TODO(RAT-2173): do this check server-side
    ...
    def _start_generative_stress_test(
        ...
        Example:
            This example assumes that evaluation dataset is registered
            with identifiers "bar", and that a model with the
            unique identifier `model_uuid` is registered.

        .. code-block:: python

            config = {
                "data_info": {"eval_dataset_id": "bar"},
                "model_id": model_uuid,
                "run_name": "My Stress Test Run",
    ...
    @deprecated("Firewall methods will not be supported from Robust Intelligence v2.4.")
    def get_firewall_for_project(self, project_id: str) -> Firewall:
        ...
        **Note that the Firewall is deprecated in Robust Intelligence v2.2 and will
        not be supported from v2.4.**
    ...
    def create_integration(
        ...
        updated_schema = self._validate_and_convert_integration_schema(
            integration_schema
        )

        req = RimeCreateIntegrationRequest(
        ...
        with RESTErrorHandler():
            resp = api.integration_service_create_integration(body=req)
        return resp.integration_info.integration.id.uuid
