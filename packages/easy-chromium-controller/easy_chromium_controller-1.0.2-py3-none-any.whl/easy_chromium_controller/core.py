...
from .utils import KillAllChromiumProcessOnLinux, KillAllChromiumProcessOnWindows, Singleton, Log, Input, check_binary_files_istalled
...
class Browser(metaclass=Singleton):
    ...
    def open(
        ...
        if self.OS == "linux":
            chromedriver_path = binary_folder + "/bin-"+ RELEASE_VERSION +"/linux/chromedriver"
            chromium_path = binary_folder + "/bin-"+ RELEASE_VERSION +"/linux/chrome/chromium"
            check_binary_files_istalled(binary_folder+"/bin-"+ RELEASE_VERSION, "/linux/chromedriver")
        else:
            chromedriver_path = binary_folder + "\\bin-"+ RELEASE_VERSION +"\\win\\chromedriver.exe"
            chromium_path = binary_folder + "\\bin-"+ RELEASE_VERSION +"\\win\\chrome\\chromium.exe"
            check_binary_files_istalled(binary_folder+"\\bin-"+ RELEASE_VERSION, "\\win\\chromedriver.exe")
        ...
        if window_size:
            options.add_argument("--window-size=" + window_size.x + "," + window_size.y)
        if window_position:
            options.add_argument("--window-position=" + window_position.x + "," + window_position.y)
        if configForDocker:
        ...
        if headless and not configForDocker:
            options.add_argument("--headless")
        service = ChromiumService(chromedriver_path)
        browser_name = "chrome"
        vendor_prefix = "goog:"
    ...
    def __registryNewWindow(self):
        index = len(self.driver.window_handles) - 1
        new_window = {self.browser_windows(index).name : self.driver.window_handles[index]}
    ...
    def switchToWindow(self, window = None):
        if window == None:
            window = self.browser_windows(0)
        self.driver.switch_to.window(self.windows_list[window.name])
