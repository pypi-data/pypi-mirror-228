...
class Singleton(type):
    """Add this class as metaclass to any class to make it a singleton"""
...
class Explorer(metaclass=Singleton):
    """Objeto responsable de interactuar con el navegador.\n
    ``Solo existe una instancia de este objeto.``\n

    Dependencias:
        - La enumeracion de ventanas "EXPLORER_WINDOWS" : necesario saber el numero de ventanas\n
    ...
    def __init__(
        ...
        if os.getenv("ENV") == "prod":
            ...
            options.binary_location = binary_location + "/bin/linux/chrome/chrome"
            options.headless = True
        else:
            service = ChromiumService(binary_location + "\\bin\\win\\chromedriver.exe")
            options.binary_location = binary_location + "\\bin\\win\\chrome\\chrome.exe"
        ...
        # Setup windows
        for i in range(len(EXPLORER_WINDOWS) - 1):
            self.__openNewWindow()
        self.switchToWindow()

    def Log(*args, **kwargs):
        """
        Funcion para gestionar los logs del Bot.\n
        Los parametros funcionan igual que la funcion print().
        """
        print("🤖   >> " + "".join(map(str, args)), **kwargs)

    def Input(start_of_line: str = ""):
        """
        Funcion para gestionar los inputs del Bot.\n
        Los parametros funcionan igual que la funcion input().
        """
    ...
    def switchToWindow(self, window: EXPLORER_WINDOWS = EXPLORER_WINDOWS.BETBURGER):
        index = int(window.value)
        self.driver.switch_to.window(self.driver.window_handles[index])

    def __openNewWindow(self):
        self.driver.execute_script("window.open();")

    """ def __closeWindow(self, window : EXPLORER_WINDOWS):
    ...
    def executeScript(self, args=[], script=""):
        ...
        Ejemplo:
        ```js
            console.self.log(args[0]); // Imprime el primer argumento
        ```
        """
    ...
    def handleCaptcha(self, captcha_iframe_xPath: str):
        ...
        def solveCaptcha():
            ...
            if len(tiles_list) == 16:
                ...
                x_offset = 136
                y_offset = 134
            self.Log("Tamaño de captcha : ", matrix_size, " x ", matrix_size)
            ...
            self.Log("Por favor, resuelve el captcha y pulsa enter")
            self.Log("imagen => <API URL>/image/robot_image_text")
            self.Log("Formato: numeros separados por puntos o comas ")
            ...
            while True:
                try:
                    ...
                    self.Log("Por favor, escribe un numero entre 1 y ", count - 1)
                    str_input = None
            self.Log("Clickando en los cuadrados: ", blocks_to_click)
            for index in blocks_to_click:
                tiles_list[index - 1].click()
                self.sleep(0.5)
            self.click(By.XPATH, '//*[@id="recaptcha-verify-button"]')
            self.Log("Comprobando...")
            self.sleep(6)
            try:
                ...
                self.Log("Captcha resuelto  ✔️")
                return
            self.Log("Captcha no resuelto aún, intentando de nuevo")
            solveCaptcha()
            return
