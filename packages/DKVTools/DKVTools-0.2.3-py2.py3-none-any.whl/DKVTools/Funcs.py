...
def addTempPathForOs(path):
    '''临时添加path到系统的PATH环境变量中，暂时只支持win32'''
    if isWindows():
        os.environ['PATH']+= ';' +path

def addTempPathForPy(path):
    '''临时添加path到python运行环境的PATH环境变量中'''
...
def zipDir( root, zipPath, filters=(), filterSkip=False, isRe=False, inZipPath=None, clearMacInfo=True ) :
    '''压缩文件夹
    root:           需要压缩的文件路径。
    zipPath:        生成的zip文件路径。
    filters:        过滤器列表，默认为()，表示压缩所有文件。当filterSkip=False压缩匹配的文件，反之则压缩不匹配的文件。
    filterSkip:     匹配过滤器的文件是否调过打包，默认False。
    isRe:           过滤器是否是正则表达式字符串，默认False，此时使用*做通配符。
    inZipPath:      压缩包的根目录，默认None,此时压缩的文件以相对于root的路径放置。若设置inZipPath，将文件放到inZipPath下。
    clearMacInfo:   是否清理mac文件系统信息，默认True，表示清理.DS_Store文件和__MACOSX文件夹。
    '''
    import re
    fileInfos = []

    def _isFilterMatch( path, patterns ) :
        '''如果patterns 是 None, 直接返回True( not filterSkip )'''
        if patterns is None :
            # return True
            return not filterSkip
        for p in patterns :
            if p.search(path) :
                return True
        return False
    ...
    if len(filters) > 0 :
        if not isRe :
            patterns = [re.compile('^%s$'%p.replace('.', r'\.').replace('*', '.+')) for p in filters]
        else :
            patterns = [re.compile('^%s$'%p) for p in filters]

    for d, fds, fs in os.walk(root) :
        if clearMacInfo and os.path.split(d)[-1] == '__MACOSX' :
            continue
        ...
        for f in fs :
            ...
            p = pathJoin(d, f)
            asb = getRelativePath(p, root)[0]
            if inZipPath is not None :
                asb = pathJoin(inZipPath, asb)

            find = _isFilterMatch(p, patterns)
            # if (find and not filterSkip) or (not find and filterSkip) :
            # find 和 filterSkip 异或，与上一行(已注释)等价
            if find ^ filterSkip :
                fileInfos.append((p, asb))
                if(p.endswith('.json')) :
                    print(asb)
    ...
    if len(fileInfos) > 0 :
        if zipPath is None :
            zipPath = root + '.zip'
        # TODO: tryMakeParentDir 优化，调用tryMakeDir
        tryMakeParentDir(zipPath)

        zipF = zipfile.ZipFile(zipPath, 'w', zipfile.ZIP_DEFLATED)
        for f, af in fileInfos :
            zipF.write(f, af)
        zipF.close()
        print('zip "%s" finished!\nTotal files: %d'%(zipPath, len(fileInfos)))
        selectInFolder(zipPath)
        return fileInfos
...
def removeMacConfFiles( tar ) :
    '''删除Mac文件系统的配置文件(夹)，如：.DS_Store、__MACOSX'''
    if os.path.isdir(tar) :
        d = os.path.basename(tar)
        if d in ('__MACOSX',) :
            removeTree(d)
        else :
            for f in os.listdir(tar) :
                removeMacConfFiles(pathJoin(tar, f))
    elif os.path.isfile(tar) :
        f = os.path.basename(tar)
        if f in ('.DS_Store', ) :
            removeFile(tar)
...
def selectInFolder( path ) :
    ...
    if os.path.exists(path) :
        ...
        if isWindows() :
            ...
            tryCmd('explorer.exe /select, "%s"'%path.replace('/', '\\'))
        else :
...
def getRelativePath(oriPath, relative2) :
    ...
    rp = relativePath(oriPath, relative2)
    return rp, oriPath != rp

def relativePath(oriPath, relative2) :
    '''获取相对路径'''
    return formatPath(os.path.relpath(oriPath, relative2))
...
def tryCmd(cmd) :
    ...
    # if isPython3() :
    #     cmd = cmd.decode(locale.getpreferredencoding())
    out_temp = None
    lines = []
...
def tryCmdAsync(cmd, callback=None, onCompeleted=None, **args) :
    '''尝试异步运行命令
    params:
        callback:           每输出一行的回调，参数(lineStr, PopenObj)，默认None
        onCompeleted:       线程退出回调，命令执行完成，默认None
        **args:             不定参数，包含key：sleep(time.sleep时间，单位秒, 默认0.0001)  notDaemon(不是守护线程，默认False)  encoding(编码，默认计算机默认编码)
    '''
    import threading
    import subprocess
    import traceback
    ...
    sleep = args.get('sleep') or 0.0001
    notDaemon = args.get('notDaemon') or False
    shell = args.get('shell') or True
    def func() :
        try:
            # encoding = args.get('encoding') or locale.getpreferredencoding()
            # obj = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=shell, encoding=encoding)
            obj = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=shell)
            count = 0
            while True :
                buff = obj.stdout.readline().splitlines()
                if obj.poll() is not None and len(buff) == 0  :
                    if onCompeleted :
                        onCompeleted()
                    break
                elif callback :
            ...
            print (u'Exception:', traceback.format_exc())
            raise e

    t = threading.Thread(target=func)
    t.setDaemon(not notDaemon)
    t.start()
...
def jsonDumps(data, **params) :
    ...
    if isPython3() and 'encoding' in list(params.keys()) :
        del params['encoding']
    if params.get('ensure_ascii') is None :
...
def getSitePkgPath( ) :
    '''获取python Lib/site-packages文件夹的路径'''
    import site
    paths = site.getsitepackages()
    if len(paths) == 2 :
        return formatPath(paths[1])
    return None
...
def countStr( s ) :
    ...
    p = re.compile(r'([\u4e00-\u9fa5\u0800-\u4e00\uac00-\ud7ff])|([a-zA-Z_\d]+)')
    rst = p.findall(s)
    # print(rst)
    return len(rst)

# ----------------------------------------------- 异或加密相关 -----------------------------------------------
def encryptXor( contnet, secretKey ) :
    '''使用secretKey异或加(解)密bytes
    params:
        content: bytes      需加密(解)密的内容
        secretKey: str      密钥
    return:
        bytearray
    '''
    buffer = bytearray(contnet)
    k = secretKey.encode()
    for c in k :
...
def encryptFile( path: str, secretKey: str ) :
    '''使用秘钥加密文件'''
    if os.path.isfile(path) :
        content = []
        k = secretKey.encode()
...
# =============================================== 动态加载py模块 ============================================================
def execStr( s, raiseException=False ) :
    '''执行从字符串定义的python代码, 返回locals环境，示例：
    help(execStr('from DKVTools.Funcs import pyData2LuaLines as test')['test'])
    help(execStr('from DKVTools.Funcs import pyData2LuaLines')['pyData2LuaLines'])
    '''
    loc = locals()
    try :
        exec(s)
...
def __main() :
    ...
    selectInFolder(r"C:/Users/Dekiven2/Desktop/proto/run.bat")
    # help(execStr('from DKVTools.Funcs import pyData2LuaLines')['pyData2LuaLines'])
    # s = 'test str 12345'
    # key = 'test'
    # s2 = encryptXor(s.encode(), key)
    # print(s2, encryptXor(s2, key).decode())
    # print(getSitePkgPath())
    # print(countStr(u'aaaaa何时when 杖尔看see南雪snow，我me与梅花plum blossom两白头哈哈哈11123 2323'))
