...
def init_trainer(
    ...
    init_autodeploy: bool = typer.Option(True, help="Whether to init with autodeploy files."),
    ...
    if init_autodeploy:
        ...
        shutil.copy(os.path.join(base_path, "autodeploy", f"{training_type}-autodeploy.yaml"), autodeploy_dir)
        shutil.copy(os.path.join(base_path, "autodeploy", f"{training_type}-requirements.txt"), autodeploy_dir)
...
def init_cortex(
    ...
    cpu: int = typer.Option(
        2,
        help="Number of CPUs to use for the Cortex deployment. Defaults to 2. Can be an integer between 1 and 48",
    ),
    memory: float = typer.Option(
        14.5,
        help="Amount of memory (in GB) to use for the Cortex deployment. Defaults to 14.5GB . Can be a float between 2.0 and 256.0 depending on hardware selection.",
    ),
    ...
    if not os.path.exists(init_dir):
        ...
        if os.listdir(init_dir) and not overwrite:
            print("Directory is not empty. Please specify an empty directory or use the --overwrite flag.")
            sys.exit(1)
...
"""
    ...
    with open(os.path.join(init_dir, "config.yaml"), "w") as f:
        yaml.dump(config, f, version=(1, 2), sort_keys=True)
...
def deploy(
    ...
    cpu: int = typer.Option(
        0,
        help="Number of CPUs to use for the Cortex deployment. Defaults to 2. Can be an integer between 1 and 48 on the new hardware. GPU and A10 have a max of 4.",
    ),
    memory: float = typer.Option(
        0.0,
        help="Amount of memory (in GB) to use for the Cortex deployment. Defaults to 14.5. Can be a float between 2.0 and 256.0 depending on hardware selection.",
    ),
    ...
    memory = memory if memory else params.get("memory", None)
    force_rebuild = (
        bool(force_rebuild)
...
def train(
    ...
    autodeploy: Union[str, None] = typer.Option(None, help="The path to a config.yaml if you would like to autodeploy. Defaults to None and will not autodeploy. Your cortex deploy parameters can also be included under 'autodeploy' in your training config.yaml."),
    autodeploy_requirements_file: Union[str, None] = typer.Option(None, help="Path to the requirements file for autodeployment. This is the same requirements that are needed for deploying the model normally."),
    autodeploy_pkglist_file: Union[str, None] = typer.Option(None, help="Path to the pkglist file for autodeployment. This is the same pkglist that is needed for deploying the model normally."),
    ...
    autodeploy_requirements_file = autodeploy_requirements_file if autodeploy_requirements_file else autodeploy.get("autodeploy_requirements_file", None)
    autodeploy_pkglist_file = autodeploy_pkglist_file if autodeploy_pkglist_file else autodeploy.get("autodeploy_pkglist_file", None)
