...
import importlib
from .ui_styles import set_style, sty
...
# def push_mount_point(mount_point):
#     assert aci.the_starlette_app is not None
#     aci.route_mount_path.append(mount_point)

# def pop_mount_point(mount_point):
#     assert aci.the_starlette_app is not None
#     aci.route_mount_path.append(mount_point)
...
class MountCtx:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        mount_obj = Mount("/" + self.mount_point_name, routes=routes)
...
def add_jproute(path, endpoint, **kwargs):
    ...
    aci.mount_route_stack[-1].append(Route(path, starlette_endpoint, **kwargs))
...
def create_endpoint_impl(wp_template):
    ...
    def wp_endpoint(request):
        sm = get_session_manager(request.session_id)
        with sessionctx(sm):
...
def default_page_builder(key=None, childs=[], **kwargs):
    return Mutable.WebPage(key=key, childs=childs, **kwargs)
