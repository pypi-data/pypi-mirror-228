...
Button = gen_HC_type(HCType.mutable,
                     "Button",
                     TR.ButtonMixin,
...
Circle = gen_HC_type(HCType.mutable,
                     "Circle",
                     TR.ButtonMixin,
                     mutableShell_addonMixins=[mutable_TF_mixins.StaticCoreSharer_ValueMixin],
...
Label = gen_HC_type(HCType.mutable,
                    "Label",
                    TR.LabelMixin,
                    stytags_getter_func=lambda m=ui_styles: m.sty.label)

Span = gen_HC_type(HCType.mutable, "Span", TR.SpanMixin,
                   stytags_getter_func=lambda m=ui_styles: m.sty.span)

TextInput = gen_HC_type(HCType.mutable, "TextInput", TR.TextInputMixin,
...
class ChartJSMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        print("CREATING A CHART WITH NAME = ", kwargs.get("chart_name"))
...
StackH = gen_Div_type(HCType.mutable, "Div", TR.DivMixin, stytags_getter_func=lambda m=ui_styles: m.sty.stackh)
StackV = gen_Div_type(HCType.mutable, "StackV", TR.DivMixin, stytags_getter_func=lambda m=ui_styles: m.sty.stackv)
...
class FormShellMixin:
    def __init__(self, *args, **kwargs):
        print("Put per connection specific data here")
        self.local_data = 5
...
def form_hook(ufunc):
    ...
    def validate_wrapper(dbref, msg, to_shell_target):
        ...
        print("num comps = ", len(dbref.childs))
        for c in traverse(dbref):
            # look for data-validators in c and
            # verify its input

            print("Validate input component ", c)
            pass
...
class HCCStatic:
    ...
    StackV = gen_Div_type(
        HCType.hcc_static_div, "HCCStaticDiv", TR.DivMixin,
        stytags_getter_func=lambda m=ui_styles: m.sty.container
    )
...
class HCCMutable:
    ...
    StackV = gen_Div_type(HCType.hcc_mutable_div, "Div", TR.DivMixin, stytags_getter_func=lambda
    m=ui_styles: m.sty.stackv
    )

    StackH = gen_Div_type(HCType.hcc_mutable_div, "Div", TR.DivMixin, stytags_getter_func=lambda
    m=ui_styles: m.sty.stackh
    )

    StackW = gen_Div_type(HCType.hcc_mutable_div, "Div", TR.DivMixin, stytags_getter_func=lambda
    m=ui_styles: m.sty.stackw
    )

    Container = gen_Div_type(HCType.hcc_mutable_div, "Div", TR.DivMixin, stytags_getter_func=lambda
        m=ui_styles: m.sty.container
    )
    
    def Halign(content, align="center", **kwargs):
        """
        there isn't yet settled way to find the type of  content. Hence content_type
        is the current fix/workaround
        """
        twsty_tags = conc_twtags(*ui_styles.sty.halign(align), *kwargs.pop("twsty_tags", []))
