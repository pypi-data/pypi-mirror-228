...
class TestLangchain:
    ...
    @pytest.mark.parametrize("model_type", [model.value for model in ModelTypes])
    def test_01_llm_chain(self, setup_class, model_type):
    ...
    @pytest.mark.parametrize("model_type", [model.value for model in ModelTypes])
    def test_02_llm_chain_model_with_params(self, setup_class, model_type):
        params = {
    ...
    def test_03_sequential_chain(self, setup_class):
    ...
    @pytest.mark.parametrize("model_type", [model.value for model in ModelTypes])
    def test_04_simple_sequential_chain(self, setup_class, model_type):
        params = {
        ...
        llm_1 = WatsonxLLM(model_id=model_type, credentials=self.wml_credentials, project_id=self.project_id,
                           params=params)
        llm_2 = WatsonxLLM(model_id=model_type, credentials=self.wml_credentials, project_id=self.project_id,
                           params=params)
    ...
    def test_05_transformation_chain(self, setup_class):
    ...
    def test_06_router_chain(self, setup_class):
    ...
    def test_07_conversation_buffer_memory_chain(self, setup_class):
        llm = WatsonxLLM(
            model_id='google/flan-ul2',
            credentials=self.wml_credentials,
            project_id=self.project_id)
        ...
        assert response_1['response'] in history[1]

    def test_08_llm_bash_chain(self, setup_class):
        llm = WatsonxLLM(
            model_id='eleutherai/gpt-neox-20b',
            credentials=self.wml_credentials,
            project_id=self.project_id)
        qa = "Please write a bash script that prints 'Hello World' to the console."
        bash_chain = LLMBashChain.from_llm(llm, verbose=True)
        response = bash_chain(qa)
        assert qa == response['question']
        assert response['answer']

    @pytest.mark.parametrize("model_type", [model.value for model in ModelTypes if model.value not in ['google/flan-t5-xxl', 'google/flan-ul2']])
    def test_09_math_chain(self, setup_class, model_type):
        llm = WatsonxLLM(
            model_id=model_type,
            credentials=self.wml_credentials,
            project_id=self.project_id)
        llm_math = LLMMathChain.from_llm(llm, verbose=True)
        qa = "What is 13 raised to the 2 power?"
