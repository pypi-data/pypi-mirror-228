############################################################################
######################## Print Tricks & More - version 0.3.128 #############
############################################################################
...
ReadMe_Quick = """
    Quick Summary:
        ...
        For example: if my_var = 123, then typing in "pt(my_var)" will output: "pt(my_var): 123 - int, Length: 3, (Line :75, #myFunction, myFile.py)"
    ...
    Functions in Print Tricks Class:
        ...
        pt()          = Printing replacement - Examples:
                        =>  my_var = 123
                        =>  pt(my_var)              = pt(my_var): 123 - int, Length: 3, (Line :75, #myFunction, myFile.py)
                        =>  pt(my_var, 'myString')  = myString - pt(my_var, 'myString'): 123 - int, Length: 3, (Line :75, #myFunction, myFile.py)
...
ReadMe_Detailed = """
    ...
    pt.l()      = Location
                    Examples:
                        ...
                        pt.l('nameOfFile.py') ## Returns 3 items, now includes the current location plus whatever filepath we specified. 
                        locations = pt.l()
                    ...
                    Features:
                        ...
                        Optionally:
                            - Prints all of these, formatted nicely if the command is like this:
                                - "pt.l(printThis=True)" or "pt.l('newFile.py', printThis=True)"
    pt.p()      = Pause
...
TODO_QUICK = """
    ...
    - Core1k colored text for Ursina:
        - Make my C colors compatible with ursina's Text() class for text that shows up on the screen. 
...
TODO_LONG_TERM = """
    ...
    - pt.speedup()
        ...
        - use the same concept in the profiler code: the import statement for the speedup will copy the file it is in, edit all of the pt statements to include line numbers, then at the very end, include a PT.ex() to ensure that the original file never runs.
        ...
        - Easiest FASTEST speedup for all of PT:
            ...
            - starting up the fast pt printing:
                ...
                - speedup will start the process of reading the user's file in whatever file called the pt statement. 
                ...
                - All pt _information is stored in a dictionary with line number determining it's status. 
                - try to put the pt statements into some sort of buffer for quicker printing? 
                    - Of course, strings are immutable so I can't save the entire pt statement in a buffer (with all of the colors, formatting etc) when the variables in the middle of the string might chagne. Is there another way to accomplish this? 
    ...
    - pt.debugger: inside pt
        ...
        - NOTE: Will most likely fail:
            ...
            not actually being a instanced method of the class, but instead is just a 
...
if os.path.exists("print_tricks_debugger.py"):
    ...
    ...  ## NOTE DO NOT REMOVE THIS Elipsis - for pt debugger #do not remove ...do not...do_not
...
# ... ###db(stack)
# filePath, lineNo, funcName, code = stack[0]
# ... ###db(code)
# ... ###db('---------------')
# for frame_summary in stack:
#     filePath, lineNo, funcName, code = frame_summary
#     ... ###db(code)
...
class pt:
    ...
    """ WARNING NOTE WARNING: Do not set these manually using "pt.print_deletes" etc.. 
        Always use "pt.disable(functionType)", so that it's handled correctly with a warning & reminder message. """
    print_Deletes = True
    ...
    def __init__(
        ...
        *variables,
        ...
        """ Parameters:
            ...
            - printAlways: If true, will print this statement even if all other pt statement prints have been turned off from printing. 
        ...
        simp_trace_output = UTI._simple_trace(variablesLen)
        # simp_trace_output = UTI._simple_trace_new(variablesLen)
        ...
        lineNo = f'{lineNo}.{pt.current_pt_on_multi_line + 1}' if pt.current_pt_on_multi_line != 0 else lineNo

        ## REMOVED ALL STRINGS "FEATURE"
        # all_is_str = UTI._allStringsQ(args)
        # ... ###db(argsWithSpecials, argsOnly, formatted_args, fmtArgsList)
        if variablesLen == 0 and strFront is None:
            print(f"pt() (Line :{lineNo}, {func_name_fmt}{fileName})")
        ...
        #     print(f'{C.t1}{strfinal}{C.er} - pt({C.t2}{argsOnly}{C.er}) - str{fromType}, Length: {len(strfinal)}, (Line :{lineNo}, {func_name_fmt}{fileName})')
        else:
            UTI._info(
                simp_trace_output,
                variablesLen,
    ...
    def c(
        ...
        you want"""
        if pt.print_colors == False and printAlways != True:
            return
        combined_colors = ""
    ...
    def ci(
        ...
        -
        """
        if pt.print_colors_tags == False and printAlways != True:
            return
        (
            fileName,
            filePath,
        ...
        if type(colors) is list or type(colors) is tuple:
            ...
            combined_colors = colors  ## if there is just one color
        print(
            f"{combined_colors}{string}{C.er} - pt.ci({C.t2}{argsOnly}{C.er}) (Line :{lineNo}, {func_name_fmt}{fileName})",
            end="\n",
            sep=" ",
            file=None,
            flush=False,
    ...
    def delete(
        ...
        If disableThisAfterPause is set to true within an app that has pause functions, calling this pt.delete()
        function will auto-disable the disableThisAfterPause parameter as a safety mechanism to ensure that the pt.delete
    ...
    def e(
        ...
        if msgType == "full" or msgType == "fullType" or full_trace is True:
            full = True
        if pt.print_Exceptions == False and printAlways != True:
            return
        ...
        if full == False:
            ...
            print(
                f"{C.t2}<<<Error>>> {C.er}"
                f"{C.t1}{myString}{C.er}{filler}pt.e(): {C.t3}{errorType}{C.er}"
    ...
    @staticmethod
    def easy_imports(project_name=None):
        # print("\nPython path:")
        ...
        if project_name is not None and project_name != '':
            # Convert project_name to lowercase for case insensitive comparison
            project_name = project_name.lower()
            while parent.name.lower() != project_name and parent != parent.parent:
                parent = parent.parent

        # pt(parent)
        sys.path.append(str(parent))
        if str(parent) in sys.path:
            print(f'pt.easy_import() successful - {parent} is now in python path for finding modules')
            # print(f'{sys.path=}')
        else:
            print(f"pt.easy_import() unsuccessful - The directory {parent} is not in the system path.")

        # files = (os.listdir(parent))

        # print(f'{len(files)} Files: in {parent}')
    ...
    def ex(*args, print_originating_code=True, _lineNo=None):
        ...
        if print_originating_code == True:
            print(
                f"{C.t2}>>>Exited App<<<{C.er} - pt.ex({C.t2}{formatted_args}{C.er}): (Line :{lineNo}, {func_name_fmt}{fileName})")
        else:
    ...
    def hr(
        ...
        """This function shortcuts all data to the pt.h() function, but sets "getResults=True" just to save time."""
        return pt.h(functionThread, getResults, strFront, strBack, daemon, printAlways)
        # end def hr
    ...
    def l(
        ...
        if printThis == True:
            if pt.print_Locations == False and printAlways != True:
                return new_File, this_File, dir_only

            print(
                f"pt.l({formatted_args}) - {C.t2}New File:  {C.t3}{new_File}{C.er} - (Line :{str(lineNo)}, {func_name_fmt}{fileName})"
            )
            print(
                f"pt.l({formatted_args}) - {C.t2}This File: {C.t3}{this_File}{C.er} - (Line :{str(lineNo)}, {func_name_fmt}{fileName})"
            )
            print(
                f"pt.l({formatted_args}) - {C.t2}Path Only: {C.t3}{dir_only}{C.er} - (Line :{str(lineNo)}, {func_name_fmt}{fileName})"
            )
...
## pt_line = f"{NAM.inst_nam}.props({C.t1}{formatted_args}{C.er}) - (Line :{str(lineNo)}, {func_name_fmt}{fileName})\n
    ...
    def props(passed_obj):
        ...
        output = f"{NAM.inst_nam}.props({C.t1}{formatted_args}{C.er}) - (Line :{str(lineNo)}, {func_name_fmt}{fileName})\n"
    ...
    def p(
        ...
        if pt.print_Pauses == False and printAlways != True:
            return
        if disableThisAfterPause == True and pt.pauseCompleted > 0:
            return
        if loops2Activate != None:
            if pt.loopsThroughPause == loops2Activate - 1:
                pt.loopsThroughPause = 0
            else:
                pt.loopsThroughPause += 1
                return
    ...
    def prefs(
        ...
        if pt.print_prefs == False and printAlways != True:
            return
        (
            fileName,
            filePath,
        ...
        # print('pt.prefs('+ C.t2 +argsOnly+C.er+'): '+
        # ' - (Line :'+str(lineNo)+', @'+funcName+', ' +'\\'+fileName+')')

        print(
            f"pt.prefs({C.t2}{argsOnly}{C.er}): - (Line :{str(lineNo)}, {func_name_fmt}{fileName})"
        )
    ...
    def r_old2(
        ...
        if this_r_key not in pt.release_enable:
            pt.release_enable[this_r_key] = {"loopNum": 1, "secondsNum": time.time()}
            return False
        else:
            re_dict = pt.release_enable[this_r_key]
    ...
    def run(file_to_run: str) -> None:
        ...
        if successful_run is True:
            print(f"pt.run({formatted_args}) - {C.t2}Successful run: {C.t3}{full_file_path}{C.er} - (Line :{str(lineNo)}, {func_name_fmt}{fileName})")
        else:
            print(f"pt.run({formatted_args}) - {C.t2}File not found: {C.t3}{full_file_path}{C.er} - (Line :{str(lineNo)}, {func_name_fmt}{fileName})")
    ...
    def size(varObject, print_sources=False, print_sources_count=False, _lineNo=None):
        """
        ...
        thisBytes, sources, sources_count = UTI._bytesSize(varObject, print_sources, print_sources_count)
        formatted_bytesSize = UTI._bytesSize_formatter(thisBytes, argsOnly, print_sources, sources, print_sources_count, sources_count)
    ...
    def timeall(
        ...
        strFront=None,
        strBack="",
        printThis=True,  ## Disables pt.timeall() from printing the results (but the results still return for you)
        printAlways=False,
        *args,
        **kwargs,
    ):
        ...
        pt.t(f"TIMING_TEST - {func.__name__}", printThis=False)
        result = func(*args, **kwargs)
        one_loop_time = pt.t(f"TIMING_TEST - {func.__name__}", printThis=False)
        ...
        for i in range(
            ...
            pt.t(func.__name__, printThis=print_pt_t, _lineNo="timeall")
            result = func(*args, **kwargs)
        ...
        if printThis == True:
            ...
            print(
                f"{C.t1}{myString}{C.er}{filler}pt.timeall({formatted_args}): "
        ...
        **kwargs,
        ...
        for i in range(
            ...
            for count, func in enumerate(functions):
                pt.t(func.__name__, printThis=print_pt_t)
                result = func(*args, **kwargs)
    ...
    def t(
        *sequences,
        ...
        if pt.print_Timers == False and printAlways != True:
            return
        # ... ###db('a')
        ...
        for sequence in sequences:
            ...
            if sequence not in pt.sequencesDict:
                (
                    fileName,
                    filePath,
                    lineNo,
                ...
                if len_sequences == 1:
                    if printThis == True:
                        print(
                            f"{C.t1}{myString}{C.er}{filler}{NAM.inst_nam}.t({formatted_args}): "
                ...
                pt.sequencesDict[sequence] = [[time.perf_counter_ns(), 0.0, 0.0]]
                ...
                if (
                    ...
                    pt.c(
                        f"{C.t2}<<<Error>>: {C.er}You cannot pass arguments on line {lineNo}: {NAM.inst_nam}.t({C.t2}{formatted_args}) ({func_name_fmt}{fileName}). \n"
                ...
                strFront = pt.sequence_args_dict.get(sequence)[1]
                strBack = pt.sequence_args_dict.get(sequence)[2]
                printAlways = pt.sequence_args_dict.get(sequence)[3]
                ...
                time_taken_last_function = pt.sequencesDict.get(sequence)[-1][2]
                startTime = pt.sequencesDict.get(sequence)[-1][0]
                ...
                if (
                    ...
                    tot_time = 0.0
                    for i in range(len(pt.sequencesDict.get(sequence))):
                        tot_time += pt.sequencesDict.get(sequence)[i][1]
                ...
                for i, magnitude_ns in enumerate(mag_last_n_total_seconds):
                    ...
                    # length = len(pt.sequencesDict.get(sequence)) + 1 ## we are adding 1 to account for the sequences append that we are going to do at the bottom of this function to account for the time of this function
                    length = len(pt.sequencesDict.get(sequence))
                    ...
                    if printThis == True:
                        print(
                            ...
                            f"{pbi}{magnitude_specific_str}{C.t1} {specific_format}{C.er} "
                            f"{magnitude_user_formatted_str}({magnitude_ns:,.2f} FPS): "
            ...
            pt.sequencesDict[sequence].append(
                [timeNow, magnitude_ns, time.perf_counter_ns() - timeNow]
            )
        return_multi = [
            pt.sequencesDict.get(str(key))[-1][1] for key in sequences
    ...
    def w(
        ...
        elif tagStr in pt.tagWaitsDict:
            # ... ###db('Use already existing entry for this tagStr')
            taggedWait = pt.tagWaitsDict[tagStr][0]
            # ... ###db(taggedWait)
    ...
    def pt(functionType="", printAlways=False):
        ...
        if functionType != "":
            ...
            help_info = help_info.replace(
                "pt(my_var): 123 - int, Length: 3, (Line :75, #myFunction, myFile.py)",
                pt_newExample1,
            )
            ...
            help_info = help_info.replace(
                "myString - pt(my_var, 'myString'): 123 - int, Length: 3, (Line :75, #myFunction, myFile.py)",
                pt_newExample2,
            )
...
############## Incorrect PT class catch###########
class PT:
    def __init__(self, *args, **kwargs):
        """Not a functional class. Instead use "from print_tricks import pt (all lowercase), 
        and then type in your pt statements in lowercase like this: pt(1)
        """
        # print(f"{C.t1}Error:{C.er} \n"
        #       f"You've used capital letters.\n"
...
############### Aliases for class pt #######################
# class p(pt):
#     """ Functional inherited alias of class pt"""
#     # def __init__(
#     #     self,
#     #     *variables,
#     #     strFront=None,
...
#     #     super().__init__()
#     #     print('what')
#     #     UTI.is_p_alias_imported = True
#     is_p_alias_imported = False
#     def __init__(self, **kwargs):
#         self.is_p_alias_imported = True
#         ... ###db('p.init')
#         global p
#         p=pt
...
class UTI:
    ...
    def _enable_disable_type_className_funcType(functionType=None):
        ...
        if "pt." in functionType:
            functionType = functionType.replace("pt.", "")

        ## set default classname to 'pt.' for printing purposes, but if the functiontype to disable is actually a generic pt() statement, then remove it from the string because we would be repeating _information.
        className = "pt."
    ...
    def _bytesSize(varObject, print_sources=False, print_sources_count=False, reducedSize_Q=False, num_of_divisions_from_original=0):
    ...
    def _bytesSize_formatter(thisBytes, argsOnly, print_sources, sources, print_sources_count, sources_count):
        ...
        if lenB <= 3:
            ...
            size_colored = (
                f"pt.size({C.t2}{argsOnly}{C.er}): {C.t3}{thisBytes} {sizeType}{C.er}"
            )
        ...
        size_colored = f"pt.size({C.t2}{argsOnly}{C.er}): ~{C.t3}{formattedSize}{C.er} {sizeType}, or {C.t3}{thisBytes}{C.er} Bytes"
    ...
    def _delete_it(myFile, deleteWhat, replaceWith, numInstances):
        ...
        backupDirOnly = pt.l("pt.delete(BackupFiles)\\")
    ...
    def _Turn_On_Off_Functions(functionType, enable_or_disable=type(str)):
        ...
        # print('pt.'+enable_or_disable+'('+ C.t2 +argsOnly+C.er+'): '+
        # ' - (Line :'+str(lineNo)+', '+func_name_fmt+'\\'+fileName+') - '+msg_disable_all+' - For performance measurements, Type pt.pt("disable")')

        # print(
        #     f'pt.{enable_or_disable}({C.t2}{argsOnly}{C.er}):'
        #     f' (Line :{lineNo}, {func_name_fmt}{fileName})'
    ...
    def _info(
        ...
        # fileName, filePath, lineNo, funcName, func_name_fmt, code, argsWithSpecials, argsOnly, formatted_args, fmtArgsList  = UTI._simple_trace(argsLen)
        fileName = simp_trace_output[0]
        filePath = simp_trace_output[1]
        lineNo = simp_trace_output[2]
        funcName = simp_trace_output[3]
        func_name_fmt = simp_trace_output[4]
        code = simp_trace_output[5]
        formatted_args = simp_trace_output[8]
        fmtArgsList = simp_trace_output[9]
        lineNo = f'{lineNo}.{pt.current_pt_on_multi_line + 1}' if pt.current_pt_on_multi_line != 0 else lineNo
        ...
        printStr = f"{strFront}{filler}{NAM.inst_nam}({formatted_args}) Length: {C.eu}{C.t3}{variablesLen} variables{C.er} (Line :{lineNo}, {func_name_fmt}{fileName}) - {strBack}"
        ...
        for v in range(variablesLen):
            ...
            ### Get optional lengthVar
            try:
                ## TODO arg = fmtArgsList[v] doesn't really do anything.... somehow converts it, but probs a better way. 
                ##        I'm pretty sure formatted_args returns the same thing??? But these get sent to lengthVar which is supposed
                ...
                # ... ###db(fmtArgsList, arg, formatted_args)
            except:
                arg = '_unknown_'
            ...
            if variablesLen == 1:
                ...
                printStr = f"{strFront}{filler}{NAM.inst_nam}({formatted_args}): "  ## Reset the printStr to accomdate for the singular values.
                totLenStr = (
                ...
                if _bytesSize < 800 or totLenStr < 145:
                    ...
                    printStr += (
                        f"{C.t3}({C.er}{lengthVar} elements below{C.t3}){C.er} {NAM.inst_nam}.size = {byteSizeFormatted} - {varType_asStr}, Length: {lengthVar}, (Line :{lineNo}, {func_name_fmt}{fileName})\n"
                        f"{C.t3}{var_as_Str}{C.er}\n"  ### These lines are already indented due to the 'var_as_Str = textrap.fill() command above
                        f"\t    ^ ^ ^ {NAM.inst_nam}({C.t2}{formatted_args}{C.er}): {C.t3}({C.er}{lengthVar} elements above this{C.t3}){C.er} {NAM.inst_nam}.size = {byteSizeFormatted} - {varType_asStr}, Length: {lengthVar}, (Line :{lineNo}, {func_name_fmt}{fileName})"
                    )
                ...
                if _bytesSize < 800 or totLenStr < 145:
                    ...
                    printStr += (
                        f"\n\t>{num}>  {C.t2}{fmtArgsList[v]}{C.er}: {C.t3}({C.er}{lengthVar} elements below{C.t3}){C.er} pt.size = {byteSizeFormatted} - {varType_asStr}, Length: {lengthVar}, (Line :{lineNo}, {func_name_fmt}{fileName})\n"
                        f"{C.t3}{var_as_Str}{C.er}\n"  ### These lines are already indented due to the 'var_as_Str = textrap.fill() command above
                        f"\t    ^ ^ ^ pt({C.t2}{fmtArgsList[v]}{C.er}): {C.t3}({C.er}{lengthVar} elements above this{C.t3}){C.er} pt.size = {byteSizeFormatted} - {varType_asStr}, Length: {lengthVar}, (Line :{lineNo}, {func_name_fmt}{fileName})"
                    )
        # ... ###db('--test start--', printStr, '--- test over')
        print(printStr, end=end, sep=sep, file=file, flush=flush)
        ...
        # pt.numPT_count += 1
        # print(pt.numPT_count)
        # # print('printStr: \n', printStr)
        # pt.rapid_pt_bulk_print_block += printStr+'\n'
        # pt.bulkPrintList.append(printStr)
        # # print('bulk: \n', pt.rapid_pt_bulk_print_block)

        # # print('numPT_count', ': ', pt.numPT_count)

        # if thisTime >= pt.time_to_print:
        #     pt.time_to_print = thisTime + 1.
        #     print('if')
        #     # print(str(pt.bulkPrintList))
        #     # print('len bulk print list: ', len(pt.bulkPrintList))
        #     # finalString = "\n".join(pt.bulkPrintList)
        #     # print(finalString)
    ...
    def _simple_trace_new(argsLen=1):
        ...
        try:
            while True:
                ...
                # filePath, lineNo, funcName, code = traceB[pt.new_trace_level]
                fi = sys._getframe(2)
                ...
                # elif "pt(" in code or "pt." in code or "pts(" in code or "... ###db(" in code: ## NOTE: deleted last two checks. no longer needed I think. 
                    ...
                    (
                        ...
                        formatted_args,
                        fmtArgsList,
                    ) = UTI._trace_processing(filePath, lineNo, code, argsLen)
    ...
    def _trace_processing(filePath, lineNo, code, argsLen):
        ...
        if count_a != count_b:  # This is a multi-line pt statement
            ...
            while True:
                ...
                nextLine = traceback.linecache.getline(filePath, check_line)
        ...
        formatted_args = f"{C.t2}{argsOnly}{C.er}"  ## This is placed here for everytime there is just one argument
        # ... ###db(formatted_args)
        ...
        # print(f'specials: {argsWithSpecials} argsOnly: {argsOnly} frmArgs: {formatted_args} fmtAlist: {fmtArgsList}')
        ## --prints:
        ## specials:  'times:',  4*(1,2,3
        ## argsOnly:  'times:',  4*(1,2,3
        ## frmArgs: 'times:',  4*(1, 2, 3  ## But these are colored
        ## fmtAlist: ["'times:'", ' 4*(1', '2', '3']
        return code, argsWithSpecials, argsOnly, formatted_args, fmtArgsList
    ...
    def _getVarLength(variable, varType, arg=None):
        ...
        if (
            ...
            ## pt.size() or UTI._bytesSize() on this would make sense.
...
class Profiler:
    def __init__(self):
        self.calling_file = inspect.stack()[1].filename
        self.modify_file()
...
#             node.body.append(ast.Expr(value=ast.Call(func=ast.Attribute(value=ast.Name(id='profiler_instance', ctx=ast.Load()), attr='pt', ctx=ast.Load()), args=[ast.Constant(value=node.lineno)], keywords=[])))
...
#                     new_node = ast.Expr(value=ast.Call(func=ast.Name(id="pt", ctx=ast.Load()), args=[ast.Constant(value=node.lineno)], keywords=[]))
