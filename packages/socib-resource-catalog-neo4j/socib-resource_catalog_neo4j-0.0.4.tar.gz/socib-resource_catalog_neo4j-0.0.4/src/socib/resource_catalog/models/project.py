#!/usr/bin/env python3 
# -*- coding: utf-8 -*-

import hashlib
import inspect
import json
import logging
import os
import pkg_resources
import re
import sys
import traceback
import uuid

from datetime import datetime
from functools import reduce, partial, lru_cache
from pathlib import Path
from validator_collection import checkers, errors

from neomodel import config
from neomodel import db

from neomodel import StructuredNode
from neomodel import StructuredRel
from neomodel import Relationship, RelationshipFrom, RelationshipTo

from neomodel import ArrayProperty
from neomodel import BooleanProperty
from neomodel import DateTimeFormatProperty
from neomodel import EmailProperty
from neomodel import FloatProperty
from neomodel import IntegerProperty
from neomodel import JSONProperty
from neomodel import RegexProperty
from neomodel import StringProperty
from neomodel import UniqueIdProperty

#######################################################################
## PROJECT (implementation)
#######################################################################
class ProjectImpl (object): 

    license = EmailProperty() 
    member_of = RelationshipTo("socib.resource_catalog.models.Program", "IS_MEMBER_OF")
    partners = RelationshipTo("socib.resource_catalog.models.Organization", "IS_PARTNER_OF")
    funders = RelationshipTo("socib.resource_catalog.models.Organization", "IS_FUNDER_OF")
    contributors = RelationshipTo("socib.resource_catalog.models.Organization", "IS_CONTRIBUTOR_TO")    
    
    def __init__ (self):
        logging.debug("[ProjectImpl::__init__] entering method")
