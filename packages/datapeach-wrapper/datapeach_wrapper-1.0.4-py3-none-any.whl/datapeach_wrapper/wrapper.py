...
class DPFileConfigStreamReader:
    def __init__(self, dt_conf_file, udf, dry_mode=False):
        self.udf = udf
        # refactor, instead of conf file, dict of conf
        self.dry_mode = dry_mode

        # currently do nothing with operation configuration
        if get_ignore_dp_config():
            self.dt_config = None
            return
        ...
        try:
            with open(dt_conf_file, "r") as conf_file:
                self.dt_config = json.load(conf_file)
                print(json.dumps(self.dt_config, indent=4))
        except:
            traceback.print_exc()
            print(
                f"We have a problem to handle the data stream config file: {dt_conf_file}"
            )
            self.dt_config = None
            return
    ...
    def _batch_pull_data(self, size=1, type="pandas", async_mode=False):
        ...
        if self.dry_mode == False:
            ...
            for i in range(size):
                msg = self.consumer.receive()
                # logging.debug(f'from topic={msg.topic_name()}, id={msg.message_id()}, event_time={msg.event_timestamp()}')
                # value=msg.value()
                data = msg.data().decode("utf-8")
                value = json.loads(data)
                print("message value receive", value)
                # if (type=="pandas"):
                #     #using schema to encode it into json string and conver to normal dict
                #     #we can handle the msg.data() but i have not checked.
                #     if (self.schema_type=="JSON"):
                #         dict_record=json.loads(self.dataSchema.encode(value))
    ...
    async def _batch_process(self, result_queue, udf, n, type="pandas"):
        ...
        operators = self.dt_config["build_operation"]["ops"]
        for op in operators:
            result = udf(input_data, **op["value"])
            result_queue.put(result)
        # return current_queue
