...
class TelemetryHelper:
    def __init__(self,
                 ...
                 max_enqueued_events: int = 5) -> None:
        ...
        with requests.Session() as session:
            if not self.telemetry_token:
                ...
                try:
                    ...
                    self.telemetry_token = next((r.get('telemetry_token', None) for r in regions if r['api_host'] == self.tb_host), None)
                    if self.telemetry_token:
...
def add_telemetry_sysinfo_event() -> None:
    ...
    add_telemetry_event('system_info',
