...
class BM25:
    ...
    def encode_documents(self, texts: Union[List[str], List[int]]) -> Dict:
        ...
        if isinstance(texts, list):
            if isinstance(texts[0], int) or isinstance(texts[0], str):
                return self._convert_to_dict(self.bm25.encode_document(texts))
            else:
    ...
    def encode_queries(self, texts: Union[List[int], List[str]]) -> Dict:
        ...
        if isinstance(texts, list):
            if isinstance(texts[0], int) or isinstance(texts[0], str):
                return self._convert_to_dict(self.bm25.encode_query(texts))
            else:
                raise TypeError("input texts must be List[str] or List[int]")
        else:
            raise TypeError("input texts must be List[str] or List[int]")
    ...
    # from map to dict
    @staticmethod
    def _convert_to_dict(input_dict: Dict) -> Dict:
        indices = list(input_dict.keys())
        scores = list(input_dict.values())
