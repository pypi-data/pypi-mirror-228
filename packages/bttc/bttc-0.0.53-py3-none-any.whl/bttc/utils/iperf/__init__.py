...
class IPerfFacade:
  ...
  1. Open Hotstop of phone as iPerf server.
  2. Connect phone as iPerf client to the phone as iPerf server.
  """

  def __init__(self, iperf_server: ANDROID_DEVICE, iperf_client: ANDROID_DEVICE):
  ...
  def iperf_client_wifi_ip(self) -> str:
    ...
    matcher = re.search(
        'Gateway\s+([0-9]+[.][0-9]+[.][0-9]+[.][0-9]+)\s+DNS', stdout)
    if not matcher:
      raise errors.IPerfClientWifiError(
  ...
  def start_test(self, time_sec: int = 5) -> IPerfTestResult:
    ...
    try:
      ...
      cmd = (
          ...
          f'{self.iperf_client_wifi_ip} -t {time_sec}')
...
def get_iperf_facade(
    ...
    iperf_client: Union[ANDROID_DEVICE, str]) -> IPerfFacade:
  ...
  iperf_facade = IPerfFacade(iperf_server, iperf_client)
