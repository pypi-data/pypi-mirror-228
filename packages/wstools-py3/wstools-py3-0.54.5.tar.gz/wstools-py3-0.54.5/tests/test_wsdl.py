    ...
    def test_all(self):
        try:
            if self.path[:7] == 'http://':
                ...
                self.wsdl = WSDLReader().loadFromFile(Path(__file__).parent / Path(f"./{self.path}"))
...
def setUpOptions(section):
    ...
    cp.optionxform = str
    configTxtFilePath = Path(__file__).parent / Path("./config.txt")
    cp.read(configTxtFilePath)
    if not cp.sections():
