...
class SocketServer(asyncore.dispatcher):
    ...
    def clear_invalid(self):
        ...
        for sock_addr in sock_addrs:
            sock = self.sockets.get(sock_addr)
            if sock is not None and sock.check_invalid() is False:
                try:
                    if hasattr(sock, 'exit'):
                        sock.exit()
                except Exception as e:
...
class SocketAccessor:
    ...
    def start(self, thread: bool = True):
        ...
        if self.config.POOL == 0:
            if thread is True:
                self.thread = Thread(target=self.asyncore_loop, name=f"socket{self} loop", daemon=False)
            else:
                self.asyncore_loop()
        else:
            if thread is True:
                self.thread = Thread(target=self.asyncore_loop, name=f"socket({self}) loop", daemon=False, kwargs={'timeout': self.config.TIME_OUT})
            else:
...
class SocketIPHandler:
    ...
    @daemon_thread
    def receive(self):
        currentThread().setName(f"{self.reg_tag} receive")
...
class SocketIPAccssor:
    ...
    def start(self, thread: bool = True):
        if self.config.MODE in [SocketType.TCP_SERVER, SocketType.UDP_SERVER]:
            if thread is True:
                self.thread = Thread(target=self.create_server, name=f"create_server")
            else:
                self.create_server()
        else:
            if thread is True:
                self.thread = Thread(target=self.create_client, name=f"create_client")
            else:
                self.create_client()
    ...
    def send_broadcast(self, data: Union[str, bytes]):
        if self.accessor:
            self.accessor.sendto(data if isinstance(data, bytes) else data.encode('utf-8'), ('<broadcast>', self.config.PORT))
