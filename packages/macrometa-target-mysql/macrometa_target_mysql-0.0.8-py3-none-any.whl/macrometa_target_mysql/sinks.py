...
class MySQLConnector(SQLConnector):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.connection_config["ssl_check_hostname"] = self.config.get(
            "ssl_check_hostname"
        )
...
class MySQLSink(SQLSink):
    ...
    def include_sdc_metadata_properties(self) -> bool:
        return self.config.get("add_metadata_columns", False) or self.config.get(
            "hard_delete"
        )
    ...
    def process_batch(self, context: dict) -> None:
        ...
        try:
            ...
            if self.key_properties:
                ...
                if self.include_sdc_metadata_properties and self.config.get(
                    "hard_delete", False
                ):
    ...
    def merge_upsert_from_table(
        ...
        upsert_on_condition = ", ".join(
            [f"{key} = VALUES({key})" for key in schema["properties"].keys()]
        )
