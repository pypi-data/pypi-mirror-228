...
def test(agent, mdp_train, mdp_test, core_train, core_test, logger, n_episodes_test, n):
...
class Weighted_Average:
    def __init__(self,
        ...
        self.weight_datapoint = (1-weight_final)/(total_epochs-start_tracking_epoch)
        self.data = list()
        ...
        try:   
            if agent.train_directly:
                if agent.train_mode == "direct":
                    ...
                    for i in trange(n_epochs):
                        if i==0:
                            J, R = test(agent, mdp_train, mdp_val, core_train, core_val, logger, n_episodes_test, i)
                        else:
