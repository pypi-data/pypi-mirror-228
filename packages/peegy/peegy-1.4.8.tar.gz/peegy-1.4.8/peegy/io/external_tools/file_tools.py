...
def get_files_and_meta_files(measurement_path: str = '',
    ...
    :param split_trigger_code: if provided, bdf file will be marked in chunks starting at each of these trigger
    events. The time stamps indicating the beginning and end of eeach block will be passed in  time the ini_time and
    ...
    for i, _full_file_name in enumerate(data_files):
        ...
        if split_trigger_code is not None:
...
def get_data_and_meta_data_files(measurement_path: str = '',
                                 ...
                                 deep_match_in_exclude_path: bool = True,
                                 deep_match_in_exclude_file: bool = True,
                                 meta_data_file_extension: [str] = ['.json'],
                                 rows_to_skip=0,
                                 ) -> [DataLinks]:
    """
    ...
    data_files = []
    for files in types:
        data_files.extend(Path(measurement_path).glob(files))
