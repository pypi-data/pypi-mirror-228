...
def _get_workflows2(params: Dict[str, Any]) -> List[str]:
    response = _get(
        _observer(),
        '/workflows',
...
def list_workflows() -> None:
    ...
    generate_output(
        list(_get_manifests(_get_workflows(_make_params_from_selectors()))),
        DEFAULT_COLUMNS,
        WIDE_COLUMNS,
    )
...
def delete_workflow(workflow_ids: List[str]) -> None:
    ...
    workflow_ids = _ensure_either_uuids_or_selectors(
        workflow_ids, _get_workflows2, {'fieldSelector': 'status.phase==RUNNING'}
    )
