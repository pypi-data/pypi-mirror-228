...
RK23_C = np.asarray((0, 1 / 2, 3 / 4), dtype=np.float64, order='C')
RK23_B = np.asarray((2 / 9, 1 / 3, 4 / 9), dtype=np.float64, order='C')
RK23_E = np.asarray((5 / 72, -1 / 12, -1 / 9, 1 / 8), dtype=np.float64, order='C')
...
# Turn arrays into readonly
RK23_C.setflags(write=False)
RK23_B.setflags(write=False)
RK23_E.setflags(write=False)
RK23_A.setflags(write=False)
...
RK45_C = np.asarray((0, 1 / 5, 3 / 10, 4 / 5, 8 / 9, 1), dtype=np.float64, order='C')
RK45_B = np.asarray((35 / 384, 0, 500 / 1113, 125 / 192, -2187 / 6784, 11 / 84), dtype=np.float64, order='C')
RK45_E = np.asarray((-71 / 57600, 0, 71 / 16695, -71 / 1920, 17253 / 339200, -22 / 525, 1 / 40), dtype=np.float64, order='C')
...
# Turn arrays into readonly
RK45_C.setflags(write=False)
RK45_B.setflags(write=False)
RK45_E.setflags(write=False)
RK45_A.setflags(write=False)
...
DOP_C_REDUCED = np.empty(12, dtype=np.float64, order='C')
for i_ in range(12):
    DOP_C_REDUCED[i_] = DOP_C[i_]

DOP_A = np.zeros((16, 16), dtype=np.float64, order='C')
...
DOP_A_REDUCED = np.empty((12, 12), dtype=np.float64, order='C')
DOP_B = np.empty(12, dtype=np.float64, order='C')
...
DOP_E3 = np.empty(13, dtype=np.float64, order='C')
...
DOP_E5 = np.zeros(13, dtype=np.float64, order='C')
...
# Turn arrays into readonly
DOP_C.setflags(write=False)
DOP_C_REDUCED.setflags(write=False)
DOP_B.setflags(write=False)
DOP_E3.setflags(write=False)
DOP_E5.setflags(write=False)
DOP_A.setflags(write=False)
DOP_A_REDUCED.setflags(write=False)
...
DOP_A_view         = DOP_A
DOP_A_REDUCED_view = DOP_A_REDUCED
DOP_B_view         = DOP_B
DOP_C_view         = DOP_C
DOP_C_REDUCED_view = DOP_C_REDUCED
DOP_E3_view        = DOP_E3
DOP_E5_view        = DOP_E5
