...
class Commits(AbstractGitLab):
	...
	def get_by_project_files(self, project_id):
		...
		try:
			logging.info("Start function: get_commit_project")
			project = self.gl.projects.get(project_id)

			logging.info("Start function: get_commit_projetc {}".format(project.name))
			...
			for commit in commits:
				...
				for change in diff:
					...
					if file_path not in file_details:
						...
						file_details[file_path]["action"] = "modified"
