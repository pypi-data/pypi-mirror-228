...
class WizataDSAPIClient:
    ...
    def upsert_component(self, component: SolutionComponent):
        """
        upsert a component based on its ID.
        """
        if component is None:
            raise ValueError("component cannot be null")
        if component.twin_id is None or not isinstance(component.twin_id, uuid.UUID):
            raise ValueError("component must be associated to a valid UUID Twin ID")
        if component.solution_type is None:
            raise ValueError("component must have a valid solution type")
        if component.content is None:
            raise ValueError("component must have a valid content")
        if component.category is None:
            raise ValueError("component must have at least a category")
    ...
    def delete_component(self, component_id: uuid.UUID):
        response = requests.delete(self.__url() + "templates/" + str(component_id) + '/',
                                   headers=self.__header())
        if response.status_code == 200:
            return
        else:
