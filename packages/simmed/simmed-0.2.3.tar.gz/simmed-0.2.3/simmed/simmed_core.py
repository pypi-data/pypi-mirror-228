        ...
        if 'MONGO_HOST' in nacos_config:
            if "MONGO_SHARD" in nacos_config and int(nacos_config['MONGO_SHARD']) == 1:
                app.config['MONGOCLIENT_CONN'] = get_shard_mongo_connection(
                    nacos_config["MONGO_HOST"], int(nacos_config["MONGO_PORT"]), nacos_config["MONGO_USER"], nacos_config["MONGO_PASSWORD"])
            else:
...
def get_mongo_connection(MONGO_HOST, MONGO_PORT, MONGO_USER, MONGO_PASSWORD):
    """获取MongoDB连接"""
...
def get_shard_mongo_connection(MONGO_HOST, MONGO_PORT, MONGO_USER, MONGO_PASSWORD):
    """获取MongoDB分片集群连接"""
    if not MONGO_USER or not MONGO_PASSWORD:
