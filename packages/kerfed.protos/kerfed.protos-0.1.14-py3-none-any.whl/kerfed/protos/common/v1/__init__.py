...
from .shipping_pb2 import ShippingRate, ShippingParcel
...
from .scene_pb2 import Scene, GeometrySummary
from .outcome_pb2 import Outcome
from .fileblob_pb2 import FileBlob
from .geometry_pb2 import Mesh, Box, Cylinder, Geometry
from .ndarray_pb2 import NDArray, RigidTransform
from .combo_pb2 import Combo, Combos, ComboDescription
from .shop_pb2 import Shop, ShopManufacturing, ShopDisbursement
...
def numpy_to_rigid(matrix: NumpyLike) -> RigidTransform:
    ...
    return RigidTransform(values=matrix[:3, :].astype(
...
def rigid_to_numpy(rigid: RigidTransform) -> NumpyLike:
    ...
    if len(rigid.values) == 0:
        ...
        matrix[:3, :] = np.array(
            rigid.values, dtype=np.float64).reshape((3, 4))
...
def proto_to_numpy(message: NDArray) -> NumpyLike:
    ...
    if len(message.float64) > 0:
        return np.array(message.float64, dtype=np.float64).reshape(
            message.shape)
...
def proto_to_datetime(stamp: Timestamp) -> datetime:
    ...
    return datetime.fromtimestamp(
        float(stamp.seconds) + float(stamp.nanos) * 1e-9)
...
def mesh_to_proto(mesh: Trimesh) -> Geometry:
    ...
    if isinstance(mesh, trimesh.primitives.Box):
        ...
        return Geometry(mesh=Mesh(blob=FileBlob(name='neutral.glb',
                                                data=export)))
    else:
