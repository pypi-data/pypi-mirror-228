...
def rotate(inputs: Image.Image, angle, interpolation=T.RESIZE_MODE.BILINEAR, border_mode=T.BORDER_MODE.CONSTANT, border_value=0, **kwargs) -> Image.Image:
...
def affine(inputs: Image.Image, angle: float, tx: float, ty: float, scale: float, shear: float, interpolation=T.RESIZE_MODE.BILINEAR, **kwargs) -> Image.Image:
  return tvf.affine(inputs, angle=angle, translate=(tx, ty), scale=scale, shear=(shear, shear), interpolation=T.RESIZE_MODE_TO_PIL[interpolation])
