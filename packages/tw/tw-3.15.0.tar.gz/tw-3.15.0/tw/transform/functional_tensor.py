...
def rotate(inputs: torch.Tensor, angle, interpolation=T.RESIZE_MODE.BILINEAR, border_mode=T.BORDER_MODE.CONSTANT, border_value=0, **kwargs) -> torch.Tensor:
...
def affine(inputs: torch.Tensor, angle: float, tx: float, ty: float, scale: float, shear: float, interpolation=T.RESIZE_MODE.BILINEAR, **kwargs) -> torch.Tensor:
  return tvf.affine(inputs, angle=angle, translate=(tx, ty), scale=scale, shear=(shear, shear), interpolation=T.RESIZE_MODE_TO_TVF[interpolation])
