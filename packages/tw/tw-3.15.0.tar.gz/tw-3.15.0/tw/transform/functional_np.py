...
@autotype
def bilateral_usm_sharpen(inputs: np.ndarray, kernel_size=3, sigma_blur=0.8, sigma_color=10, sigma_space=1, coeff=1.2, **kwargs) -> np.ndarray:
  inputs = inputs.astype('float32')
...
@autotype
def advanced_blur(inputs: np.ndarray, kernel_size=3, sigma=0.8, rotate=0, beta=1.0, noise=(0.75, 1.25), **kwargs) -> np.ndarray:
...
@autotype
def multiplicative_noise(inputs: np.ndarray, multiplier=(0.9, 1.1), per_channel=True, elementwise=True, **kwargs) -> np.ndarray:
...
class QualityAwareTransform():
  ...
  def __init__(self, num_augs=5, iou_range=[0.1, 0.3], patch_size=160, num_order=1, num_pairs=1, method='pair_crop', **kwargs):
...
# noise
  ...
  def random_multiplicative_noise(self, x, coeffs=[(0.95, 1.05), (0.90, 1.10), (0.85, 1.15), (0.80, 1.20), (0.75, 1.25)], ids=None):
    ids = random.choice(range(len(coeffs))) if ids is None else ids
...
def quality_aware_transforms(inputs, num_augs=5, iou_range=[0.1, 0.3], patch_size=160, num_order=1, num_pairs=1, method='pair_crop', **kwargs):
