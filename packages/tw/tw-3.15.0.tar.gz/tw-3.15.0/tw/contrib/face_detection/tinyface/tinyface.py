...
class TinyFace():
  ...
  def __init__(self, device='cuda', pretrain='/cephFS/video_lab/checkpoints/detection/face_detector/tiny_face_on_widerface.pth'):
    tstamp = time.time()
    self.device = device
    ...
    state_dict = torch.load(pretrain, map_location=self.device)['model']
