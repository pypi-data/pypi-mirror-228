...
class BigoNeurOP(torch.utils.data.Dataset):
  ...
  def __init__(self, augment_dir, mask_dir, transform, phase, **kwargs):
    super(BigoNeurOP, self).__init__()
    self.transform = transform
    self.targets = {}
    self.masks = {} # all degree use identical mask

    # load all imgs
    imgs, vids = tw.media.collect(augment_dir, if_check_path=False)
    for file in imgs:
    ...
    # load masks
    imgs, vids = tw.media.collect(mask_dir, if_check_path=False)
    for file in imgs:
      filename = os.path.splitext(os.path.basename(file))[0]
      if filename not in self.targets:
        print(f'{filename} failed to find in augmented dir.')
        continue
      self.masks[filename] = file
  ...
  def __getitem__(self, idx):
    ...
    path_mask = self.masks[self.ids_to_names[idx]]

    (path1, degree1), (path2, degree2) = random.choices(album, k=2)

    img1 = np.array(imageio.imread(path1), dtype='float32') / 255.0
    img2 = np.array(imageio.imread(path2), dtype='float32') / 255.0
    mask = np.array(cv2.imread(path_mask, cv2.IMREAD_GRAYSCALE)) / 255.0

    img_1_meta = T.ImageMeta(binary=img1, source=T.COLORSPACE.RGB)
    img_1_meta.label = (degree2 - degree1) / 20.0
    img_2_meta = T.ImageMeta(binary=img2, source=T.COLORSPACE.RGB)
    img_1_mask = T.ImageMeta(binary=mask, source=T.COLORSPACE.HEATMAP)

    # meta or not
    return self.transform([img_1_meta, img_2_meta, img_1_mask])

class BigoColorEnhance(torch.utils.data.Dataset):
  """
  """

  def __init__(self, source_dir, target_dir, mask_dir, transform, phase, **kwargs):
    ...
    # load source
    imgs, vids = tw.media.collect(source_dir, if_check_path=False)
    for file in imgs:
      filename = os.path.splitext(os.path.basename(file))[0]
      if filename not in self.sources:
        self.sources[filename] = file
    
    # load target
    imgs, vids = tw.media.collect(target_dir, if_check_path=False)
    for file in imgs:
      filename = os.path.splitext(os.path.basename(file))[0]
      if filename not in self.sources:
        print(f'{filename} failed to find in source dir.')
        continue
      self.targets[filename] = file
    ...
    # split dataset by 9:1
    total = int(len(self.sources) * 0.9)
    self.ids_to_names = {}
    count = 0
    for i, filename in enumerate(self.sources):
      if (i < total and phase == tw.phase.train) or (i >= total and phase in [tw.phase.val, tw.phase.test]):
        self.ids_to_names[count] = filename
        count += 1
  ...
  def __getitem__(self, idx):
    ...
    source_path = self.sources[self.ids_to_names[idx]]
    target_path = self.targets[self.ids_to_names[idx]]
    mask_path = self.masks[self.ids_to_names[idx]]

    source = np.array(imageio.imread(source_path), dtype='float32') / 255.0
    target = np.array(imageio.imread(target_path), dtype='float32') / 255.0
    mask = np.array(cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)) / 255.0

    source_meta = T.ImageMeta(binary=source, source=T.COLORSPACE.RGB)
    target_meta = T.ImageMeta(binary=target, source=T.COLORSPACE.RGB)
    mask_meta = T.ImageMeta(binary=mask, source=T.COLORSPACE.HEATMAP)

    # meta or not
    return self.transform([source_meta, target_meta, mask_meta])