...
class BaseNet(nn.Module):
    ...
    def __init__(self, layers, channels, strides=[2, 2, 2, 2, 1], deploy=False):
...
class LPSNet1Path(BaseSegNet):
    ...
    def __init__(self, depth, width, resolution, num_classes=19, strides=[2, 2, 2, 2, 1], deploy=False):
        super(LPSNet1Path, self).__init__(depth, width, resolution, num_classes)
        self._check_resolution()
        self.net = BaseNet(self.depth, self.width, strides, deploy)
...
class LPSNet2Path(BaseSegNet):
    ...
    def __init__(self, depth, width, resolution, num_classes=19, strides=[2, 2, 2, 2, 1], deploy=False):
        ...
        self._check_resolution()
        self.netH = BaseNet(self.depth, self.width, strides, deploy)
        self.netL = BaseNet(self.depth, self.width, strides, deploy)
...
class LPSNet3Path(BaseSegNet):
    ...
    def __init__(self, depth, width, resolution, num_classes=19, strides=[2, 2, 2, 2, 1], deploy=False):
        ...
        self._check_resolution()
        self.net1 = BaseNet(self.depth, self.width, strides, deploy)
        self.net2 = BaseNet(self.depth, self.width, strides, deploy)
        self.net3 = BaseNet(self.depth, self.width, strides, deploy)
...
def lpsnet(depth, width, resolution, num_classes=19, strides=[2, 2, 2, 2, 1], deploy=False):
    ...
    if len(resolution_sorted) == 1:
        return LPSNet1Path(depth, width, resolution_sorted, num_classes, strides, deploy)
    elif len(resolution_sorted) == 2:
        return LPSNet2Path(depth, width, resolution_sorted, num_classes, strides, deploy)
    elif len(resolution_sorted) == 3:
        return LPSNet3Path(depth, width, resolution_sorted, num_classes, strides, deploy)
    else:
        raise NotImplementedError


def lpsnet_s(deploy=False):
    depth = (1, 3, 3, 10, 10)
    width = (8, 24, 48, 96, 96)
    resolution = (3 / 4, 1 / 4, 0)
    net = lpsnet(depth, width, resolution, deploy=deploy)
    return net


def lpsnet_m(deploy=False):
    depth = (1, 3, 3, 10, 10)
    width = (8, 24, 48, 96, 96)
    resolution = (1, 1 / 4, 0)
    net = lpsnet(depth, width, resolution, deploy=deploy)
