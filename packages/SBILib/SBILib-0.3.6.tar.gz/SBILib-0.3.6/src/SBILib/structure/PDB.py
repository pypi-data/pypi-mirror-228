...
class PDB(StorableObject):
    ...
    def graft(self, PDB2, selection):
        ...
        nguide = query_chain.extract(int(query_start_1)-1 , int(query_end_1) - 1 , backbone=False)
        addition = query_chain.extract(int(query_start_2)-1 , int(query_end_2) - 1 , backbone=False)
        ...
        nquery = target_chain.extract(int(target_start_1) -1 , int(target_end_1)-1 , backbone=False)
        addition = target_chain.extract(int(target_start_2) -1 , int(target_end_2)-1 , backbone=False)
