...
class ProjectString(ProjectInIt):
    ...
    def get_api_fun_str(self, name, row):
        ...
        api_str += '    kwargs["_host"] = kwargs.get("host", _host)\n'
    ...
    def get_api_testcase_header_str(self, fun_name):
        ...
        header_str += f"@allure.epic(\"{self.name3 + '，' if self.name3 else ''}接口测试\")\n"
        header_str += f"@allure.feature(\"场景：{api.title + '，' if api.title else ''}接口（{api.url}）\")\n"
