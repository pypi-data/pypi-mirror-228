...
def write_VESTA(
    ...
    _write_specific_format(structure, grid, vd, out_filename, format=format, compact=compact, inorm=inorm)


def write_delta_rho_vesta(total, individuals, output="delta_rho.cube", format="cube", compact=False, inorm=False):
    """电荷密度差分可视化
    ...
    _write_specific_format(
        structure, grid, volumetricData, filename=output, format=format, compact=compact, inorm=inorm
    )
...
def _write_specific_format(structure, grid, volumetricData, filename, format="cube", compact=False, inorm=False):
    ...
    compact : bool
        all data in one line, default False
    inorm : bool
        normalize volumetricData to 1, default False
    """
    ...
    if format.lower() == "cube":
        ...
        with open(absfile, "w") as fileobj:
            ...
            # 换行格式建议: https://h5cube-spec.readthedocs.io/en/latest/cubeformat.html
            if compact:
                # 牺牲可读性，将vd的空格替换成换行符，且不强制0.开头，可以缩小文件体积，且依旧可解析
                # 减小的字节数 = (ngx*ngy*ngz)，500x500x500个格点，可减小约 119.2 MB
                reshaped_vd.T.flatten().tofile(fileobj, sep="\n", format="%.5e")
            else:
                for iz in range(ngz):
                    for iy in range(ngy):
                        for ix in range(ngx):
                            fileobj.write(f"{reshaped_vd[ix, iy, iz]:12.5e} ")
                            if ix % 6 == 5:
                                fileobj.write("\n")
                        fileobj.write("\n")
        ...
        with open(absfile, "w") as file:
            ...
            if compact:
                ...
                for ix in range(ngx):
                    for iy in range(ngy):
                        for iz in range(ngz):
                            file.write(f"{reshaped_vd[ix, iy, iz]:12.5e} ")
                            count += 1 # 换行逻辑不同于cube，固定5个数就换行
                            if count % 5 == 0:
                                file.write("\n")
