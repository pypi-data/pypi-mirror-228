            ...
            if not path.exists(day_file_save_path):
                ...
                if hours_with_target_file != 0:
                    ...
                    df_participant = df_participant[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "LOG_TIME", "LOG_TIMESTAMP",
                         "STEPS_LAST_HOUR", "ACCUMULATED_STEPS"]]
                    df_participant.to_csv(day_file_save_path, index=False)

                    day_file_name_stepcount_day = file_save_date_path + sep + device + "_" + file_shortname + "_day_" + date + ".csv"
                    df_stepcount_day = get_stepcount_matrix_day(df_participant)
                    df_stepcount_day["PARTICIPANT_ID_TEXT"] = [pid] * len(df_stepcount_day)
                    df_stepcount_day["PARTICIPANT_ID_NUMERIC"] = [numerical_id] * len(df_stepcount_day)
                    df_stepcount_day = df_stepcount_day[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "YEAR", "MONTH", "DAY", "TOTAL_STEPS"]]
                    df_stepcount_day.to_csv(day_file_name_stepcount_day, index=False)
