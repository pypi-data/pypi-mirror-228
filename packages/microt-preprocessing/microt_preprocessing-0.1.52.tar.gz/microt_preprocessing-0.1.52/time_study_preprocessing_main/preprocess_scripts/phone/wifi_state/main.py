...
from .wifi_state_hour import *
from .wifi_state_minute import *
from .wifi_state_day import *
from .parse import *
import warnings
            ...
            if not path.exists(day_file_save_path):
                ...
                if hours_with_target_file != 0:
                    ...
                    df_participant = df_participant[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "LOG_TIME", "LOG_TIMESTAMP", "IS_WIFI_ON"]]
                    df_participant.to_csv(day_file_save_path, index=False)

                    day_file_name_wifi_minute = file_save_date_path + sep + device + "_" + file_shortname + "_minute_" + date + ".csv"
                    day_file_name_wifi_hour = file_save_date_path + sep + device + "_" + file_shortname + "_hour_" + date + ".csv"
                    day_file_name_wifi_day = file_save_date_path + sep + device + "_" + file_shortname + "_day_" + date + ".csv"
                    if not os.path.exists(day_file_name_wifi_minute):
                        # Add a module for minute-level mims sample number
                        df_wifi_minute = get_wifi_matrix_minute(df_participant)
                        df_wifi_minute["PARTICIPANT_ID_TEXT"] = [pid] * len(df_wifi_minute)
                        df_wifi_minute["PARTICIPANT_ID_NUMERIC"] = [numerical_id] * len(df_wifi_minute)
                        df_wifi_minute = df_wifi_minute[
                            ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "YEAR", "MONTH", "DAY", "HOUR", "MINUTE",
                             "WIFI_STATUS"]]
                        df_wifi_minute.to_csv(day_file_name_wifi_minute, index=False)

                        df_wifi_hour = get_wifi_matrix_hour(df_wifi_minute)
                        df_wifi_hour["PARTICIPANT_ID_TEXT"] = [pid] * len(df_wifi_hour)
                        df_wifi_hour["PARTICIPANT_ID_NUMERIC"] = [numerical_id] * len(df_wifi_hour)
                        df_wifi_hour = df_wifi_hour[
                            ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "YEAR", "MONTH", "DAY", "HOUR",
                             "WIFI_ON", "MISSING"]]
                        df_wifi_hour.to_csv(day_file_name_wifi_hour, index=False)

                        df_wifi_day = get_wifi_matrix_day(df_wifi_hour)
                        df_wifi_day["PARTICIPANT_ID_TEXT"] = [pid] * len(df_wifi_day)
                        df_wifi_day["PARTICIPANT_ID_NUMERIC"] = [numerical_id] * len(df_wifi_day)
                        df_wifi_day = df_wifi_day[
                            ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "YEAR", "MONTH", "DAY",
                             "WIFI_ON", "MISSING"]]
                        df_wifi_day.to_csv(day_file_name_wifi_day, index=False)
