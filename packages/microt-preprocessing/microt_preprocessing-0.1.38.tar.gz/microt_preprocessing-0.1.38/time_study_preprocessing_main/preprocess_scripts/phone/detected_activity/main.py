            ...
            if not path.exists(day_file_save_path):
                ...
                if hours_with_target_file != 0:
                    ...
                    if not os.path.exists(day_file_name_activity_minute):
                        ...
                        df_activity_minute["PARTICIPANT_ID"] = [pid] * len(df_activity_minute)
                        df_activity_minute["NUMERICAL_ID"] = [numerical_id] * len(df_activity_minute)
                        ...
                        df_activity_hour["PARTICIPANT_ID"] = [pid] * len(df_activity_hour)
                        df_activity_hour["NUMERICAL_ID"] = [numerical_id] * len(df_activity_hour)
                        df_activity_hour.to_csv(day_file_name_activity_hour, index=False)

                        df_activity_day = get_activity_matrix_day(df_activity_hour)
                        df_activity_day["PARTICIPANT_ID"] = [pid] * len(df_activity_day)
                        df_activity_day["NUMERICAL_ID"] = [numerical_id] * len(df_activity_day)
