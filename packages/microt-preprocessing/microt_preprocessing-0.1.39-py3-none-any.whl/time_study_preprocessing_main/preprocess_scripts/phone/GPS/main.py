...
def pre_process(microT_root_path, intermediate_file_save_path, p_id, decrypt_password, date):
    ...
    file_save_date_path = intermediate_file_save_path + sep + "intermediate_file" + sep + p_id + sep + date
    ind_cluster_label_intermediate_path_minute = os.path.join(file_save_date_path, "phone_location_cluster_minute.csv")
    ind_cluster_label_intermediate_path_hour = os.path.join(file_save_date_path, "phone_location_cluster_hour.csv")
    ind_cluster_label_intermediate_path_day = os.path.join(file_save_date_path, "phone_location_cluster_day.csv")
    if not path.exists(ind_cluster_label_intermediate_path_minute) or not path.exists(ind_cluster_label_intermediate_path_hour):
        ...
        if path.exists(ind_cluster_label_table_path):
            ...
            if df_cluster_minute is not None:
                ...
                df_cluster_minute["PARTICIPANT_ID"] = [pid] * len(df_cluster_minute)
                df_cluster_minute["NUMERICAL_ID"] = [numerical_id] * len(df_cluster_minute)
                ...
                df_cluster_hour["PARTICIPANT_ID"] = [pid] * len(df_cluster_hour)
                df_cluster_hour["NUMERICAL_ID"] = [numerical_id] * len(df_cluster_hour)
                ...
                df_cluster_day["PARTICIPANT_ID"] = [pid] * len(df_cluster_day)
                df_cluster_day["NUMERICAL_ID"] = [numerical_id] * len(df_cluster_day)
                # df_cluster_hour["LOG_TIMESTAMP"] = [convert_readable_time_list_to_timestamp(x) for x in
                #                                    df_cluster_hour["LOG_TIME"]]
