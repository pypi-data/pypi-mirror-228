        ...
        if path.exists(date_folder_path):
            ...
            if not path.exists(day_file_save_path_dnd) and not path.exists(
                ...
                if hours_with_target_file != 0:
                    ...
                    df_pid_sleep_wake["PARTICIPANT_ID"] = [pid]*len(df_pid_sleep_wake)
                    df_pid_burst_status["PARTICIPANT_ID"] = [pid] * len(df_pid_burst_status)
                    numerical_id = getNumericalID(pid)
                    df_pid_sleep_wake["NUMERICAL_ID"] = [numerical_id]*len(df_pid_sleep_wake)
                    df_pid_burst_status["NUMERICAL_ID"] = [numerical_id] * len(df_pid_burst_status)
