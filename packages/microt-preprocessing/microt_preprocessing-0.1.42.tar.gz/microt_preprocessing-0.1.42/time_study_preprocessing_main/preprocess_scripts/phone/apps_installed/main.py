            ...
            if not path.exists(day_file_save_path):
                ...
                for hour in validated_hour_list:
                    ...
                    if len(target_file_matched) > 0:
                        ...
                        try:
                            df_hour_raw = pd.read_csv(target_file_path, header=None, usecols=[i for i in range(4)])
                        except:
                ...
                if hours_with_target_file != 0:
                    ...
                    df_participant["LOG_TIMESTAMP"] = [convert_readable_time_list_to_timestamp(x) for x in df_participant["LOG_TIME"]]

                    df_participant = df_participant[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "LOG_TIME", "LOG_TIMESTAMP", "APP_PACKAGE_NAME", "APP_VERSION_CODE", "APP_VERSION_NAME"]]
