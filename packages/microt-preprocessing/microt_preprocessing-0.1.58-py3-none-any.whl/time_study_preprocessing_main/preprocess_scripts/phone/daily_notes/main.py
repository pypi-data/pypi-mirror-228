...
def pre_process(microT_root_path, intermediate_file_save_path, p_id, decrypt_password, date):
    ...
    if path.exists(area_folder_path):
        ...
        if path.exists(date_folder_path):
            ...
            day_file_name_sleep_wake = device + "_" + file_shortname + "_" + sleep_wake_notes + "_clean_" + date + ".csv"
            day_file_name_burst_status = device + "_" + file_shortname + "_" + burst_notes + "_clean_" + date + ".csv"
            day_file_name_dnd_status = device + "_" + file_shortname + "_" + dnd_status + "_clean_" + date + ".csv"
            ...
            if not path.exists(day_file_save_path_dnd) and not path.exists(
                ...
                if hours_with_target_file != 0:
                    ...
                    df_pid_sleep_wake["PARTICIPANT_ID_TEXT"] = [pid]*len(df_pid_sleep_wake)
                    df_pid_burst_status["PARTICIPANT_ID_TEXT"] = [pid] * len(df_pid_burst_status)
                    df_pid_dnd_status["PARTICIPANT_ID_TEXT"] = [pid] * len(df_pid_dnd_status)

                    numerical_id = getNumericalID(pid)
                    df_pid_sleep_wake["PARTICIPANT_ID_NUMERIC"] = [numerical_id]*len(df_pid_sleep_wake)
                    df_pid_burst_status["PARTICIPANT_ID_NUMERIC"] = [numerical_id] * len(df_pid_burst_status)
                    df_pid_dnd_status["PARTICIPANT_ID_NUMERIC"] = [numerical_id] * len(df_pid_dnd_status)

                    df_pid_sleep_wake["LOG_TIMESTAMP"] = [convert_readable_time_list_to_timestamp(x) for x in
                                                       df_pid_sleep_wake["LOG_TIME"]]
                    df_pid_burst_status["LOG_TIMESTAMP"] = [convert_readable_time_list_to_timestamp(x) for x in
                                                       df_pid_burst_status["LOG_TIME"]]
                    df_pid_dnd_status["LOG_TIMESTAMP"] = [convert_readable_time_list_to_timestamp(x) for x in
                                                            df_pid_dnd_status["LOG_TIME"]]

                    df_pid_sleep_wake = df_pid_sleep_wake[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "LOG_TIME", "LOG_TIMESTAMP", 'SLEEP_OR_WAKE', 'SLEEP_OR_WAKE_TIME']]
                    df_pid_burst_status = df_pid_burst_status[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "LOG_TIME", "LOG_TIMESTAMP", 'IS_BURST_DAY']]
                    df_pid_dnd_status = df_pid_dnd_status[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "LOG_TIME", "LOG_TIMESTAMP", 'IS_IN_DND']]
...
if __name__ == "__main__":
    microT_root_path = r"C:\Users\jixin\Desktop\testing\logs"
    intermediate_file_save_path = r"C:\Users\jixin\Desktop\testing"
    p_id_list = ["rippingpeprepose@timestudy_com"]
    decrypt_password = ""
    date = "2021-11-17"

    pre_process(microT_root_path, intermediate_file_save_path, p_id, decrypt_password, date)
