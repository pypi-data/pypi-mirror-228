        ...
        if path.exists(date_folder_path):
            ...
            if path.exists(swan_dest):
                ...
                if not swan_hour_minute_exist:
                    ...
                    if df_swan_minute is not None:
                        df_swan_minute["PARTICIPANT_ID"] = [pid] * len(df_swan_minute)
                        df_swan_minute["NUMERICAL_ID"] = [numerical_id] * len(df_swan_minute)
                        df_swan_minute.to_csv(day_file_name_swan_minute, index=False)

                        df_swan_hour = get_swan_matrix_hour(df_swan_minute)
                        df_swan_hour["PARTICIPANT_ID"] = [pid] * len(df_swan_hour)
                        df_swan_hour["NUMERICAL_ID"] = [numerical_id] * len(df_swan_hour)
                        df_swan_hour.to_csv(day_file_name_swan_hour, index=False)

                        df_swan_day = get_swan_matrix_day(df_swan_hour)
                        df_swan_day["PARTICIPANT_ID"] = [pid] * len(df_swan_day)
                        df_swan_day["NUMERICAL_ID"] = [numerical_id] * len(df_swan_day)
            ...
            if not mims_dest_exists or not har_dest_all_exist or not mims_hour_minute_exist:
                ...
                if df_participant_mims.shape[0] != 0:
                    df_participant_mims["PARTICIPANT_ID"] = [pid] * len(df_participant_mims)
                    df_participant_mims["NUMERICAL_ID"] = [numerical_id] * len(df_participant_mims)
                    df_participant_mims["LOG_TIMESTAMP"] = [convert_readable_time_list_to_timestamp(x) for x in
                                                       df_participant_mims["HEADER_TIME_STAMP"]]
                    df_participant_mims.to_csv(day_file_save_path_mims, index=False)
                if df_participant_har_intensity.shape[0] != 0:
                    df_participant_har_intensity["PARTICIPANT_ID"] = [pid] * len(df_participant_har_intensity)
                    df_participant_har_intensity["NUMERICAL_ID"] = [numerical_id] * len(df_participant_har_intensity)
                    df_participant_har_intensity.to_csv(day_file_save_path_har_intensity, index=False)
                if df_participant_har_activity.shape[0] != 0:
                    df_participant_har_activity["PARTICIPANT_ID"] = [pid] * len(df_participant_har_activity)
                    df_participant_har_activity["NUMERICAL_ID"] = [numerical_id] * len(df_participant_har_activity)
                    df_participant_har_activity.to_csv(day_file_save_path_har_activity, index=False)
                if df_participant_har_posture.shape[0] != 0:
                    df_participant_har_posture["PARTICIPANT_ID"] = [pid] * len(df_participant_har_posture)
                    df_participant_har_posture["NUMERICAL_ID"] = [numerical_id] * len(df_participant_har_posture)
                    df_participant_har_posture.to_csv(day_file_save_path_har_posture, index=False)
                if df_mims_minute is not None:
                    df_mims_minute["PARTICIPANT_ID"] = [pid] * len(df_mims_minute)
                    df_mims_minute["NUMERICAL_ID"] = [numerical_id] * len(df_mims_minute)
                    ...
                    df_mims_hour["PARTICIPANT_ID"] = [pid] * len(df_mims_hour)
                    df_mims_hour["NUMERICAL_ID"] = [numerical_id] * len(df_mims_hour)
                    df_mims_hour.to_csv(day_file_name_mims_hour, index=False)

                    df_mims_day = get_mims_matrix_day(df_mims_hour)
                    df_mims_day["PARTICIPANT_ID"] = [pid] * len(df_mims_day)
                    df_mims_day["NUMERICAL_ID"] = [numerical_id] * len(df_mims_day)
            ...
            if not auc_hour_minute_exists:
                ...
                if df_auc_minute is not None:
                    df_auc_minute["PARTICIPANT_ID"] = [pid] * len(df_auc_minute)
                    df_auc_minute["NUMERICAL_ID"] = [numerical_id] * len(df_auc_minute)
                    df_auc_minute.to_csv(day_file_name_auc_minute, index=False)

                    df_auc_hour = get_auc_matrix_hour(df_auc_minute)
                    df_auc_hour["PARTICIPANT_ID"] = [pid] * len(df_auc_hour)
                    df_auc_hour["NUMERICAL_ID"] = [numerical_id] * len(df_auc_hour)
                    df_auc_hour.to_csv(day_file_name_auc_hour, index=False)

                    df_auc_day = get_auc_matrix_day(df_auc_hour)
                    df_auc_day["PARTICIPANT_ID"] = [pid] * len(df_auc_day)
                    df_auc_day["NUMERICAL_ID"] = [numerical_id] * len(df_auc_day)
