...
from .screen_status_minute import *
from .screen_status_hour import *
from .screen_status_15min import *
from .screen_status_day import *
...
def pre_process(microT_root_path, intermediate_file_save_path, p_id, decrypt_password, date):
    ...
    if path.exists(area_folder_path):
        ...
        if path.exists(date_folder_path):
            ...
            day_screen_minute_save_path = file_save_date_path + sep + device + "_screen_status_minute" + "_clean_" + date + ".csv"
            day_screen_hour_save_path = file_save_date_path + sep + device + "_screen_status_hour" + "_clean_" + date + ".csv"
            day_screen_min15_save_path = file_save_date_path + sep + device + "_screen_status_minute15" + "_clean_" + date + ".csv"
            day_screen_day_save_path = file_save_date_path + sep + device + "_screen_status_day" + "_clean_" + date + ".csv"

            if True:
                # if not path.exists(day_file_save_path) or not path.exists(day_screen_minute_save_path) or not path.exists(
                #         day_screen_hour_save_path):
                ...
                if hours_with_target_file != 0:
                    ...
                    df_participant["LOG_TIMESTAMP"] = [convert_readable_time_list_to_timestamp(x) for x in
                                                       df_participant["LOG_TIME"]]

                    # step 3:  write out day file for participant
                    df_participant = df_participant[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "LOG_TIME", "LOG_TIMESTAMP", "PHONE_EVENT"]]
                    df_participant.to_csv(day_file_save_path, index=False)
                    # screen on/off status, unlock events
                    df_day_screen_minute = get_screen_status_matrix_minute(df_participant, date)
                    df_day_screen_minute["PARTICIPANT_ID_TEXT"] = [pid] * len(df_day_screen_minute)
                    df_day_screen_minute["PARTICIPANT_ID_NUMERIC"] = [numerical_id] * len(df_day_screen_minute)
                    df_day_screen_minute = df_day_screen_minute[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "YEAR", "MONTH", "DAY", "HOUR", "MINUTE",
                         "SCREEN_STATUS", "SCREEN_ON_SECONDS", "UNLOCK_EVENTS_NUM"]]

                    df_day_screen_hour = get_screen_status_matrix_hour(df_day_screen_minute)
                    df_day_screen_hour["PARTICIPANT_ID_TEXT"] = [pid] * len(df_day_screen_hour)
                    df_day_screen_hour["PARTICIPANT_ID_NUMERIC"] = [numerical_id] * len(df_day_screen_hour)
                    df_day_screen_hour = df_day_screen_hour[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "YEAR", "MONTH", "DAY", "HOUR",
                         "SCREEN_ON_SECONDS", "UNLOCK_EVENTS_NUM"]]

                    # df_day_screen_minute15 = get_screen_status_matrix_minute15(df_day_screen_minute)
                    df_day_screen_day = get_screen_status_matrix_day(df_day_screen_hour)
                    df_day_screen_day["PARTICIPANT_ID_TEXT"] = [pid] * len(df_day_screen_day)
                    df_day_screen_day["PARTICIPANT_ID_NUMERIC"] = [numerical_id] * len(df_day_screen_day)
                    df_day_screen_day = df_day_screen_day[
                        ["PARTICIPANT_ID_TEXT", "PARTICIPANT_ID_NUMERIC", "YEAR", "MONTH", "DAY",
                         "SCREEN_ON_SECONDS", "UNLOCK_EVENTS_NUM"]]

                    df_day_screen_minute.to_csv(day_screen_minute_save_path, index=False)
                    df_day_screen_hour.to_csv(day_screen_hour_save_path, index=False)
                    # df_day_screen_minute15.to_csv(day_screen_min15_save_path, index=False)
                    df_day_screen_day.to_csv(day_screen_day_save_path, index=False)
