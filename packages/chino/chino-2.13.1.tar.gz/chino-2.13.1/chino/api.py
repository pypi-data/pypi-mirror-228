...
from .objects import Repository, ListResult, User, Group, Schema, Document, \
    BulkGetResult
from .objects import Blob, BlobDetail, UserSchema, Collection, Permission, IDs
from .objects import Application, Consent, DumpFile
...
class ChinoAPIBase(object):  # PRAGMA: NO COVER
    ...
    def __init__(self, auth, url, timeout, session=True, force_https=True):
        """
        Init the class, auth is ref, so it can be changed and changes
        ...
        :return:
        """
        # we have to ensure https, the 302 on http is forwarded
        # to the GET operation even if it's a POST PUT DELETE
        if force_https:
    ...
    def apicall(self, method, url, params=None, data=None, form=None,
        ...
        if method == 'CHUNK':
            ...
            if method == 'GET':
                ...
                res = self._apicall_put(url, data, params)
            elif method == 'DELETE':
                res = self._apicall_delete(url, params)
            elif method == 'PATCH':
                res = self._apicall_patch(url, data, params)
            else:
    ...
    def _apicall_put(self, url, data, params):
        if hasattr(data, 'to_dict()'):
            d = data.to_dict()
        else:
            d = data
        r = self.req.put(url, auth=self._get_auth(), params=params,
                         data=json.dumps(d), timeout=self.timeout)
        return r

    def _apicall_patch(self, url, data, params):
        if hasattr(data, 'to_dict()'):
            d = data.to_dict()
        else:
            d = data
        r = self.req.patch(url, auth=self._get_auth(), params=params,
                           data=json.dumps(d), timeout=self.timeout)
        return r
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        message = r.text
                logger.exception("Decoding result {} {}".format(r, message))
                raise CallError(
...
class ChinoAPIUsers(ChinoAPIBase):
    ...
    def code(self, code, redirect_uri, client_id, client_secret):
        ...
        # self.auth = None
        url = "auth/token/"
        pars = dict(code=code, redirect_uri=redirect_uri, client_id=client_id,
    ...
    def login(self, username, password):
        ...
        try:
            ...
            self.auth.refresh_token = result['refresh_token']
            self.auth.bearer_token = result['access_token']
            self.auth.bearer_exp = datetime.now() + timedelta(
    ...
    def refresh(self, refresh_token=None):
        ...
        # self.auth = None
        url = "auth/token/"
        pars = dict(grant_type='refresh_token', client_id=self.auth.client_id,
                    client_secret=self.auth.client_secret,
    ...
    def logout(self):
        ...
        auth = self.auth
        pars = dict(client_id=self.auth.client_id,
                    client_secret=self.auth.client_secret,
    ...
    def bulk_get(self, user_ids):
        url = 'users/bulk_get'

        if not isinstance(user_ids, list):
            raise TypeError('user_ids parameter needs to be list.')

        for document_id in user_ids:
            if not isinstance(document_id, str):
                raise TypeError(
                    'user_ids list must contain only strings.'
                )

        data = dict(ids=user_ids)
        result = self.apicall('POST', url, data=data)

        return BulkGetResult(User, result)
    ...
    def create(self, user_schema_id, username, password, attributes=None,
        ...
        return User(
            **self.apicall('POST', url, data=data, params=params)['user'])

    def update(self, user_id, check_reuse=False, **kwargs):
        url = "users/%s" % user_id
        params = {}
        if check_reuse:
            params['check_reuse'] = 'true'
        u_updated = self.apicall('PUT', url, data=kwargs, params=params)[
            'user']
        return User(**u_updated)

    def partial_update(self, user_id, consistent=False, check_reuse=False,
                       **kwargs):
        url = "users/%s" % user_id
        params = dict()
        if consistent:
        ...
        if check_reuse:
            params['check_reuse'] = 'true'
        u_updated = self.apicall('PATCH', url, data=kwargs, params=params)[
            'user']
        return User(**u_updated)
    ...
    def search(self, user_schema_id, query, result_type="FULL_CONTENT",
               sort=None, **kwargs):
        url = 'search/users/%s' % user_schema_id
        if not sort:
            sort = []
        data = dict(result_type=result_type, query=query)
        if sort:
            data['sort'] = sort
        if result_type == "COUNT":
            return self.apicall('POST', url, data=data, params=kwargs)['count']
        elif result_type == "EXISTS" or result_type == "USERNAME_EXISTS":
            return self.apicall('POST', url, data=data, params=kwargs)[
                'exists']
        else:
            return ListResult(User, self.apicall('POST', url, data=data,
                                                 params=kwargs))

    def list_blocked_users(self, user_id=None, ip=None, retrieve_users=False):
        url = 'auth/blocked_users'
        payload = {}
        if user_id:
        ...
        response = self.apicall('POST', url, data=payload)
        if not retrieve_users:
            return response
        else:
            user_ids = [i['user_id'] for i in response]
            bulk = self.bulk_get(user_ids)
            for i, item in enumerate(response):
                response[i]['user'] = bulk.users.get(item['user_id'], None)
            return response

    def unblock_user(self, user_id, ip):
        url = 'auth/unblock_user'
        payload = {"user_id": user_id, "ip": ip}
        return self.apicall("POST", url, data=payload)
...
class ChinoAPIGroups(ChinoAPIBase):
    ...
    def list(self, **pars):
        ...
        :param: usual for a list ``offset``, ``limit``. Use ``name`` to
    ...
    def add_users_to_group(self, group_id, user_schema_id):
        url = "groups/%s/user_schemas/%s" % (group_id, user_schema_id)
        return self.apicall('POST', url)
...
class ChinoAPIPermissions(ChinoAPIBase):
    ...
    def resources(self, action, resource_type, subject_type, subject_id,
    ...
    def resource(self, action, resource_type, resource_id, subject_type,
                 subject_id,
                 manage=None, authorize=None):
    ...
    def resource_children(self, action, resource_type, resource_id,
                          resource_child_type, subject_type, subject_id,
                          manage=None, authorize=None, created_document=None):
        url = "perms/%s/%s/%s/%s/%s/%s" % (
            action, resource_type, resource_id, resource_child_type,
    ...
    def read_perms_document(self, document_id):
        ...
        return [Permission(**p) for p in
                self.apicall('GET', url)['permissions']]
...
class ChinoAPIRepositories(ChinoAPIBase):
    ...
    def list(self, **pars):
        ...
        :param: usual for a list ``offset``, ``limit``. Use ``descr`` to
                filter by description
        :return: dict containing ``count``,``total_count``,``limit``,``offset``
    ...
    def update(self, repository_id, **kwargs):
        ...
        url = "repositories/%s" % repository_id
        return Repository(
            **self.apicall('PUT', url, data=kwargs)['repository'])
    ...
    def delete(self, repository_id, force=False, all_content=False):
        ...
        :param repository_id: (id) the id of the repository
        :param force: to force a deletion
        :param all_content: to delete in cascade
        :return: None
        """
        url = "repositories/%s" % repository_id
        params = dict()
...
class ChinoAPISchemas(ChinoAPIBase):
    ...
    def list(self, repository_id, **pars):
        ...
        :param: usual for a list ``offset``, ``limit``. Use ``descr`` to
                filter by description
        :return: dict containing ``count``,``total_count``,``limit``,
            ``offset``,``repositories``
        """
    ...
    def convert_datatypes_submit(self, schema_id, fields=None, defaults=None):
        """
        Submit a new convert datatypes job.

        :param schema_id: (id) of the schema
        :param fields: 'fields' (dict) field name: new datatype
        :param defaults: (dict) field name: new default
        :return: message
        """
        url = "schemas/convert/%s" % schema_id
        data = {}
        if fields:
            data['fields'] = fields
        if defaults:
    ...
    def convert_datatypes_status(self, schema_id):
        """
        Read the current status of the datatype conversion job

        :param schema_id: (id) of the schema
        :return: dict
        """
        url = "schemas/convert/%s/status" % schema_id
        return self.apicall('GET', url)

    def dump_submit(self, schema_id, query, sort=None, fields=None,
                    format='json'):
        """
        Submit a new dump schema job.

        :param schema_id: (id) of the schema
        :param query: (dict) a query to limit the documents to dump
        :param sort: (dict) sort info
        :param fields: (list) a list to limit the fields to dump
        :param format: (str) the format of the ouput file (json, csv)
        :return: dict with submit recap and the job id (dump_id)
        """
        url = "schemas/dump/%s" % schema_id
    ...
    def dump_status(self, dump_id):
        """
        Read the status of the dump job.

        :param dump_id: (str) the id of the dump job (uuid)
        :return: (dict) info about the status of the job
        """
        url = "schemas/dump/%s/status" % dump_id
        return self.apicall('GET', url)

    def dump_download(self, dump_id):
        """
        Download the file with the dump.

        :param dump_id: (str) the id of the dump job (uuid)
        :retrieve_status: (bool) if True fill the DumpFile obj with
                                 status info (default)
        :return: (stream) the stream of the dump file
        """
        status = self.dump_status(dump_id)
        if not status['status'] == 'COMPLETED':
            raise CallError(400, "Download not ready")

        url = "schemas/dump/%s/download" % dump_id
        dump_resp = self.apicall('GET', url, raw=True)
        if dump_resp.status_code == 200:
            return DumpFile(dump_id, dump_resp.content, status_info=status)
        else:
            return dump_resp
...
class ChinoAPIDocuments(ChinoAPIBase):
    ...
    def list(self, schema_id, full_document=False, **pars):
        ...
        # validate is_active and convert bool to the required bool string
        is_active = pars.get('is_active', None)
        if is_active is not None:
            if is_active not in (True, False):
                raise TypeError("is_active must be bool")
            pars['is_active'] = str(is_active).lower()

        # validate datetime params and convert to the required datetime string
        datetime_params = [
            'insert_date', 'insert_date__gt', 'insert_date__lt',
            'last_update', 'last_update__gt', 'last_update__lt',
        ]
        for param in datetime_params:
            d = pars.get(param, None)
            if d is None:
                continue

            if not isinstance(d, datetime):
                raise TypeError("{} must be datetime".format(param))

            # if it's not a naive datetime (has timezone info), make it UTC
            if d.tzinfo is not None and d.tzinfo.utcoffset(d) is not None:
                d = d.astimezone(pytz.utc)

            pars[param] = d.strftime("%Y-%m-%dT%H:%M:%S")

        return ListResult(Document, self.apicall('GET', url, params=pars))

    def bulk_get(self, document_ids):
        url = 'documents/bulk_get'

        if not isinstance(document_ids, list):
            raise TypeError('document_ids parameter needs to be list.')

        for document_id in document_ids:
            if not isinstance(document_id, str):
                raise TypeError(
                    'document_ids list must contain only strings.'
                )

        data = dict(ids=document_ids)
        result = self.apicall('POST', url, data=data)

        return BulkGetResult(Document, result)
    ...
    def create(self, schema_id, content, consistent=False):
        ...
        if consistent:
            params['consistent'] = 'true'
        return Document(
            **self.apicall('POST', url, data=data, params=params)['document'])
    ...
    def update(self, document_id, **kwargs):
        # if partial=True is passed, use PATCH instead of PUT
        partial = kwargs.pop('partial', False)
        method = 'PATCH' if partial else 'PUT'

        # data = dict(content=content)
        url = "documents/%s" % document_id
        return Document(**self.apicall(method, url, data=kwargs)['document'])
    ...
    def search(self, schema_id, query, result_type="FULL_CONTENT", sort=None,
               **kwargs):
        ...
        if result_type == "COUNT":
            ...
            return bool(
                self.apicall('POST', url, data=data, params=kwargs)['exists'])
        elif result_type == "ONLY_ID":
            return ListResult(IDs, self.apicall('POST', url, data=data,
                                                params=kwargs))
        else:
            return ListResult(Document, self.apicall('POST', url, data=data,
                                                     params=kwargs))
...
class ChinoAPIBlobs(ChinoAPIBase):
    ...
    def send(self, document_id, blob_field_name, file_path,
             chunk_size=12 * 1024):
        ...
        sha1 = hashlib.sha1()
        offset = 0
        for chunk in self._read_bytes_from_file(file_path,
    ...
    def chunk(self, upload_id, data, length, offset):
        ...
        return self.apicall('CHUNK', url, data=data,
                            params=dict(length=length, offset=offset))['blob']
    ...
    def detail(self, blob_id):
        ...
        return Blob(filename=fname, content=res.content, blob_id=blob_id)

    def delete(self, blob_id):
        url = 'blobs/%s' % blob_id
        return self.apicall('DELETE', url)

    def generate(self, blob_id, one_time=None, duration=None):
        url = 'blobs/{}/generate'.format(blob_id)
        data = dict()
        if one_time:
            # even if false is not sent, but false is the default
            data['one_time'] = one_time
        if duration:
            data['duration'] = duration
        return self.apicall('POST', url, data=data)

    def detail_token(self, blob_id, token, filename, mimetype=None):
        url = 'blobs/url/%s?token=%s' % (blob_id, token)
        if mimetype:
            url += "&mimetype=%s" % mimetype
        # this is different
        res = self.apicall('GET', url, raw=True)
        # fname = res.headers['Content-Disposition'].split(';')[1].split('=')[1]
        return Blob(filename=filename, content=res.content, blob_id=blob_id)
...
class ChinoAPISearches(ChinoAPIBase):
    ...
    @deprecated(deprecated_in="2.4.0", removed_in="3.0.0",
                details="Use ChinoAPIDocuments.search instead")
    def search(self, schema_id, result_type="FULL_CONTENT", filter_type="and",
               sort=None, filters=None, **kwargs):
        return self.documents(schema_id, result_type, filter_type, sort,
                              filters, **kwargs)

    @deprecated(deprecated_in="2.4.0", removed_in="3.0.0",
                details="Use ChinoAPIDocuments.search instead")
    def documents(self, schema_id, result_type="FULL_CONTENT",
                  filter_type="and", sort=None, filters=None, **kwargs):
        url = 'search/documents/%s' % schema_id
        if not sort:
            sort = []
        if not filters:
    ...
    @deprecated(deprecated_in="2.4.0", removed_in="3.0.0",
                details="Use ChinoAPIDocuments.search instead")
    def documents_complex(self, schema_id, query, result_type="FULL_CONTENT",
                          sort=None,
                          **kwargs):
        url = 'search/documents/%s' % schema_id
        if not sort:
    ...
    @deprecated(deprecated_in="2.4.0", removed_in="3.0.0",
                details="Use ChinoAPIUsers.search instead")
    def users(self, user_schema_id, result_type="FULL_CONTENT",
              filter_type="and", sort=None, filters=None, **kwargs):
        url = 'search/users/%s' % user_schema_id
        if not sort:
            sort = []
    ...
    @deprecated(deprecated_in="2.4.0", removed_in="3.0.0",
                details="Use ChinoAPIUsers.search instead")
    def users_complex(self, user_schema_id, query, result_type="FULL_CONTENT",
                      sort=None, **kwargs):
        url = 'search/users/%s' % user_schema_id
        if not sort:
            sort = []
...
class ChinoAuth(object):
    ...
    def __init__(self, customer_id, customer_key=None, bearer_token=None,
        ...
        :param customer_key: optional, if specified the auth is set as
                chino customer (admin)
...
class ChinoAPIUserSchemas(ChinoAPIBase):
    ...
    def create(self, description, fields):
        ...
        url = "user_schemas"
        return UserSchema(
            **self.apicall('POST', url, data=data)['user_schema'])
    ...
    def update(self, user_schema_id, **kwargs):
        ...
        url = "user_schemas/%s" % user_schema_id
        return UserSchema(
            **self.apicall('PUT', url, data=kwargs)['user_schema'])
    ...
    def convert_datatypes(self, user_schema_id, fields=None, defaults=None):
        """
        Submit a new convert datatypes job.

        :param user_schema_id: (id) of the user schema
        :param fields: 'fields' (dict) field name: new datatype
        :param defaults: (dict) field name: new default
        :return: message
        """
        url = "user_schemas/convert/%s" % user_schema_id
        data = {}
        if fields:
            data['fields'] = fields
        if defaults:
    ...
    def convert_datatypes_status(self, user_schema_id):
        """
        Read the current status of the datatype conversion job
        ...
        :return: dict
        """
        url = "user_schemas/convert/%s/status" % user_schema_id
        return self.apicall('GET', url)

    def dump_submit(self, user_schema_id, query, sort=None, fields=None,
                    format='json'):
        """
        Submit a new dump schema job.
        ...
        :return: dict with submit recap and the job id (dump_id)
        """
        url = "user_schemas/dump/%s" % user_schema_id
        data = {'query': query, 'format': format}
        if sort:
            data['sort'] = sort
        if fields:
    ...
    def dump_status(self, dump_id):
        ...
        :return: (dict) info about the status of the job
        """
        url = "user_schemas/dump/%s/status" % dump_id
        return self.apicall('GET', url)
    ...
    def dump_download(self, dump_id):
        ...
        url = "user_schemas/dump/%s/download" % dump_id
        dump_resp = self.apicall('GET', url, raw=True)
        if dump_resp.status_code == 200:
            return DumpFile(dump_id, dump_resp.content, status_info=status)
        else:
...
class ChinoAPICollections(ChinoAPIBase):
    ...
    def list(self, **pars):
        ...
        :param: usual for a list ``offset``, ``limit``. Use ``document_id``
                to list only the collections with a specific document
        :return: dict containing ``count``,``total_count``,``limit``,
                ``offset``,``repositories``
        """
        # if you specify 'document_id' as kwarg:
        #    collections = client.collections.list(document_id=<doc_id>)
        # use the url with the filter, otherwise the base LIST url
        if 'document_id' in pars.keys():
            url = "collections/document/%s" % pars.pop('document_id')
        else:
            url = "collections"
    ...
    def update(self, collection_id, **kwargs):
        url = "collections/%s" % collection_id
        return Collection(
            **self.apicall('PUT', url, data=kwargs)['collection'])
    ...
    def search(self, name, contains=False, **pars):
        ...
        return ListResult(Collection,
                          self.apicall('POST', url, params=pars, data=data))
...
class ChinoAPIApplication(ChinoAPIBase):
    ...
    def create(self, name, grant_type='password', redirect_url='',
        ...
        :param redirect_url: the redirect url (if grant_type allows it)
        :param client_type: optional parameter client_type can be either
                'public' or
                'confidential'."confidential" is the default value.
        ...
        url = "auth/applications"
        return Application(
            **self.apicall('POST', url, data=data)[Application.__str_name__])
    ...
    def detail(self, application_id):
        ...
        return Application(
            **self.apicall('GET', url)[Application.__str_name__])

    def update(self, application_id, **kwargs):
        url = "auth/applications/%s" % application_id
        return Application(
            **self.apicall('PUT', url, data=kwargs)[Application.__str_name__])
...
class ChinoAPIConsents(ChinoAPIBase):
    ...
    @deprecated(deprecated_in="2.4.0", removed_in="3.0.0",
                details="Deprecated in favor of the new Consenta API: "
                        "https://docs.chino.io/consent/consentame/docs/v1")
    def list(self, user_id=None, **pars):
        ...
        :param pars: dict with fields ``offset``, ``limit``
                (to navigate through paged results of a list)
        :return: dict containing ``count``,``total_count``,``limit``,
                ``offset``,``repositories``
        """
        url = "consents"
    ...
    def create(self, user_id, details, data_controller, purposes):
        ...
        :param user_id: string that identifies uniquely a user.
                    Can be a chino.objects.User user_id attribute
                    or any other valid string
                    (i.e. an email address)
        :param details: a dict that contains all the following fields:
                    ``description``, ``policy_url``,
                    ``policy_version`` and ``collection_mode``
        :param data_controller: a dict that must contain all the following
                fields: ``company``, ``contact``, ``address``, ``email``,
                ``VAT`` and ``on_behalf``
        :param purposes: a list that contains one or more dicts.
                The dicts must contain all the following fields:
                ``authorized``, ``purpose`` and ``description``
        ...
        return Consent(
            **self.apicall("POST", url, data=consent_obj)['consent'])
    ...
    def history(self, consent_id, **pars):
        ...
        :param pars: dict with fields ``offset``, ``limit``
                to navigate through paged results of a list
        :return: a ListResult with the history of the Consent object
                with the specified consent_id
        """
    ...
    def update(self, consent_id, user_id, details, data_controller, purposes):
        """
        Creates a new Consent with the specified informations and withdraws
        the old one which has the same ID.
        ...
        :param details: a dict that contains all the following fields:
                ...
                ``policy_version`` and ``collection_mode``
        :param data_controller: a dict that must contain all the following
                fields: ``company``, ``contact``, ``address``,
                `email``, ``VAT`` and ``on_behalf``
        :param purposes: a list that contains one or more dicts.
                The dicts must contain all the following fields:
                ``authorized``, ``purpose`` and ``description``
        :return: the created Consent object
        ...
        url = "consents/%s" % consent_id
        return Consent(
            **self.apicall("PUT", data=updated_consent, url=url)['consent'])
    ...
    def withdraw(self, consent_id):
        """
        Withdraws the Consent which has the specified ID. Withdrawn consents
        can not be further updated, but remain in the database for further
        reference.
...
class ChinoAPIClient(object):
    ...
    def __init__(self, customer_id, customer_key=None, bearer_token=None,
                 client_id=None, client_secret=None,
                 version='v1', url='https://api.chino.io/', timeout=30,
                 session=True, force_https=True):
        """
        Init the class

        :param customer_id: mandatory
        :param customer_key: optional, if specified the auth is set as chino
                        customer (admin)
        :param bearer_token: optional, if specified the auth is as user
