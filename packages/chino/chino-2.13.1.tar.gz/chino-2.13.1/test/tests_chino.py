...
class SchemaChinoTest(BaseChinoTest):
    ...
    def test_dump(self):
        ...
        res = self.chino.schemas.dump_submit(created.schema_id, query=query,
                                             sort=sort)
...
class PermissionChinoTest(BaseChinoTest):
    ...
    def test_create_a_repository(self):
        with self.assertRaises(
                CallError):  # user has no create permission, expected an error.
        ...
        self.chino.permissions.resources('grant', 'repositories', 'users',
                                         ...
                                         manage=['R', 'U', 'L'])
        with self.assertRaises(
                CallError):  # must fail because user has no create permission
        ...
        with self.assertRaises(
                CallError):  # must fail because user has no "D" permission
            self.chino_user1.repositories.delete(self.repository._id)
        self.chino_user0.permissions.resource('grant', 'repositories',
                                              self.repository._id, 'users',
                                              self.user1._id,
                                              manage=['D'])
        with self.assertRaises(
                CallError):  # must fail because user is not authorized
            self.chino_user1.permissions.resource('grant', 'repositories',
                                                  self.repository._id, 'users',
                                                  self.user0._id,
                                                  manage=['D'])
        self.chino_user1.repositories.delete(self.repository._id)
    ...
    def test_create_documents(self):
        ...
        content = dict(fieldInt=123, fieldString='test', fieldBool=False,
                       fieldDate='2015-02-19',
                       fieldDateTime='2015-02-19T16:39:47')
        self.document = self.chino_user0.documents.create(self.schema,
                                                          content=content)._id
        with self.assertRaises(
                CallError):  # must fail because user is not authorized
        ...
        self.chino_user0.permissions.resource_children('grant', 'schemas',
                                                       self.schema,
                                                       'documents', 'users',
                                                       self.user1._id,
                                                       manage=['L'])
