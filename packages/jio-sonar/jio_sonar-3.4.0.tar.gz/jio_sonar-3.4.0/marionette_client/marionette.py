...
class Marionette(object):
    ...
    def cleanup(self):
        if self.session is not None:
            try:
                self.delete_session()
            except (errors.KaiOSMarionetteException, socket.error, IOError):
    ...
    def _send_message(self, name, params=None, key=None):
        ...
        reason, if ``execute_script``, or ``execute_async_script`` is
        called, it will loop until all commands requested from the
        server have been exhausted, and we receive our expected response.
        ...
        if not self.session_id and name != "WebDriver:NewSession":
            raise errors.SessionNotCreatedException("Please start a session")

        try:
            print("data_now", self.protocol)
            if self.protocol < 3:
                print("in if")
                data = {"name": name}
                if params:
    ...
    def _handle_error(self, obj):
        if self.protocol == 1:
            print("IN HERE HANDLE ERROR")
            if "error" not in obj or not isinstance(obj["error"], dict):
                raise errors.KaiOSMarionetteException(
    ...
    def start_session(self, desired_capabilities=None, session_id=None, timeout=60):
        ...
        # 
        # try:
        #     resp = self._send_message("WebDriver:NewSession", body)
