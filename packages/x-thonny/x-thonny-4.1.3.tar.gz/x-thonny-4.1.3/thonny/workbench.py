...
class Workbench(tk.Tk):
    ...
    创建主窗口
    维护布局（_init_containers）
    加载插件（_init_plugins，add_view，add_command）
    提供主要组件的引用（editor_notebook和runner）
    各组件间的通信（参见event_generate和bind）
    配置服务（get_option，set_option，add_defaults）
    加载翻译
    维护字体（命名字体，增大和减小字体大小）
    在工作台和插件加载后，会开始出现三种类型的事件：

    用户事件（按键、鼠标点击、菜单选择等）
    虚拟事件（主要通过get_workbench().event_generate触发）。这些事件包括：
    通过Tk事件系统报告和分发的事件；
    通过增强的get_workbench().event_generate报告的WorkbenchEvent事件。
    来自后台进程的事件（程序输出通知、输入请求、调试器进度通知）
    ...
    def _init_icon(self) -> None:
        ...
        if running_on_linux() and ui_utils.get_tk_version_info() >= (8, 6):
            self.iconphoto(True, self.get_image("edu-yh.png"))
        else:
            icon_file = os.path.join(self.get_package_dir(), "res", "edu-yh.ico")
            print(icon_file)
            try:
    ...
    """
    @Author ：yb
    @Date   ：2023-08-13 17:16:35
    @Desc   ： 
    self是实体类Workbench的实例
    class Workbench(tk.Tk):
    ...
    def _init_containers(self) -> None:
        ...
        """
        @Author ：yb
        @Date   ：2023-08-13 16:05:05
        @Desc   ：定义布局中各个组件的宽度和高度
        """
