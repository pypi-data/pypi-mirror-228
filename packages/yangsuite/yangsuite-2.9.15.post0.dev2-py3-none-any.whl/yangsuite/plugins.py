...
def check_public_versions() -> list:
    ...
    for app_name in all_apps_names:
        ...
        if resp.status_code == 200:
            ...
            # Don't report pre-releases.
            filtered_rel = [r for r in releases if ".post" not in r]
            # Don't report yanked releases.
            for rel in filtered_rel:
                for blob in data["releases"][rel]:
                    if blob["yanked"] is True:
                        filtered_rel.remove(rel)
                        break
            releases = filtered_rel
            ...
            plugins.append(
                {
                    "package_name": app_name,
                    "description": "Something went wrong. Request status to "
                    + f"pypi.org: {resp.status_code} - '{responses[resp.status_code]}'",
                    "latest_version": "None known",
...
def update_plugins(plugins):
    ...
    for plugin_info in plugins:
        ...
        if version:
            ...
            args.append(plugin)
        # TODO: remove this condition at yangtree version 2.0.1
        if plugin == "yangsuite-yangtree" and version == "2.0.0":
            special_reboot = True
    ...
    try:
        ...
        for plugin_info in plugins:
            ...
            if not output:
                ...
                "Successfully installed .*{0}".format(plugin), output
            ):
