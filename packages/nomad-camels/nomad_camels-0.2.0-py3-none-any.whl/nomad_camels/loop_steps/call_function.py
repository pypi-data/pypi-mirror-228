...
class Call_Function(Loop_Step):
    def __init__(self, name='', parent_step=None, step_info=None, **kwargs):
        ...
        if step_info is None:
            step_info = {}
        self.call_functions = step_info['call_functions'] if 'call_functions' in step_info else []
    ...
    def get_protocol_string(self, n_tabs=1):
        ...
        for func in self.call_functions:
            if func not in available_functions:
                raise Exception(f'Trying to call function "{func}" but it is not available!\nCheck whether you maybe renamed an instrument.')
            dev, fun = func.split('.')
...
class Call_Function_Config(Loop_Step_Config):
    def __init__(self, loop_step:Call_Function, parent=None):
        ...
        self.table = Call_Functions_Table(headerLabels=labels, parent=self,
                                          info_dict=info,
                                          title='Functions')
