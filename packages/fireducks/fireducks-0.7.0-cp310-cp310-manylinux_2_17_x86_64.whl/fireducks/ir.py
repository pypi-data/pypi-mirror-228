...
Op_make_scalar_time_point_ns = fire.Opcode(
    "fireducks.make_scalar.time_point_ns"
)
...
Op_groupby_select_head = fire.Opcode("fireducks.groupby_select_head")
Op_groupby_select_tail = fire.Opcode("fireducks.groupby_select_tail")
...
def concat(tables, ignore_index):
    return _build_op(
        Op_concat,
        [TableType],
        [tables, create_i1_attr("ignore_index", ignore_index)],
        chaining=True,
    ).outs[0]
...
def drop_duplicates(table, subset, keep):
    return _build_op(
        Op_drop_duplicates,
        [TableType],
        [table, subset, keep],
...
def dropna(table, subset, is_any, thresh):
    return _build_op(
        Op_dropna,
        [TableType],
        [
...
def filter(table, array):
    return _build_op(
        Op_filter, [TableType], [table, array], chaining=True
    ).outs[0]
...
def fillna_scalar(table, fill_value):
    return _build_op(
        Op_fillna_scalar,
        [TableType],
        [table, fill_value],
...
def groupby_agg(table, groupkeys, funcs, columns, relabels, as_index, sort):
    return _build_op(
        Op_groupby_agg,
        [TableType],
        [
...
def groupby_head_or_tail(table, groupkeys, n, is_head):
    if is_head:
        return _build_op(
            Op_groupby_head,
            [TableType],
...
def groupby_select_agg(
    table, groupkeys, funcs, columns, relabels, selector, as_index, sort
):
    return _build_op(
...
def groupby_select_head_or_tail(table, groupkeys, selector, n, is_head):
    if is_head:
        return _build_op(
            Op_groupby_select_head,
            [TableType],
            [table, groupkeys, selector, n],
            chaining=True,
        ).outs[0]
    else:
        return _build_op(
            Op_groupby_select_tail,
            [TableType],
            [table, groupkeys, selector, n],
            chaining=True,
        ).outs[0]
...
def read_parquet(filename):
    return _build_op(Op_read_parquet, [TableType], [filename]).outs[0]
...
def set_index(table, key):
    return _build_op(
        Op_set_index, [TableType], [table, key], chaining=True
    ).outs[0]
...
def replace_scalar(table, to_replace, value):
    return _build_op(
        Op_replace_scalar,
        [TableType],
...
def reset_index(table, drop, is_series):
    return _build_op(
        Op_reset_index,
        [TableType],
        [
...
def rolling_aggregate(table, window, funcs):
    return _build_op(
...
def setitem(table, cols, rhs, from_list, from_ndarray):
    return _build_op(
        Op_setitem,
        [TableType],
        [
            ...
            cols,
            rhs,
            create_i1_attr("from_list", from_list),
            create_i1_attr("from_ndarray", from_ndarray),
        ],
        implicit_defs=[table],
        chaining=True,
    ).outs[0]
...
def sort_values(table, keys, orders, na_pos, is_series):
    return _build_op(
        Op_sort_values,
        [TableType],
        [table, keys, orders, na_pos, create_i1_attr("is_series", is_series)],
        chaining=True,
    ).outs[0]
...
def to_csv(table, index):
    return _build_op(
        Op_to_csv,
        [StringType],
        [table, create_i1_attr("index", index)],
        chaining=True,
    ).outs[0]


def value_counts(table, sort, ascending, dropna):
    return _build_op(
        Op_value_counts,
        [TableType],
        [
...
def write_csv(table, filename, index):
    return _build_op(
        Op_write_csv,
        [],
        [table, filename, create_i1_attr("index", index)],
        chaining=True,
    ).outs[0]
