...
class FireDucksObject:
    ...
    def _evaluate(self, options=None):
        logger.debug("FireDucksObject._evaluate")
        fireducks.core.evaluate([self._value], options)
        return self
...
def _install_fallback_may_inplace(cls, name, pos=-1, default=False):
    def wrapper(self, *args, **kwargs):
        reason = f"{name} is not yet implemented"
    ...
    if not utils._hasattr(cls, name):
        type.__setattr__(cls, name, wrapper)
...
def _from_pandas_to_value(obj):
    ...
    raise RuntimeError(
        "fireducks._from_panad_to_value: unknown object is given: "
        f"{type(obj)}"
    )
...
def _setup_FireDucksPandsCompat(cls):
    ...
    utils.install_fallbacks(cls, ["__repr__"], override=True)
...
class FireDucksPandasCompat(
    ...
    def __init__(self, value, pandas_object=None):
        super().__init__(value)
        metadata = FireDucksMetadata()
    ...
    def _fallback_call_packed(
        self, method, args=None, kwargs=None, *, reason=None, stacklevel=6
    ):
        return utils.fallback_call_packed(
            self._unwrap,
            method,
    ...
    def astype(self, dtype, copy=True, errors="raise"):
        ...
        if isinstance(dtype, Series):
            ...
            # Workaround until dfkl backend supports casting to category
            # GT #1932
            reason = "category is not supported"
    ...
    def fillna(self, *args, **kwargs):
        arg = utils.decode_args(args, kwargs, pandas.DataFrame.fillna)
        if arg.axis is None:
        ...
        return self._create_or_inplace(
            ir.fillna_scalar(self._value, arg.value), arg.inplace
        )
    ...
    def replace(self, *args, **kwargs):
        ...
        elif arg.value is pandas_lib.no_default or not irutils.irable_scalar(
            arg.value
        ):
        ...
        return self.__class__._create(
            ir.replace_scalar(self._value, arg.to_replace, arg.value)
        )
    ...
    def _logical_func(self, func, *args, **kwargs):
        ...
        if arg.axis is None:
            ...
            reason = arg.is_not_default(["bool_only", "skipna", "level"])
    ...
    def sum(self, *args, **kwargs):
        ...
        reason = arg.is_not_default(
            ["skipna", "level", "numeric_only", "min_count", "kwargs"]
        )
...
def setup_Scalar(cls):
    ...
    # FIXME: fireducks is incorrect. fireducks returns a exception in bitwise
    # operations.
    # @see test_bitwise_funcs, test_bitwise_funcs_lhs_bool,
    # test_bitwise_funcs_rhs_bool
    ...
    for op in bin_ops:
        ...
        reason = f"Scalar.{method_name} is called"

        if op[0] == "r":
            operator_func = getattr(operator, op[1:])
            wrapper = get_reverse_wrapper(method_name, operator_func, reason)
        else:
            operator_func = getattr(operator, op)
            wrapper = get_wrapper(method_name, operator_func, reason)

        utils.install_wrapper(cls, method_name, wrapper, None, True)
