...
class SENTIAFF(nn.Module):
    ...
    def forward(self, inp):
        ...
        return output
class SENTIAMHA(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0,
                 pre_lnorm=False):
        super(SENTIAMHA, self).__init__()
...
class SENTIATransformerBlock(nn.Module):
    ...
    def __init__(self, hidden_dim, num_heads):
        ...
        self.masked_multihead_attention = SENTIAMHA(num_heads, hidden_dim, hidden_dim // 8, 0)
...
class SENTIA(nn.Module):
    ...
    @staticmethod
    def generate_causal_mask(x: torch.Tensor):
        """
        ...
        seq_length = x.size(1)
        batch_size = x.size(0)
        mask = torch.triu(torch.ones(batch_size, seq_length), diagonal=1).to(x.device)
        return mask.bool()
    ...
    def top_k_top_p_filtering(logits, top_k=0, top_p=0.0, filter_value=-float('Inf')):
        ...
        if top_p > 0.0:
            ...
            logits[indices_to_remove] = filter_value
        return logits
    def fit(self, num_epochs, dataloader, tokenizer, optimizer, val_dataloader, scheduler, lr=3e-4,):
        """
...
class ConversationDataset(Dataset):
    def __init__(self, tokenizer, max_length=256, data=None):
        self.data = data
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.type = type
...
def map_fn(index, flags):
    ...
    data = load_dataset("roneneldan/TinyStories", split="train[:150000]")
    val_data = load_dataset("lighteval/mmlu", "all", split="validation")
    data = SENTIADataset(tokenizer, data, flags['batch_size'])
    val_data = ConversationDataset(tokenizer, data=val_data)
    ...
    hidden_dim = 1024
    model = SENTIA(len(tokenizer), embedding_dim, num_heads, num_layers, hidden_dim).to(device)
    para_train_loader = pl.ParallelLoader(train_loader, [device]).per_device_loader(device)
    para_val_loader = pl.ParallelLoader(test_loader, [device]).per_device_loader(device)
    optimizer = AdamW
    scheduler = StepLR
    wandb.init("SENTIA-session-2", 'D:\\Projects\\chatTulu\\', project="SENTIA")
    try:
        model.fit(flags['epochs'], para_train_loader, tokenizer, optimizer, para_val_loader, scheduler)
