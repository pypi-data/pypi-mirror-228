...
class SENTIAPreTrainedModel(PreTrainedModel):
    ...
    def generate_causal_mask(x):
        ...
        seq_length = x.size(1)
        batch_size = x.size(0)
        mask = torch.tril(torch.ones(batch_size, 1, seq_length, seq_length), diagonal=1).to(torch.device(x.device))
        return mask.bool()    
    def to(self, device: Optional[torch.device] = None, dtype: Optional[torch.dtype] = None, *args, **kwargs):
        if dtype == torch.float16:
            warnings.warn(message="You have loaded this model in float16, if you are using Adam based optimizers, make sure to set the eps to at least 1e-4 to prevent nan loss")
    ...
    @staticmethod
    def fit(model, num_epochs, dataloader, tokenizer, optimizer, val_dataloader, scheduler, device: torch.device, lr=8e-5):
        def lr_lambda(current_step):
            if current_step < 100:
                return current_step / 100
            return max(
                0.1,
                (num_epochs * len(dataloader) - current_step) / (num_epochs * len(dataloader) - 50)
            )
        """
        ...
        for epoch in range(num_epochs):
            ...
            for i, batch in tqdm(enumerate(dataloader)):
                ...
                outputs = model(input_ids=input_ids, labels=input_ids, keytoken_ids=keytoken_ids)
...
class MEPA(nn.Module):
    ...
    class MEPALayer(nn.Module):
        ...
        def forward(self, x):
            ...
            """
            biased = x + self.bias
            scaled = self.rms((biased * self.scaling_matrix) + biased)
            ffn_out = self.ffn(scaled) 
            return self.layer_norm(ffn_out + scaled)
...
class SENTIADecoderBlock(nn.Module):
    ...
    def __init__(self, config: SENTIAConfig, *args, **kwargs):
        ...
        self.masked_attention = SENTIAAttention(config, "global")
