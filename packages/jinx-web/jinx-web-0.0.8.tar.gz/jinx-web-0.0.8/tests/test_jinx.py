import pytest
from . import helpers
from jinx import Jinx, Middleware
...
def test_assets_are_served(tmpdir_factory):
    ...
    _, path, body = helpers.create_static(static_dir)
    api = Jinx(static_dir=str(static_dir))
    client = api.test_session()

    response = client.get(f'http://testserver/static/{path}')
