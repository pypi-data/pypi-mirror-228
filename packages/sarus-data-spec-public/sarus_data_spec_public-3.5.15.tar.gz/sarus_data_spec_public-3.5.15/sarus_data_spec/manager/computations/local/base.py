...
class LocalComputationWithLRU(LocalComputation[T]):
    """Extension of the general class that caches some results in-process
    to avoid building the same objects"""

    def __init__(self, computing_manager: Base, lru_size: int = 16):
        super().__init__(computing_manager)
        self.cache: collections.OrderedDict[str, T] = collections.OrderedDict()
        self.lru_size = lru_size

    async def task_result(self, dataspec: st.DataSpec, **kwargs: t.Any) -> T:
        """Returns the task result after caching it."""
        uuid = dataspec.uuid()
        if uuid not in self.cache:
            value: T = await super().task_result(dataspec, **kwargs)
            ...
            print(f"cache hit for {self.task_name}: {uuid}")
            self.cache.move_to_end(uuid)
