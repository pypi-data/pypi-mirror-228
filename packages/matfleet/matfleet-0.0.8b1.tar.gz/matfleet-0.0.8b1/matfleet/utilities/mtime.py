...
import six
import datetime
from matfleet.utilities.constant import TIME_FORMAT_D, TIME_FORMAT_FS
...
def reconstitute_dates(obj_dict):
    ...
    if isinstance(obj_dict, six.string_types):
        try:
            ...
            try:
                return datetime.datetime.strptime(obj_dict, TIME_FORMAT_D)
            except:
                pass
    return obj_dict


def now_time(as_str=True, only_day=False):
    if only_day:
        fmtstr = TIME_FORMAT_D
    else:
        fmtstr = TIME_FORMAT_FS
    dd = datetime.datetime.now().strftime(fmtstr)
