...
app.mount(
    "/static", StaticFiles(directory=static_files_dir, check_dir=False, html=True)
)


@deployment(name="Perspective_Web_Server", num_replicas=1, route_prefix="/")
@ingress(app)
class PerspectiveRayServer:
    def __init__(self, args: PerspectiveRayServerArgs = None):
        ...
        self._schemas = {}
        self._tables = {}
        self._psp_thread = Thread(
            target=perspective_thread, args=(self._manager,), daemon=True
        )
    ...
    async def new_table_rest(self, tablename: str, request: Request) -> Response:
        ...
        try:
            ...
            raise HTTPException(503, "Exception during schema parsing")
    ...
    async def update_table_rest(self, tablename: str, request: Request) -> Response:
        ...
        try:
            ...
            raise HTTPException(503, "Exception during data parsing")

        try:
            self.update(tablename, data)
        except BaseException:
            raise HTTPException(
                503, f"Exception during data ingestion: {tablename} / {format_exc()}"
            )
...
@deployment(name="Perspective_Proxy_Server", route_prefix="/perspective")
...
def main(args: PerspectiveRayServerArgs = None) -> Application:
    ...
    raise Exception(
        "Perspective server requires websockets, rerun with RAY_SERVE_ENABLE_EXPERIMENTAL_STREAMING=1"
    )
