...
class ToolbarController():
     ...
     def leftClick(self, event, item):
         ...
         if self.view.selected_tool() in (labels.SOURCES_NM,
             ...
             self.new_model_item(event.pos(), self.view.selected_tool())
     ...
     def new_model_item(self, pos, item_label, interactive=True):
        # copy values of item that are currently in view to new model

        pos_cm = self.mouse_pos_to_cm(pos)
        pos_pixels = self.mouse_pos_to_pixels(pos)
        ...
        if item_label == labels.WALLS:      
            ...
            item_model.shielding = controller.item_in_view().name
