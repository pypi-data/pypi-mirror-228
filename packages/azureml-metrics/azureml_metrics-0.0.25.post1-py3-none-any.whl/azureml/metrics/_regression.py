...
class RegressionMetric(Metric):
    ...
    def __init__(self,
        ...
        if y_test.shape[0] != y_pred.shape[0]:
            message = "Mismatched input shapes: y_test={}, y_pred={}".format(y_test.shape, y_pred.shape)
            raise DataErrorException(message,
                                     target="y_pred", reference_code="_regression.RegressionMetric.__init__",
                                     safe_message="Mismatched input shapes: y_test, y_pred", has_pii=True)
...
class Residuals(RegressionMetric, NonScalarMetric):
    ...
    def compute(self):
        ...
        if self._y_std is None:
            safe_message = "y_std required to compute Residuals."
            raise DataErrorException(safe_message, target="_y_std", reference_code="_regression.Residuals.compute",
                                     safe_message=safe_message, has_pii=False)
...
class PredictedTrue(RegressionMetric, NonScalarMetric):
    ...
    def compute(self):
        ...
        if self._bin_info is None:
            safe_message = "bin_info is required to compute PredictedTrue."
            raise DataErrorException(safe_message, target="bin_info",
                                     reference_code="_regression.PredictedTrue.compute",
                                     has_pii=False, safe_message=safe_message)
        if not isinstance(self._bin_info, Dict):
            safe_message = "bin_info is required in the format to compute PredictedTrue. "
            sample = "bin_info = {'number_of_bins': 2, 'bin_starts': 1, 'bin_ends': [1, 2]}"
            raise DataErrorException(safe_message + sample, target="bin_info",
                                     reference_code="_regression.PredictedTrue.compute",
                                     has_pii=False, safe_message=safe_message)

        n_bins = self._bin_info.get('number_of_bins', None)
        bin_starts = self._bin_info.get('bin_starts', None)
        bin_ends = self._bin_info.get('bin_ends', None)
        if n_bins is None or bin_starts is None or bin_ends is None:
            required_keys = [
                'number_of_bins',
                'bin_starts',
                'bin_ends'
            ]
            check_keys = [key for key in required_keys if key not in self._bin_info]
            if len(check_keys) > 0:
                safe_message = "{} keys are missing in bin_info.".format(check_keys)
                raise DataErrorException(safe_message, target="bin_info",
                                         reference_code="_regression.PredictedTrue.compute",
                                         has_pii=False, safe_message=safe_message)
