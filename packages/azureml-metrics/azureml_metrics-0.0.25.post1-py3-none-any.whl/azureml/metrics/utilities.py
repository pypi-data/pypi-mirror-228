...
from azureml.metrics.exceptions import ClientException, HFEvaluateClientException
...
def segregate_scalar_non_scalar(metrics: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    for metric_name, metric_value in metrics.items():
        try:
            ...
            aggregate_metric_scores(metric_name, metric_value, metrics_result)
...
def aggregate_metric_scores(metric_name, metric_value, metrics_result):
    """Helper function to aggregate the possible artifact metrics"""
    if metric_name in constants.Metric.NON_AGGREGATED_METRICS:
    ...
    elif metric_name in constants.Metric.NON_SCALAR_RAG_EVALUATION_SET:
...
def extract_common_kwargs(kwargs):
    ...
    return common_args, custom_dimensions, log_activity, log_traceback
