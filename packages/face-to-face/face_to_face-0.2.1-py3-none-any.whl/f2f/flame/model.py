...
class FLAME(nn.Module):
    ...
    def forward(
        ...
        args:
            ...
            eyes_rotation: (N, 6) in radians (3 for left, 3 for right) or (N, 8) in quaternions
                axis direction:
                    x: head center -> left ear
                    y: head center -> top
                    z: head center -> forward of the face
        return:
            vertices: (N, 5023, 3)
            dynamic_lm17: (N, 17, 3)
            static_lm68: (N, 68, 3)
    ...
    def _find_dynamic_lm_idx_and_bcoords(
        ...
        Args:
            rotations: shape (N, 15), rotations of global 3, eyes 6, jaw 3, neck 3
        Returns:
            dynamic_lm_faces_idx:
...
class FLAMETexture(nn.Module):
    ...
    def get_texture_kwargs(
        ...
        return dict(maps=texture, faces_uvs=faces_uvs, verts_uvs=verts_uvs)
