...
collections = [collection for collection in os.listdir(os.path.join(os.path.dirname(__file__), 'mongo', 'collections'))]
index_cache = []

def load_indexes(collection_scope, collection_name):
	try:
		module_name = f"database.mongo.collections.{collection_scope}.{collection_name}"
		print(f"module_name={module_name}")
		collection_module = __import__(module_name, fromlist=['indexes'])
		print(f"collection_module={collection_module}")
		collection_indexes = getattr(collection_module, 'indexes')
		print(f"collection_indexes={collection_indexes}")
