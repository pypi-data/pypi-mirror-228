...
def deploy_libraries(w3, account, config, libraries):
    # Solidity source files
    source_files = list(map(lambda x: x["source"], list(libraries.values())))

    # Compile the contracts
    compiled_files = compile_files(source_files, ["abi", "bin"])
...
def deploy_contracts(w3, account, config, libraries, contracts):
    # Solidity source files
