...
def calculate_RMSD_between_motif_and_similar_motifs(
        ...
        index_folder_path: Path, RMSD_atoms: str, RMSD_threshold: float, n_cores: int, results_output_path: Path
    ) -> None:
    """
    ...
    """
    ...
    n_RMSD_to_compute = get_n_RMSD_to_compute(PDBs_with_similar_motifs)
    tqdm_progress_bar = get_tqdm_progress_bar(total=n_RMSD_to_compute, desc='RMSD calculations')
    ...
    print('Writting results file ...')
    with open(results_output_path, 'wt') as file_handle:
        # Column names
        file_handle.write('matched_motif,similar_motif_found,RMSD,n_mutations,PDB_ID,header_description\n')
