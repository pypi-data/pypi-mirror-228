...
# (But no!  As of 2023 Aug 11, the new code is 15K lines.)
...
# implemented *tonto* but are not attempted in *Tonto2*.
...
import xdg           # *python3-xdg* Debian Package
...
EXTERNALS = [
    ...
    'q0',
    'tonto2_code_tables',
    'tonto2_parse_international_addresses',
    ]
for external in EXTERNALS:
    try:
        # module in a package
        mod = importlib.import_module(f'.{external}', 'tonto2')
    except ImportError:
        mod = None
    if mod:
        pass
        ...
        # independent module
        mod = importlib.import_module(external)
    globals()[external] = mod
...
def repl_case(valu, targ, repl=None, is_highlighted=False):
    ...
    while True:
        ...
        if (targ_len == ZERO) or (new_pos == NA):
            result += valu[old_pos:]
            break
        else:
            count += 1
...
#                    itm.edt.set_visual(itm.val_disp)  #  Can't because *edt* won't always be set.
    return result
...
def announce(txt):
    ...
    if COMMON['CONFIG'].festival_type_voice_synth in BLANK_VALUES:
        ...
        subprocess.run(
            [COMMON['CONFIG'].festival_type_voice_synth, '--tts'],
            text=True,
            input=txt,
            )
    return
...
def generate_qr_image(txt):
    ...
    if COMMON['CONFIG'].qr_code_generator in BLANK_VALUES:
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.png') as unit:
            proc = subprocess.run(
                f'{COMMON["CONFIG"].qr_code_generator} --output "{unit.name}" "{txt}"',
                shell=True,
                text=True,
                capture_output=True,
                )
...
class ItmDateTime(Itm):
    ...
    def __init__(self, tag=None):
        super().__init__(tag)
        self.fmt_disp = locale.nl_langinfo(locale.D_T_FMT)
...
class ItmDate(ItmDateTime):
    ...
    def __init__(self, tag=None):
        super().__init__(tag)
        self.fmt_disp = locale.nl_langinfo(locale.D_FMT)
...
class ItmTime(ItmDateTime):
    ...
    def __init__(self, tag=None):
        super().__init__(tag)
        self.fmt_disp = locale.nl_langinfo(locale.T_FMT)
...
class ItmURI(ItmText):
    ...
    def event_open(self):
        ...
        if link in BLANK_VALUES:
            ...
            browser_open_tab(uri=self.val_comp)
        return
    ...
    def event_drag_and_drop(self, event):
        ...
        uri = None
        title = None
        for (mime, has_title) in allowable:
            if mime in available:
                text = event.mimeData().data(mime).data().decode('utf-8')
                lines = text.splitlines()
                if lines:
#                    print(f'{mime}:  {repr(lines)}')
                    uri = lines.pop(ZERO)
    ...
    *icon* is not used ... yet.
    ...
    def icon(self, x):
        if x in BLANK_VALUES:
            ...
            self._icon = pathlib.Path(x).expanduser()
...
class RelCalendar(Rel):
    ...
    def initialize_holidays(self):        
        self.load_recs(HOLIDAYS.splitlines()[1:], fieldnames=self.pages.get_tag_collection())
        return
...
class Config:
    ...
    fn = 'tonto.ini'
    ini_path = pathlib.Path(xdg.BaseDirectory.xdg_config_home, 'Tonto2', fn)
    ...
    def load(self):
        ...
        def load_globals():
            ...
            self.ledger_font = self.ini[sect].get('ledger_font', self.default_ledger_font)
            if self.ledger_font in BLANK_VALUES:
                self.ledger_font = self.default_ledger_font
            self.icon_file = self.ini[sect].get('icon_file', self.default_icon_file)
            if self.icon_file in BLANK_VALUES or (not pathlib.Path(self.icon_file).exists()):
                self.icon_file = self.default_icon_file
            self.alert_wav = self.ini[sect].get('alert_wav', self.default_alert_wav)
            if self.alert_wav in BLANK_VALUES or (not pathlib.Path(self.alert_wav).exists()):
                self.alert_wav = self.default_alert_wav
            self.festival_type_voice_synth = self.ini[sect].get('festival_type_voice_synth', self.default_festival_type_voice_synth)
            if self.festival_type_voice_synth in BLANK_VALUES or (not shutil.which(pathlib.Path(self.festival_type_voice_synth))):
                self.festival_type_voice_synth = self.default_festival_type_voice_synth
            self.qr_code_generator = self.ini[sect].get('qr_code_generator', self.default_qr_code_generator)
            if self.qr_code_generator in BLANK_VALUES or (not shutil.which(pathlib.Path(self.qr_code_generator))):
                self.qr_code_generator = self.default_qr_code_generator
            self.browser = self.ini[sect].get('browser', self.default_browser)
...
class Tab:
    ...
    def ledger_fill(self):
        ...
        self.ledger_layout()
        self.wgt.add_wgt(self.ledger.get_view())
        self.reset()
        return self
...
are not in the current relation are appended as text items.'''),
    ...
    def event_contents(self):
        browser_open_tab(f'https://lacusveris.com/Tonto/HowTo/index.html', can_update_traversed_date=False)
        return
...
class DlgViewText(q0.Q0DialogModal):
    ...
    def event_view_preview(self):
        txt = self.wgt_view.get_html()
        with tempfile.NamedTemporaryFile(mode='w', delete=False) as unit:
            unit.write(txt)
            tmp_name = unit.name
...
class Alarm:
    ...
    essential_attribs = [
        ...
        ('mins_adv', TAG_ADVANCE_ALARM, ZERO),
    ...
    def __str__(self):
        ...
        result = f'<td><tt>{txt_time}</tt></td><td><b>{txt_title}</b></td>'
        return result
