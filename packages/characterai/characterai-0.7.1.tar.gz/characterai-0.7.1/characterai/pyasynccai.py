...
class PyAsyncCAI:
    ...
    async def upload_image(
        ...
        await _page.goto('https://beta.character.ai/chat')

        close1 = _page.locator('svg#mobile-app-modal-close')
        ...
        async with _page.expect_response(
            ...
            async with _page.expect_file_chooser() as file_info:
                await _page.click('input.form-control')

            file_chooser = await file_info.value
            await file_chooser.set_files(path)

        response = await (await response_info.value).json()

        if response['status'] != 'OK':
            raise errors.ImageUploadError(response['error'])
        else:
            return response
    ...
    class user:
        ...
        async def characters(self, *, wait: bool = False, token: str = None):
            return await _GetResponse(
