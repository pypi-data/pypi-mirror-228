...
def mix_datafile_coeff(ff1, ff2):
    lj_coeff2, bond_coeff2, ang_coeff2, dih_coeff2, imp_coeff2 = ff2
    lj_coeff1, bond_coeff1, ang_coeff1, dih_coeff1, imp_coeff1 = ff1

    lj_coeff1["type"] = lj_coeff1["type"] + lj_coeff2["type"].max()
    total_lj_coeff = pd.concat([lj_coeff2, lj_coeff1], axis=0).reset_index(drop=True)
    total_lj_coeff.index = total_lj_coeff.index + 1
    ff_sec_names = ["\n\nPair Coeffs \n\n"]
    ff_sec_strs = [total_lj_coeff.to_string(index=None, header=None)]

    if type(bond_coeff2) != type(None):
        if len(bond_coeff2) > 0:
            total_bond_coeff = bond_coeff2.copy(deep=True)
    if type(bond_coeff1) != type(None):
        if len(bond_coeff1) > 0:
            bond_coeff1["type"] = bond_coeff1["type"] + bond_coeff2["type"].max()
            total_bond_coeff = pd.concat([bond_coeff2, bond_coeff1], axis=0).reset_index(drop=True)
            total_bond_coeff.index = total_bond_coeff.index + 1
            ff_sec_names.append("\n\nBond Coeffs \n\n")
            ff_sec_strs.append(total_bond_coeff.to_string(index=None, header=None))
    
    
    if type(ang_coeff2) != type(None):
        if len(ang_coeff2) > 0:
            total_ang_coeff = ang_coeff2.copy(deep=True)
    if type(ang_coeff1) != type(None):
        if len(ang_coeff1) > 0:
            ang_coeff1["type"] = ang_coeff1["type"] + ang_coeff2["type"].max()
            total_ang_coeff = pd.concat([ang_coeff2, ang_coeff1], axis=0).reset_index(drop=True)
            total_ang_coeff.index = total_ang_coeff.index + 1
            ff_sec_names.append("\n\nAngle Coeffs \n\n")
            ff_sec_strs.append(total_ang_coeff.to_string(index=None, header=None))
    
    if type(dih_coeff2) != type(None):
        if len(dih_coeff2) > 0:
            total_dih_coeff = dih_coeff2.copy(deep=True)
    if type(dih_coeff1) != type(None):
        if len(dih_coeff1) > 0:
            dih_coeff1["type"] = dih_coeff1["type"] + dih_coeff2["type"].max()
            total_dih_coeff = pd.concat([dih_coeff2, dih_coeff1], axis=0).reset_index(drop=True)
            total_dih_coeff.index = total_dih_coeff.index + 1
            ff_sec_names.append("\n\nDihedral Coeffs \n\n")
            ff_sec_strs.append(total_dih_coeff.to_string(index=None, header=None))
    
    if type(imp_coeff2) != type(None):
        if len(imp_coeff2) > 0:
            total_imp_coeff = imp_coeff2.copy(deep=True)
    if type(imp_coeff1) != type(None):
        if len(imp_coeff1) > 0:
            imp_coeff1["type"] = imp_coeff1["type"] + imp_coeff2["type"].max()
            total_imp_coeff = pd.concat([imp_coeff2, imp_coeff1], axis=0).reset_index(drop=True)
            total_imp_coeff.index = total_imp_coeff.index + 1
            ff_sec_names.append("\n\nImproper Coeffs \n\n")
            ff_sec_strs.append(total_imp_coeff.to_string(index=None, header=None))
    return ff_sec_names, ff_sec_strs