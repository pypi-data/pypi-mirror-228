...
async def start_ssh_for_app(
    app_spec: fragments.AppSpec, *, use_case: str | None = None, sdk: SlingshotSDK
    ...
    assert app_spec.app_type == schemas.AppType.CUSTOM, "This method only supports custom apps"

    use_case = use_case or 'SSH'
    formatted_app_type = describe_app_type(app_spec.app_type, app_spec.app_sub_type)
    console.print(f"Starting {use_case} for {formatted_app_type} '{app_spec.app_spec_name}'")

    if app_spec.app_instance_status != schemas.AppInstanceStatus.READY:
        raise SlingshotException(
            f"Cannot {use_case} while {formatted_app_type} is not ready. Current status: {app_spec.app_instance_status}"
        )
    ...
    if not ssh_port:
        ...
        resp = await sdk.start_app_ssh(app_spec.app_spec_id)
        if resp.error:
...
async def start_ssh_for_run(run: fragments.Run, *, sdk: SlingshotSDK) -> SshConnectionDetails:
    ...
    if run.job_status != schemas.JobStatus.ACTIVE:
        raise SlingshotException(f"Cannot SSH to a run that is not active. Current status: {run.job_status}")
