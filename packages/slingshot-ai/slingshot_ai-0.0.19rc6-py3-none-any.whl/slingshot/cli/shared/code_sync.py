...
from slingshot.cli.shared.formatting import describe_app_type
...
async def start_code_sync(sync_path: Path, app_spec: fragments.AppSpec, *, sdk: SlingshotSDK) -> None:
    """
    ...
    :param app_spec: Spec of the app to sync against (may be a session or other app such as a web app)
    :param sdk: Slingshot SDK
    """

    ssh_connection_details = await start_ssh_for_app(app_spec, use_case='code sync', sdk=sdk)
    ssh_connection_str = (
        f"{ssh_connection_details.username}@{ssh_connection_details.hostname}:{ssh_connection_details.port}"
    )
    formatted_app_type = describe_app_type(app_spec.app_type, app_spec.app_sub_type)
    target_sync_path = (
        "/slingshot/session" if app_spec.app_sub_type == schemas.AppSubType.SESSION else "/mnt/slingshot/code"
    )

    await _start_unison(formatted_app_type, ssh_connection_str, sync_path, target_sync_path, verbose=sdk.verbose)


async def _start_unison(
    app_type: str, ssh_connection_str: str, sync_path: Path | None, target_sync_path: str, verbose: bool = False
) -> None:
    ...
    for _ in range(3):  # retry 3 times
        console.print(f"[blue]Syncing {p} to your {app_type}...[/blue]")
        try:
