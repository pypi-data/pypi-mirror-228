...
class SlingshotAPI:
    ...
    async def start_app(
        ...
        *,
        ...
        if mount_specs is None:
            mount_specs = [schemas.remote_mount_spec_to_local(mount_spec) for mount_spec in app_spec.mount_specs]
    ...
    async def start_app_ssh(self, app_spec_id: str, *, project_id: str) -> schemas.Response[schemas.SshPort]:
        """Start SSH for an app, exposing it on a port that we can access from the public Internet."""
        return await self._client.make_request(
            url=f"project/{project_id}/apps/{app_spec_id}/ssh/start",
            method="post",
