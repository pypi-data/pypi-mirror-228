...
class MountSpec(BaseGraphQLEntity):
    ...
    mode: Literal["DOWNLOAD", "UPLOAD", "VOLUME"] = Field(..., alias="mode")
    name: str = Field(..., alias="name")
    tag: Optional[str] = Field(None, alias="tag")


class _AppSpecAppInstanceProjection(BaseModel):
...
class _AppSpecRunProjection(BaseModel):
...
class AppSpec(BaseGraphQLEntity):
    ...
    _fragment = """
        fragment SlingshotAppSpec on AppSpecs {
            ...
            trainingRuns(orderBy:{ createdAt:DESC }, limit: 1) {
                trainingRunId
                createdAt
                jobStatus
                sshPort
    ...
    app_instances: list[_AppSpecAppInstanceProjection] = Field(..., alias="appInstances")
    deployments: list[Deployment] = Field(..., alias="deployments")
    training_runs: list[_AppSpecRunProjection] = Field(..., alias="trainingRuns")
