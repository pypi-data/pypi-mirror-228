...
class SlingshotSDK:
    ...
    async def upsert_dataset_artifact(self, upsert: schemas.Upsert | Path, dataset_tag: str) -> fragments.BlobArtifact:
        ...
        """
        upsert_artifact_name = "dataset_upsert"
        project_id = await self._get_current_project_id_or_raise()
        if isinstance(upsert, schemas.Upsert):
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                upsert_artifact = await self.upload_artifact(
                    upsert_filename, blob_artifact_name=upsert_artifact_name, as_zip=False
                )
        else:  # Path
            upsert_artifact = await self.upload_artifact(upsert, blob_artifact_name=upsert_artifact_name, as_zip=False)
        ...
        resp = await self._api.upsert_dataset_artifact(
            upsert_artifact_id=upsert_artifact.blob_artifact_id,
            dataset_artifact_name=dataset_tag,
            project_id=project_id,
        )
    ...
    async def get_logs(self, *, run_id: str | None = None, spec_id: str | None = None) -> list[LogLine]:
    ...
    async def start_run(
        ...
        resp = await self._api.start_run(
            ...
            cmd=cmd,
            # We're blindly converting a fragment here to the CLI mount schema, if it works it's by accident
            # TODO: We don't seem to be using the mount spec argument here and we don't have it for apps/deployments,
            #   remote it here and from the API?
            mount_specs=mount_specs,  # type: ignore
    ...
    async def start_app_ssh(self, spec_id: str) -> Response[SshPort]:
        """Starts SSH access for an app."""
        project_id = await self._get_current_project_id_or_raise()
        return await self._api.start_app_ssh(spec_id=spec_id, project_id=project_id)

    async def start_run_ssh(self, run_id: str) -> Response[SshPort]:
    ...
    async def stop_app(self, *, app_name: str) -> None:
        ...
        """
        project_id = await self._get_current_project_id_or_raise()
        app_spec = await self._api.get_component_spec_by_name(spec_name=app_name, project_id=project_id)
        if not app_spec:
            raise SlingshotException(f"Could not find app with name {app_name}")
        await self._api.stop_app(spec_id=app_spec.spec_id, project_id=project_id)
    ...
    async def list_apps(self) -> list[fragments.ComponentSpec]:
    ...
    async def list_run_templates(self) -> list[fragments.ComponentSpec]:
        ...
        return [app_spec for app_spec in existing_app_specs if app_spec.component_type == schemas.ComponentType.RUN]
    ...
    async def list_deployments(self) -> list[fragments.ComponentSpec]:
        ...
        existing_app_specs = await self._api.list_app_specs(project_id=project_id)
        return [
            app_spec for app_spec in existing_app_specs if app_spec.component_type == schemas.ComponentType.DEPLOYMENT
        ]
    ...
    async def delete_app(self, spec_id: str) -> schemas.ResponseOK:
        """Delete an app with the given id."""
        project_id = await self._get_current_project_id_or_raise()
        return await self._api.delete_app(spec_id=spec_id, project_id=project_id)
    ...
    async def _wait_for_deployment_status(
        self,
        deployment_spec: schemas.HasComponentSpecId,
    ...
    async def _wait_for_app_status(
        self: SlingshotSDK,
        app: schemas.HasComponentSpecId,
        status: schemas.AppInstanceStatus,
        *,
        max_wait: float | None = None,
    ) -> None:
