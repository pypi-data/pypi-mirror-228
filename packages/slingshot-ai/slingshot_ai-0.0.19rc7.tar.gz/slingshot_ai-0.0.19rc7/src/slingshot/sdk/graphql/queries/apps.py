...
class AppInstancesByAppTypeQuery(BaseGraphQLQuery[AppInstancesResponse]):
    _query = """
        query AppInstancesByAppType($componentType: AppTypeEnumEnum!, $projectId: String!) {
            appInstances(where: {_and: {componentType: {_eq: $componentType}, componentSpec: {projectId: {_eq: $projectId}}}}) {
                ...AppInstance
            }
        }
    """
    ...
    def __init__(self, component_type: str, project_id: str):
        super().__init__(
            variables={"componentType": component_type, "projectId": project_id}, response_model=AppInstancesResponse
        )
