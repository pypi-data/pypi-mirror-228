...
class ApplyService:
    ...
    async def plan_apps(self, config_: schemas.ProjectManifest) -> ComponentSpecPlan:
        ...
        for app_sub_type in schemas.AppSubType:
            ...
            (
                ...
                existing_app_specs, apps_from_sub_type, schemas.ComponentType.APP, app_sub_type  # noqa
            )
    ...
    async def apply_app_specs(self, app_spec_plan: ComponentSpecPlan, env_spec_name_to_id: dict[str, str]) -> None:
        ...
        run_specs_to_delete = [
            app_spec for app_spec in app_specs_to_delete if app_spec.component_type == schemas.ComponentType.RUN
        ]
    ...
    async def create_component_spec(
        self,
        app_spec: schemas.AbstractComponentSpec,
        exec_env_spec_id: str | None = None,
        import_run_spec: schemas.HasComponentSpecId | None = None,
        export_run_spec: schemas.HasComponentSpecId | None = None,
    ) -> schemas.HasComponentSpecId:
        console.print(f"Creating '{app_spec.name}'...", end="")
        component_type = _get_component_type(app_spec)
        ...
        spec_id = get_data_or_raise(app_spec_id_response)
        console.print("✅")
        return spec_id
    ...
    async def delete_app_spec(self, app_spec: fragments.ComponentSpec) -> None:
        console.print(
            f"Deleting {app_spec.friendly_component_type} '{app_spec.spec_name}' from the remote project manifest...",
            end="",
        )
        await self._sdk.delete_app(spec_id=app_spec.spec_id)
        console.print("✅ ")

    async def apply_changes_to_remote(self, env_spec_plan: EnvSpecPlan, app_spec_plan: ComponentSpecPlan) -> None:
        """
    ...
    async def restart_updated_apps(self, env_spec_plan: EnvSpecPlan, app_spec_plan: ComponentSpecPlan) -> None:
        _, app_specs_to_update, _ = app_spec_plan

        # We only want to restart deployments and custom apps
        apps_or_deployments_to_restart = [
            (app_spec, component_type)
            for app_spec, _ in app_specs_to_update
            if (component_type := _get_component_type(app_spec))
        ...
        for app_spec, component_type in apps_or_deployments_to_restart:
...
def _get_app_sub_type(spec: schemas.AbstractComponentSpec) -> schemas.AppSubType | None:
    if isinstance(spec, schemas.WebappComponentSpec):
        return schemas.AppSubType.WEBAPP
    if isinstance(spec, schemas.SessionComponentSpec):
        return schemas.AppSubType.SESSION
    if isinstance(spec, schemas.RedisComponentSpec):
        return schemas.AppSubType.REDIS
    if isinstance(spec, schemas.LabelStudioComponentSpec):
...
def _diff_existing_app_yaml_specs(
    all_existing_app_specs: list[fragments.ComponentSpec],
    yaml_app_specs: typing.Sequence[schemas.AbstractComponentSpec],
    ...
    spec_display_name = _get_spec_display_name(component_type, app_sub_type)
    app_specs_to_create: list[schemas.AbstractComponentSpec] = []
    app_specs_to_update: list[tuple[schemas.AbstractComponentSpec, str]] = []
    app_specs_to_delete: list[fragments.ComponentSpec] = []

    existing_app_specs = [spec for spec in all_existing_app_specs if spec.component_type == component_type]
    existing_spec_name_to_spec: dict[str, fragments.ComponentSpec] = {
        app_spec.spec_name: app_spec for app_spec in existing_app_specs
    }
    ...
    for app_spec in yaml_app_specs:
        ...
        if len(app_spec_diff) > 0:
            app_specs_to_update.append((app_spec, existing_app_spec.spec_id))

    for existing_app_spec in existing_app_specs:
        if existing_app_spec.spec_name not in yaml_app_spec_names and (
            _normalize_app_sub_type(existing_app_spec) == app_sub_type
        ):
            console.print(f"[red](-)[/red] {spec_display_name} '{existing_app_spec.spec_name}'")
...
def _normalize_app_sub_type(app: fragments.ComponentSpec) -> Optional[schemas.AppSubType]:
    """
    Ensures that a custom app always has a sub component type. Prior to Aug 2023, apps without a
...
async def _remote_project_manifest(sdk: "SlingshotSDK") -> schemas.ProjectManifest:
    ...
    # Build app specs
    app_specs: list[schemas.AbstractComponentSpec] = []
